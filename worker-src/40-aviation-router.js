...
export default{async fetch(request,env){
 ...
 if(mode==='official-water'){
  try{return json({...await pegelOnlineStation(lat,lon),version:WORKER_VERSION,checkedAt:new Date().toISOString()},200,{'cache-control':'public, max-age=120, stale-while-revalidate=300'})}
  catch(error){return json({available:false,error:error instanceof Error?error.message:String(error),provider:'WSV PEGELONLINE REST API v2',version:WORKER_VERSION},502,{'cache-control':'no-store'})}
 }
