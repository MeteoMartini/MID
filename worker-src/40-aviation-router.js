...
export default{async fetch(request,env){
 ...
 if(mode==='flight-cross-section'){if(!FLIGHT_CROSS_SECTION_ENABLED)return json({error:'Cross Section ist vorerst deaktiviert.',status:'to-be-continued',version:WORKER_VERSION,checkedAt:new Date().toISOString()},410,{'cache-control':'no-store'});try{return json({...await flightCrossSection(u),version:WORKER_VERSION},200,{'cache-control':'public, max-age=900'})}catch(error){return json({error:error instanceof Error?error.message:String(error),version:WORKER_VERSION,checkedAt:new Date().toISOString()},502,{'cache-control':'no-store'})}}
