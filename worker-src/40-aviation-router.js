...
export default{async fetch(request,env){
 ...
 if(mode==='dach-extreme-outlook'){try{return json({...await dachExtremeOutlookData(),version:WORKER_VERSION},200,{'cache-control':'public, max-age=600, stale-while-revalidate=3600'})}catch(error){return json({scope:'DACH',periods:[],cells:[],error:error instanceof Error?error.message:String(error),version:WORKER_VERSION,checkedAt:new Date().toISOString()},502,{'cache-control':'no-store'})}}
