...
export type Hour={time:string;epoch:number;timezone:string;temperature:number;apparent:number;humidity:number;dewPoint:number;pressure:number;precipitation:number;rain:number;showers:number;snowfall:number;probability:number;code:number;wind:number;gust:number;gustAdjusted?:boolean;direction:number;cloud:number;lowCloud:number;midCloud?:number;highCloud?:number;uvIndex:number;visibility:number;ceiling?:number;freezingLevel?:number;snowline?:number;surfaceTemperature?:number;snowDepth?:number;cape:number;liftedIndex?:number;convectiveInhibition?:number;columnWaterVapour?:number;sunshineDuration?:number|null;weatherSourceId?:string;weatherSourceLabel?:string;weatherBundleKind?:'best-match'|'coherent-model'|'nowcast';localAdjustment?:number;localAdjustmentSourceLabel?:string;sunriseEpoch?:number;sunsetEpoch?:number;isDay:boolean};
export type Minute15={time:string;epoch:number;timezone:string;precipitation:number;rain:number;showers:number;snowfall:number;probability:number;code:number;sunshineDuration?:number|null;sunriseEpoch?:number;sunsetEpoch?:number;isDay?:boolean};
...
export type Day={date:string;code:number;max:number;min:number;sunrise?:string;sunset?:string;sunshineDuration:number|null;sunshineDurationMeta?:SunshineDurationDiagnostic;precipitation:number;rain?:number;showers?:number;snowfall?:number;precipitationHours?:number;probability:number;probabilitySignificant?:number;probabilityWindows?:PrecipitationProbabilityWindow[];probabilitySource?:'ensemble-members-dwd'|'daily-wet-derived'|'hourly-max-fallback';probabilityMemberCount?:number;wind:number;gust:number;gustAdjusted?:boolean;direction:number;uvMax:number;weatherSourceId?:string;weatherSourceLabel?:string};
export type EnsembleModelDay={id:string;label:string;family?:string;independenceGroup?:string;max:number;min:number;precipitation:number;precipitationProbability:number;precipitationProbabilitySignificant?:number;probabilityQuality?:'native-members'|'estimated-spread';memberCount:number;wind?:number;gust?:number;sunshineDuration?:number|null;weatherCode?:number};
export type EnsembleScenarioPoint={date:string;max:number;min:number;precipitation:number;sunshineDuration:number|null;gust:number};
...
export type OfficialAlert={id:string;headline:string;description:string;instruction?:string;language?:string;level:OfficialAlertLevel;severity?:string;event?:string;source:string;area?:string;areaCodes?:string[];effective?:string;onset?:string;expires?:string;updatedAt?:string;url?:string};
export type ModelRunUsageStatus='active'|'fallback'|'unavailable'|'adapter-not-configured'|'reserve';
export type ModelRunMeta={id:string;label:string;kind:'forecast'|'ensemble';initialisationTime?:string;availabilityTime?:string;updateIntervalSeconds?:number;temporalResolutionSeconds?:number;metadataModelId?:string;metadataSource?:string;rapidUpdate?:boolean;resolutionKm?:number;forecastHorizonHours?:number;members?:number;availabilityOnly?:boolean;usageStatus?:ModelRunUsageStatus;statusNote?:string;metadataUnavailable?:boolean;usedInCanonical?:boolean;usedDays?:number;usedHours?:number;fusionLoaded?:boolean;usageDetail?:string};
...
const meanModels:EnsembleMeanModel[]=[
 ...
 {id:'dwd_icon_eu_eps_ensemble_mean',label:'DWD ICON-EU EPS Mittel/Spread',metaId:'dwd_icon_eu_eps',family:'dwd-icon-eps',independenceGroup:'dwd-ensemble',resolutionKm:13,updateHours:6,maxDays:5,bbox:[-25,30,45,72]},
 ...
 {id:'dwd_icon_eps_ensemble_mean',label:'DWD ICON Global EPS Mittel/Spread',metaId:'dwd_icon_eps',family:'dwd-icon-eps',independenceGroup:'dwd-ensemble',resolutionKm:26,updateHours:12,maxDays:7.5},
 ...
 {id:'ncep_gefs025_ensemble_mean',label:'NOAA GEFS 0.25° Mittel/Spread',metaId:'ncep_gefs025',family:'noaa-gefs',independenceGroup:'noaa-ensemble',resolutionKm:25,updateHours:6,maxDays:10},
 {id:'ncep_gefs05_ensemble_mean',label:'NOAA GEFS 0.5° Mittel/Spread',metaId:'ncep_gefs05',family:'noaa-gefs',independenceGroup:'noaa-ensemble',resolutionKm:50,updateHours:6,maxDays:35},
...
const forecastCandidates:ForecastCandidate[]=[
 {id:'icon-d2-ruc',label:'DWD ICON-D2-RUC',kind:'forecast',metaSource:'dwd-ruc',rapidUpdate:true,resolutionKm:2,forecastHorizonHours:14,countries:['DE','CH','AT'],bbox:[-6,43,26,58]},
 ...
 {id:'jma_seamless',label:'JMA Seamless',kind:'forecast',resolutionKm:5,forecastHorizonHours:264,countries:['JP','KR'],bbox:[118,20,155,52]},
...
async function modelRunMeta(candidate:ModelMetaCandidate,signal?:AbortSignal):Promise<ModelRunMeta|null>{
 if(candidate.metaSource==='dwd-ruc'){
  try{
   ...
   return{id:candidate.id,label:candidate.label,kind:candidate.kind,initialisationTime:new Date(initMs).toISOString(),availabilityTime:Number.isFinite(availableMs)?new Date(availableMs).toISOString():undefined,updateIntervalSeconds:Number(data.update_interval_seconds)||3600,temporalResolutionSeconds:Number(data.temporal_resolution_seconds)||undefined,metadataModelId:candidate.id,metadataSource:`DWD Open Data · ${candidate.label} Laufmetadaten`,rapidUpdate:true,resolutionKm:Number(data._mid_resolution_km)||candidate.resolutionKm,forecastHorizonHours:Number(data._mid_forecast_horizon_hours)||candidate.forecastHorizonHours,members:Number(data._mid_members)||candidate.members,statusNote:candidate.id==='icon-d2-ruc'?'Zustandskern stündlich bis +14 h · Niederschlag 5 min bis +6 h · Konvektionsdiagnostik 15 min bis +6 h':'RUC-EPS · stündlicher probabilistischer Kurzfristpfad bis +14 h'};
  }catch{return null}
 }
...
function modelRunCandidate(model:EnsembleModel):ModelMetaCandidate{return{id:model.id,label:model.label,kind:'ensemble',metaIds:[model.metaId],rapidUpdate:model.updateHours<=1,resolutionKm:model.resolutionKm,forecastHorizonHours:model.maxDays*24}}
...
export async function bestMatchModelInfo(lat:number,lon:number,country?:string,signal?:AbortSignal):Promise<BestMatchModelInfo>{
 ...
 try{const runs=await modelRunMetas(selected,signal),rapidAvailable=runs.filter(run=>run.rapidUpdate).map(run=>run.label),value={summary:`Open-Meteo Best Match bildet die Leitprognose. MID prüft und fusioniert zusätzlich geeignete Rapid-Update-, Regional- und unabhängige Globalmodelle${rapidAvailable.length?` (${rapidAvailable.join(', ')})`:''}. DWD ICON-D2-RUC wird aus DWD Open Data numerisch in die kanonische Kurzfristfusion eingebunden: der gemeinsame Mehrvariablenkern bleibt nativ stündlich bis +14 h, während native RUC-Niederschläge in 5-Minuten-Schritten bis +6 h und 15-Minuten-Diagnostik aus Niederschlag, CAPE/CIN, DBZ_CMAX sowie – soweit vollständig – MU-CAPE/CIN, LPI, Updraft Helicity, EchoTop, Hagel- und Scherungsfeldern den Nowcast-/Extremwetterpfad ergänzen. RAIN_GSP/SNOW_GSP/GRAU_GSP unterstützen die Niederschlagsphase; VIS/CEILING/HZEROCL/SNOWLMT sowie mittlere/hohe Bewölkung stehen als stündliche Spezialdiagnostik zur Verfügung; fehlende Temperatur-, Wind-, Druck- oder Wolkenzwischenwerte werden nicht interpoliert. DWD ICON-D2-RUC-EPS ergänzt den Kurzfristpfad stündlich probabilistisch. Radar/Nowcast und Beobachtungen bleiben nachgelagerte Korrekturen. Die konkrete Best-Match-/Modellbeteiligung kann je Variable, Standort und Zeitraum wechseln.`,likelyChain:'',candidateModels,runs} satisfies BestMatchModelInfo;writeBestMatchInfoCache(lat,lon,country,value);return value}catch(error){const stale=readBestMatchInfoCache(lat,lon,country,BEST_MATCH_INFO_STALE_MS);if(stale)return stale;throw error}
}
...
function cachedForecastResult(entry:ForecastCoreCache&{age:number},reason:'offline-local-cache'|'fresh-local-cache'|'network-fallback-local-cache'){const prior=entry.value._mid_core_proxy;return{...entry.value,_mid_core_proxy:{...prior,cached:true,stale:entry.age>FORECAST_CORE_FRESH_MS,ageMs:entry.age,sourceUpdatedAt:prior?.sourceUpdatedAt??new Date(entry.time).toISOString(),upstreamReason:reason}}}
...
export async function forecast(lat:number,lon:number,signal?:AbortSignal,options:ForecastRequestOptions={}){const cached=readForecastCoreCache(lat,lon),priority=options.priority??'normal',offline=typeof navigator!=='undefined'&&navigator.onLine===false,cooldownAt=openMeteoCooldownRetryAt(),cooldownActive=cooldownAt>Date.now();if(offline){if(cached)return cachedForecastResult(cached,'offline-local-cache');throw new Error('Offline: Für diesen Ort ist noch kein gespeicherter Wetterstand verfügbar.')}if(!options.forceFresh&&cached&&cached.age<=FORECAST_CORE_FRESH_MS)return cachedForecastResult(cached,'fresh-local-cache');const useDirectFirst=priority==='foreground'&&!cooldownActive;let directError:unknown=null;if(useDirectFirst){try{const value=await directOpenMeteoCoreForecast(lat,lon,signal,priority);writeForecastCoreCache(lat,lon,value);return value}catch(error){if(signal?.aborted)throw error;directError=error}}const proxied=await workerForecastCore(lat,lon,signal,options.forceFresh===true,options.timeZone,options.elevation);if(proxied){writeForecastCoreCache(lat,lon,proxied);return proxied}if(!useDirectFirst&&!cooldownActive){try{const value=await directOpenMeteoCoreForecast(lat,lon,signal,priority);writeForecastCoreCache(lat,lon,value);return value}catch(error){if(signal?.aborted)throw error;directError=error}}if(cached)return cachedForecastResult(cached,'network-fallback-local-cache');if(cooldownActive)throw new OpenMeteoRateLimitError(cooldownAt);if(directError)throw directError;throw new Error('Open-Meteo Best Match ist vorübergehend nicht erreichbar.')}

const AIR_QUALITY_CACHE_PREFIX='mid:air-quality:v1:';
const AIR_QUALITY_FRESH_MS=15*60000;
const AIR_QUALITY_STALE_MS=2*3600000;
type AirQualityPayload={current?:Record<string,number|string>;hourly?:Record<string,(number|string|null)[]>;hourly_units?:Record<string,string>;timezone?:string;utc_offset_seconds?:number};
...
export async function airQuality(lat:number,lon:number,signal?:AbortSignal,forceRefresh=false){const cached=forceRefresh?null:readAirQualityCache(lat,lon);if(cached)return cached;const pollutantAqi=['european_aqi','european_aqi_pm2_5','european_aqi_pm10','european_aqi_nitrogen_dioxide','european_aqi_ozone','european_aqi_sulphur_dioxide'],pollutants=['pm10','pm2_5','nitrogen_dioxide','sulphur_dioxide','ozone'],p=new URLSearchParams({latitude:String(lat),longitude:String(lon),timezone:'auto',past_days:'1',forecast_days:'2',current:[...pollutantAqi,...pollutants,'uv_index'].join(','),hourly:[...pollutantAqi,...pollutants].join(',')});try{const value=await j<AirQualityPayload>(`https://air-quality-api.open-meteo.com/v1/air-quality?${p}`,signal);writeAirQualityCache(lat,lon,value);return value}catch(error){if(signal?.aborted)throw error;const stale=readAirQualityCache(lat,lon,AIR_QUALITY_STALE_MS);if(stale)return stale;throw error}}
...
export function dailyWetProbabilityFromHours(date:string,hours:Hour[],fallback=NaN){const values=hours.filter(hour=>hour.time.slice(0,10)===date).map(hour=>Number(hour.probability)).filter(Number.isFinite);const derived=canonicalDailyWetProbability(values);return Number.isFinite(derived)?derived:fallback}
function dailyWetProbabilityFromWeather(w:Weather,date:string,fallback:number){const times=(w.hourly?.time??[]) as string[],values=(w.hourly?.precipitation_probability??[]) as (number|string|null)[],probabilities=times.map((time,index)=>String(time).slice(0,10)===date?n(values[index],NaN):NaN).filter(Number.isFinite),derived=canonicalDailyWetProbability(probabilities);return Number.isFinite(derived)?derived:fallback}
...
function aggregateMembers(results:ModelResult[],runs:ModelRunMeta[]=[]){
 ...
 for(let lead=0;lead<allDates.length;lead++){
  ...
  for(const r of representatives){
   ...
   const medMax=quantile(memberRows.map(x=>x.max),.5),medMin=quantile(memberRows.map(x=>x.min),.5),filtered=memberRows.filter(x=>Math.abs(x.max-medMax)<=8&&Math.abs(x.min-medMin)<=8),rows=filtered.length>=Math.max(3,Math.ceil(memberRows.length*.55))?filtered:memberRows,groupDivisor=Math.max(1,groupCounts.get(r.model.independenceGroup)??1),freshness=freshnessByModel.get(r.model.id)??.94,tempWeight=modelDayWeight(r.model,lead,rows.length,'temperature',freshness)/groupDivisor,precipWeight=modelDayWeight(r.model,lead,rows.length,'precipitation',freshness)/groupDivisor,windWeight=modelDayWeight(r.model,lead,rows.length,'wind',freshness)/groupDivisor,generalWeight=modelDayWeight(r.model,lead,rows.length,'general',freshness)/groupDivisor;if(!rows.length||Math.max(tempWeight,precipWeight,windWeight,generalWeight)<=0)continue;
  ...
  maxVals=robustWeighted(maxVals,9);minVals=robustWeighted(minVals,9);rainVals=robustWeighted(rainVals,25);cumulativeRainVals=robustWeighted(cumulativeRainVals,25*Math.sqrt(lead+1));sunVals=robustWeighted(sunVals,21600);windVals=robustWeighted(windVals,35);gustVals=robustWeighted(gustVals,45);if(groupsUsed.size<2||maxVals.length<6||minVals.length<6)continue;
...
async function fetchEnsembleWeather(lat:number,lon:number,forecastDays:number,modelId:string,signal?:AbortSignal,hourly='temperature_2m,precipitation,wind_speed_10m,wind_gusts_10m,sunshine_duration',priority:OpenMeteoPriority='normal'){
 const forecast_days=Math.max(1,Math.min(14,Math.ceil(forecastDays))),request=async(rawVariables:string)=>{
  ...
  const p=new URLSearchParams({latitude:String(lat),longitude:String(lon),timezone:'auto',forecast_days:String(forecast_days),models:modelId,hourly:variables,wind_speed_unit:'kn'}),weather=await fetchEnsembleRequest(`https://ensemble-api.open-meteo.com/v1/ensemble?${p}`,signal,priority);if(modelId==='ecmwf_aifs_europe_ensemble'&&!aifsEuropeEnsembleContractValid(weather))throw new Error('ECMWF AIFS Europa Ensemble: Bewölkungs-/Niederschlagswerte außerhalb des Open-Meteo-Vertrags');return weather
 };
