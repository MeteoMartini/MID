...
export type ForecastFusionRapidMinute15={time:string;epoch:number;precipitation:number;cape:number;convectiveInhibition:number;peakRateMmh?:number;dbzCmax?:number;rain?:number;snowfallWaterEquivalent?:number;graupelWaterEquivalent?:number;capeMu?:number;capeMuEpoch?:number;cinMu?:number;cinMuEpoch?:number;lpi?:number;lpiMax?:number;uhMax?:number;uhMaxLow?:number;uhMaxMed?:number;echoTopM?:number;hailGsp?:number;lapseRate?:number;srh?:number;windShearU?:number;windShearV?:number;updraftMax?:number;vorticityMax?:number;shortwaveNet?:number;shortwaveDirect?:number;shortwaveDiffuse?:number;temperature?:number;dewPoint?:number;humidity?:number;pressure?:number;wind?:number;gust?:number;direction?:number;cloud?:number;lowCloud?:number;midCloud?:number;highCloud?:number;visibility?:number;ceilingM?:number;freezingLevelM?:number;snowlineM?:number;surfaceTemperatureC?:number;source?:string;nativePrecipitationSeconds?:number;nativeStateSeconds?:number;nativeVisibilitySeconds?:number;nativeCeilingSeconds?:number};
...
function smoothLeadTransition(leadHours:number,startHours:number,endHours:number,startWeight:number,endWeight:number){const span=Math.max(.001,endHours-startHours),t=clamp((leadHours-startHours)/span,0,1),smooth=t*t*(3-2*t);return startWeight+(endWeight-startWeight)*smooth}
export function mosmixHourlyLeadStrength(leadHours:number){const lead=Math.max(0,Number(leadHours)||0);if(lead<=3)return .18;if(lead<12)return smoothLeadTransition(lead,3,12,.18,.38);if(lead<=42)return .38;if(lead<60)return smoothLeadTransition(lead,42,60,.38,.3);if(lead<=108)return .3;if(lead<132)return smoothLeadTransition(lead,108,132,.3,.16);return .16}
...
type HourlyDayFusionAdjustment={temperatureScale:number;temperatureOffset:number;windScale:number;gustScale:number};
function dayFusionAdjustment(base:Day|undefined,fused:Day|undefined):HourlyDayFusionAdjustment|undefined{if(!base||!fused||base===fused)return undefined;const baseCenter=(base.max+base.min)/2,fusedCenter=(fused.max+fused.min)/2,baseRange=Math.max(2,base.max-base.min),temperatureScale=clamp((fused.max-fused.min)/baseRange,.78,1.24),temperatureOffset=fusedCenter-baseCenter*temperatureScale,windScale=base.wind>=2?clamp(fused.wind/base.wind,.72,1.38):1,gustScale=base.gust>=3?clamp(fused.gust/base.gust,.72,1.38):1;return{temperatureScale,temperatureOffset,windScale,gustScale}}
function blendDayFusionAdjustment(a:HourlyDayFusionAdjustment,b:HourlyDayFusionAdjustment,t:number):HourlyDayFusionAdjustment{const f=clamp(t,0,1),mix=(x:number,y:number)=>x+(y-x)*f;return{temperatureScale:mix(a.temperatureScale,b.temperatureScale),temperatureOffset:mix(a.temperatureOffset,b.temperatureOffset),windScale:mix(a.windScale,b.windScale),gustScale:mix(a.gustScale,b.gustScale)}}
function localHourFraction(time:string){const match=/T(\d{2})(?::(\d{2}))?/.exec(time);if(!match)return 12;return clamp(Number(match[1])+(Number(match[2])||0)/60,0,23.999)}
function temporalDayFusionAdjustment(time:string,date:string,orderedDates:string[],adjustments:Map<string,HourlyDayFusionAdjustment>):HourlyDayFusionAdjustment|undefined{const current=adjustments.get(date);if(!current)return undefined;const index=orderedDates.indexOf(date),hour=localHourFraction(time);if(index<0)return current;if(hour<12&&index>0){const previous=adjustments.get(orderedDates[index-1]);if(previous)return blendDayFusionAdjustment(previous,current,(hour+12)/24)}if(hour>=12&&index<orderedDates.length-1){const next=adjustments.get(orderedDates[index+1]);if(next)return blendDayFusionAdjustment(current,next,(hour-12)/24)}return current}
...
export type RapidThunderRisk={
 ...
 diagnostics:{
  ...
  muIngredientFresh:boolean;
  muIngredientAgeMinutes?:number;
 };
};
...
function rapidTimedIngredient(value:unknown,valueEpoch:unknown,sampleEpoch:unknown,maxAgeMinutes=35){const numeric=rapidThunderFinite(value),valid=rapidThunderFinite(valueEpoch),target=rapidThunderFinite(sampleEpoch),ageMinutes=Number.isFinite(valid)&&Number.isFinite(target)?Math.abs(valid-target)/60000:Number.POSITIVE_INFINITY;return{value:Number.isFinite(numeric)&&ageMinutes<=maxAgeMinutes?numeric:Number.NaN,ageMinutes:Number.isFinite(ageMinutes)?ageMinutes:undefined,fresh:Number.isFinite(numeric)&&ageMinutes<=maxAgeMinutes}}
...
export function rapidThunderRisk(sample:RapidThunderSample):RapidThunderRisk|null{
 const cape=Math.max(0,rapidThunderFinite(sample.cape,0)),sampleEpoch=rapidThunderFinite(sample.epoch),muCape=rapidTimedIngredient(sample.capeMu,sample.capeMuEpoch,sampleEpoch),muCin=rapidTimedIngredient(sample.cinMu,sample.cinMuEpoch,sampleEpoch),capeMu=muCape.fresh?Math.max(cape,Math.max(0,muCape.value)):cape,cin=minimumFinite(Math.abs(rapidThunderFinite(sample.convectiveInhibition)),muCin.fresh?Math.abs(muCin.value):Number.NaN),effectiveCin=Number.isFinite(cin)?cin:120,muIngredientFresh=muCape.fresh||muCin.fresh,muIngredientAgeMinutes=Math.min(muCape.ageMinutes??Number.POSITIVE_INFINITY,muCin.ageMinutes??Number.POSITIVE_INFINITY);
 ...
 if(instabilitySupport>=.2)signals.push(`${muCape.fresh&&capeMu>cape?'MU-CAPE':'ML-CAPE'} ${Math.round(capeMu)} J/kg`);
...
// Niederschlagsmengen und -wahrscheinlichkeiten sind bei Open-Meteo und DWD/MOSMIX
// rückblickende Akkumulationsintervalle: der Zeitstempel bezeichnet das INTERVALLENDE.
...
export function blendRadarAtTarget({radar,targetEpoch,intervalMinutes,modelAmount,modelProbability,now=Date.now()}:RadarTargetBlendInput):RadarTargetBlend|null{
 ...
 if(nearbyFrames.length){const nearbyPeak=nearbyFrames.reduce((maximum,frame)=>Math.max(maximum,radarFinite(frame.nearbyRate)),0),localProbability=clamp(Math.min(55,Math.max(rateProbability(nearbyPeak,nearby,true),radarFinite(radar.radarProbability)*.55)),0,55),coverage=clamp(frames.length/expectedFrames,0,1),probabilityWeight=clamp(probabilityBase(radar)*probabilityFactor*(leadMinutes<=DIRECT_RADAR_HORIZON_MINUTES?(.72+.20*coverage):.42),0,leadMinutes<=DIRECT_RADAR_HORIZON_MINUTES?.90:.48),probability=safeModelProbability*(1-probabilityWeight)+localProbability*probabilityWeight,siteDryAmountWeight=leadMinutes<=DIRECT_RADAR_HORIZON_MINUTES?clamp(qualityBase(radar)*amountFactor*(.76+.19*coverage),0,.90):0,amount=safeModelAmount*(1-siteDryAmountWeight);
  // Nahe Echos dürfen die Eintrittswahrscheinlichkeit stützen, sind aber KEIN Standorttreffer.
  // Innerhalb des direkten Nowcast-Horizonts wird deshalb eine nasse NWP-Stundenmenge
  // am lokal trockenen Radarweg gedämpft, statt unverändert durchzureichen.
  return{amount:Math.max(0,amount),probability:clamp(probability,0,100),radarRateMmh:nearbyPeak,radarWeight:siteDryAmountWeight,probabilityWeight,mode:'proximity',hitClass:'nearby',nearbyOnly:true,nearestWetKm,siteSupport:0,frameCount:frames.length,siteFrameCount:0,interrupted,intervalStartAt:iso.startAt,intervalEndAt:iso.endAt}}
 const exactDryCoverage=frames.length>0&&dryFrames.length===frames.length&&!siteIntervals.length;
 if(exactDryCoverage&&probabilityFactor>0){const localDryProbability=0,dryWeight=clamp(probabilityBase(radar)*probabilityFactor*(frames.length>=Math.max(1,expectedFrames-1)?1:.72),0,.95),blendedProbability=safeModelProbability*(1-dryWeight)+localDryProbability*dryWeight,probability=safeModelProbability<=5?0:blendedProbability,amountWeight=leadMinutes<=DIRECT_RADAR_HORIZON_MINUTES?clamp(Math.max(dryWeight,qualityBase(radar)*amountFactor*.88),0,.95):0,amount=safeModelAmount*(1-amountWeight);
  // Kein Sonderweg für große Modellmengen: Gerade konvektive NWP-Ausreißer müssen im
  // 0–2-h-Fenster an einer belastbaren trockenen Radar-Strecke korrigiert werden.
  return{amount:Math.max(0,amount),probability:clamp(probability,0,100),radarWeight:amountWeight,probabilityWeight:dryWeight,mode:'dry',hitClass:'dry',nearestWetKm,siteSupport:0,frameCount:frames.length,siteFrameCount:0,interrupted,intervalStartAt:iso.startAt,intervalEndAt:iso.endAt}}
 return null;
}
...
export function applyHyperlocalForecastHours(hours:Hour[],anchor:ForecastLocalAnchor|undefined,now=Date.now(),referenceHours:Hour[]=hours){
 ...
 const modelNow=nearestForecastHour(referenceHours,now,90*60000),modelNowPrecipitation=trailingAccumulationHour(referenceHours,now);if(!modelNow)return hours;
...
export function finalizeForecastMinute15(minutes15:Minute15[],baseHours:Hour[],finalHours:Hour[],options:ForecastMinute15FinalizationOptions={}):Minute15[]{
 ...
 const result=minutes15.map(row=>{
  const baseHour=nearestForecastHour(baseHours,row.epoch,45*60000),finalHour=nearestForecastHour(finalHours,row.epoch,45*60000),baseAccumulationHour=trailingAccumulationHour(baseHours,row.epoch)??baseHour,finalAccumulationHour=trailingAccumulationHour(finalHours,row.epoch)??finalHour;let precipitation=Math.max(0,Number(row.precipitation)||0),rain=Math.max(0,Number(row.rain)||0),showers=Math.max(0,Number(row.showers)||0),snowfall=Math.max(0,Number(row.snowfall)||0),probability=clamp(Number(row.probability)||0,0,100),code=Math.round(Number(row.code)||0);
  if(baseAccumulationHour&&finalAccumulationHour){const probabilityDelta=Number(finalAccumulationHour.probability)-Number(baseAccumulationHour.probability);if(Number.isFinite(probabilityDelta)){if(probabilityDelta>=0)probability=clamp(probability+probabilityDelta,0,100);else if(baseAccumulationHour.probability>0)probability=clamp(probability*(Math.max(0,finalAccumulationHour.probability)/baseAccumulationHour.probability),0,100)}if(finalHour&&baseHour&&finalHour.code!==baseHour.code&&finalAccumulationHour.probability>=Math.max(30,probability-5))code=finalHour.code}
  const rucRapid=nearestForecastHour(options.rucRapidMinutes15??[],row.epoch,8*60000);if(rucRapid){const leadHours=(row.epoch-now)/3600000,weight=leadHours<=2?.38:leadHours<=4?.62:leadHours<=6?.50:0;if(weight>0){const rapidAmount=Math.max(0,Number(rucRapid.precipitation)||0),nextAmount=precipitation+(rapidAmount-precipitation)*weight,total=rain+showers+snowfall,scale=total>.001?nextAmount/total:1;precipitation=nextAmount;if(total>.001){rain*=scale;showers*=scale;snowfall*=scale}else if(finalHour?.temperature!=null&&finalHour.temperature<=1)snowfall=nextAmount;else rain=nextAmount;const rapidRain=Number(rucRapid.rain),rapidSnow=Number(rucRapid.snowfallWaterEquivalent),phaseTotal=(Number.isFinite(rapidRain)?Math.max(0,rapidRain):0)+(Number.isFinite(rapidSnow)?Math.max(0,rapidSnow):0);if(phaseTotal>.001&&nextAmount>.001){const snowShare=Math.max(0,rapidSnow)/phaseTotal;snowfall=nextAmount*snowShare;rain=nextAmount*(1-snowShare);showers=0}const peak=Math.max(0,Number(rucRapid.peakRateMmh)||0),cape=Math.max(0,Number(rucRapid.cape)||0),cin=Math.max(0,Number(rucRapid.convectiveInhibition)||0),dbz=Number(rucRapid.dbzCmax),baseConvective=clamp((cape-250)/1200,0,1)*clamp((220-cin)/200,.08,1)*Math.max(clamp(peak/20,0,1),Number.isFinite(dbz)?clamp((dbz-28)/25,0,1):0),rapidThunder=rapidThunderRisk(rucRapid),convectiveSupport=Math.max(baseConvective,rapidThunder?.support??0),rapidDry=rapidAmount<=.005&&peak<=.05&&(!Number.isFinite(dbz)||dbz<12)&&convectiveSupport<.12;if(rapidDry)probability=clamp(probability*(1-weight*.55),0,100);else probability=Math.max(probability,clamp((rapidAmount>.02?35:10)+rapidAmount*80+convectiveSupport*35,0,92),rapidThunder?.signalScore??0);if(rapidThunder&&(rapidThunder.level==='likely'||rapidThunder.level==='high')&&precipitation>.08&&![68,69,71,73,75,76,77,78,79,83,84,85,86,87,88,89,90,93,94,96,99].includes(code))code=rapidThunderForecastCode(code,rapidThunder);else if(convectiveSupport>.55&&precipitation>.08&&![68,69,71,73,75,76,77,78,79,83,84,85,86,87,88,89,90,93,94,96,99].includes(code))code=Math.max(code,82)}}
  const nativeState=Boolean(rucRapid&&Number(rucRapid.nativeStateSeconds)===900),stateLeadHours=Math.max(0,(row.epoch-now)/3600000),stateWeight=nativeState?(stateLeadHours<=2?.86:stateLeadHours<=4?.74:stateLeadHours<=6?.62:0):0,stateTemperature=stateWeight?rapidStateBlend(finalHour?.temperature,rucRapid?.temperature,stateWeight,4,-80,65):row.temperature,stateDewPoint=stateWeight?rapidStateBlend(finalHour?.dewPoint,rucRapid?.dewPoint,stateWeight,5,-90,50):row.dewPoint,stateHumidity=stateWeight?rapidStateBlend(finalHour?.humidity,rucRapid?.humidity,stateWeight,35,0,100):row.humidity,statePressure=stateWeight?rapidStateBlend(finalHour?.pressure,rucRapid?.pressure,stateWeight,8,850,1100):row.pressure,stateWind=stateWeight?rapidStateBlend(finalHour?.wind,rucRapid?.wind,stateWeight,12,0,180):row.wind,stateGust=stateWeight?rapidStateBlend(finalHour?.gust,rucRapid?.gust,stateWeight,18,0,220):row.gust,stateDirection=stateWeight?rapidStateDirection(finalHour?.direction,rucRapid?.direction,stateWeight):row.direction,stateCloud=stateWeight?rapidStateBlend(finalHour?.cloud,rucRapid?.cloud,stateWeight,100,0,100):row.cloud,stateLowCloud=stateWeight?rapidStateBlend(finalHour?.lowCloud,rucRapid?.lowCloud,stateWeight,100,0,100):row.lowCloud,stateMidCloud=stateWeight?rapidStateBlend(finalHour?.midCloud,rucRapid?.midCloud,stateWeight,100,0,100):row.midCloud,stateHighCloud=stateWeight?rapidStateBlend(finalHour?.highCloud,rucRapid?.highCloud,stateWeight,100,0,100):row.highCloud,stateVisibility=stateWeight?rapidStateBlend(finalHour?.visibility,rucRapid?.visibility,stateWeight,80000,50,100000):row.visibility,stateCeiling=stateWeight?rapidStateBlend(finalHour?.ceiling,rucRapid?.ceilingM,stateWeight,12000,0,15240):row.ceiling,stateFreezingLevel=stateWeight?rapidStateBlend(finalHour?.freezingLevel,rucRapid?.freezingLevelM,stateWeight,5000,0,10000):row.freezingLevel,stateSnowline=stateWeight?rapidStateBlend(finalHour?.snowline,rucRapid?.snowlineM,stateWeight,5000,0,10000):row.snowline,stateSurfaceTemperature=stateWeight?rapidStateBlend(finalHour?.surfaceTemperature,rucRapid?.surfaceTemperatureC,stateWeight,8,-90,70):row.surfaceTemperature;
  const radarBlend=blendRadarAtTarget({radar:options.radar,targetEpoch:row.epoch,intervalMinutes:15,modelAmount:precipitation,modelProbability:probability,now});if(radarBlend){const componentTotal=rain+showers+snowfall,scale=componentTotal>.001?radarBlend.amount/componentTotal:1;precipitation=radarBlend.amount;probability=radarBlend.probability;if(radarBlend.mode==='dry'&&radarBlend.radarWeight>=.6){precipitation=0;rain=0;showers=0;snowfall=0;code=dryMinute15SkyCode(row,finalHour)}else if(componentTotal>.001){rain*=scale;showers*=scale;snowfall*=scale}else if(finalHour?.temperature!=null&&finalHour.temperature<=1)snowfall=precipitation;else rain=precipitation}
  // Ein Modellcode ist keine ortsfeste Beobachtung. Bei 0,0 mm bleibt eine Restwahrscheinlichkeit
  // sichtbar, aber die 90-Minuten-Karte darf daraus keinen Regen erzeugen oder ikonisieren.
  if(precipitation<=.005&&probability<30&&precipitationWeatherCode(code)){rain=0;showers=0;snowfall=0;code=dryMinute15SkyCode(row,finalHour)}
  if(Number(anchorPrecipitation)>.01){const offsetMinutes=Math.max(0,(row.epoch-now)/60000),weight=localAdjustmentWeight(offsetMinutes,45),observedRate=Number(anchorPrecipitation)*60/Math.max(1,Number(anchor?.precipitationMinutes)||60),observedAmount=Math.max(0,observedRate*.25),modelNow=baseAccumulationHour?Math.max(0,baseAccumulationHour.precipitation*.25):precipitation,localAmount=localAssimilatedValue(observedAmount,modelNow,precipitation,offsetMinutes,45,0,80,20);if(localAmount>precipitation){const total=rain+showers+snowfall,scale=total>.001?localAmount/total:1;precipitation=localAmount;if(total>.001){rain*=scale;showers*=scale;snowfall*=scale}else if(finalHour?.temperature!=null&&finalHour.temperature<=1)snowfall=localAmount;else rain=localAmount}probability=Math.max(probability,90*weight);if(Number.isFinite(Number(anchorCode))&&offsetMinutes<=30)code=Math.round(Number(anchorCode))}
  const isDay=row.isDay??finalHour?.isDay,signal=reconcileForecastPrecipitation({precipitation,rain,showers,snowfall,probability,code,intervalSeconds:15*60,humidity:finalHour?.humidity,cloud:finalHour?.cloud,lowCloud:finalHour?.lowCloud,cape:finalHour?.cape,liftedIndex:finalHour?.liftedIndex,convectiveInhibition:finalHour?.convectiveInhibition,sunshineDuration:row.sunshineDuration??finalHour?.sunshineDuration,isDay,leadHours:(row.epoch-now)/3600000}),sunshineDuration=coherentSunshineDurationSeconds({valueSeconds:row.sunshineDuration,intervalSeconds:15*60,daylightSeconds:forecastIntervalDaylightSeconds(row.epoch,15*60,finalHour?.sunriseEpoch,finalHour?.sunsetEpoch),isDay,weatherCode:signal.code,precipitation:signal.precipitation,rain:signal.rain,showers:signal.showers,snowfall:signal.snowfall,precipitationProbability:signal.probability,cloudCover:finalHour?.cloud,lowCloudCover:finalHour?.lowCloud}),next={...row,precipitation:signal.precipitation,rain:signal.rain,showers:signal.showers,snowfall:signal.snowfall,probability:signal.probability,code:signal.code,temperature:stateTemperature,dewPoint:stateDewPoint,humidity:stateHumidity,pressure:statePressure,wind:stateWind,gust:stateGust,direction:stateDirection,cloud:stateCloud,lowCloud:stateLowCloud,midCloud:stateMidCloud,highCloud:stateHighCloud,visibility:stateVisibility,ceiling:stateCeiling,freezingLevel:stateFreezingLevel,snowline:stateSnowline,surfaceTemperature:stateSurfaceTemperature,nativeStateSeconds:stateWeight?900:row.nativeStateSeconds,isDay:row.isDay??finalHour?.isDay,sunshineDuration};if(!stateWeight&&Math.abs(next.precipitation-row.precipitation)<.001&&Math.abs(next.probability-row.probability)<.1&&next.code===row.code&&next.sunshineDuration===row.sunshineDuration)return row;changed=true;return next
...
function reconcileForecastHourPrecipitation(hour:Hour){
 ...
 const sunshineDuration=coherentSunshineDurationSeconds({valueSeconds:hour.sunshineDuration,intervalSeconds:3600,daylightSeconds:forecastIntervalDaylightSeconds(hour.epoch,3600,hour.sunriseEpoch,hour.sunsetEpoch),isDay:hour.isDay,weatherCode:signal.code,precipitation:signal.precipitation,rain:signal.rain,showers:signal.showers,snowfall:signal.snowfall,precipitationProbability:signal.probability,cloudCover:hour.cloud,lowCloudCover:hour.lowCloud});
 if(signal.precipitation===hour.precipitation&&signal.rain===hour.rain&&signal.showers===hour.showers&&signal.snowfall===hour.snowfall&&signal.probability===hour.probability&&signal.code===hour.code&&sunshineDuration===hour.sunshineDuration)return hour;
 return{...hour,precipitation:signal.precipitation,rain:signal.rain,showers:signal.showers,snowfall:signal.snowfall,probability:signal.probability,code:signal.code,sunshineDuration};
}
...
export function applyForecastFusionHours(hours:Hour[],baseDays:Day[],fusedDays:Day[],fusion?:ForecastFusionResult|null){
 if(hours.length===0)return hours;
 const dayAdjustment=baseDays!==fusedDays,mosmixHours=Array.isArray(fusion?.hours)?fusion.hours:[],weatherHours=Array.isArray(fusion?.weatherHours)?fusion.weatherHours:[],mosmixQuality=clamp(Number(fusion?.mosmix?.quality)||0,0,1),mosmixUsable=Boolean(fusion?.active&&fusion?.mosmix?.available&&fusion?.mosmix?.applied&&mosmixHours.length&&mosmixQuality>=.42),baseByDate=new Map(baseDays.map(day=>[day.date,day])),fusedByDate=new Map(fusedDays.map(day=>[day.date,day])),orderedAdjustmentDates=baseDays.map(day=>day.date).filter(date=>fusedByDate.has(date)).sort(),dayAdjustments=new Map<string,HourlyDayFusionAdjustment>(orderedAdjustmentDates.map(date=>[date,dayFusionAdjustment(baseByDate.get(date),fusedByDate.get(date))]).filter((entry):entry is [string,HourlyDayFusionAdjustment]=>Boolean(entry[1])));let changed=false;
...
export function dryRadarNowcastProbability(modelProbability:number,radar:RadarNowcast|null|undefined,leadMinutes=0):DryRadarNowcastProbabilityBlend|null{
 ...
 const directFactor=minutes<=120?clamp(1-minutes/180,.33,1):clamp(1-(minutes-120)/Math.max(1,dryHorizon-120),0,.33),radarWeight=(radar.quality==='high'?.94:radar.quality==='medium'?.82:.58)*directFactor,safeModelProbability=clamp(Number(modelProbability)||0,0,100),probability=safeModelProbability<=5&&radarProbability<=5?0:clamp(safeModelProbability*(1-radarWeight)+radarProbability*radarWeight,0,100);
...
export function reconcileForecastDaysWithHours(days:Day[],hours:Hour[]){
 ...
 const now=Date.now(),timezone=hours.find(hour=>hour.timezone)?.timezone||'UTC',today=dateKeyInTimezone(now,timezone),displayHours=precipitationPresentationHours(hours),hoursByDate=new Map<string,Hour[]>();
 for(const hour of displayHours){const date=hour.time.slice(0,10),bucket=hoursByDate.get(date);if(bucket)bucket.push(hour);else hoursByDate.set(date,[hour])}
 let changed=false;const result=days.map(day=>{
  const relevant=hoursByDate.get(day.date);if(!relevant?.length)return day;
  const isCurrentDay=day.date===today,futureFraction=(hour:Hour)=>{const start=Number(hour.epoch),end=start+3600000;if(!Number.isFinite(start)||end<=now)return 0;if(start>=now)return 1;return clamp((end-now)/3600000,0,1)},futureRelevant=relevant.filter(hour=>futureFraction(hour)>0),nearTerm=isCurrentDay&&futureRelevant.some(hour=>hour.epoch<=now+6*3600000),precipitationHours=nearTerm?futureRelevant:relevant,precipitationFraction=(hour:Hour)=>nearTerm?futureFraction(hour):1,completeCoverage=relevant.length>=DAY_HOURLY_FULL_COVERAGE_MIN_HOURS,temperatures=relevant.map(hour=>Number(hour.temperature)).filter(Number.isFinite),hourlyMax=temperatures.length?Math.max(...temperatures):Number.NaN,hourlyMin=temperatures.length?Math.min(...temperatures):Number.NaN;
