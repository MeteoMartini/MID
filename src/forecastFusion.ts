...
export function finalizeForecastMinute15(minutes15:Minute15[],baseHours:Hour[],finalHours:Hour[],options:ForecastMinute15FinalizationOptions={}):Minute15[]{
 ...
 const result=minutes15.map(row=>{
  ...
  const stateLeadHours=Math.max(0,(row.epoch-now)/3600000),rapidStateWeight=stateLeadHours<=2?.86:stateLeadHours<=4?.74:stateLeadHours<=6?.62:0,visibilityWeight=rucRapid&&Number(rucRapid.nativeVisibilitySeconds)===900?rapidStateWeight:0,ceilingWeight=rucRapid&&Number(rucRapid.nativeCeilingSeconds)===900?rapidStateWeight:0,stateVisibility=visibilityWeight?rapidStateBlend(finalHour?.visibility,rucRapid?.visibility,visibilityWeight,80000,50,100000):row.visibility,stateCeiling=ceilingWeight?rapidStateBlend(finalHour?.ceiling,rucRapid?.ceilingM,ceilingWeight,12000,0,15240):row.ceiling;
