...
export function MidWeatherThread({points=[],label='Temperaturverlauf',className='',minimumSpan=4,verticalGridDivisions=2,horizontalInset=0,temperatureColorMode='solid',markers='both'}:{points:number[];label?:string;className?:string;minimumSpan?:number;verticalGridDivisions?:number;horizontalInset?:number;temperatureColorMode?:'solid'|'ecmwf';markers?:'none'|'start'|'both'}){
 ...
 return <svg className={`mid-weather-thread${className?` ${className}`:''}`} viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label={label}><g className="mid-weather-thread-grid" aria-hidden="true">{gridY.map(value=><line key={`h-${value}`} x1={safeInset} y1={value} x2={100-safeInset} y2={value}/>)}{gridX.map((value,index)=><line key={`v-${index}`} x1={value} y1="8" x2={value} y2="92"/>)}</g><line className="mid-weather-thread-guide" x1={safeInset} y1={guideY} x2={100-safeInset} y2={guideY}/>{temperatureColorMode==='ecmwf'?<g className="mid-weather-thread-temperature-ecmwf">{colorSegments.map(segment=><line key={segment.index} x1={segment.x1} y1={segment.y1} x2={segment.x2} y2={segment.y2} stroke={segment.color}/>)}</g>:segments.map((path,index)=><path key={index} d={path}/>)}
