...
function ShortTermRibbon({hours,minutes15,timezone,unit,onSelectedDate,anchor,radarNowcast}:{hours:Hour[];minutes15:Minute15[];timezone:string;unit:WindUnit;onSelectedDate:(date:string)=>void;anchor?:ShortTermAnchor;radarNowcast?:RadarNowcast}){
 ...
 return <div className="cockpit-short-term" data-hyperlocal={locallyAdjusted?'true':'false'}>
  ...
  <div className="cockpit-short-insight" aria-label="Kurzfristkompass für die nächsten Stunden">
   <div className="cockpit-short-insight-grid">
    <article className="cockpit-short-spotlight narrative"><small>Nächste Stunden</small><b>{previewSummary}</b><span>{resolution==='1h'?'Stündliches Raster im Diagramm aktiv.':'Grafik verdichtet, Vorschau darunter stündlich.'}</span></article>
   ...
   <div className="cockpit-hourly-preview" role="list" aria-label="Stündliche Kurzfristvorschau">{previewPoints.map(point=>{const precipFill=Math.max(10,Math.round(clamp(point.probability,0,100))),precipStyle={width:`${precipFill}%`,background:precipitationColor({precipitation:point.precipitation,rain:point.precipitation,showers:0,snowfall:0,probability:point.probability,code:point.code,time:'',epoch:point.epoch,timezone})} as CSSProperties;return <article key={point.epoch} className="cockpit-hourly-chip" role="listitem" title={`${point.timeLabel}: ${point.weatherLabel} · ${Math.round(point.temperature)} °C · ${formatDecimalFixed(point.precipitation,1)} mm · ${Math.round(point.probability)} % · Wind ${wind(point.wind,unit)}, Böen ${wind(point.gust,unit)}`}><time>{point.timeLabel}</time><span className="cockpit-hourly-chip-head"><WeatherPictogram code={point.code} day={point.isDay} size={24} title={point.weatherLabel} cloud={point.cloud} lowCloud={point.lowCloud} midCloud={point.midCloud} highCloud={point.highCloud}/><b>{Math.round(point.temperature)}°</b></span><small>{shortTermCompactWeatherLabel(point.weatherLabel)}</small><span className="cockpit-hourly-rain"><i/><b style={precipStyle}/><small>{Math.round(point.probability)} %</small></span><span className="cockpit-hourly-wind"><InlineWindArrow direction={point.direction} gust={point.gust} size={14}/><small>{wind(point.wind,unit)}</small></span></article>})}</div>
  </div>
 </div>
}
