...
function ShortTermRibbon({hours,minutes15,timezone,unit,onSelectedDate,anchor,radarNowcast}:{hours:Hour[];minutes15:Minute15[];timezone:string;unit:WindUnit;onSelectedDate:(date:string)=>void;anchor?:ShortTermAnchor;radarNowcast?:RadarNowcast}){
 ...
 const previewPoints=hourlyPoints.slice(0,Math.min(12,hourlyPoints.length)),timelinePoints=hourlyPoints.slice(0,Math.min(24,hourlyPoints.length)),previewLead=previewPoints[0]??points[0],previewTail=previewPoints.at(-1)??previewLead,timelineLead=timelinePoints[0]??previewLead,timelineTail=timelinePoints.at(-1)??previewTail,peakGustPoint=previewPoints.reduce((best,item)=>item.gust>best.gust?item:best,previewLead),peakRainPoint=previewPoints.reduce((best,item)=>item.precipitation>best.precipitation?item:best,previewLead),peakWarmPoint=previewPoints.reduce((best,item)=>item.temperature>best.temperature?item:best,previewLead),coolestPoint=previewPoints.reduce((best,item)=>item.temperature<best.temperature?item:best,previewLead),firstWetPoint=previewPoints.find(item=>(item.precipitation>=.05&&item.probability>=25)||item.probability>=60),trendLabel=shortTermTrendLabel(previewPoints),previewSummary=shortTermPointSummary(previewPoints,unit);
 const width=Math.max(resolution==='1h'?780:700,points.length*(resolution==='1h'?74:84)),height=254,left=48,right=20,top=34,bottom=height-72,plotWidth=width-left-right,x=(index:number)=>left+(points.length===1?plotWidth/2:index/(points.length-1))*plotWidth;
 ...
 const precipMax=Math.max(.2,...points.map(point=>Math.max(point.precipitation,.05))),yPrecip=(value:number)=>bottom-clamp(value,0,precipMax)/precipMax*44;
 const line=points.map((point,index)=>`${index?'L':'M'}${x(index).toFixed(1)} ${yTemp(point.temperature).toFixed(1)}`).join(' '),baselineY=yTemp(referenceTemperature),barWidth=Math.max(12,Math.min(24,plotWidth/Math.max(1,points.length)*.38));
 return <div className="cockpit-short-term" data-hyperlocal={locallyAdjusted?'true':'false'}>
  ...
  <div className="cockpit-chart-wrap" data-cockpit-horizontal-scroll="true"><svg className="cockpit-short-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Kurzfristige Temperatur-, Niederschlags- und Windsignale">
   <rect x={left} y={top-16} width={plotWidth} height={bottom-top+24} rx="12" fill="currentColor" opacity=".025"/><line x1={left} x2={width-right} y1={bottom} y2={bottom} stroke="currentColor" opacity=".22"/><line x1={left} x2={left} y1={top} y2={bottom} stroke="currentColor" opacity=".22"/><text x={left} y={top-18} fontSize="11" fontWeight="700" fill="currentColor" opacity=".72">Temperatur °C</text><text x={width-right} y={height-14} textAnchor="end" fontSize="10" fill="currentColor" opacity=".62">Windrichtung · Zeit</text><line x1={left} x2={width-right} y1={baselineY} y2={baselineY} stroke="#ff9a62" opacity=".45" strokeDasharray="5 4"/>
   ...
   {points.map((point,index)=>{const barTop=yPrecip(Math.max(point.precipitation,.02)),pointX=x(index),iconY=top-2,tempCurveY=yTemp(point.temperature),tempLabelBelow=tempCurveY<top+26,tempLabelY=tempLabelBelow?Math.min(bottom-12,tempCurveY+18):tempCurveY-10,arrowY=height-42,timeY=height-18,hitWidth=plotWidth/points.length;return <g key={point.epoch} opacity={index>=Math.ceil(points.length*.72)?.72:1}><line x1={pointX} x2={pointX} y1={bottom-2} y2={bottom+6} stroke="currentColor" opacity=".14"/><rect x={pointX-barWidth/2} y={barTop} width={barWidth} height={Math.max(4,bottom-barTop)} rx="4" fill={precipitationColor({precipitation:point.precipitation,rain:point.precipitation,showers:0,snowfall:0,probability:point.probability,code:point.code,time:'',epoch:point.epoch,timezone})} opacity={Math.max(.18,Math.min(.95,point.probability/100))}/><WeatherPictogram code={point.code} day={point.isDay} size={28} x={pointX-14} y={iconY} title={point.weatherLabel} cloud={point.cloud} lowCloud={point.lowCloud} midCloud={point.midCloud} highCloud={point.highCloud}/><SvgWindDirectionArrow x={pointX} y={arrowY} direction={point.direction} size={14} color={windSignalColor(point.gust)}/><text x={pointX} y={tempLabelY} textAnchor="middle" dominantBaseline={tempLabelBelow?'hanging':'auto'} fontSize="12" fontWeight="700" fill="currentColor">{Math.round(point.temperature)}°</text><text x={pointX} y={timeY} textAnchor="middle" fontSize="11" fill="currentColor" opacity=".72">{point.timeLabel}</text><rect x={pointX-hitWidth/2} y={top-14} width={hitWidth} height={bottom-top+70} fill="transparent"><title>{`${point.timeLabel} · ${Math.round(point.temperature)} °C · ${formatDecimalFixed(point.precipitation,1)} mm · ${Math.round(point.probability)} % · ${wind(point.wind,unit)} / ${wind(point.gust,unit)}`}</title></rect></g>})}
   <path d={line} fill="none" stroke="#ff7a37" strokeWidth="3" vectorEffect="non-scaling-stroke"/>
  </svg></div>
  <div className="cockpit-short-insight" aria-label="Kurzfristkompass für die nächsten Stunden">
   <div className="cockpit-short-insight-grid premium">
    <article className="cockpit-short-spotlight narrative"><small>Nächste Stunden</small><b>{previewSummary}</b><span>{resolution==='1h'?'Stündliches Raster im Diagramm aktiv.':'Diagramm verdichtet, 24-h-Leiste darunter stündlich.'}</span></article>
    <article className="cockpit-short-spotlight"><small>Temperaturspanne</small><b>{Math.round(coolestPoint.temperature)}° / {Math.round(peakWarmPoint.temperature)}°</b><span>{trendLabel} bis {previewTail.timeLabel}</span></article>
    <article className="cockpit-short-spotlight"><small>Windspitze</small><b>{wind(peakGustPoint.gust,unit)}</b><span>{peakGustPoint.timeLabel} · {cardinal(peakGustPoint.direction)}</span></article>
    <article className="cockpit-short-spotlight"><small>Niederschlag</small><b>{firstWetPoint?`ab ${firstWetPoint.timeLabel}`:'vorerst trocken'}</b><span>max. {Math.round(Math.max(...points.map(item=>item.probability)))} % · bis {formatDecimalFixed(peakRainPoint.precipitation,1)} mm</span></article>
   </div>
   <section className="cockpit-hourly-preview-shell" aria-label="Stündliche 24-Stunden-Vorschau">
    <header className="cockpit-hourly-preview-head"><span><small>24-Stunden-Leiste</small><b>{formatDate(dateOnlyFromEpoch(timelineLead.epoch,timezone),{weekday:'long',day:'2-digit',month:'2-digit'})} · {timelineLead.timeLabel}–{timelineTail.timeLabel}</b></span><em>{timelinePoints.length>=18?'Seitlich wischbar':'Komprimierte Stundenübersicht'}</em></header>
    <div className="cockpit-hourly-preview" role="list" aria-label="Stündliche Kurzfristvorschau">{timelinePoints.map((point,index)=>{const pointDate=dateOnlyFromEpoch(point.epoch,timezone),previousDate=index?dateOnlyFromEpoch(timelinePoints[index-1].epoch,timezone):'',showDateMarker=!index||pointDate!==previousDate,precipFill=Math.max(8,Math.round(clamp(point.probability,0,100))),precipStyle={width:`${precipFill}%`,background:precipitationColor({precipitation:point.precipitation,rain:point.precipitation,showers:0,snowfall:0,probability:point.probability,code:point.code,time:'',epoch:point.epoch,timezone})} as CSSProperties;return <article key={point.epoch} className={`cockpit-hourly-chip${point.precipitation>=.05||point.probability>=45?' wet':''}${point.isDay?' day':' night'}`} role="listitem" title={`${formatDate(pointDate,{weekday:'long',day:'2-digit',month:'2-digit'})} ${point.timeLabel}: ${point.weatherLabel} · ${Math.round(point.temperature)} °C · ${formatDecimalFixed(point.precipitation,1)} mm · ${Math.round(point.probability)} % · Wind ${wind(point.wind,unit)}, Böen ${wind(point.gust,unit)}`}>
      ...
      <div className="cockpit-hourly-chip-top"><time>{point.timeLabel}</time><b>{Math.round(point.temperature)}°</b></div>
