...
function ensembleSeries(ensemble:EnsembleDay[],days:Day[]){
 const dayMap=new Map(days.map(day=>[day.date,day]));return ensemble.slice(0,14).map((item,index)=>({date:item.date,best:dayMap.get(item.date)?.max??item.maxMean,temperatureMean:item.maxMean,temperatureLow:item.maxLow,temperatureHigh:item.maxHigh,precipitationProbability:item.precipitationProbability,precipitationMean:item.precipitationMean,precipitationLow:item.precipitationLow,precipitationHigh:item.precipitationHigh,windMean:item.windMean,windLow:item.windLow,windHigh:item.windHigh,gustMean:item.gustMean,modelCount:item.modelCount,index}));
}
...
function MiniRibbon({horizon,hours,days,ensemble,timezone}:{horizon:ForecastHorizon;hours:Hour[];days:Day[];ensemble:EnsembleDay[];timezone:string}){
 ...
 const series=ensembleSeries(ensemble,days);return <span className="cockpit-mini-ribbon fourteen">{series.slice(0,14).map(item=><i key={item.date} style={{height:`${clamp(ensembleConsistency(item),16,100)}%`,opacity:item.index>=7?.55:.9}} title={`${formatDate(item.date,{weekday:'short'})}: Konsistenz ${ensembleConsistency(item)} %`}/>)}</span>
}
