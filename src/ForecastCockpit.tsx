...
function nextNinetyMinutes(minutes15:Minute15[],timezone:string){
 const now=Date.now();
 const relevant=minutes15
  ...
  .map(item=>({
   ...item,
   clock:formatClock(item.epoch,timezone),
   parts:plausiblePrecipitation(item),
   strength:Math.max(0,Math.min(100,(Math.max(0,item.probability)*.55)+Math.min(1,Math.max(0,item.precipitation)/2)*45))
  }));
 return relevant;
}
...
function ShortTermRibbon({hours,minutes15,timezone,unit,onSelectedDate}:{hours:Hour[];minutes15:Minute15[];timezone:string;unit:WindUnit;onSelectedDate:(date:string)=>void}){
 ...
 const now90=useMemo(()=>nextNinetyMinutes(minutes15,timezone),[minutes15,timezone]);
 ...
 return <div className="cockpit-short-term">
  <div className="cockpit-brief"><span><small>Kurzfrist</small><strong>{summary}</strong></span><div className="cockpit-inline-toggle" role="group" aria-label="Auflösung der Kurzfristvorhersage"><button type="button" className={resolution==='3h'?'active':''} aria-pressed={resolution==='3h'} onClick={()=>setResolution('3h')}>3 h</button><button type="button" className={resolution==='1h'?'active':''} aria-pressed={resolution==='1h'} onClick={()=>setResolution('1h')}>stündlich</button></div></div>
  {now90.length?<div className="cockpit-now90"><header><b>Nächste 90 min</b><small>{now90[0]?.clock} – {now90.at(-1)?.clock}</small></header><div className="cockpit-now90-grid">{now90.map(item=><span key={item.epoch} className={`cockpit-now90-slot${item.parts.type!=='none'?' wet':''}`} title={`${item.clock}: ${item.parts.weatherLabel} · ${formatDecimalFixed(item.precipitation,1)} mm · ${Math.round(item.probability)} %`}><small>{item.clock}</small><i style={{height:`${Math.max(10,item.strength)}%`,background:precipitationColor(item)}}/><b>{Math.round(item.probability)} %</b></span>)}</div></div>:null}
  <div className="cockpit-chart-wrap" data-cockpit-horizontal-scroll="true"><svg className="cockpit-short-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Kurzfristige Temperatur-, Niederschlags- und Windsignale">
   <line x1={left} x2={width-right} y1={baselineY} y2={baselineY} stroke="currentColor" opacity=".16" strokeDasharray="4 4"/>
   <text x={width-right} y={baselineY-6} textAnchor="end" fontSize="11" fill="currentColor" opacity=".6">Mittel {Math.round(referenceTemperature)}°</text>
...
function anomalyLabel(value:number){if(!Number.isFinite(value))return'–';return `${value>=0?'+':''}${formatDecimalFixed(value,1)}°`}
...
function SvgWindDirectionArrow({x,y,direction,size=14}:{x:number;y:number;direction:number;size?:number}){const to=((direction%360)+540)%360,scale=size/14,description=`Wind aus ${Math.round(((direction%360)+360)%360)}°`;return <g className="svg-wind-direction-arrow" transform={`translate(${x.toFixed(2)} ${y.toFixed(2)}) rotate(${to.toFixed(1)}) scale(${scale.toFixed(3)})`} role="img" aria-label={description}><title>{description}</title><line x1="0" y1="5" x2="0" y2="-5"/><path d="M -3 -2 L 0 -6 L 3 -2"/></g>}
