...
function ShortTermRibbon({hours,minutes15,timezone,unit,onSelectedDate,anchor,radarNowcast}:{hours:Hour[];minutes15:Minute15[];timezone:string;unit:WindUnit;onSelectedDate:(date:string)=>void;anchor?:ShortTermAnchor;radarNowcast?:RadarNowcast}){
 ...
 return <div className="cockpit-short-term" data-hyperlocal={locallyAdjusted?'true':'false'}>
  ...
  <div className="cockpit-focus-card short-term"><span><b>Auf einen Blick</b><small>{resolution==='3h'?'3-Stunden-Raster':'Stündlich'} · Linie = Temperaturmittel</small></span><span><b>{`Böen bis ${wind(points.reduce((best,item)=>item.gust>best.gust?item:best,points[0]).gust,unit)}`}</b><small>Windspitze</small></span><span><b>{Math.round(Math.max(...points.map(item=>item.probability)))} %</b><small>max. Niederschlagsrisiko</small></span></div>
 </div>
}
...
export function ForecastCockpit({mode,hours,minutes15,days,ensemble,scenarios,climate,timezone,unit,selectedDate,onSelectedDate,availability,sourceLabel,updatedLabel,ensembleLoading,cockpitDetails,sevenDaySummary,shortTermAnchor,radarNowcast}:ForecastCockpitProps){
