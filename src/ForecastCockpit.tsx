...
function SevenDayBand({days,hours,climate,unit,selectedDate,onSelectedDate,summary,hourlyDetail}:{days:Day[];hours:Hour[];climate:ClimateDay[];unit:WindUnit;selectedDate:string;onSelectedDate:(date:string)=>void;summary?:string;hourlyDetail?:ReactNode}){
 ...
 return <div className="cockpit-seven-day">
  ...
  <div className="cockpit-seven-grid" data-cockpit-horizontal-scroll="true" style={{'--cockpit-day-count':visible.length} as CSSProperties}>{visible.map(day=>{const dayHours=hours.filter(hour=>hour.time.startsWith(day.date)),weather=dayWeatherCharacter(day,dayHours),dayVisual=cockpitPeriodVisual(dayHours,true,weather.code,dayWeatherCharacterText(weather)),nightVisual=cockpitPeriodVisual(dayHours,false,weather.code,weather.label),left=(day.min-allMinimum)/temperatureRange*100,width=Math.max(8,(day.max-day.min)/temperatureRange*100),warning=windWarningLevel(day.gust),regime=dayRegime(day,dayHours),climateDay=climateByDate.get(day.date),minTone=dailyTemperatureTone(day.min,climateDay?.minMean,'min'),maxTone=dailyTemperatureTone(day.max,climateDay?.maxMean,'max'),isExpanded=expandedDate===day.date;return <button type="button" key={day.date} className={`cockpit-day regime-${regime}${selected.date===day.date?' active':''}${isExpanded?' hourly-open':''}`} data-regime={regimeLabel(regime)} title={`${regimeLabel(regime)} · ${dayWeatherCharacterText(weather)}`} onClick={()=>chooseDay(day.date)} aria-pressed={selected.date===day.date} aria-expanded={hourlyDetail?isExpanded:undefined}>
   ...
   <span className="cockpit-day-weather-pair"><WeatherPictogram className="cockpit-day-main-icon" code={dayVisual.code} day size={40} title={dayVisual.title} cloud={dayVisual.cloud} lowCloud={dayVisual.lowCloud} midCloud={dayVisual.midCloud} highCloud={dayVisual.highCloud}/>{nightVisual.available&&<span className="cockpit-day-night-icon"><WeatherPictogram className="cockpit-day-night-glyph" code={nightVisual.code} day={false} size={21} compact title={nightVisual.title} cloud={nightVisual.cloud} lowCloud={nightVisual.lowCloud} midCloud={nightVisual.midCloud} highCloud={nightVisual.highCloud}/></span>}</span>
