...
import {currentIndex,dayWeatherCharacter,dayWeatherCharacterText,label as weatherCodeLabel,wind,type ClimateDay,type Day,type EnsembleDay,type EnsembleScenarioCluster,type Hour,type Minute15,type WindUnit} from './weather';
...
function shortTermPointSummary(points:ShortTermForecastPoint[],unit:WindUnit){
 ...
 return `${kind} ab ${first.timeLabel} · ${gustPart}`;
}
...
function ShortTermRibbon({hours,minutes15,timezone,unit,onSelectedDate,anchor}:{hours:Hour[];minutes15:Minute15[];timezone:string;unit:WindUnit;onSelectedDate:(date:string)=>void;anchor?:ShortTermAnchor}){
 ...
 return <div className="cockpit-short-term" data-hyperlocal={locallyAdjusted?'true':'false'}>
  ...
  <div className="cockpit-chart-wrap" data-cockpit-horizontal-scroll="true"><svg className="cockpit-short-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Kurzfristige Temperatur-, Niederschlags- und Windsignale">
   ...
   {points.map((point,index)=>{const barTop=yPrecip(Math.max(point.precipitation,.02));return <g key={point.epoch} opacity={index>=Math.ceil(points.length*.72)?.72:1}><line x1={x(index)} x2={x(index)} y1={bottom-2} y2={bottom+6} stroke="currentColor" opacity=".14"/><rect x={x(index)-barWidth/2} y={barTop} width={barWidth} height={Math.max(4,bottom-barTop)} rx="4" fill={precipitationColor({precipitation:point.precipitation,rain:point.precipitation,showers:0,snowfall:0,probability:point.probability,code:point.code,time:'',epoch:point.epoch,timezone})} opacity={Math.max(.18,Math.min(.95,point.probability/100))}/><WeatherPictogram code={point.code} day={point.isDay} size={30} x={x(index)-15} y={top-10} title={point.weatherLabel}/><SvgWindDirectionArrow x={x(index)} y={bottom+24} direction={point.direction} size={14} color={windSignalColor(point.gust)}/><text x={x(index)} y={yTemp(point.temperature)-10} textAnchor="middle" fontSize="12" fontWeight="700" fill="currentColor">{Math.round(point.temperature)}°</text><text x={x(index)} y={height-18} textAnchor="middle" fontSize="11" fill="currentColor" opacity=".72">{point.timeLabel}</text><rect x={x(index)-plotWidth/points.length/2} y={top-12} width={plotWidth/points.length} height={bottom-top+40} fill="transparent"><title>{`${point.timeLabel} · ${Math.round(point.temperature)} °C · ${formatDecimalFixed(point.precipitation,1)} mm · ${Math.round(point.probability)} % · ${wind(point.wind,unit)} / ${wind(point.gust,unit)}`}</title></rect></g>})}
...
export function ForecastCockpit({mode,hours,minutes15,days,ensemble,scenarios,climate,timezone,unit,selectedDate,onSelectedDate,availability,sourceLabel,updatedLabel,ensembleLoading,details,cockpitDetails,sevenDaySummary,shortTermAnchor}:ForecastCockpitProps){
 ...
 const shortPoints=useMemo(()=>buildShortTermForecast(minutes15,hours,timezone,Date.now(),shortTermAnchor),[minutes15,hours,timezone,shortTermAnchor]);
