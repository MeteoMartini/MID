...
function maximum(values:(number|null)[]){const usable=values.filter((value):value is number=>value!=null);return usable.length?Math.max(...usable):null}
function minimum(values:(number|null)[]){const usable=values.filter((value):value is number=>value!=null);return usable.length?Math.min(...usable):null}
...
function summarizeTimeline(timeline:EventTimelinePoint[],fusion:ForecastFusionResult|null,eventProbability?:EventPrecipitationProbabilityAssessment|null,weatherTwinApplied=false):EventSummary{
 ...
 const probabilityRows=timeline.map(point=>({probability:Number(point.precipitationProbability),minutes:Number(point.durationMinutes)})).filter(row=>Number.isFinite(row.probability)&&row.probability>=0&&Number.isFinite(row.minutes)&&row.minutes>0),coveredMinutes=probabilityRows.reduce((sum,row)=>sum+row.minutes,0),windowAverageProbability=coveredMinutes>0?probabilityRows.reduce((sum,row)=>sum+row.probability*row.minutes,0)/coveredMinutes:mean(precipitationProbabilities)
 const precipitationProbabilityRelevant=eventProbability?.probability??windowAverageProbability
...
export async function buildEventPlan(options:BuildEventPlanOptions):Promise<EventPlan>{
 ...
 if(Number.isFinite(startEpoch)&&Number.isFinite(endEpoch)&&endEpoch<startEpoch)endEpoch=startEpoch+3600000
 ...
 const advice=evaluateEvent(summary,eventEnvironment,eventActivity),fusionState=forecastFusionLabel(canonicalActive?canonical!.fusion:fusion),source=[canonicalActive?'Aktive Ortsvorhersage · identische MID-Endstufe':fusionState||'Open-Meteo Best Match · gemeinsame MID-Plausibilisierung',weatherTwinApplied?'Wetterzwilling · lokal validierte Temperatur-/Böenkorrektur':'',eventProbability?`Event-Niederschlagswahrscheinlichkeit ${eventStartTime}–${eventEndTime} · Ensemble-Member >0,2 mm`:`Niederschlagswahrscheinlichkeit ${eventStartTime}–${eventEndTime} · zeitgewichtetes Stundenmittel`,nowcastApplied?'Radar-Nowcast':'',thunderApplied?'Konvektiv-/Gewitter-Nowcast':'',eventActivity==='flight'&&summary.flightHazards?.available?'Druckniveau-Flugwetterdiagnose':''].filter(Boolean).join(' · ')
