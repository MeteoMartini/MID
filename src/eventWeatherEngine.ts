...
function weatherSeverity(code:number|null){if(code==null)return 0;if([95,96,97,98,99].includes(code))return 6;if([68,69,71,73,75,76,77,78,79,83,84,85,86,87,88,89,90,93,94].includes(code))return 5;if([65,67,82].includes(code))return 4;if([63,66,81].includes(code))return 3;if([61,80,51,53,55,56,57].includes(code))return 2;if([45,48].includes(code))return 1;return 0}
...
function summarizeTimeline(timeline:EventTimelinePoint[],fusion:ForecastFusionResult|null,eventProbability?:EventPrecipitationProbabilityAssessment|null,weatherTwinApplied=false):EventSummary{
 ...
 const probabilityRows=timeline.map(point=>({probability:point.precipitationProbability==null?NaN:Number(point.precipitationProbability),minutes:Number(point.durationMinutes)})).filter(row=>Number.isFinite(row.probability)&&row.probability>=0&&Number.isFinite(row.minutes)&&row.minutes>0),coveredMinutes=probabilityRows.reduce((sum,row)=>sum+row.minutes,0),timelineMinutes=timeline.reduce((sum,point)=>sum+Math.max(0,Number(point.durationMinutes)||0),0),probabilityCoverageComplete=timeline.length>0&&probabilityRows.length===timeline.length&&coveredMinutes>=timelineMinutes-1,windowAverageProbability=coveredMinutes>0?probabilityRows.reduce((sum,row)=>sum+row.probability*row.minutes,0)/coveredMinutes:mean(precipitationProbabilities)
 const fallbackProbability=!eventProbability&&probabilityCoverageComplete&&windowAverageProbability!=null&&Number.isFinite(windowAverageProbability)?windowAverageProbability:null,precipitationProbabilityRelevant=eventProbability?.probability??fallbackProbability,precipitationProbabilitySource=eventProbability?'ensemble-members-dwd-event':fallbackProbability!=null?'hourly-window-average-fallback':'unavailable'
...
function timelineForWindow(hours:Hour[],date:string,startTime:string,endTime:string,timezone?:string){
 ...
 for(let index=0;index<rows.length;index++){
  ...
  const intervalSeconds=(overlapEnd-overlapStart)/1000,fraction=(overlapEnd-overlapStart)/step,scaled=(value:number|null|undefined)=>value!=null&&Number.isFinite(value)?Number(value)*fraction:null,precipitation=scaled(hour.precipitation),rain=scaled(hour.rain),showers=scaled(hour.showers),snowfall=scaled(hour.snowfall),rawSunshineDuration=scaled(hour.sunshineDuration),hasIntervalPrecipitation=eventIntervalHasPrecipitation(precipitation,rain,showers,snowfall),intervalCode=hasIntervalPrecipitation?hour.code:eventIntervalSkyCode(hour,rawSunshineDuration,intervalSeconds),sunshineDuration=coherentSunshineDurationSeconds({valueSeconds:rawSunshineDuration,intervalSeconds,isDay:hour.isDay,weatherCode:intervalCode,precipitation:precipitation??0,rain:rain??0,showers:showers??0,snowfall:snowfall??0,precipitationProbability:hour.probability,cloudCover:hour.cloud,lowCloudCover:hour.lowCloud})
  // Open-Meteo's precipitation/sunshine fields describe the preceding hour, while
  // weather_code is instantaneous at the timestamp. A boundary rain code therefore
  // must not retroactively label a dry 16–17 h interval as rainy. Accumulations lead
  // the interval presentation; only supported precipitation keeps the endpoint code.
  const part=precipitationParts({time:hour.time,epoch:hour.epoch,timezone:hour.timezone,precipitation:precipitation??0,rain:rain??0,showers:showers??0,snowfall:snowfall??0,probability:hour.probability,code:intervalCode,temperature:hour.temperature,dewPoint:hour.dewPoint,humidity:hour.humidity,cloud:hour.cloud,lowCloud:hour.lowCloud,cape:hour.cape,liftedIndex:hour.liftedIndex,convectiveInhibition:hour.convectiveInhibition,sunshineDuration:sunshineDuration??undefined,isDay:hour.isDay})
  result.push({time:clockFromCivilStamp(overlapEnd,timezone),periodLabel:`${clockFromCivilStamp(overlapStart,timezone)}–${clockFromCivilStamp(overlapEnd,timezone)}`,durationMinutes:Math.max(1,Math.round((overlapEnd-overlapStart)/60000)),temperature:hour.temperature,apparent:hour.apparent,precipitationProbability:hour.probability,precipitation,rain,showers,snowfall,weatherCode:Number.isFinite(intervalCode)?part.displayCode:null,weatherIntensity:part.intensity,weatherPhenomenon:part.phenomenon,weatherLabel:part.type==='none'?label(part.displayCode):part.weatherLabel,wind:hour.wind,gust:hour.gust,uv:hour.uvIndex,visibility:hour.visibility,humidity:hour.humidity,cloud:hour.cloud,lowCloud:hour.lowCloud,cape:hour.cape,liftedIndex:hour.liftedIndex,convectiveInhibition:hour.convectiveInhibition,sunshineDuration,isDay:hour.isDay,weatherSourceId:hour.weatherSourceId,weatherSourceLabel:hour.weatherSourceLabel})
 }
 return result
}
...
export async function buildEventPlan(options:BuildEventPlanOptions):Promise<EventPlan>{
 ...
 if(eventActivity==='flight'&&Number.isFinite(startEpoch)&&Number.isFinite(endEpoch))summary.flightHazards=await loadEventFlightHazards(location.latitude,location.longitude,location.elevation??weather.elevation??0,startEpoch,endEpoch,signal,location.country_code||location.country)
 summary.coverageComplete=summary.coverageComplete&&timeline.reduce((sum,point)=>sum+Number(point.durationMinutes||0),0)*60000>=endEpoch-startEpoch-60000;const advice=evaluateEvent(summary,eventEnvironment,eventActivity),fusionState=forecastFusionLabel(canonicalActive?canonical!.fusion:fusion),source=[canonicalActive?'Aktive Ortsvorhersage · identische MID-Endstufe':fusionState||'Open-Meteo Best Match · gemeinsame MID-Plausibilisierung',weatherTwinApplied?'Wetterzwilling · lokal validierte Temperatur-/Böenkorrektur':'',summary.precipitationProbabilitySource==='ensemble-members-dwd-event'?`Event-Niederschlagswahrscheinlichkeit ${eventStartTime}–${eventEndTime} · Ensemble-Member >0,2 mm`:summary.precipitationProbabilitySource==='hourly-window-average-fallback'?`Niederschlagswahrscheinlichkeit ${eventStartTime}–${eventEndTime} · zeitgewichtetes Stunden-PoP-Mittel als Ersatzsignal`:`Ereigniswahrscheinlichkeit nicht verfügbar`,nowcastApplied?'Radar-Nowcast':'',thunderApplied?'Konvektiv-/Gewitter-Nowcast':'',eventActivity==='flight'&&summary.flightHazards?.available?'Druckniveau-Flugwetterdiagnose':''].filter(Boolean).join(' · ')
