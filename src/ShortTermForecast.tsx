...
import {label,wind,type Hour,type Minute15,type Station,type Weather,type WindUnit} from './weather';
...
export function buildShortTermForecast(minutes15:Minute15[],hours:Hour[],timezone:string,now=Date.now(),anchor:ShortTermAnchor={}):ShortTermForecastPoint[]{
 ...
 for(const target of buildTargetEpochs(now)){
  ...
  const code=reconciledWeatherCode(sourceCode,anchorCode,cloud,lowCloud,visibility,humidity,temperature,precipitation,probability,offsetMinutes,localAdjustment),ratio=sourcePrecipitation>.001?precipitation/sourcePrecipitation:1,rain=sourceRain*ratio,showers=sourceShowers*ratio,snowfall=sourceSnowfall*ratio,parts=precipitationParts({time:quarter?.time??base.time,epoch:target,timezone,precipitation,rain,showers,snowfall,probability,code,temperature,dewPoint,humidity,cloud,lowCloud,cloudBaseHft:offsetMinutes<=60?anchor.cloudBaseHft:undefined,ceilingHft:offsetMinutes<=60?anchor.ceilingHft:undefined}),thunder=significantHourlyThunderRisk({code:parts.displayCode,cape:base.cape,liftedIndex:base.liftedIndex,convectiveInhibition:base.convectiveInhibition,columnWaterVapour:base.columnWaterVapour,temperature,dewPoint,humidity,precipitation,rain,showers,probability});
