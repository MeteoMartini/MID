...
import {label,wind,type Hour,type Minute15,type RadarNowcast,type RadarNowcastFrame,type Station,type Weather,type WindUnit} from './weather';
...
function radarFrameEpoch(frame:RadarNowcastFrame){return Date.parse(frame.time)}
function radarSiteThreshold(radar:RadarNowcast){return Math.max(.02,Number(radar.siteEchoThreshold)||0.05)}
function radarNearbyThreshold(radar:RadarNowcast){return Math.max(radarSiteThreshold(radar),Number(radar.nearbyEchoThreshold)||Math.max(.08,radarSiteThreshold(radar)*1.8))}
function radarRateToProbability(rate:number,threshold:number,nearby=false){
 const scaled=threshold>0?rate/threshold:0,base=nearby?26:34,span=nearby?50:58;
 return clampValue(base+scaled*span,0,nearby?78:96);
}
function radarNowcastSignal(radar:RadarNowcast|undefined,target:number,intervalMinutes:number){
 if(!radar||radar.source==='model'||radar.coverage===false)return null;
 const siteThreshold=radarSiteThreshold(radar),nearbyThreshold=radarNearbyThreshold(radar),frames=(radar.nowcastSeries??[]).map(frame=>({...frame,epoch:radarFrameEpoch(frame)})).filter(frame=>Number.isFinite(frame.epoch));
 const close=frames.filter(frame=>Math.abs(frame.epoch-target)<=10*60000),best=close.sort((a,b)=>Math.max(Number(b.rate)||0,Number(b.nearbyRate)||0)-Math.max(Number(a.rate)||0,Number(a.nearbyRate)||0))[0];
 if(best){
  const rate=Math.max(0,Number(best.rate)||0),nearbyRate=Math.max(rate,Number(best.nearbyRate)||0),siteWet=rate>=siteThreshold,nearbyWet=nearbyRate>=nearbyThreshold,dominant=siteWet?rate:nearbyRate;
  if(siteWet||nearbyWet)return{precipitation:dominant*intervalMinutes/60,probability:radarRateToProbability(dominant,siteWet?siteThreshold:nearbyThreshold,!siteWet&&nearbyWet)};
 }
 const now=Date.now(),minutes=(target-now)/60000,arrival=Number(radar.arrivalMinutes),hasArrival=Number.isFinite(arrival),windowStart=hasArrival?Math.max(0,arrival-10):Number.POSITIVE_INFINITY,rawEnd=Number.isFinite(Number(radar.endMinutes))?Number(radar.endMinutes):hasArrival?arrival+75:Number.NEGATIVE_INFINITY,windowEnd=hasArrival?Math.max(windowStart+15,rawEnd):Number.NEGATIVE_INFINITY,currentRate=Math.max(0,Number(radar.currentRate)||0,Number(radar.peakRate)||0),radarProbability=clampValue(Number(radar.radarProbability)||0,0,100);
 const activeWindow=hasArrival&&minutes+intervalMinutes>=windowStart&&minutes-intervalMinutes<=windowEnd;
 if(activeWindow&&radarProbability>=15){
  const rate=Math.max(currentRate,radarProbability>=55?siteThreshold:0),probability=Math.max(nearbyThreshold?radarRateToProbability(rate||nearbyThreshold,nearbyThreshold,false):0,radarProbability*.92);
  return{precipitation:Math.max(0,rate*intervalMinutes/60),probability:clampValue(probability,0,96)};
 }
 if(radarProbability>=45&&minutes>=0&&minutes<=45)return{precipitation:Math.max(0,currentRate*intervalMinutes/60),probability:Math.max(30,Math.min(90,radarProbability*.82))};
 return null;
}
...
export function buildShortTermForecast(minutes15:Minute15[],hours:Hour[],timezone:string,now=Date.now(),anchor:ShortTermAnchor={},radarNowcast?:RadarNowcast):ShortTermForecastPoint[]{
 ...
 for(const target of buildTargetEpochs(now)){
  ...
  if(!base)continue;
  const intervalMinutes=quarter?15:60,radarSignal=offsetMinutes<=90?radarNowcastSignal(radarNowcast,target,intervalMinutes):null,sourcePrecipitation=Math.max(quarter?.precipitation??base.precipitation,radarSignal?.precipitation??0),sourceRain=Math.max(quarter?.rain??base.rain,radarSignal?.precipitation??0),sourceShowers=quarter?.showers??base.showers,sourceSnowfall=quarter?.snowfall??base.snowfall,sourceProbability=Math.max(quarter?.probability??base.probability,radarSignal?.probability??0),sourceCode=quarter?.code??base.code,observedRate=Number(anchorPrecipitation)*60/Math.max(1,Number(anchor.precipitationMinutes)||60),observedAmount=Number.isFinite(observedRate)?Math.max(0,observedRate*intervalMinutes/60):undefined,modelNowAmount=Number.isFinite(Number(modelNow?.precipitation))?Math.max(0,Number(modelNow?.precipitation)*intervalMinutes/60):undefined;
  ...
  points.push({id:`${offsetMinutes}:${target}`,offsetMinutes,offsetLabel:offsetLabel(offsetMinutes),timeLabel:clock(target,timezone),intervalLabel:quarter?'15 min':'1 h',epoch:target,source:quarter?'15-min':'hourly',temperature,apparent,humidity,dewPoint,pressure,precipitation:signal.precipitation,probability:signal.probability,code:parts.displayCode,weatherLabel:parts.type==='none'?label(parts.displayCode):parts.weatherLabel,wind:windPair.wind,gust:windPair.gust,direction,cloud,lowCloud,visibility,isDay:base.isDay,localAdjustment,thunderPercent:thunder?.percent});
 }
 return points;
}
...
export function ShortTermForecast({minutes15,hours,timezone,unit,anchor,forecastSourceLabel,radarNowcast}:{minutes15:Minute15[];hours:Hour[];timezone:string;unit:WindUnit;anchor?:ShortTermAnchor;forecastSourceLabel?:string;radarNowcast?:RadarNowcast}){
