...
import {icon,label,wind,type Hour,type Minute15,type WindUnit} from './weather';
...
function offsetLabel(minutes:number){if(minutes<60)return`+${minutes} min`;return`+${minutes/60} h`}
...
export function buildShortTermForecast(minutes15:Minute15[],hours:Hour[],timezone:string,now=Date.now()):ShortTermForecastPoint[]{
 ...
 for(const offsetMinutes of OFFSETS){
  const target=now+offsetMinutes*60000,quarter=offsetMinutes<=60?nearest(minutes15,target,12*60000):undefined,base=interpolatedHour(hours,target);
  if(!base)continue;
...
function DirectionArrow({direction}:{direction:number}){return <Navigation size={13} style={{transform:`rotate(${Number.isFinite(direction)?direction:0}deg)`}} aria-hidden="true"/>}
