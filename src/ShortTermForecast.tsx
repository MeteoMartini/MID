...
export function ShortTermForecast({minutes15,hours,timezone,unit,anchor,forecastSourceLabel}:{minutes15:Minute15[];hours:Hour[];timezone:string;unit:WindUnit;anchor?:ShortTermAnchor;forecastSourceLabel?:string}){
 const points=useMemo(()=>buildShortTermForecast(minutes15,hours,timezone,Date.now(),anchor),[minutes15,hours,timezone,anchor]),[selectedId,setSelectedId]=useState(''),selected=points.find(point=>point.id===selectedId),locallyAdjusted=Boolean(anchor?.active&&points.some(point=>point.localAdjustment>0)),sourceLabel=locallyAdjusted?(anchor?.sourceLabel||'Lokal angepasst'):(forecastSourceLabel||'Best Match'),selectPoint=(pointId:string)=>setSelectedId(current=>current===pointId?'':pointId);
