...
const NETWORK_ATTEMPT_TIMEOUT_MS=30_000;
...
async function guardedFetchInternal(url:string,init:RequestInit,priority:OpenMeteoPriority,maxRetries:number){let attempt=0;const signal=init.signal??undefined;for(;;){if(signal?.aborted)throw abortError(signal);if(priority==='background'&&cooldownUntil>Date.now())throw new OpenMeteoRateLimitError(cooldownUntil);const release=await acquire(priority,signal);let response:Response;const controller=new AbortController(),abort=()=>controller.abort(signal?.reason),timer=globalThis.setTimeout(()=>controller.abort(new DOMException('Open-Meteo-Netzwerkanfrage hat das Zeitlimit überschritten.','TimeoutError')),NETWORK_ATTEMPT_TIMEOUT_MS);if(signal?.aborted)abort();else signal?.addEventListener('abort',abort,{once:true});try{response=await fetch(url,{...init,signal:controller.signal,cache:init.cache??'no-store',headers:{Accept:'application/json',...(init.headers??{})}})}catch(error){release();if(signal?.aborted)throw error;if(attempt>=maxRetries)throw error;attempt++;await wait(500*attempt,signal);continue}finally{globalThis.clearTimeout(timer);signal?.removeEventListener('abort',abort)}release();if(response.status===429){const retryAt=setCooldown(response,attempt);if(priority==='background'||attempt>=maxRetries)throw new OpenMeteoRateLimitError(retryAt);attempt++;await wait(Math.max(0,retryAt-Date.now()+20),signal);continue}if([500,502,503,504].includes(response.status)&&attempt<maxRetries){attempt++;await wait(500*attempt,signal);continue}return response}}
