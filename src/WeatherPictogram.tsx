...
function dryCloudProfileKind(code:number,cloud:unknown):WeatherPictogramKind|undefined{
 ...
 if(cover===undefined||c<0||c>3)return undefined;
 const oktas=Math.max(0,Math.min(8,Math.round(cover/12.5)));
 if(oktas===0)return'clear';
 if(oktas<=2)return'mostly-clear';
...
export function synopticPhenomenonDescription(value:string):string{
 ...
 if(has('SN'))return raw.startsWith('-')?'Leichter Schneefall':raw.startsWith('+')?'Starker Schneefall':'Mäßiger Schneefall';
