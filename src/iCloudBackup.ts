...
const SENSITIVE_KEYS=new Set(['mid:lastTrackedLocation']);

export type ICloudBackupSummary={createdAt:string;appVersion:string;localEntries:number;weatherTwinLocations:number;captures:number;references:number;observations:number;bytes:number};
...
function isDurableKey(key:string){
 ...
 if(SENSITIVE_KEYS.has(key)||TRANSIENT_PREFIXES.some(prefix=>key.startsWith(prefix))||SENSITIVE_PREFIXES.some(prefix=>key.startsWith(prefix)))return false;
 return true;
}
...
function applyLocalState(values:Record<string,string>){for(const[key,value]of Object.entries(values)){if(!isDurableKey(key)||typeof value!=='string')continue;localStorage.setItem(key,value)}}

export async function restoreICloudBackup(file:File):Promise<{summary:ICloudBackupSummary;weatherTwin:ForecastVerificationArchiveStats}>{
 ...
 await persistStateNow();applyLocalState(bundle.localState.values);const weatherTwin=await importForecastVerificationArchive(bundle.weatherTwin);await persistStateNow();
 const summary:ICloudBackupSummary={...bundle.summary,bytes:file.size};try{localStorage.setItem(LAST_BACKUP_KEY,JSON.stringify({...summary,restoredAt:new Date().toISOString()}))}catch{}return{summary,weatherTwin};
}
