...
export type ExtremeWeatherOutlook={
 ...
 quality:{ensembleMembers:number;probabilityMethod:string;modelResolution:string;displayGrid:string;dataCoveragePct?:number;diagnosticCoveragePct:number;nowcastObservation?:{provider?:string;observedAt?:string;ageMinutes?:number;cells?:number};limitations:string[]};
 error?:string;
};
