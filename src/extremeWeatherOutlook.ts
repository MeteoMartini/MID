...
export type ExtremeWeatherOutlook={
 ...
 grid:{rows:number;cols:number;latStep:number;lonStep:number;pointCount:number;bounds:{south:number;west:number;north:number;east:number};analysisBounds?:{south:number;west:number;north:number;east:number}};
...
const OUTLOOK_CACHE_KEY=`${OUTLOOK_PAYLOAD_PREFIX}v4`;
...
function validOutlook(value:unknown):value is ExtremeWeatherOutlook{const data=value as ExtremeWeatherOutlook|undefined,bounds=data?.grid?.bounds;return Boolean(data&&data.scope==='Mitteleuropa'&&Array.isArray(data.periods)&&data.periods.length&&Array.isArray(data.cells)&&data.cells.length&&data.grid?.pointCount&&bounds&&bounds.west<=-3.84&&bounds.east>=20.2&&bounds.south<=43.2&&bounds.north>=57.99&&data.thresholds?.probability)}
...
export async function loadExtremeWeatherOutlook(signal?:AbortSignal):Promise<ExtremeWeatherOutlook>{
 ...
 if(!workerSkipped){
  try{
   const worker=await fetchWorkerJson<ExtremeWeatherOutlook>('dach-extreme-outlook',{}, {purpose:'general',signal,timeoutMs:48000,maxAgeMs:30*60*1000,staleIfErrorMs:6*60*60*1000,cacheKey:'dach-extreme-outlook:v4'});
