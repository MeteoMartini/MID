...
export function readWeatherTwinSettings():WeatherTwinSettings{try{const parsed=JSON.parse(localStorage.getItem(WEATHER_TWIN_SETTINGS_KEY)||'{}') as Partial<WeatherTwinSettings>;return{enabled:parsed.enabled!==false,useAsMainForecast:Boolean(parsed.useAsMainForecast),nowcastAssimilation:parsed.nowcastAssimilation!==false,probabilityCalibration:parsed.probabilityCalibration!==false,biasCorrection:parsed.biasCorrection!==false,personalRecommendations:parsed.personalRecommendations!==false,privateSensorUrl:PRIVATE_SENSOR_INTEGRATION_ENABLED?String(parsed.privateSensorUrl||''):'',privateSensorLabel:String(parsed.privateSensorLabel||'Privater Sensor'),activities:mergedActivities(parsed.activities)}}catch{return{enabled:true,useAsMainForecast:false,nowcastAssimilation:true,probabilityCalibration:true,biasCorrection:true,personalRecommendations:true,privateSensorUrl:'',privateSensorLabel:'Privater Sensor',activities:defaultActivities()}}}
