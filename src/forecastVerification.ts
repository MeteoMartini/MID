import {DWD_PRECIPITATION_PROBABILITY_THRESHOLD_MM,dailyWetProbabilityFromHours,stationFieldObservationUsable,type Day,type EnsembleDay,type Hour,type Location,type RadarNowcast,type Station} from './weather';
...
export type ForecastPrediction={
 ...
 consensusRole?:ModelConsensusRole;
 probabilityKind?:'daily-wet-native'|'daily-wet-derived'|'spread-estimate'|'hourly-max';
 sourceId?:string;
};

export type AdditionalForecastPrediction={id:string;label:string;days:Day[];family?:string;independenceGroup?:string;consensusRole?:ModelConsensusRole};
...
export function readWeatherTwinSettings():WeatherTwinSettings{try{const parsed=JSON.parse(localStorage.getItem(WEATHER_TWIN_SETTINGS_KEY)||'{}') as Partial<WeatherTwinSettings>;return{enabled:parsed.enabled!==false,useAsMainForecast:Boolean(parsed.useAsMainForecast),nowcastAssimilation:parsed.nowcastAssimilation!==false,probabilityCalibration:parsed.probabilityCalibration!==false,biasCorrection:parsed.biasCorrection!==false,personalRecommendations:parsed.personalRecommendations!==false,learnAllFavorites:parsed.learnAllFavorites===true,privateSensorUrl:PRIVATE_SENSOR_INTEGRATION_ENABLED?String(parsed.privateSensorUrl||''):'',privateSensorLabel:String(parsed.privateSensorLabel||'Privater Sensor'),activities:mergedActivities(parsed.activities)}}catch{return{enabled:true,useAsMainForecast:false,nowcastAssimilation:true,probabilityCalibration:true,biasCorrection:true,personalRecommendations:true,learnAllFavorites:false,privateSensorUrl:'',privateSensorLabel:'Privater Sensor',activities:defaultActivities()}}}
...
function storeContentSignature(store:Store){return JSON.stringify({version:store.version,captures:store.captures,references:store.references,observations:store.observations})}
...
export async function restoreForecastVerificationArchive(locationKey:string){if(ARCHIVE_LOADED.has(locationKey))return false;try{const local=readStore(locationKey),db=await openArchiveDb(),archived=await new Promise<Store|null>((resolve,reject)=>{const tx=db.transaction(DB_STORE,'readonly'),request=tx.objectStore(DB_STORE).get(locationKey);request.onsuccess=()=>resolve(request.result??null);request.onerror=()=>reject(request.error)});db.close();ARCHIVE_LOADED.add(locationKey);if(!archived)return false;const merged=mergeStores(local,migrateStore(archived)),changed=storeContentSignature(merged)!==storeContentSignature(local);if(changed)saveStore(locationKey,merged);return changed}catch{ARCHIVE_LOADED.delete(locationKey);return false}}
...
export async function importForecastVerificationArchive(bundle:ForecastVerificationArchiveBundle):Promise<ForecastVerificationArchiveStats>{if(!bundle||bundle.schema!=='mid-weather-twin-archive'||bundle.version!==1||!bundle.locations||typeof bundle.locations!=='object')throw new Error('Das Wetterzwilling-Langzeitarchiv besitzt ein ungültiges Format.');const entries=Object.entries(bundle.locations).slice(0,1000),mergedLocations:Record<string,Store>={};for(const[rawKey,rawStore]of entries){const locationKey=String(rawKey||'').trim();if(!locationKey||locationKey.length>240)continue;const local=readStore(locationKey),merged=mergeStores(local,migrateStore(rawStore)),changed=storeContentSignature(merged)!==storeContentSignature(local);ARCHIVE_LOADED.add(locationKey);if(changed)saveStore(locationKey,merged);mergedLocations[locationKey]=changed?merged:local}await waitForForecastVerificationArchiveWrites();return archiveStats(mergedLocations,bundle.updatedAt)}
...
function upsertObservation(store:Store,row:TwinLiveObservation){const index=store.observations.findIndex(item=>item.id===row.id);if(index>=0){if(JSON.stringify(store.observations[index])===JSON.stringify(row))return false;store.observations[index]=row;return true}store.observations.push(row);return true}
...
function bestMatchPrediction(day:Day,hours:Hour[]=[]):ForecastPrediction{const probability=day.probabilitySource==='ensemble-members-dwd'?finite(day.probability):dailyWetProbabilityFromHours(day.date,hours,finite(day.probability)),probabilityKind=day.probabilitySource==='ensemble-members-dwd'?'daily-wet-native':day.probabilitySource==='daily-wet-derived'?'daily-wet-derived':'hourly-max';return{id:'best_match',label:'Open-Meteo Best Match',max:finite(day.max),min:finite(day.min),precipitation:Math.max(0,finite(day.precipitation)),probability:clamp(probability,0,100),gust:Number.isFinite(day.gust)?day.gust:undefined,sunshineDuration:Number.isFinite(day.sunshineDuration)?day.sunshineDuration:undefined,weatherCode:day.code,consensusRole:'anchor',probabilityKind}}
...
function representativeEnsembleModels(row:EnsembleDay|undefined){const byIdentity=new Map<string,NonNullable<EnsembleDay['modelSummaries']>[number]>();for(const model of row?.modelSummaries??[]){const identity=stableModelIdentity(model),current=byIdentity.get(identity.id),score=(model.probabilityQuality==='native-members'?1000:0)+Math.max(0,model.memberCount);if(!current||(score>((current.probabilityQuality==='native-members'?1000:0)+Math.max(0,current.memberCount))))byIdentity.set(identity.id,model)}return[...byIdentity.values()]}
function currentPredictions(day:Day,row:EnsembleDay|undefined,hours:Hour[]=[]){const models=representativeEnsembleModels(row).map(model=>{const identity=stableModelIdentity(model);return{id:identity.id,sourceId:model.id,label:model.label,family:identity.family,independenceGroup:identity.independenceGroup,consensusRole:'independent' as const,max:model.max,min:model.min,precipitation:model.precipitation,probability:model.precipitationProbability,probabilityKind:model.probabilityQuality==='estimated-spread'?'spread-estimate' as const:'daily-wet-native' as const,gust:model.gust,sunshineDuration:model.sunshineDuration,weatherCode:model.weatherCode}});return[bestMatchPrediction(day,hours),...models].filter(predictionValid)}
...
function normalizedPredictionForLearning(prediction:ForecastPrediction){if(!modelPredictionIsLearnable(prediction))return prediction;const identity=stableModelIdentity(prediction);return{...prediction,id:identity.id,family:identity.family,independenceGroup:identity.independenceGroup}}
function statsFor(evaluations:Evaluation[],filter?:(evaluation:Evaluation)=>boolean){
 const grouped=new Map<string,{id:string;label:string;consensusRole?:ModelConsensusRole;dates:Map<string,Record<ForecastParameter,ParamAccumulator>>}>();
 for(const evaluation of evaluations){
  if(filter&&!filter(evaluation))continue;
  for(const rawPrediction of evaluation.capture.predictions){
   const prediction=normalizedPredictionForLearning(rawPrediction),row=grouped.get(prediction.id)??{id:prediction.id,label:prediction.label,consensusRole:prediction.consensusRole,dates:new Map<string,Record<ForecastParameter,ParamAccumulator>>()},day=row.dates.get(evaluation.date)??emptyParameters(),errors=modelParameterErrors(prediction,evaluation.reference);
   for(const parameter of Object.keys(errors) as ForecastParameter[])addParameter(day[parameter],errors[parameter]);
   row.dates.set(evaluation.date,day);grouped.set(prediction.id,row)
  }
 }
 return[...grouped.values()].map(row=>{
  const metrics:Partial<Record<ForecastParameter,ParameterMetric>>={};
  for(const parameter of['temperature','precipitation','probability','gust','sunshine'] as ForecastParameter[]){
   const dayErrors=[...row.dates.values()].map(day=>day[parameter]).filter(acc=>acc.count>0).map(acc=>acc.sum/acc.count);
   if(dayErrors.length){const error=dayErrors.reduce((sum,value)=>sum+value,0)/dayErrors.length;metrics[parameter]={parameter,samples:dayErrors.length,error,score:parameterScore(parameter,error)}}
  }
  const temperatureMae=metrics.temperature?.error??NaN,precipitationMae=metrics.precipitation?.error??NaN,brier=metrics.probability?.error??NaN,score=priorityWeightedMetricScore(metrics),days=row.dates.size;
  return{id:row.id,label:row.label,days,temperatureMae,precipitationMae,brier,gustMae:metrics.gust?.error,sunshineMae:metrics.sunshine?.error,score,weight:0,confidence:confidenceFromSamples(days),parameterMetrics:metrics} satisfies ForecastModelScore
 }).sort((a,b)=>a.score-b.score)
}
function modelBiases(evaluations:Evaluation[],regime:WeatherRegime,horizon:ForecastHorizon){const rows=new Map<string,{label:string;dates:Set<string>;sum:Record<ForecastParameter,number>;count:Record<ForecastParameter,number>}>();for(const evaluation of evaluations){if(evaluation.regime!==regime||evaluation.horizon!==horizon)continue;for(const rawPrediction of evaluation.capture.predictions){if(!modelPredictionIsLearnable(rawPrediction))continue;const prediction=normalizedPredictionForLearning(rawPrediction),row=rows.get(prediction.id)??{label:prediction.label,dates:new Set<string>(),sum:{temperature:0,precipitation:0,probability:0,gust:0,sunshine:0},count:{temperature:0,precipitation:0,probability:0,gust:0,sunshine:0}};row.dates.add(evaluation.date);const temp=((prediction.max-evaluation.reference.max)+(prediction.min-evaluation.reference.min))/2;row.sum.temperature+=temp;row.count.temperature++;row.sum.precipitation+=prediction.precipitation-evaluation.reference.precipitation;row.count.precipitation++;if(Number.isFinite(prediction.gust)&&Number.isFinite(evaluation.reference.gust)){row.sum.gust+=Number(prediction.gust)-Number(evaluation.reference.gust);row.count.gust++}if(Number.isFinite(prediction.sunshineDuration)&&Number.isFinite(evaluation.reference.sunshineDuration)){row.sum.sunshine+=(Number(prediction.sunshineDuration)-Number(evaluation.reference.sunshineDuration))/3600;row.count.sunshine++}rows.set(prediction.id,row)}}return[...rows].map(([id,row])=>{const samples=row.dates.size,shrink=samples/(samples+5),corrections:Partial<Record<ForecastParameter,number>>={};for(const parameter of['temperature','precipitation','gust','sunshine'] as ForecastParameter[])if(row.count[parameter])corrections[parameter]=-(row.sum[parameter]/row.count[parameter])*shrink;return{id,label:row.label,regime,horizon,samples,confidence:confidenceFromSamples(samples),corrections} satisfies BiasCorrection})}
function calibrationBins(evaluations:Evaluation[],modelId='best_match'){
 const daily=new Map<string,{probabilitySum:number;count:number;observed:number}>();
 for(const evaluation of evaluations){
  const prediction=evaluation.capture.predictions.find(row=>row.id===modelId);if(!prediction||prediction.id==='best_match'&&!prediction.probabilityKind||['spread-estimate','hourly-max'].includes(String(prediction.probabilityKind||'')))continue;
  const probability=clamp(prediction.probability,0,100),row=daily.get(evaluation.date)??{probabilitySum:0,count:0,observed:evaluation.reference.precipitation>DWD_PRECIPITATION_PROBABILITY_THRESHOLD_MM?1:0};row.probabilitySum+=probability;row.count++;daily.set(evaluation.date,row)
 }
 const buckets=Array.from({length:5},(_,index)=>({lower:index*20,upper:(index+1)*20,samples:0,forecastSum:0,observed:0}));
 for(const row of daily.values()){const probability=row.probabilitySum/Math.max(1,row.count),index=Math.min(4,Math.floor(probability/20)),bucket=buckets[index];bucket.samples++;bucket.forecastSum+=probability;bucket.observed+=row.observed}
 return buckets.filter(bucket=>bucket.samples).map(bucket=>{const forecastMean=bucket.forecastSum/bucket.samples,observedFrequency=100*bucket.observed/bucket.samples,shrink=bucket.samples/(bucket.samples+8),calibrated=forecastMean*(1-shrink)+observedFrequency*shrink;return{lower:bucket.lower,upper:bucket.upper,samples:bucket.samples,forecastMean,observedFrequency,calibrated} satisfies ProbabilityCalibrationBin})
}
...
function localValidationEvidence(evaluations:Evaluation[]){
 const byDay=new Map<string,{weighted:number;best:number;count:number}>();
 for(const evaluation of evaluations){const weighted=evaluation.capture.predictions.find(row=>row.id==='mid_local_weighted'),best=evaluation.capture.predictions.find(row=>row.id==='best_match');if(!weighted||!best)continue;const weightedScore=compositePredictionScore(weighted,evaluation.reference),bestScore=compositePredictionScore(best,evaluation.reference);if(!Number.isFinite(weightedScore)||!Number.isFinite(bestScore))continue;const row=byDay.get(evaluation.date)??{weighted:0,best:0,count:0};row.weighted+=weightedScore;row.best+=bestScore;row.count++;byDay.set(evaluation.date,row)}
 const dayRows=[...byDay.values()].map(row=>({weighted:row.weighted/Math.max(1,row.count),best:row.best/Math.max(1,row.count)})),weightedMean=dayRows.length?dayRows.reduce((sum,row)=>sum+row.weighted,0)/dayRows.length:NaN,bestMean=dayRows.length?dayRows.reduce((sum,row)=>sum+row.best,0)/dayRows.length:NaN,improvement=dayRows.length?100*(bestMean-weightedMean)/Math.max(.1,bestMean):undefined;
 return{samples:dayRows.length,cases:evaluations.length,days:dayRows.length,improvement}
}
function adaptiveWeightCap(evaluations:Evaluation[],regime:WeatherRegime,horizon:ForecastHorizon){const globalDays=new Set(evaluations.map(item=>item.date)).size,segmentDays=new Set(evaluations.filter(item=>item.regime===regime&&item.horizon===horizon).map(item=>item.date)).size,validation=localValidationEvidence(evaluations);let max=globalDays<6?48:globalDays<12?52:globalDays<18?56:globalDays<36?60:62;if(segmentDays<3)max=Math.min(max,50);else if(segmentDays<6)max=Math.min(max,54);else if(segmentDays<12)max=Math.min(max,58);if(validation.days<6)max=Math.min(max,58);else if(Number.isFinite(validation.improvement)){if(validation.improvement!>=10)max+=3;else if(validation.improvement!>=3)max+=2;else if(validation.improvement!>0)max+=1;else if(validation.improvement!<=-5)max=Math.min(max,52);else max=Math.min(max,55)}max=clamp(Math.round(max),48,65);const evidenceSamples=Math.min(globalDays,segmentDays?Math.max(segmentDays,Math.round((globalDays+segmentDays)/2)):globalDays),confidence=confidenceFromSamples(evidenceSamples);return{max,confidence,samples:evidenceSamples,segmentDays,globalDays,validationSamples:validation.samples,validationImprovement:validation.improvement}}
...
function distributeGroupBudget<T extends {independenceGroup:string}>(rows:T[],read:(row:T)=>number,write:(row:T,value:number)=>void){
 const groups=new Map<string,T[]>();for(const row of rows){const items=groups.get(row.independenceGroup)??[];items.push(row);groups.set(row.independenceGroup,items)}
 const budgets=[...groups.entries()].map(([id,items])=>({id,items,budget:Math.max(0,...items.map(read))})),totalBudget=budgets.reduce((sum,row)=>sum+row.budget,0)||1;
 for(const group of budgets){const within=group.items.reduce((sum,row)=>sum+Math.max(0,read(row)),0)||1,groupPercent=100*group.budget/totalBudget;for(const row of group.items)write(row,groupPercent*Math.max(0,read(row))/within)}
}
function regularizedWeights(evaluations:Evaluation[],regime:WeatherRegime,horizon:ForecastHorizon,beforeDate?:string){
 const eligibleEvaluations=evaluations.filter(item=>!beforeDate||item.date<beforeDate),uniqueDays=new Set(eligibleEvaluations.map(item=>item.date)).size;if(uniqueDays<5)return[];
 const global=statsFor(eligibleEvaluations).filter(row=>modelPredictionIsLearnable({id:row.id})),segment=statsFor(eligibleEvaluations,item=>item.regime===regime&&item.horizon===horizon).filter(row=>modelPredictionIsLearnable({id:row.id})),globalById=new Map(global.map(row=>[row.id,row])),segmentById=new Map(segment.map(row=>[row.id,row])),ids=[...new Set([...global.map(row=>row.id),...segment.map(row=>row.id)])],cap=adaptiveWeightCap(eligibleEvaluations,regime,horizon);
 const weighted=ids.map(id=>{const identity=stableModelIdentity({id}),globalRow=globalById.get(id),segmentRow=segmentById.get(id),days=segmentRow?.days??0,globalScore=globalRow?.score??8,score=((segmentRow?.score??globalScore)*days+globalScore*5)/(days+5),parameterWeights:Partial<Record<ForecastParameter,number>>={},parameterSamples:Partial<Record<ForecastParameter,number>>={};for(const parameter of['temperature','precipitation','probability','gust','sunshine'] as ForecastParameter[]){const segmentMetric=segmentRow?.parameterMetrics[parameter],globalMetric=globalRow?.parameterMetrics[parameter],segmentCount=segmentMetric?.samples??0,baseScore=globalMetric?.score??score,combined=((segmentMetric?.score??baseScore)*segmentCount+baseScore*5)/(segmentCount+5);parameterWeights[parameter]=1/Math.max(.18,combined);parameterSamples[parameter]=segmentCount||globalMetric?.samples||0}return{id,label:segmentRow?.label??globalRow?.label??id,independenceGroup:identity.independenceGroup,days:Math.max(days,globalRow?.days??0),score,raw:1/Math.max(.35,score),parameterWeights,parameterSamples}}).filter(row=>row.days>=2).sort((a,b)=>a.score-b.score);if(new Set(weighted.map(row=>row.independenceGroup)).size<2)return[];
 const normalized=weighted.map(row=>({...row,weight:0,confidence:confidenceFromSamples(row.days),parameterCaps:{} as Partial<Record<ForecastParameter,number>>}));
 distributeGroupBudget(normalized,row=>row.raw,(row,value)=>{row.weight=value});capPercentages(normalized,row=>row.weight,(row,value)=>{row.weight=value},cap.max);
 for(const parameter of['temperature','precipitation','probability','gust','sunshine'] as ForecastParameter[]){const maxSamples=Math.max(0,...normalized.map(row=>row.parameterSamples[parameter]??0)),parameterCap=parameterWeightCap(cap.max,maxSamples);distributeGroupBudget(normalized,row=>row.parameterWeights[parameter]??0,(row,value)=>{row.parameterWeights[parameter]=value});for(const row of normalized)row.parameterCaps[parameter]=parameterCap;capPercentages(normalized,row=>row.parameterWeights[parameter]??0,(row,value)=>{row.parameterWeights[parameter]=value},parameterCap)}
 return normalized.sort((a,b)=>b.weight-a.weight).map(row=>({id:row.id,label:row.label,independenceGroup:row.independenceGroup,weight:row.weight,days:row.days,confidence:row.confidence,maxWeight:cap.max,capConfidence:cap.confidence,capSamples:cap.samples,validationSamples:cap.validationSamples,validationImprovement:cap.validationImprovement,parameterWeights:row.parameterWeights,parameterCaps:row.parameterCaps} satisfies ModelWeight))
}
...
function equalWeightedPrediction(predictions:ForecastPrediction[]):ForecastPrediction|undefined{
 const usable=predictions.map(normalizedPredictionForLearning).filter(modelPredictionIsLearnable),unique=[...new Map(usable.map(row=>[row.id,row])).values()],groups=new Map<string,ForecastPrediction[]>();for(const row of unique){const group=stableModelIdentity(row).independenceGroup,items=groups.get(group)??[];items.push(row);groups.set(group,items)}if(groups.size<2)return undefined;
 const groupMean=(rows:ForecastPrediction[],selector:(prediction:ForecastPrediction)=>number)=>{const values=rows.map(selector).filter(Number.isFinite);return values.length?values.reduce((sum,value)=>sum+value,0)/values.length:NaN},mean=(selector:(prediction:ForecastPrediction)=>number)=>{const values=[...groups.values()].map(rows=>groupMean(rows,selector)).filter(Number.isFinite);return values.length?values.reduce((sum,value)=>sum+value,0)/values.length:NaN},max=mean(row=>row.max),min=mean(row=>row.min),precipitation=mean(row=>row.precipitation),probability=mean(row=>row.probability),gust=mean(row=>Number(row.gust)),sunshineDuration=mean(row=>Number(row.sunshineDuration));if(![max,min,precipitation,probability].every(Number.isFinite))return undefined;return{id:'equal_weighted',label:'Einfaches Multimodellmittel · unabhängige Gruppen gleich gewichtet',max,min,precipitation:Math.max(0,precipitation),probability:clamp(probability,0,100),gust:Number.isFinite(gust)?gust:undefined,sunshineDuration:Number.isFinite(sunshineDuration)?sunshineDuration:undefined,consensusRole:'derived'}
}
...
export function recordForecastCapture(locationKey:string,days:Day[],ensemble:EnsembleDay[],location?:Location,hours:Hour[]=[],additional:AdditionalForecastPrediction[]=[]){if(!readWeatherTwinSettings().enabled||!locationKey||!days.length)return;const now=Date.now(),slot=Math.floor(now/(3*3600000)),store=readStore(locationKey),ensembleByDate=new Map(ensemble.map(row=>[row.date,row])),historical=chosenEvaluations(store),profile=readTwinSiteProfile(locationKey,location??{id:0,name:locationKey,latitude:0,longitude:0}),timezone=location?.timezone;let changed=false;for(const day of days.slice(0,3)){const target=dateNoon(day.date,timezone),leadHours=(target-now)/3600000;if(!Number.isFinite(leadHours)||leadHours<-12||leadHours>84)continue;const regime=predictedRegime(day,profile,hours),horizon=horizonBucket(leadHours),predictions=currentPredictions(day,ensembleByDate.get(day.date),hours);for(const source of additional){const extra=source.days.find(row=>row.date===day.date);if(extra)predictions.push({id:source.id,label:source.label,max:extra.max,min:extra.min,precipitation:Math.max(0,extra.precipitation),probability:clamp(extra.probability,0,100),gust:extra.gust,sunshineDuration:extra.sunshineDuration,weatherCode:extra.code,family:source.family,independenceGroup:source.independenceGroup,consensusRole:source.consensusRole??'derived',probabilityKind:'daily-wet-derived'})}const weights=regularizedWeights(historical,regime,horizon,day.date),settings=readWeatherTwinSettings(),biases=settings.biasCorrection?modelBiases(historical.filter(item=>item.date<day.date),regime,horizon):[],calibrations=settings.probabilityCalibration?calibrationBins(historical.filter(item=>item.date<day.date)):[],local=weightedPrediction(predictions,weights,biases,calibrations),equal=equalWeightedPrediction(predictions);if(equal)predictions.push(equal);if(local)predictions.push(local);const signature=`${day.date}:${slot}`,index=store.captures.findIndex(item=>`${item.targetDate}:${Math.floor(Date.parse(item.issuedAt)/(3*3600000))}`===signature),capture:ForecastCapture={id:signature,targetDate:day.date,issuedAt:new Date(now).toISOString(),leadHours,timezone,predictedRegime:regime,predictions},content=JSON.stringify({targetDate:capture.targetDate,timezone:capture.timezone,predictedRegime:capture.predictedRegime,horizon:horizonBucket(capture.leadHours),predictions:capture.predictions});if(index>=0){const existing=store.captures[index],existingContent=JSON.stringify({targetDate:existing.targetDate,timezone:existing.timezone,predictedRegime:existing.predictedRegime,horizon:horizonBucket(existing.leadHours),predictions:existing.predictions});if(existingContent!==content){store.captures[index]=capture;changed=true}}else{store.captures.push(capture);changed=true}}if(changed)saveStore(locationKey,store)}
...
async function retrospectiveBestMatchRequest(location:Location,signal?:AbortSignal){const params=new URLSearchParams({latitude:String(location.latitude),longitude:String(location.longitude),timezone:'auto',past_days:'30',forecast_days:'1',models:'best_match',wind_speed_unit:'kn',daily:REFERENCE_DAILY}),response=await guardedOpenMeteoFetch(`https://api.open-meteo.com/v1/forecast?${params}`,{signal,cache:'no-store'},{priority:'background',maxRetries:0});if(!response.ok)throw new Error(`Open-Meteo Rückblick HTTP ${response.status}`);return referenceRows(await response.json(),'Open-Meteo Best-Match-Rückblick','model-fallback',.28)}
async function reanalysisReferenceRequest(location:Location,signal?:AbortSignal){const base={latitude:String(location.latitude),longitude:String(location.longitude),timezone:'auto',start_date:localDateOffset(-120,location.timezone),end_date:localDateOffset(-1,location.timezone),models:'era5_land',wind_speed_unit:'kn'},request=async(daily:string)=>{const params=new URLSearchParams({...base,daily});if(Number.isFinite(location.elevation))params.set('elevation',String(location.elevation));return guardedOpenMeteoFetch(`https://archive-api.open-meteo.com/v1/archive?${params}`,{signal,cache:'no-store'},{priority:'background',maxRetries:0})};let response=await request(REFERENCE_DAILY);if(!response.ok)response=await request('temperature_2m_max,temperature_2m_min,precipitation_sum,weather_code');if(!response.ok)throw new Error(`Open-Meteo Reanalyse HTTP ${response.status}`);return referenceRows(await response.json(),'Open-Meteo ERA5-Land-Reanalyse','reanalysis',.72)}
...
function mainForecastStatus(currentForecasts:LocalWeightedForecast[],weightingReady:boolean,validationDays:number,weightedImprovement?:number):TwinMainForecastStatus{
 ...
 for(const row of ready){const dayGroups=new Map<string,{label:string;weight:number}>();for(const weight of row.weights){if(!modelPredictionIsLearnable({id:weight.id})||!Number.isFinite(weight.weight)||weight.weight<=0)continue;const group=weight.independenceGroup||stableModelIdentity({id:weight.id}).independenceGroup,current=dayGroups.get(group)??{label:weight.label,weight:0};current.weight+=weight.weight;if(weight.weight>current.weight/2)current.label=weight.label;dayGroups.set(group,current)}for(const[group,day]of dayGroups){const current=totals.get(group)??{id:group,label:day.label,sum:0,count:0};current.sum+=day.weight;current.count++;totals.set(group,current)}}
 ...
 let reason='';if(!weightingReady)reason='Mindestens fünf abgeschlossene Vergleichstage und zwei unabhängige Modellgruppen sind erforderlich.';else if(ready.length<requiredReady)reason='Für mindestens zwei der nächsten drei Tage fehlt noch eine belastbare lokal gewichtete Prognose.';else if(modelCount<2)reason='Der aktuelle Modellmix enthält noch nicht mindestens zwei unabhängige Modellgruppen.';else if(validationDays<6)reason=`Noch ${Math.max(0,6-validationDays)} abgeschlossene Kontrolltage bis zur Freigabe gegen Best Match.`;else if(!Number.isFinite(improvement))reason='Der Kontrollvergleich mit Best Match ist noch nicht vollständig berechenbar.';else if(Number(improvement)<0)reason=`MID liegt im Kontrollvergleich derzeit ${Math.round(Math.abs(Number(improvement)))} % hinter Best Match.`;else reason=`MID ist nach ${validationDays} Kontrolltagen mindestens gleichwertig zu Best Match (${Number(improvement)>=0?'+':''}${Math.round(Number(improvement))} %).`;
 const sourceLabel=dominantModel?`Best-Match-Nachkorrektur · Diagnose-Schwerpunkt ${dominantModel.label} ${Math.round(dominantModel.weight)} %`:'Best-Match-Nachkorrektur';return{eligible,confidence,readyDays:ready.length,modelCount,validationDays,improvement,dominantModel,sourceLabel,reason}
}
