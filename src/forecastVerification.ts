...
function compositePredictionScore(prediction:ForecastPrediction,reference:ForecastReference){const errors=modelParameterErrors(prediction,reference),scores=(Object.keys(errors) as ForecastParameter[]).map(parameter=>parameterScore(parameter,errors[parameter])).filter(Number.isFinite);return scores.length?scores.reduce((sum,value)=>sum+value,0)/scores.length:NaN}
