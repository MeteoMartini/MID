...
function queueMirrorStore(locationKey:string,store:Store){const previous=ARCHIVE_WRITES.get(locationKey)??Promise.resolve(),next=previous.catch(()=>undefined).then(()=>mirrorStore(locationKey,store));ARCHIVE_WRITES.set(locationKey,next);void next.finally(()=>{if(ARCHIVE_WRITES.get(locationKey)===next)ARCHIVE_WRITES.delete(locationKey)})}
...
function saveStore(locationKey:string,store:Store){const full=mergeStores(store,ARCHIVE_MEMORY.get(locationKey)??emptyStore()),updated={...full,updatedAt:new Date().toISOString()};ARCHIVE_MEMORY.set(locationKey,updated);const compact={...updated,captures:updated.captures.slice(-MAX_CAPTURES),references:updated.references.slice(-MAX_REFERENCES),observations:updated.observations.slice(-MAX_OBSERVATIONS)};try{localStorage.setItem(storeKey(locationKey),JSON.stringify(compact))}catch{}queueMirrorStore(locationKey,updated)}
...
export function applyLocalTwinForecast(locationKey:string,days:Day[],ensemble:EnsembleDay[],radar?:RadarNowcast|null,location?:Location){const settings=readWeatherTwinSettings();if(!settings.enabled||!settings.useAsMainForecast)return days;const report=buildForecastVerificationReport(locationKey,days,ensemble,location),byDate=new Map(report.currentForecasts.map(row=>[row.date,row]));let changed=false;const result=days.map((day,index)=>{const row=byDate.get(day.date);if(!row?.ready||!row.prediction)return day;const adjusted=index===0&&settings.nowcastAssimilation?nowcastAdjustedPrediction(row.prediction,radar).prediction:row.prediction,gust=Number.isFinite(adjusted.gust)?Math.max(day.wind,Number(adjusted.gust)):day.gust;changed=true;return{...day,max:adjusted.max,min:adjusted.min,precipitation:adjusted.precipitation,probability:adjusted.probability,gust,sunshineDuration:Number.isFinite(adjusted.sunshineDuration)?clamp(Number(adjusted.sunshineDuration),0,86400):day.sunshineDuration}});return changed?result:days}
