...
function weightedPrediction(predictions:ForecastPrediction[],weights:ModelWeight[],biases:BiasCorrection[]=[],calibrations:ProbabilityCalibrationBin[]=[]):ForecastPrediction|undefined{
 ...
 const weatherRepresentative=[...usable].filter(row=>Number.isFinite(row.prediction.precipitation)&&Number.isFinite(row.prediction.probability)).sort((a,b)=>{
  const score=(row:{weight:ModelWeight})=>((row.weight.parameterWeights.precipitation??row.weight.weight)+(row.weight.parameterWeights.probability??row.weight.weight)+(row.weight.parameterWeights.sunshine??row.weight.weight))/3;
