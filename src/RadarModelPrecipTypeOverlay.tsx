...
function phaseFor(frame:WeatherPhaseGridFrame,index:number):PhaseResult{
 const code=Math.round(finite(frame.weatherCode[index])||0),temperature=finite(frame.temperature2m[index]),humidity=finite(frame.relativeHumidity2m[index]),directWetBulb=finite(frame.wetBulbTemperature2m[index]),wetBulb=Number.isFinite(directWetBulb)?directWetBulb:wetBulbStull(temperature,humidity),precipitation=Math.max(0,finite(frame.precipitation[index])||0),rain=Math.max(0,finite(frame.rain[index])||0),showers=Math.max(0,finite(frame.showers[index])||0),snowfall=Math.max(0,finite(frame.snowfall[index])||0),rucRain=Math.max(0,finite(frame.rucRain?.[index])||0),rucSnow=Math.max(0,finite(frame.rucSnowfallWaterEquivalent?.[index])||0),rucGraupel=Math.max(0,finite(frame.graupelWaterEquivalent?.[index])||0),snowfallHeight=finite(frame.snowfallHeight[index]),freezingLevel=finite(frame.freezingLevelHeight[index]),elevation=finite(frame.elevation[index]),explicitFreezing=[56,57,66,67].includes(code),explicitSnow=[71,73,75,85,86].includes(code),explicitIceCrystals=code===76,explicitSnowGrains=code===77,explicitSnowStars=code===78,explicitIcePellets=code===79,explicitMixed=[68,69,83,84].includes(code),explicitGraupel=[87,88].includes(code),explicitHail=[89,90].includes(code),explicitGraupelHail=[96,99].includes(code),explicitThunderPrecip=[95,97].includes(code),explicitWintryAfterThunder=[93,94].includes(code),explicitRain=[51,53,55,61,63,65,80,81,82,91,92].includes(code),liquid=rain+showers,snowLevelSupports=Number.isFinite(snowfallHeight)&&Number.isFinite(elevation)&&snowfallHeight<=elevation+220,liquidLevelSupports=Number.isFinite(snowfallHeight)&&Number.isFinite(elevation)&&snowfallHeight>=elevation+320,freezeSupports=Number.isFinite(freezingLevel)&&Number.isFinite(elevation)&&freezingLevel<=elevation+280,rucEvidence=rucRain>=.003||rucSnow>=.003||rucGraupel>=.003,coldEnough=coldSupport(temperature,wetBulb),modelEvidence=precipitation>=.003||liquid>=.003||snowfall>=.003||rucEvidence||explicitFreezing||explicitSnow||explicitIceCrystals||explicitSnowGrains||explicitSnowStars||explicitIcePellets||explicitMixed||explicitGraupel||explicitHail||explicitGraupelHail||explicitThunderPrecip||explicitWintryAfterThunder||explicitRain;
 ...
 if(rucGraupel>=.003&&rucGraupel>=Math.max(rucRain*.65,rucSnow*.8))return{phase:'graupel',label:'Graupel · ICON-D2-RUC',confidence:'hoch',modelEvidence};
 if(rucSnow>=.003&&rucRain>=.003)return{phase:'mixed',label:'Mischphase · ICON-D2-RUC',confidence:'hoch',modelEvidence};
 if(rucSnow>=.003){if(((Number.isFinite(wetBulb)&&wetBulb<=1.2)||snowLevelSupports)&&coldEnough)return{phase:'snow',label:'Schnee · ICON-D2-RUC',confidence:'hoch',modelEvidence};return{phase:'mixed',label:'Mischphase · ICON-D2-RUC',confidence:'mittel',modelEvidence}}
 if(rucRain>=.003){if(coldEnough&&((Number.isFinite(wetBulb)&&wetBulb<=-.25)||(freezeSupports&&Number.isFinite(temperature)&&temperature<=1.2)))return{phase:'freezing',label:'gefrierende Phase möglich · ICON-D2-RUC',confidence:'mittel',modelEvidence};return{phase:'rain',label:'Regen · ICON-D2-RUC',confidence:'hoch',modelEvidence}}
 if(explicitFreezing&&coldEnough)return{phase:'freezing',label:'gefrierender Niederschlag',confidence:Number.isFinite(wetBulb)&&wetBulb<=.8?'hoch':'mittel',modelEvidence};
 ...
 if(explicitSnow||snowfall>=.003){if(coldEnough&&((Number.isFinite(wetBulb)&&wetBulb<=.9)||snowLevelSupports))return{phase:'snow',label:'Schnee',confidence:explicitSnow?'hoch':'mittel',modelEvidence};if(Number.isFinite(wetBulb)&&wetBulb<=1.7)return{phase:'mixed',label:'Mischphase möglich',confidence:'mittel',modelEvidence}}
 ...
 if(modelEvidence&&Number.isFinite(wetBulb)){
  if(coldEnough&&wetBulb<=.1&&snowLevelSupports&&precipitation>=.05)return{phase:'graupel',label:'Graupel möglich',confidence:'mittel',modelEvidence};
  if(coldEnough&&wetBulb<=.35&&snowLevelSupports)return{phase:'snow',label:'Schneephase wahrscheinlich',confidence:'mittel',modelEvidence};
...
function phaseMinimumDbz(phase:SymbolPhase){return phase==='hail'?15:phase==='graupel-hail'?12:phase==='graupel'?9:['snow-grains','snow-stars','ice-crystals','ice-pellets'].includes(phase)?4:(phase==='freezing'?9:phase==='snow'?6:7)}
...
function buildPhaseSymbols(data:WeatherPhaseGridData,raster:OperaRaster):PhaseSymbol[]{
 ...
 for(let row=0;row<data.lats.length;row++)for(let col=0;col<data.lons.length;col++){
  ...
  const temperature=finite(frame.temperature2m[index]),humidity=finite(frame.relativeHumidity2m[index]),directWetBulb=finite(frame.wetBulbTemperature2m[index]),wetBulb=Number.isFinite(directWetBulb)?directWetBulb:wetBulbStull(temperature,humidity),phase=radarBackedThermalPhase(phaseFor(frame,index),frame,index,echo),typed=asSymbolPhase(phase.phase);if(!typed||phase.confidence==='eingeschränkt')continue;
  if(typed==='freezing'&&(!coldSupport(temperature,wetBulb)||(Number.isFinite(temperature)&&temperature>2.5)||(Number.isFinite(wetBulb)&&wetBulb>.8)))continue;
  if((typed==='snow'||typed==='mixed')&&(Number.isFinite(temperature)&&temperature>4)&&(Number.isFinite(wetBulb)&&wetBulb>1.5))continue;
  if(echo.maxDbz<phaseMinimumDbz(typed))continue;
  candidates.push({id:`${row}:${col}:${typed}`,lat,lon,phase:typed,label:`${phase.label} · ${Math.round(echo.maxDbz)} dBZ · ${data.modelLabel||'Regionalmodell'}`,dbz:echo.maxDbz});
 }
