...
function phaseFor(frame:WeatherPhaseGridFrame,index:number):PhaseResult{
 ...
 if(rucSnow>=.003){if((Number.isFinite(wetBulb)&&wetBulb<=1.2)||snowLevelSupports)return{phase:'snow',label:'Schnee · ICON-D2-RUC',confidence:'hoch',modelEvidence};return{phase:'mixed',label:'Mischphase · ICON-D2-RUC',confidence:'mittel',modelEvidence}}
 if(rucRain>=.003){if((Number.isFinite(wetBulb)&&wetBulb<=.25)||freezeSupports)return{phase:'freezing',label:'gefrierende Phase möglich · ICON-D2-RUC',confidence:'mittel',modelEvidence};return{phase:'rain',label:'Regen · ICON-D2-RUC',confidence:'hoch',modelEvidence}}
