...
type PhaseSymbol={id:string;lat:number;lon:number;phase:'mixed'|'snow'|'freezing';symbol:string;label:string;dbz:number};
...
function phaseFor(frame:WeatherPhaseGridFrame,index:number):PhaseResult{
 ...
 if(modelEvidence&&Number.isFinite(wetBulb)){if(wetBulb<=.35&&snowLevelSupports)return{phase:'snow',label:'Schneephase wahrscheinlich',confidence:'mittel',modelEvidence};if(wetBulb>=1.7&&!freezeSupports)return{phase:'rain',label:'flüssige Phase wahrscheinlich',confidence:'mittel',modelEvidence}}
...
function phaseSymbol(phase:'mixed'|'snow'|'freezing'){return phase==='snow'?'❄️':phase==='freezing'?'🧊':'🌨️'}
function buildPhaseSymbols(data:WeatherPhaseGridData,raster:OperaRaster):PhaseSymbol[]{
 ...
 for(let row=0;row<data.lats.length;row++)for(let col=0;col<data.lons.length;col++){
  ...
  const phase=radarBackedThermalPhase(phaseFor(frame,index),frame,index,echo);if(!['mixed','snow','freezing'].includes(phase.phase)||phase.confidence==='eingeschränkt')continue;
  const typed=phase.phase as 'mixed'|'snow'|'freezing',minimumDbz=typed==='snow'||typed==='freezing'?5:7;if(echo.maxDbz<minimumDbz)continue;
 ...
 const selected:PhaseSymbol[]=[];for(const item of candidates.sort((a,b)=>b.dbz-a.dbz)){if(selected.some(existing=>distanceKm(existing,item)<14))continue;selected.push(item);if(selected.length>=90)break}return selected;
}
