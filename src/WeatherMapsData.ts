...
export type WeatherPhaseGridData={modelId:'icon-d2';modelLabel:string;targetTime:string;referenceTime?:string;lats:number[];lons:number[];frame:WeatherPhaseGridFrame;provider?:string;checkedAt?:string;gridSpacingKm?:number;coverage?:string;error?:string};
...
export async function loadWeatherPhaseGrid(lat:number,lon:number,targetTime:string,signal?:AbortSignal){const parsed=Date.parse(targetTime),quarter=15*60000,normalizedTarget=Number.isFinite(parsed)?new Date(Math.round(parsed/quarter)*quarter).toISOString():targetTime;return fetchWorkerJson<WeatherPhaseGridData>('precipitation-phase-grid',{lat,lon,target:normalizedTarget},{purpose:'radar',signal,timeoutMs:28000,maxAgeMs:10*60000,staleIfErrorMs:20*60000,cacheKey:`precipitation-phase-grid:${lat.toFixed(2)}:${lon.toFixed(2)}:${normalizedTarget.slice(0,16)}`})}
