...
export type WeatherPhaseGridFrame={time:string;temperature2m:number[];relativeHumidity2m:number[];wetBulbTemperature2m:number[];weatherCode:number[];precipitation:number[];rain:number[];showers:number[];snowfall:number[];snowfallHeight:number[];freezingLevelHeight:number[];elevation:number[];rucRain?:number[];rucSnowfallWaterEquivalent?:number[];graupelWaterEquivalent?:number[]};
