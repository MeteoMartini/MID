...
export async function loadPx250Metadata(lat:number,lon:number):Promise<Px250Meta>{
 ...
 if(!configured)return{available:false,nativeResolutionM:250,reason:'Das 250-m-Radar benötigt den Cloudflare Worker v0.7.30; ein direkter Browserabruf beim DWD ist wegen CORS nicht zuverlässig möglich.'};
 try{
  const url=new URL(configured,location.href);url.searchParams.set('mode','px250-meta');url.searchParams.set('lat',String(lat));url.searchParams.set('lon',String(lon));url.searchParams.set('_mid','0.7.30');
