...
export async function loadPx250Metadata(lat:number,lon:number):Promise<Px250Meta>{
 ...
 if(!configured)return{available:false,nativeResolutionM:250,reason:'PX250 benötigt den Cloudflare Worker v0.7.21; ein direkter Browserabruf beim DWD ist wegen CORS nicht zuverlässig möglich.'};
 try{
