...
export type MountainSnowLineModel={id:string;label:string;provider:string;members:number;maxDays:number;points:MountainSnowLineModelPoint[]};
...
const SNOWLINE_ENSEMBLE_MODELS=[
 {id:'dwd_icon_eps_ensemble_mean_seamless',label:'ICON-EPS',provider:'DWD',members:40,maxDays:7},
 {id:'ecmwf_ifs_europe_ensemble_mean',label:'IFS ENS',provider:'ECMWF',members:51,maxDays:15},
 {id:'ecmwf_aifs_europe_ensemble_mean',label:'AIFS ENS',provider:'ECMWF',members:51,maxDays:15},
 {id:'ncep_gefs_ensemble_mean_seamless',label:'GEFS',provider:'NOAA',members:31,maxDays:14},
 {id:'cmc_gem_geps_ensemble_mean',label:'GEPS',provider:'CMC',members:21,maxDays:14},
 {id:'ukmo_global_ensemble_mean_20km',label:'MOGREPS-G',provider:'UKMO',members:18,maxDays:8},
 {id:'google_weathernext2_ensemble_mean',label:'WeatherNext 2',provider:'Google',members:64,maxDays:15}
] as const;
...
async function fetchSnowLineMeanModel(loc:Location,definition:(typeof SNOWLINE_ENSEMBLE_MODELS)[number],signal?:AbortSignal):Promise<MountainSnowLineModel>{
 const variables=['snowfall_height','snowfall_height_spread','freezing_level_height','freezing_level_height_spread'],params=new URLSearchParams({latitude:String(loc.latitude),longitude:String(loc.longitude),elevation:'nan',timezone:'auto',forecast_days:String(Math.min(14,definition.maxDays)),cell_selection:'nearest',models:definition.id,hourly:variables.join(',')}),response=await fetch(`${ENSEMBLE_ENDPOINT}?${params}`,{signal,cache:'no-store'});if(!response.ok)throw new Error(`${definition.label} HTTP ${response.status}`);const payload=await response.json() as SnowLineMeanPayload;if(payload.error||!payload.hourly)throw new Error(payload.reason||`${definition.label} ohne Ensemble-Mittel`);const hourly=payload.hourly,times=Array.isArray(hourly.time)?hourly.time.map(String):[],snow=Array.isArray(hourly.snowfall_height)?hourly.snowfall_height:[],snowSpread=Array.isArray(hourly.snowfall_height_spread)?hourly.snowfall_height_spread:[],freezing=Array.isArray(hourly.freezing_level_height)?hourly.freezing_level_height:[],freezingSpread=Array.isArray(hourly.freezing_level_height_spread)?hourly.freezing_level_height_spread:[],offset=Number(payload.utc_offset_seconds)||0,points:MountainSnowLineModelPoint[]=[];
 for(let index=0;index<times.length;index++){let mean=Number(snow[index]),spread=Number(snowSpread[index]);if(!Number.isFinite(mean)){const freeze=Number(freezing[index]);if(Number.isFinite(freeze))mean=Math.max(0,freeze-250)}if(!Number.isFinite(spread)){const freezeSpread=Number(freezingSpread[index]);spread=Number.isFinite(freezeSpread)?Math.max(0,freezeSpread):0}const epoch=snowLineTimeEpoch(times[index],offset);if(!Number.isFinite(mean)||!Number.isFinite(epoch))continue;const sigma=Math.max(0,spread||0),delta=1.28155*sigma;points.push({epoch,time:times[index],mean,spread:sigma,low:Math.max(0,mean-delta),high:Math.min(8000,mean+delta)})}
 if(!points.length)throw new Error(`${definition.label} ohne Schneefallgrenze`);return{id:definition.id,label:definition.label,provider:definition.provider,members:definition.members,maxDays:definition.maxDays,points};
}
function combineSnowLineModels(models:MountainSnowLineModel[]):MountainSnowLineEnsemble|undefined{if(!models.length)return undefined;const start=Math.ceil(Math.min(...models.flatMap(model=>model.points.map(point=>point.epoch)))/(3*3600000))*(3*3600000),end=Math.max(...models.flatMap(model=>model.points.map(point=>point.epoch))),points:MountainSnowLineEnsemblePoint[]=[];for(let epoch=start;epoch<=end;epoch+=3*3600000){const rows=models.flatMap(model=>{let best:MountainSnowLineModelPoint|undefined,distance=Infinity;for(const point of model.points){const current=Math.abs(point.epoch-epoch);if(current<distance){distance=current;best=point}}return best&&distance<=95*60000?[{model,point:best}]:[]});if(!rows.length)continue;const means=rows.map(row=>row.point.mean),pseudo=rows.flatMap(row=>[row.point.low,row.point.mean,row.point.high]);points.push({epoch,time:new Date(epoch).toISOString(),median:snowLineQuantile(means,.5),p25:snowLineQuantile(pseudo,.25),p75:snowLineQuantile(pseudo,.75),p10:snowLineQuantile(pseudo,.1),p90:snowLineQuantile(pseudo,.9),modelCount:rows.length,memberEquivalent:rows.reduce((sum,row)=>sum+row.model.members,0)})}return points.length?{models,points,source:'Open-Meteo Ensemble Mean API · gleichgewichtete Modell-Systeme; 10–90-%-Band aus Modellmittel ± Ensemble-Spread'}:undefined}
async function mountainSnowLineEnsemble(loc:Location,signal?:AbortSignal){const settled=await Promise.allSettled(SNOWLINE_ENSEMBLE_MODELS.map(model=>fetchSnowLineMeanModel(loc,model,signal))),models=settled.flatMap(result=>result.status==='fulfilled'?[result.value]:[]);return combineSnowLineModels(models)}
...
export async function mountainSportsForecast(loc:Location,config:MountainConfig,signal?:AbortSignal):Promise<MountainSportsForecast>{
 const snowLineEnsemblePromise=mountainSnowLineEnsemble(loc,signal).catch(()=>undefined),points=configuredPoints(loc,config),latitudes=points.map(point=>point.latitude).join(','),longitudes=points.map(point=>point.longitude).join(','),elevations=points.map(point=>point.elevation).join(','),surfaceVariables=['temperature_2m','apparent_temperature','relative_humidity_2m','dew_point_2m','precipitation_probability','precipitation','rain','showers','snowfall','snow_depth','weather_code','cloud_cover','cloud_cover_low','visibility','freezing_level_height','wet_bulb_temperature_2m','wind_speed_10m','wind_gusts_10m','wind_direction_10m','uv_index','cape','is_day'],profileVariables=MOUNTAIN_CLOUD_PROFILE_LEVELS.flatMap(level=>[`cloud_cover_${level}hPa`,`geopotential_height_${level}hPa`]),params=new URLSearchParams({latitude:latitudes,longitude:longitudes,elevation:elevations,timezone:'auto',forecast_hours:'72',past_hours:'24',models:'best_match',wind_speed_unit:'kn',current:surfaceVariables.filter(key=>key!=='precipitation_probability').join(','),hourly:[...surfaceVariables,...profileVariables].join(',')}),response=await fetch(`${FORECAST_ENDPOINT}?${params}`,{signal,cache:'no-store'});if(!response.ok)throw new Error(`Höhenprognose HTTP ${response.status}`);const raw=await response.json() as MountainPointWeather[]|MountainPointWeather,rows=Array.isArray(raw)?raw:[raw];if(rows.length!==points.length)throw new Error('Die Höhenprognose lieferte nicht alle konfigurierten Niveaus.');const baseLevels=points.map((point,index)=>{const weather=rows[index],snowfall=snowfallSums(weather),depth=currentValue(weather,'snow_depth');return{...point,weather,modelSnowDepthCm:Number.isFinite(depth)?Math.max(0,depth*100):NaN,measuredSnowDepthCm:NaN,pastSnow24Cm:snowfall.past24,newSnow24Cm:snowfall.next24,newSnow48Cm:snowfall.next48} satisfies MountainLevelForecast}),measurements=await Promise.all(baseLevels.map(level=>geoSphereSnowMeasurement(level,signal))),levels=baseLevels.map((level,index)=>{const snowMeasurement=measurements[index];return{...level,measuredSnowDepthCm:snowMeasurement?.valueCm??NaN,snowMeasurement}}),season=config.season==='auto'?(autoWinter(loc.latitude,levels)?'winter':'summer'):config.season,snowLineEnsemble=await snowLineEnsemblePromise;return{levels,season,source:'Open-Meteo Best Match · höhenbezogene Koordinaten und Höhen · GeoSphere-Schneemessung bei strenger Nähe-/Höhen-/Aktualitätsprüfung',snowLineEnsemble};
}
