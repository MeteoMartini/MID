...
async function directMountainProfile(loc:Location,signal?:AbortSignal):Promise<MountainProfileResult>{
 ...
 for(const valley of valleyPool)for(const summit of summitPool){if(valley===summit)continue;const gain=Number(summit.elevation)-Number(valley.elevation),span=distanceMeters(valley.latitude,valley.longitude,summit.latitude,summit.longitude);if(gain<250||span>30000)continue;const sameLift=Boolean(valley.liftId&&valley.liftId===summit.liftId),nearest=Math.min(valley.distanceM,summit.distanceM);if(nearest>22000)continue;const score=gain/16-valley.distanceM/420-summit.distanceM/900+(valley.role==='valley'?24:0)+(summit.role==='summit'?24:0)+(sameLift?58:0)+(valley.kind==='station'?6:0)+(summit.kind==='station'?6:0);if(!best||score>best.score)best={valley,summit,score}}
...
function validProfile(result:MountainProfileResult|undefined,loc:Location){const levels=Array.isArray(result?.levels)?result.levels:[],valley=levels.find(level=>level.role==='valley'),summit=levels.find(level=>level.role==='summit'),nearFavorite=levels.every(level=>finite(level.latitude)&&finite(level.longitude)&&distanceMeters(loc.latitude,loc.longitude,Number(level.latitude),Number(level.longitude))<=32000);return Boolean(valley&&summit&&nearFavorite&&finite(valley.latitude)&&finite(valley.longitude)&&finite(valley.elevation)&&finite(summit.latitude)&&finite(summit.longitude)&&finite(summit.elevation)&&Number(summit.elevation)>Number(valley.elevation))}

export async function mountainProfile(loc:Location,signal?:AbortSignal):Promise<MountainProfileResult>{
 let workerError='';try{const result=await fetchWorkerJson<MountainProfileResult&{error?:string}>('mountain-profile',{lat:loc.latitude,lon:loc.longitude,name:loc.name,country:countryCodeFromLocation(loc.country_code)||countryCodeFromLocation(loc.country)},{purpose:'general',signal,timeoutMs:14000});if(validProfile(result,loc))return result;workerError='Worker-Höhenprofil passt räumlich nicht zum gewählten Favoriten.'}catch(error){if(signal?.aborted)throw error;workerError=error instanceof Error?error.message:String(error)}
