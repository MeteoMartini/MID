...
export function mountainDayDates(level:MountainLevelForecast){const times=(level.weather.hourly.time??[]) as string[];return[...new Set(times.map(value=>String(value).slice(0,10)).filter(value=>/^\d{4}-\d{2}-\d{2}$/.test(value)))].slice(0,7)}
