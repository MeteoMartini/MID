...
import {AlertTriangle,CalendarRange,CheckCircle2,CloudRain,MapPin,RefreshCw,Search,Snowflake,Sparkles,Sun,Thermometer,Wind} from 'lucide-react';
import {icon,label,searchLocations,type Location} from './weather';
...
function destinationLabel(location:Location){return[location.name,location.admin1,location.country].filter(Boolean).join(', ')}
...
export default function TravelPlannerPanel({initialLocation,advancedMode}:Props){
 const initialStart=addDays(localToday(),30),initialEnd=addDays(initialStart,13),[destination,setDestination]=useState<Location>(()=>storedLocation()??initialLocation),[query,setQuery]=useState(''),[searching,setSearching]=useState(false),[searchError,setSearchError]=useState(''),[searchResults,setSearchResults]=useState<Location[]>([]),[mode,setMode]=useState<PlannerMode>(()=>storageGet(MODE_KEY)==='flexible'?'flexible':'fixed'),[start,setStart]=useState(initialStart),[end,setEnd]=useState(initialEnd),[tripDays,setTripDays]=useState(14),[preference,setPreference]=useState<TravelPreference>(()=>PREFERENCES.some(item=>item.id===storageGet(PREFERENCE_KEY))?storageGet(PREFERENCE_KEY) as TravelPreference:'balanced'),[minAvgMax,setMinAvgMax]=useState(''),[maxAvgMax,setMaxAvgMax]=useState(''),[maxWetDays,setMaxWetDays]=useState(''),[minSunHours,setMinSunHours]=useState(''),[maxWind,setMaxWind]=useState(''),[minSnowDepth,setMinSnowDepth]=useState(''),[loading,setLoading]=useState(false),[error,setError]=useState(''),[analysis,setAnalysis]=useState<AnalysisState|null>(null),[activeIndex,setActiveIndex]=useState(0),searchController=useRef<AbortController|null>(null),analysisController=useRef<AbortController|null>(null);
 useEffect(()=>()=>{searchController.current?.abort();analysisController.current?.abort()},[]);
 const preferenceInfo=PREFERENCES.find(item=>item.id===preference)??PREFERENCES[0],constraints=useMemo<TravelConstraints>(()=>({minAvgMax:optionalNumber(minAvgMax),maxAvgMax:optionalNumber(maxAvgMax),maxWetDays:optionalNumber(maxWetDays),minSunHoursPerDay:optionalNumber(minSunHours),maxWindKmh:optionalNumber(maxWind),minSnowDepthCm:optionalNumber(minSnowDepth)}),[minAvgMax,maxAvgMax,maxWetDays,minSunHours,maxWind,minSnowDepth]),active=analysis?.windows[activeIndex]??analysis?.windows[0],span=dateRange(start,end).length;
 ...
 async function analyse(event:FormEvent){
  ...
  const includeSnowDepth=mode==='flexible'&&(preference==='snow'||Number.isFinite(constraints.minSnowDepthCm));setLoading(true);storageSet(PREFERENCE_KEY,preference);
 ...
 return <section className="travel-planner">
  <header className="travel-head"><div><span>KLIMATOLOGISCHE REISEPLANUNG</span><h3>Reisewetter & Reiseplaner</h3><p>Erwartbare Bedingungen für einen festen Zeitraum oder das klimatologisch passendste Reisefenster.</p></div><span className="travel-climate-badge">ERA5-Land · 1991–2020</span></header>
  ...
  <div className="travel-destination-card">
   ...
   <form className="travel-location-search" onSubmit={runSearch}><label><span>Anderes Reiseziel suchen</span><div><Search size={16}/><input value={query} onChange={(event:ValueEvent)=>setQuery(event.target.value)} placeholder="Ort, Region oder Reiseziel"/></div></label><button type="submit" className="secondary" disabled={searching}>{searching?<RefreshCw className="spin" size={16}/>:<Search size={16}/>} Suchen</button></form>
   {searchError&&<small className="travel-search-error">{searchError}</small>}
   {searchResults.length>0&&<div className="travel-search-results" role="listbox" aria-label="Reiseziele">{searchResults.slice(0,8).map(location=><button type="button" key={`${location.id}:${location.latitude}:${location.longitude}`} onClick={()=>chooseLocation(location)}><MapPin size={15}/><span><strong>{location.name}</strong><small>{[location.admin1,location.country].filter(Boolean).join(' · ')||`${number(location.latitude,2)}°, ${number(location.longitude,2)}°`}</small></span></button>)}</div>}
  </div>
  ...
  <div className="travel-method-note"><AlertTriangle size={15}/><span>Das Ergebnis ist eine klimatologische Erwartung aus vergangenen Jahren und keine Wettervorhersage für den konkreten Reisetermin.</span></div>
  {error&&<div className="error">{error}</div>}
  {loading&&<div className="travel-loading"><RefreshCw className="spin" size={20}/><div><strong>Langjährige Klimadaten werden ausgewertet</strong><span>{mode==='flexible'&&(preference==='snow'||Number.isFinite(constraints.minSnowDepthCm))?'Für die Schneelage werden zusätzlich historische Stundenwerte verarbeitet.':'Die Abfrage erfolgt erst nach dieser ausdrücklichen Auswahl und nicht automatisch.'}</span></div></div>}
  ...
  {analysis&&active&&<section className="travel-result">
   ...
   <div className="travel-metrics">
    ...
    <Metric iconNode={<Wind/>} labelText="Wind" value={`${number(active.summary.windMaxMean,0)} km/h`} detail="Ø tägliches Windmaximum"/>
