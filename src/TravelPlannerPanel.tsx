...
const PREFERENCES:{id:TravelPreference;label:string;detail:string}[]=[
 ...
 {id:'snow',label:'Hohe Schneelage',detail:'hohe historische Schneehöhe; kumulierter Schneefall zusätzlich'},
...
export default function TravelPlannerPanel({initialLocation,advancedMode,unit}:Props){
 ...
 async function analyse(event:FormEvent){
  ...
  try{
   const dataset=await fetchTravelClimatology(destination,includeSnowDepth,controller.signal);if(mode==='flexible'&&preference==='snow'&&!dataset.snowDepthIncluded)throw new Error(dataset.snowDepthWarning||'Historische Schneehöhe ist derzeit nicht verfügbar; „Hohe Schneelage“ kann deshalb nicht belastbar optimiert werden.');let windows:TravelWindowResult[];
   if(mode==='fixed'){
 ...
 return <section className="travel-planner">
  ...
  <div className="travel-method-note"><AlertTriangle size={15}/><span>Das Ergebnis ist eine klimatologische Erwartung aus vergangenen Jahren und keine Wettervorhersage für den konkreten Reisetermin. Pro gerastertem Klimapunkt erfolgt höchstens ein direkter Basisabruf für die atmosphärische Klimatologie; ERA5-Seamless kombiniert die feinere ERA5-Land-Temperatur mit ERA5 für Niederschlag, Sonne und Wind. Für Küstenorte ermittelt MID zusätzlich das tägliche NOAA-OISST-v2.1-Meeresoberflächenmittel der Normperiode 1991–2020 exakt für die Kalendertage des Reisezeitraums. Landzellen und Meeresgitter weiter als 80 km werden ausgeschlossen; aktuelle Marinewerte werden nicht verwendet. Positive Ergebnisse werden lokal gespeichert. Beim festen Zeitraum sowie für eine schneehöhenbezogene Optimierung oder Bedingung lädt MID zusätzlich historische Schneehöhendaten.</span></div>
  {error&&<div className="error">{error}</div>}
  {loading&&<div className="travel-loading"><RefreshCw className="spin" size={20}/><div><strong>Langjährige Klimadaten werden ausgewertet</strong><span>{snowDepthRequired?'Für die Schneehöhenbewertung wird einmalig ein zusätzlicher historischer Datensatz verarbeitet.':'Es wird höchstens ein kompakter Basisdatensatz geladen; Wiederholungen nutzen den lokalen Klimacache.'}</span></div></div>}
