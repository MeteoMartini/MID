...
import {marineForecast} from './weather';
...
type TravelWaterInfo={temperature:number;gridDistanceKm:number;timeLabel?:string};
...
function marineTimeLabel(value:unknown){if(typeof value!=='string'||!value)return undefined;const epoch=Date.parse(value);if(!Number.isFinite(epoch))return undefined;try{return new Intl.DateTimeFormat('de-DE',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}).format(new Date(epoch))}catch{return undefined}}
function coastalWaterInfo(location:Location,forecast:Awaited<ReturnType<typeof marineForecast>>):TravelWaterInfo|null{const temperature=Number(forecast.current?.sea_surface_temperature),gridDistanceKm=haversineKm(location.latitude,location.longitude,forecast.latitude,forecast.longitude);if(!Number.isFinite(temperature)||!Number.isFinite(gridDistanceKm)||gridDistanceKm>COASTAL_WATER_MAX_DISTANCE_KM)return null;return{temperature,gridDistanceKm,timeLabel:marineTimeLabel(forecast.current?.time)}}
...
export default function TravelPlannerPanel({initialLocation,advancedMode,unit}:Props){
 ...
 useEffect(()=>{
  if(!analysis){setWaterInfo(null);return}
  const controller=new AbortController();let cancelled=false;
  marineForecast(destination.latitude,destination.longitude,'auto',controller.signal).then(forecast=>{if(cancelled||controller.signal.aborted)return;setWaterInfo(coastalWaterInfo(destination,forecast))}).catch(()=>{if(!cancelled&&!controller.signal.aborted)setWaterInfo(null)});
  return()=>{cancelled=true;controller.abort()}
 },[analysis,destination.latitude,destination.longitude,destination.id]);
 ...
 return <section className="travel-planner">
  ...
  <div className="travel-method-note"><AlertTriangle size={15}/><span>Das Ergebnis ist eine klimatologische Erwartung aus vergangenen Jahren und keine Wettervorhersage für den konkreten Reisetermin. Pro gerastertem Klimapunkt erfolgt höchstens ein direkter Basisabruf; ERA5-Seamless kombiniert die feinere ERA5-Land-Temperatur mit ERA5 für Niederschlag, Sonne und Wind. Das Ergebnis bleibt drei Jahre lokal gespeichert. Die Schneehöhe ist ein optionaler Zusatzabruf. Der MID-Worker wird dafür nicht verwendet.</span></div>
  ...
  {analysis&&active&&<section className="travel-result">
   ...
   <div className="travel-metrics">
    ...
    {waterInfo&&<Metric iconNode={<Droplets/>} labelText="Wassertemperatur" value={`${number(waterInfo.temperature)} °C`} detail={`küstennahes Meer${waterInfo.timeLabel?` · Stand ${waterInfo.timeLabel} Uhr`:''} · nächstes Meeresgitter ${number(waterInfo.gridDistanceKm,0)} km`}/>}
   </div>
