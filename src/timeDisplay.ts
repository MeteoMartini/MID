...
export function displayTimeSuffix(mode:TimeDisplayMode=readTimeDisplayMode()){return mode==='zulu'?'Z':'LT'}
...
function displayOptionsContainClock(options:Intl.DateTimeFormatOptions){return Boolean(options.hour||options.minute||options.second||options.timeStyle)}
export function formatDisplayDateTime(value:string|number|Date,localTimeZone:string|undefined,options:Intl.DateTimeFormatOptions){const date=value instanceof Date?value:new Date(value);if(!Number.isFinite(date.getTime()))return'–';const mode=readTimeDisplayMode(),zone=displayTimeZone(localTimeZone,mode),markConvertedLocal=mode==='local'&&!localTimeZone&&displayOptionsContainClock(options);try{const text=new Intl.DateTimeFormat('de-DE',{...options,timeZone:zone}).format(date);return markConvertedLocal?`${text} LT`:text}catch{const text=new Intl.DateTimeFormat('de-DE',options).format(date);return markConvertedLocal?`${text} LT`:text}}
