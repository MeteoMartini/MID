...
export type RasterTone={saturation?:number;contrast?:number;brightnessMin?:number;brightnessMax?:number;hueRotate?:number};
const clampRasterTone=(value:number,minimum:number,maximum:number)=>Math.max(minimum,Math.min(maximum,value));
export function RasterTileLayer({id,url,opacity=1,minZoom=0,maxZoom=22,tileSize=256,attribution,zIndex=0,tone,onReady,onError}:{id:string;url:string;opacity?:number;minZoom?:number;maxZoom?:number;tileSize?:number;attribution?:string;zIndex?:number;tone?:RasterTone;onReady?:()=>void;onError?:(message?:string)=>void}){
 ...
 useEffect(()=>{if(!map||!url)return;let active=true,ready=false;const errorHandler=(event:maplibregl.ErrorEvent)=>{const source=(event as unknown as {sourceId?:string}).sourceId;if(source&&source!==sourceId)return;if(active)onError?.(event.error?.message)};const idleHandler=()=>{if(!active||ready||!map.getLayer(layerId))return;ready=true;onReady?.()};
  try{if(map.getLayer(layerId))map.removeLayer(layerId);if(map.getSource(sourceId))map.removeSource(sourceId);map.addSource(sourceId,{type:'raster',tiles:[url],tileSize,minzoom:minZoom,maxzoom:maxZoom,attribution});map.addLayer({id:layerId,type:'raster',source:sourceId,paint:{'raster-opacity':Math.max(0,Math.min(1,opacity)),'raster-fade-duration':0,'raster-resampling':'linear',...(tone?.saturation!==undefined?{'raster-saturation':clampRasterTone(tone.saturation,-1,1)}:{}),...(tone?.contrast!==undefined?{'raster-contrast':clampRasterTone(tone.contrast,-1,1)}:{}),...(tone?.brightnessMin!==undefined?{'raster-brightness-min':clampRasterTone(tone.brightnessMin,0,1)}:{}),...(tone?.brightnessMax!==undefined?{'raster-brightness-max':clampRasterTone(tone.brightnessMax,0,1)}:{}),...(tone?.hueRotate!==undefined?{'raster-hue-rotate':clampRasterTone(tone.hueRotate,0,360)}:{})}});registerMapLayerOrder(map,layerId,zIndex);map.on('error',errorHandler);map.on('idle',idleHandler)}catch(error){onError?.(error instanceof Error?error.message:String(error))}
 ...
 },[map,id,url,minZoom,maxZoom,tileSize,attribution,zIndex,tone?.saturation,tone?.contrast,tone?.brightnessMin,tone?.brightnessMax,tone?.hueRotate]);
