...
import {airQuality,cloudOktas,cloudOktasText,currentIndex,dayEffectiveUvMax,dayWeatherCharacter,ensembles,forecast,hazards,icon,label,mapDays,mapHours,mapMinutely15,officialWarnings,searchLocations,reverseLocation,station,wind,type Day,type EnsembleDay,type Hour,type Location,type Minute15,type OfficialAlert,type Station,type Weather,type WindUnit} from './weather';
...
export default function App(){
 const[loc,setLocState]=useState<Location|null>(()=>storedLocation()),[w,setW]=useState<Weather|null>(null),[air,setAir]=useState<any>(null),[ens,setEns]=useState<EnsembleDay[]>([]),[models,setModels]=useState<string[]>([]),[ensLoading,setEnsLoading]=useState(false),[ensError,setEnsError]=useState(''),[st,setSt]=useState<Station|null>(null),[stationLoading,setStationLoading]=useState(false),[official,setOfficial]=useState<OfficialAlert[]>([]),[officialLoading,setOfficialLoading]=useState(false),[officialError,setOfficialError]=useState(''),[officialProvider,setOfficialProvider]=useState(''),[loading,setLoading]=useState(false),[error,setError]=useState(''),[dark,setDark]=useState(()=>localStorage.getItem('theme')!=='light'),[unit,setUnit]=useState<WindUnit>(()=>(localStorage.getItem('windUnit') as WindUnit)||'kn'),[selected,setSelected]=useState('');
 const seq=useRef(0);
