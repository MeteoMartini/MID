...
import {airQuality,cloudOktas,countryCodeFromLocation,cloudOktasText,currentIndex,dayEffectiveUvMax,dayWeatherCharacter,ensembles,forecast,hazards,icon,label,mapDays,mapHours,mapMinutely15,officialWarnings,searchLocations,reverseLocation,station,wind,type Day,type EnsembleDay,type Hour,type Location,type Minute15,type OfficialAlert,type Station,type Weather,type WindUnit} from './weather';
...
export default function App(){
 ...
 function setLoc(next:Location){const normalized=normalizeLocation(next);localStorage.setItem(LOCATION_STORAGE_KEY,JSON.stringify(normalized));setLocState(normalized);setW(null);setAir(null);setEns([]);setModels([]);setOfficial([]);setOfficialError('');setOfficialProvider('');setSelected('')}
