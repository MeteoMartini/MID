...
export default function App(){
 const[loc,setLocState]=useState<Location|null>(()=>storedLocation()),[w,setW]=useState<Weather|null>(null),[air,setAir]=useState<any>(null),[ens,setEns]=useState<EnsembleDay[]>([]),[models,setModels]=useState<string[]>([]),[ensLoading,setEnsLoading]=useState(false),[ensError,setEnsError]=useState(''),[st,setSt]=useState<Station|null>(null),[stationLoading,setStationLoading]=useState(false),[loading,setLoading]=useState(false),[error,setError]=useState(''),[dark,setDark]=useState(()=>localStorage.getItem('theme')!=='light'),[unit,setUnit]=useState<WindUnit>(()=>(localStorage.getItem('windUnit') as WindUnit)||'kn'),[selected,setSelected]=useState('');
 const seq=useRef(0);
 ...
 function setLoc(next:Location){localStorage.setItem(LOCATION_STORAGE_KEY,JSON.stringify(next));setLocState(next);setW(null);setAir(null);setEns([]);setModels([]);setSelected('')}
...
function TrendTooltip({active,payload,label}:{active?:boolean;payload?:any[];label?:string}){if(!active||!payload?.length)return null;const row=payload[0]?.payload;return <div className="charttooltip trend-tooltip"><strong>{label}</strong><div className="tooltip-group"><b>Best Match</b><span>Tmin: {row.bestMin?.toFixed(1)??'–'} °C</span><span>Tmax: {row.bestMax?.toFixed(1)??'–'} °C</span></div><div className="tooltip-group"><b>ENS-Mittel</b><span>Tmin: {row.minMean.toFixed(1)} °C</span><span>Tmax: {row.maxMean.toFixed(1)} °C</span></div><div className="tooltip-group"><b>P10–P90</b><span>Tmin: {row.minLow.toFixed(1)} bis {row.minHigh.toFixed(1)} °C</span><span>Tmax: {row.maxLow.toFixed(1)} bis {row.maxHigh.toFixed(1)} °C</span></div><div className="tooltip-group compact"><span><b>Prognosekonsistenz:</b> {Math.round(row.confidence)} % ({row.modelCount} Modelle{row.memberCount?` · ${row.memberCount} Mitglieder`:''})</span></div></div>}
...
function Ensembles({data,models,days,loading,error}:{data:EnsembleDay[];models:string[];days:Day[];loading:boolean;error:string}){
 const[showRainProbability,setShowRainProbability]=useState(true);
 ...
 return <section className="card ensemble"><Title eye={`${models.length} verfügbare ENS-Modelle`} title="14-Tage-Ensemble-Trend"/>
  ...
  <div className="ens-summary">{hasMembers?<span><b>{summaryMembers}</b> Ensemble-Mitglieder pro Tag im Mittel</span>:<span><b>{models.length}</b> verfügbare Modellmittel</span>}<span><b>{models.length}</b> Modellfamilien aktiv</span><span><i className="dot demo" style={{background:'#45c86b'}}/> hohe Konsistenz</span><span><i className="dot demo" style={{background:'#f2c94c'}}/> mittlere Konsistenz</span><span><i className="dot demo" style={{background:'#eb5757'}}/> geringe Konsistenz</span></div>
  ...
  <div className="charthead"><h3>Temperaturtrend und Prognoseunsicherheit</h3><p>Best Match als Referenz sowie gewichtete Ensemble-Mittel und P10–P90-Intervalle.</p></div><CombinedTrendChart data={d}/>
...
function Widget({loc,days,hours,unit}:{loc:Location;days:Day[];hours:Hour[];unit:WindUnit}){
 ...
 const previewDays=days.slice(0,n).map(d=>({...d,hz:dailyHazards(d,hours.filter(x=>x.time.startsWith(d.date)))}));
