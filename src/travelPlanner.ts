...
function circularMean(values:number[]){const finite=values.filter(Number.isFinite);if(!finite.length)return Number.NaN;const vector=finite.reduce((sum,value)=>({x:sum.x+Math.sin(value*Math.PI/180),y:sum.y+Math.cos(value*Math.PI/180)}),{x:0,y:0});return(Math.atan2(vector.x,vector.y)*180/Math.PI+360)%360}
...
function emptyBucket():Bucket{return{max:[],min:[],mean:[],precipitation:[],wet:0,snow:0,sunshine:[],daylight:[],wind:[],windMean:[],gust:[],windDirection:[],snowfall:[],cloud:[],codes:new Map()}}
...
async function fetchJson<T>(url:string):Promise<T>{const controller=new AbortController(),timer=globalThis.setTimeout(()=>controller.abort(new DOMException('Historische Wetterdaten haben das Zeitlimit überschritten.','TimeoutError')),30000);try{const response=await guardedOpenMeteoFetch(url,{signal:controller.signal,cache:'force-cache',headers:{Accept:'application/json'}},{priority:'normal'}),payload=await response.json().catch(()=>({}));if(!response.ok||(payload as {error?:boolean}).error)throw new Error(String((payload as {reason?:string}).reason||`Open-Meteo HTTP ${response.status}`));return payload as T}finally{globalThis.clearTimeout(timer)}}
...
export function aggregateTravelClimate(payload:HistoricalDailyPayload):TravelClimateDataset{
 ...
 for(let index=0;index<times.length;index++){
  ...
  const max=numberAt(daily.temperature_2m_max??[],index),min=numberAt(daily.temperature_2m_min??[],index),precipitation=numberAt(daily.precipitation_sum??[],index),sunshine=numberAt(daily.sunshine_duration??[],index),daylight=numberAt(daily.daylight_duration??[],index),wind=numberAt(daily.wind_speed_10m_max??[],index),windMean=numberAt(daily.wind_speed_10m_mean??[],index),gust=numberAt(daily.wind_gusts_10m_max??[],index),windDirection=numberAt(daily.wind_direction_10m_dominant??[],index),cloud=numberAt(daily.cloud_cover_mean??[],index),snowfall=numberAt(daily.snowfall_sum??[],index),code=Math.round(numberAt(daily.weather_code??[],index)),meanValue=Number.isFinite(max)&&Number.isFinite(min)?(max+min)/2:Number.NaN,cloudProxy=Number.isFinite(cloud)?cloud:Number.isFinite(sunshine)&&Number.isFinite(daylight)&&daylight>0?Math.max(0,Math.min(100,100*(1-sunshine/daylight))):Number.NaN;
  addFinite(bucket.max,max);addFinite(bucket.min,min);addFinite(bucket.mean,meanValue);addFinite(bucket.precipitation,precipitation);addFinite(bucket.sunshine,sunshine/3600);addFinite(bucket.daylight,daylight/3600);addFinite(bucket.wind,wind);addFinite(bucket.windMean,windMean);addFinite(bucket.gust,gust);addFinite(bucket.windDirection,windDirection);addFinite(bucket.snowfall,snowfall);addFinite(bucket.cloud,cloudProxy);
  if(Number.isFinite(precipitation)&&precipitation>=1)bucket.wet++;
  if(Number.isFinite(snowfall)&&snowfall>=.1)bucket.snow++;
 ...
 for(const[key,bucket]of buckets){
  ...
  days[key]={key,maxMean:mean(bucket.max),minMean:mean(bucket.min),meanMean:mean(bucket.mean),maxP25:quantile(bucket.max,.25),maxP75:quantile(bucket.max,.75),minP25:quantile(bucket.min,.25),minP75:quantile(bucket.min,.75),precipitationMean:mean(bucket.precipitation),wetProbability:bucket.precipitation.length?bucket.wet/bucket.precipitation.length*100:NaN,sunshineMeanHours:mean(bucket.sunshine),daylightMeanHours:mean(bucket.daylight),windMaxMean:mean(bucket.wind),windMean:mean(bucket.windMean),gustMaxMean:bucket.gust.length?mean(bucket.gust):undefined,gustRecordMax:bucket.gust.length?Math.max(...bucket.gust):undefined,windDirection:circularMean(bucket.windDirection),snowfallMean:mean(bucket.snowfall),snowProbability:bucket.snowfall.length?bucket.snow/bucket.snowfall.length*100:NaN,cloudMean:mean(bucket.cloud),weatherCode:mode(bucket.codes),years};
 }
...
export async function fetchTravelClimatology(location:{latitude:number;longitude:number;elevation?:number},includeSnowDepth:boolean,signal?:AbortSignal):Promise<TravelClimateDataset>{
 ...
 if(!dataset){
  const request=sharedRequest<TravelClimateDataset>(baseKey,async()=>{const cached=readCache<TravelClimateDataset>(baseKey);if(cached)return cached;const makeParams=(daily:string)=>{const params=new URLSearchParams({latitude:String(climateLocation.latitude),longitude:String(climateLocation.longitude),start_date:'1991-01-01',end_date:'2020-12-31',daily,timezone:'auto',models:'era5_seamless',cell_selection:'land',temperature_unit:'celsius',precipitation_unit:'mm',wind_speed_unit:'kn'});if(Number.isFinite(climateLocation.elevation))params.set('elevation',String(climateLocation.elevation));return params};let payload:HistoricalDailyPayload;try{payload=await fetchJson<HistoricalDailyPayload>(`https://archive-api.open-meteo.com/v1/archive?${makeParams(DAILY_VARIABLES)}`)}catch{payload=await fetchJson<HistoricalDailyPayload>(`https://archive-api.open-meteo.com/v1/archive?${makeParams(DAILY_VARIABLES_WITHOUT_GUST)}`)}assertTravelHistoricalPayload(payload);const result=aggregateTravelClimate(payload);writeCache(baseKey,result);return result});
...
async function loadTravelWaterPeriod(location:{latitude:number;longitude:number},start:string,end:string,signal?:AbortSignal):Promise<TravelWaterPeriodCache>{
 ...
 const request=sharedRequest<TravelWaterPeriodCache>(key,async()=>{const existing=readCache<TravelWaterPeriodCache>(key);if(existing)return existing;if(!validDateParts(start)||!validDateParts(end))throw new Error('Ungültiger Reisezeitraum für die Wassertemperatur.');
  ...
  if(payload.schema!=='mid.travel-water-climate.v1')throw new Error('Die NOAA-Wassertemperatur ist über den aktuellen MID-Datendienst noch nicht verfügbar.');
  ...
  if(!Number.isFinite(temperature)||temperature<-3||temperature>45||!Number.isFinite(gridDistanceKm)||gridDistanceKm<0||gridDistanceKm>COASTAL_WATER_MAX_DISTANCE_KM||!Number.isFinite(gridLat)||!Number.isFinite(gridLon))throw new Error('Für diesen Küstenort konnte keine plausible NOAA-Wassertemperatur bestimmt werden.');
