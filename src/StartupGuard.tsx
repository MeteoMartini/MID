...
function StartupRecovery(){
 ...
 const run=async(action:()=>Promise<void>,success:string)=>{setBusy(true);setMessage('Wiederherstellung läuft …');try{await action();setMessage(success);window.setTimeout(()=>location.reload(),650)}catch{setMessage('Wiederherstellung fehlgeschlagen. Bitte erneut versuchen.')}finally{setBusy(false)}};
 return <main className="mid-startup-recovery"><section><AlertTriangle size={34}/><h1>MID konnte nicht vollständig starten</h1><p>Deine Favoriten, Einstellungen und Wetterzwillingsdaten werden nicht gelöscht. Du kannst den App-Cache reparieren oder zuerst eine Sicherung erstellen.</p><div><button type="button" disabled={busy} onClick={()=>location.reload()}><RefreshCw size={17}/>Neu laden</button><button type="button" disabled={busy} onClick={()=>void run(async()=>{await repairMidCache()},'Cache repariert. MID wird neu geladen …')}><Wrench size={17}/>App-Cache reparieren</button><button type="button" disabled={busy} onClick={()=>void run(async()=>{await resetMidServiceWorker()},'App-Cache zurückgesetzt. MID wird neu geladen …')}><ShieldCheck size={17}/>Abgesichert neu starten</button><button type="button" disabled={busy} onClick={()=>void run(async()=>{await saveICloudBackup()},'Sicherung wurde geöffnet beziehungsweise gespeichert.')}><CloudUpload size={17}/>Daten sichern</button></div>{message&&<p className="mid-startup-recovery-message" role="status">{message}</p>}</section></main>
}
...
export class StartupGuard extends Component<Props,State>{
 ...
 render(){return this.state.error?<StartupRecovery/>:this.props.children}
}
