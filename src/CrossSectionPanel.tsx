...
type TerrainPoint={fraction:number;distanceKm:number;latitude:number;longitude:number;elevation:number};
...
type CrossSectionData={route:string;waypoints:CrossWaypoint[];points:CrossPoint[];terrainProfile?:TerrainPoint[];totalDistanceKm:number;startTime:string;endTime:string;flightLevel:number;model:string;modelLabel:string;generatedAt:string;source:string;weatherSampleCount?:number;terrainSampleCount?:number;version?:string;error?:string};
...
function interpolateLevel(levels:CrossLevel[],targetHft:number,key:'temperature'|'humidity'|'cloud'|'windSpeed'|'windDirection'){
 ...
 for(let i=1;i<rows.length;i++){const a=rows[i-1],b=rows[i];if(targetHft<=b.height){const f=(targetHft-a.height)/Math.max(1,b.height-a.height);return key==='windDirection'?mixDirection(a.value,b.value,f):a.value+(b.value-a.value)*f}}
 return null;
}
...
function interpolateLevelPair(levelA:CrossLevel|null,levelB:CrossLevel|null,f:number):CrossLevel|null{
 ...
 return {pressure,height:mixNullable(levelA?.height??null,levelB?.height??null,f),temperature:mixNullable(levelA?.temperature??null,levelB?.temperature??null,f),humidity:mixNullable(levelA?.humidity??null,levelB?.humidity??null,f),cloud:mixNullable(levelA?.cloud??null,levelB?.cloud??null,f),windSpeed:mixNullable(levelA?.windSpeed??null,levelB?.windSpeed??null,f),windDirection:mixDirection(levelA?.windDirection??null,levelB?.windDirection??null,f)};
}
function interpolatePoint(a:CrossPoint,b:CrossPoint,f:number):DensePoint{
 ...
 return {fraction:lerp(a.fraction,b.fraction,f),distanceKm:lerp(a.distanceKm,b.distanceKm,f),latitude:lerp(a.latitude,b.latitude,f),longitude:lerp(a.longitude,b.longitude,f),elevation:lerp(a.elevation,b.elevation,f),validTime:time,precipitation:mixNullable(a.precipitation,b.precipitation,f),levels:BASE_PRESSURES.map(pressure=>interpolateLevelPair(levelByPressure(a.levels,pressure),levelByPressure(b.levels,pressure),f)).filter((level):level is CrossLevel=>Boolean(level))};
}
function densifyPoints(points:CrossPoint[],subdivisions=HORIZONTAL_DENSITY){
 if(points.length<2)return points;const dense:DensePoint[]=[];
 for(let i=0;i<points.length-1;i++){const a=points[i],b=points[i+1];for(let step=0;step<subdivisions;step++)dense.push(interpolatePoint(a,b,step/subdivisions))}
 dense.push(points[points.length-1]);return dense;
}
function smoothPath(coords:[number,number][],tension=.9){
 if(!coords.length)return '';if(coords.length===1)return `M ${coords[0][0].toFixed(1)} ${coords[0][1].toFixed(1)}`;
 let d=`M ${coords[0][0].toFixed(1)} ${coords[0][1].toFixed(1)}`;
 for(let i=0;i<coords.length-1;i++){
  const p0=coords[i-1]??coords[i],p1=coords[i],p2=coords[i+1],p3=coords[i+2]??p2;
  const cp1x=p1[0]+((p2[0]-p0[0])/6)*tension,cp1y=p1[1]+((p2[1]-p0[1])/6)*tension,cp2x=p2[0]-((p3[0]-p1[0])/6)*tension,cp2y=p2[1]-((p3[1]-p1[1])/6)*tension;
  d+=` C ${cp1x.toFixed(1)} ${cp1y.toFixed(1)} ${cp2x.toFixed(1)} ${cp2y.toFixed(1)} ${p2[0].toFixed(1)} ${p2[1].toFixed(1)}`;
 }
 return d;
}
function terrainAreaPath(points:TerrainPoint[]){
 const coords=points.map(point=>[xAt(point.fraction),yAtHft(Math.max(0,hft(point.elevation)??0))] as [number,number]);
 if(!coords.length)return '';
 return `${smoothPath(coords,.94)} L ${coords[coords.length-1][0].toFixed(1)} ${PLOT_BOTTOM} L ${coords[0][0].toFixed(1)} ${PLOT_BOTTOM} Z`;
}
function terrainOutlinePath(points:TerrainPoint[]){
 const coords=points.map(point=>[xAt(point.fraction),yAtHft(Math.max(0,hft(point.elevation)??0))] as [number,number]);
 return coords.length>1?smoothPath(coords,.94):'';
}
function pathFromHeights(points:CrossPoint[],height:(point:CrossPoint)=>number|null){
 ...
 return coords.length>1?smoothPath(coords,.88):'';
}
function contourLines(points:CrossPoint[]):SvgLine[]{
 ...
 if(zero)lines.push({path:zero,label:'0 °C',color:'#d22d37',labelY:yAtHft(zeroHeight)-4});
 const tropopause=pathFromHeights(points,point=>tropopauseHeight(point.levels));
 if(tropopause)lines.push({path:tropopause,label:'Trop.',color:'#ffe734',labelY:68});
 for(const [offset,threshold] of [50,75,100].entries()){
  ...
  if(path)lines.push({path,label:`${threshold} kt`,color:'#b0a026',dash:'5 4',labelY:yAtHft(lineHeight)-6-offset*2});
 }
 return lines;
}
function sampleLayer(point:CrossPoint,upperPressure:number,lowerPressure:number,topFraction:number,bottomFraction:number){
 ...
 if(!upperLevel||!lowerLevel)return null;const upperH=hft(upperLevel.height),lowerH=hft(lowerLevel.height);if(upperH===null||lowerH===null)return null;
 const top=lerp(upperH,lowerH,topFraction),bottom=lerp(upperH,lowerH,bottomFraction);
 const mix=(key:'temperature'|'humidity'|'cloud'|'windSpeed'|'windDirection')=>{const a=finite(upperLevel[key]),b=finite(lowerLevel[key]);return key==='windDirection'?mixDirection(a,b,(topFraction+bottomFraction)/2):mixNullable(a,b,(topFraction+bottomFraction)/2)};
...
function cells(points:CrossPoint[]):Cell[]{
 const result:Cell[]=[];if(points.length<2)return result;const pressurePairs=BASE_PRESSURES.slice(0,-1).map((pressure,index)=>[pressure,BASE_PRESSURES[index+1]] as const);
 for(let i=0;i<points.length-1;i++){
  ...
  for(const [upperPressure,lowerPressure] of pressurePairs){
   for(let vertical=0;vertical<VERTICAL_SUBDIVISIONS;vertical++){
    ...
    const layerA=sampleLayer(a,upperPressure,lowerPressure,topFraction,bottomFraction),layerB=sampleLayer(b,upperPressure,lowerPressure,topFraction,bottomFraction);if(!layerA||!layerB)continue;
    ...
    const cloudScore=Math.max(cloud??0,Math.max(0,(humidity??0)-58)*2.05);
    if(cloudScore>=18){const intensity:Cell['intensity']=cloudScore>=80?'strong':cloudScore>=52?'moderate':'light';result.push({path,kind:'cloud',opacity:clamp(0.08+cloudScore/130,0.08,0.92),intensity})}
    if(temperature!==null&&temperature<=1&&temperature>=-24&&cloudScore>=40){const intensity:Cell['intensity']=temperature>=-8?'strong':temperature>=-14?'moderate':'light';result.push({path,kind:'icing',opacity:clamp(.14+(cloudScore/220)+(temperature>=-8?.22:temperature>=-14?.12:.03),.10,.72),intensity})}
    const layerDepth=Math.max(8,((layerA.bottom-layerA.top)+(layerB.bottom-layerB.top))/2),verticalShear=windTop!==null&&windBottom!==null?Math.abs(windTop-windBottom)/(layerDepth/9):0;
    if(verticalShear>=0.6){const intensity:Cell['intensity']=verticalShear>=1.4?'strong':verticalShear>=0.95?'moderate':'light';result.push({path,kind:'turbulence',opacity:clamp(.12+verticalShear/4,.12,.65),intensity})}
   }
  }
 }
 return result;
...
function terrainHeightAtFraction(points:TerrainPoint[],fraction:number){if(!points.length)return 0;let best=points[0],delta=Math.abs(points[0].fraction-fraction);for(const point of points){const next=Math.abs(point.fraction-fraction);if(next<delta){best=point;delta=next}}return best.elevation}
...
function CrossSectionGraphic({data}:{data:CrossSectionData}){
 ...
 const terrainPoints=useMemo(()=>data.terrainProfile?.length?data.terrainProfile:data.points.map(point=>({fraction:point.fraction,distanceKm:point.distanceKm,latitude:point.latitude,longitude:point.longitude,elevation:point.elevation})),[data.points,data.terrainProfile]);
 ...
 return <div className="flight-cross-stage" data-flight-export="cross-section"><svg viewBox={`0 0 ${STAGE_WIDTH} ${STAGE_HEIGHT}`} role="img" aria-label={`Flugmeteorologischer Cross Section ${data.route}`}>
  <defs>
   <linearGradient id="flightSky" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#0c377b"/><stop offset=".38" stopColor="#2557ab"/><stop offset=".75" stopColor="#5d86cb"/><stop offset="1" stopColor="#86a7df"/></linearGradient>
   <linearGradient id="flightTerrain" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#e8e29c"/><stop offset=".48" stopColor="#b9b274"/><stop offset="1" stopColor="#777344"/></linearGradient>
   <linearGradient id="flightTerrainShade" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="rgba(255,255,255,.35)"/><stop offset="1" stopColor="rgba(0,0,0,.12)"/></linearGradient>
   ...
   <pattern id="turbPattern" width="7" height="7" patternUnits="userSpaceOnUse" patternTransform="rotate(35)"><path d="M 0 0 V 7" stroke="#ffb14f" strokeWidth="1.15" opacity=".84"/></pattern>
   <filter id="softCloud" x="-10%" y="-10%" width="120%" height="120%"><feGaussianBlur stdDeviation="2.4"/></filter>
   <filter id="cloudShadow" x="-10%" y="-10%" width="120%" height="120%"><feGaussianBlur stdDeviation="1.55"/></filter>
   <filter id="terrainBlur" x="-5%" y="-5%" width="110%" height="110%"><feGaussianBlur stdDeviation=".5"/></filter>
  </defs>
  <rect x="0" y="0" width={STAGE_WIDTH} height={STAGE_HEIGHT} fill="#f6f7fb"/>
  <rect x={PLOT_LEFT} y="1" width={PLOT_RIGHT-PLOT_LEFT} height="18" fill="#f0df1c"/>
  <text x={PLOT_LEFT+6} y="14" className="flight-banner-text">GRAMET FROM {data.route.replaceAll('_',' · ')} at {dateTimeLabel(data.startTime)} TO {dateTimeLabel(data.endTime)}</text>
  <rect x={PLOT_LEFT} y="21" width="110" height="13" rx="2" fill="#eb8044"/><text x={PLOT_LEFT+4} y="30" className="flight-origin-tag">MID CROSS SECTION</text>
  <rect x={PLOT_LEFT} y={PLOT_TOP} width={PLOT_RIGHT-PLOT_LEFT} height={PLOT_BOTTOM-PLOT_TOP} rx="4" fill="url(#flightSky)"/>
  {Array.from({length:23},(_,index)=>index*25).map(level=><g key={level}><line x1={PLOT_LEFT} x2={PLOT_RIGHT} y1={yAtHft(level)} y2={yAtHft(level)} stroke={level%50===0?'rgba(255,255,255,.28)':'rgba(255,255,255,.12)'} strokeDasharray={level%50===0?'2 5':'1 6'}/>{level%50===0&&<><text x="64" y={yAtHft(level)+3} textAnchor="end" className="flight-axis-label">{level}</text><text x={STAGE_WIDTH-23} y={yAtHft(level)+3} className="flight-axis-label">{level}</text></>}</g>)}
  ...
  {riskCells.filter(cell=>cell.kind==='cloud').map((cell,index)=><path key={`cs-${index}`} d={cell.path} fill={cell.intensity==='strong'?'#b6cadd':cell.intensity==='moderate'?'#d3e2ef':'#eaf2fa'} opacity={cell.opacity*.42} filter="url(#cloudShadow)"/>) }
  ...
  {densePoints.map((point,index)=>{const amount=finite(point.precipitation)??0;if(amount<.1||index%3!==0)return null;const x=xAt(point.fraction),terrainY=yAtHft(hft(terrainHeightAtFraction(terrainPoints,point.fraction))??0),height=Math.min(156,16+amount*28);return <g key={`p-${index}`} opacity={Math.min(.85,.25+amount/5)}><path d={`M ${x-3.8} ${terrainY-height} Q ${x} ${terrainY-height-14} ${x+3.8} ${terrainY-height} L ${x+6.4} ${terrainY} L ${x-6.4} ${terrainY} Z`} fill="#f3e3c8"/><text x={x} y={terrainY-height-4} textAnchor="middle" className="flight-precip-label">{amount>=1?'▾▾':'▾'}</text></g>})}
  {lines.map((line,index)=><g key={`${line.label}-${index}`}><path d={line.path} fill="none" stroke={line.color} strokeWidth={line.label==='0 °C'?2.2:1.7} strokeDasharray={line.dash}/><text x="84" y={line.labelY} fill={line.color} className="flight-contour-label">{line.label}</text></g>)}
  ...
  <path d={terrainPath} fill="url(#flightTerrain)" stroke="#776e36" strokeWidth="1.1" filter="url(#terrainBlur)"/>
  <path d={terrainPath} fill="url(#flightTerrainShade)" opacity=".28"/>
  <path d={terrainRidge} fill="none" stroke="#6a6231" strokeWidth="1.15"/>
  ...
  {data.waypoints.map((waypoint,index)=><g key={`${waypoint.id}-${index}`}><line x1={xAt(waypoint.fraction)} x2={xAt(waypoint.fraction)} y1={PLOT_TOP} y2={PLOT_BOTTOM} stroke="rgba(255,238,0,.72)" strokeWidth="1"/><rect x={xAt(waypoint.fraction)-27} y="482" width="54" height="16" rx="3" fill="#e8fff1" stroke="#88c99d"/><text x={xAt(waypoint.fraction)} y="494" textAnchor="middle" className="flight-waypoint-label">{waypoint.id}</text></g>)}
