...
export type CompositeViewMode='radar'|'satellite'|'synoptic';
export type ModelLineColor='white'|'black'|'multicolor';
export type ModelLineTone='auto'|ModelLineColor;
export type CompositeSettings={basemap:BasemapId;viewMode:CompositeViewMode;showRadar:boolean;highResolution:boolean;showPrecipitationType:boolean;showSatellite:boolean;showLightning:boolean;showNowcastObjects:boolean;showMotionOverlay:boolean;motionTimeMode:MotionTimeMode;showWarnings:boolean;modelLines:ModelLineMode;modelLineTone:ModelLineTone;isobarLineColor:ModelLineColor;isoheightLineColor:ModelLineColor;radarOpacity:number;radarColorTable:RadarColorTableId;precipitationTypeOpacity:number;satelliteOpacity:number;lightningOpacity:number;warningOpacity:number;modelOpacity:number;mapOverlayOpacity:number;playbackSeconds:number};
...
export function readCompositeSettings():CompositeSettings{
 try{
  const raw=JSON.parse(localStorage.getItem(COMPOSITE_SETTINGS_KEY)||'{}') as Partial<CompositeSettings>,layers=JSON.parse(localStorage.getItem(COMPOSITE_LAYERS_KEY)||'{}') as Partial<LayerSettings>,basemap=raw.basemap&&COMPOSITE_BASEMAPS[raw.basemap]?raw.basemap:DEFAULTS.basemap,showRadar=typeof layers.showRadar==='boolean'?layers.showRadar:typeof raw.showRadar==='boolean'?raw.showRadar:DEFAULTS.showRadar,highResolution=typeof layers.highResolution==='boolean'?layers.highResolution:raw.highResolution===true,showPrecipitationType=typeof layers.showPrecipitationType==='boolean'?layers.showPrecipitationType:typeof raw.showPrecipitationType==='boolean'?raw.showPrecipitationType:DEFAULTS.showPrecipitationType,selectedModelLines=layers.modelLines??raw.modelLines,modelLines=MODEL_LINE_MODES.includes(selectedModelLines as ModelLineMode)?selectedModelLines as ModelLineMode:DEFAULTS.modelLines,modelLineTone:ModelLineTone=raw.modelLineTone==='white'||raw.modelLineTone==='black'||raw.modelLineTone==='multicolor'?raw.modelLineTone:DEFAULTS.modelLineTone,defaultManualColor:ModelLineColor=modelLineTone==='black'?'black':modelLineTone==='multicolor'?'multicolor':'white',isobarLineColor:ModelLineColor=raw.isobarLineColor==='white'||raw.isobarLineColor==='black'||raw.isobarLineColor==='multicolor'?raw.isobarLineColor:defaultManualColor,isoheightLineColor:ModelLineColor=raw.isoheightLineColor==='white'||raw.isoheightLineColor==='black'||raw.isoheightLineColor==='multicolor'?raw.isoheightLineColor:defaultManualColor,legacySatellite=typeof layers.showSatellite==='boolean'?layers.showSatellite:raw.showSatellite===true,viewMode:CompositeViewMode=raw.viewMode==='satellite'||raw.viewMode==='synoptic'?raw.viewMode:!showRadar&&modelLines!=='off'?'synoptic':!showRadar&&legacySatellite?'satellite':'radar',radarColorTable=DEFAULT_RADAR_COLOR_TABLE,motionTimeMode:MotionTimeMode=raw.motionTimeMode==='absolute'?'absolute':'relative';
  return{basemap,viewMode,showRadar,highResolution,showPrecipitationType,showSatellite:typeof layers.showSatellite==='boolean'?layers.showSatellite:typeof raw.showSatellite==='boolean'?raw.showSatellite:DEFAULTS.showSatellite,showLightning:typeof layers.showLightning==='boolean'?layers.showLightning:typeof raw.showLightning==='boolean'?raw.showLightning:DEFAULTS.showLightning,showNowcastObjects:typeof layers.showNowcastObjects==='boolean'?layers.showNowcastObjects:typeof raw.showNowcastObjects==='boolean'?raw.showNowcastObjects:DEFAULTS.showNowcastObjects,showMotionOverlay:typeof layers.showMotionOverlay==='boolean'?layers.showMotionOverlay:typeof raw.showMotionOverlay==='boolean'?raw.showMotionOverlay:DEFAULTS.showMotionOverlay,motionTimeMode,showWarnings:typeof layers.showWarnings==='boolean'?layers.showWarnings:typeof raw.showWarnings==='boolean'?raw.showWarnings:DEFAULTS.showWarnings,modelLines,modelLineTone,isobarLineColor,isoheightLineColor,radarOpacity:clamp(storedNumber(raw.radarOpacity,DEFAULTS.radarOpacity),15,100),radarColorTable,precipitationTypeOpacity:clamp(storedNumber(raw.precipitationTypeOpacity,DEFAULTS.precipitationTypeOpacity),20,100),satelliteOpacity:clamp(storedNumber(raw.satelliteOpacity,DEFAULTS.satelliteOpacity),15,100),lightningOpacity:clamp(storedNumber(raw.lightningOpacity,DEFAULTS.lightningOpacity),20,100),warningOpacity:clamp(storedNumber(raw.warningOpacity,DEFAULTS.warningOpacity),20,100),modelOpacity:clamp(storedNumber(raw.modelOpacity,DEFAULTS.modelOpacity),20,100),mapOverlayOpacity:clamp(storedNumber(raw.mapOverlayOpacity,DEFAULTS.mapOverlayOpacity),0,100),playbackSeconds:[1.2,2.4,4.8].includes(Number(raw.playbackSeconds))?Number(raw.playbackSeconds):DEFAULTS.playbackSeconds};
