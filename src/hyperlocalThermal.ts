...
export function constrainTemperatureWithDirectObservations(input:{
 ...
 const supportN=clamp((effectiveN-1.25)/1.9,0,1),supportDistance=clamp((27-weightedDistanceKm)/20,0,1),supportAge=clamp((50-weightedAgeMinutes)/38,0,1),coherence=clamp(1-spreadK/3,0,1),calm=Number.isFinite(wind)?clamp((8-wind)/7,0,1):.35,nightBonus=isNight?.12:0,evidence=clamp(.34*supportN+.24*supportDistance+.18*supportAge+.24*coherence,0,1),largeGradientSignal=clamp((Math.abs(gap)-1.45)/3.55,0,1),strongCurrentConsensus=!isNight&&sampleCount>=3&&effectiveN>=1.8&&weightedDistanceKm<=18&&weightedAgeMinutes<=25&&spreadK<=1.35&&spanK<=2.4&&evidence>=.52,strongConsensusBonus=strongCurrentConsensus?.18*largeGradientSignal*evidence:0,rawStrength=.22+.24*supportN+.18*supportDistance+.12*supportAge+.12*coherence+nightBonus+(isNight?.08*calm:.03*calm)+largeGradientSignal*evidence*(isNight?.16:.09)+strongConsensusBonus,strength=clamp(rawStrength,isNight?.24:.20,isNight?.88:strongCurrentConsensus?.86:.64);
 ...
 // Für die aktuelle 2-m-Temperatur darf auch tagsüber ein besonders starker, enger
 // Mehrstationskonsens deutlich weiter schließen: Das verhindert, dass der schnelle
 // Beobachtungswert beim später eintreffenden Modell-Analysepass wieder mehrere Kelvin
 // in Richtung eines erkennbar falschen Zielpunktgradienten springt. Schwache, alte,
 // weit entfernte oder widersprüchliche Evidenz behält die bisherigen Tagesgrenzen.
 const adaptiveEligibility=clamp((Math.min(supportN,supportDistance,supportAge,coherence)-.18)/.62,0,1),baseCap=isNight?(Number.isFinite(wind)&&wind<=6.5?1.8:1.55):1.15,adaptiveExtra=(isNight?3.05:strongCurrentConsensus?3.35:1.85)*Math.pow(evidence,1.45)*Math.pow(adaptiveEligibility,1.25)*largeGradientSignal*(isNight?(.78+.22*calm):1),evidenceCap=baseCap+adaptiveExtra,closureCap=Math.abs(gap)*(isNight?(.72+.14*evidence):strongCurrentConsensus?(.76+.10*evidence):(.58+.10*evidence)),maxCorrection=Math.max(baseCap,Math.min(isNight?4.65:strongCurrentConsensus?4.5:3.0,evidenceCap,closureCap)),correction=clamp(gap*strength,-maxCorrection,maxCorrection),value=residual+correction;
