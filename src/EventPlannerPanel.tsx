...
function sortEvents(records:EventCenterRecord[],mode:EventSortMode,now=Date.now()){const chronological=(a:EventCenterRecord,b:EventCenterRecord)=>eventCenterStartEpoch(a)-eventCenterStartEpoch(b)||(b.updatedAt||0)-(a.updatedAt||0);return [...records].sort((a,b)=>{const aExpired=isEventCenterRecordExpired(a,now),bExpired=isEventCenterRecordExpired(b,now);if(aExpired!==bExpired)return aExpired?1:-1;if(mode==='favorites'&&a.isFavorite!==b.isFavorite)return a.isFavorite?-1:1;if(mode==='updated')return(b.updatedAt||0)-(a.updatedAt||0)||chronological(a,b);if(mode==='title')return(a.title||a.location.name).localeCompare(b.title||b.location.name,'de-DE',{sensitivity:'base'})||chronological(a,b);if(aExpired&&bExpired)return eventCenterStartEpoch(b)-eventCenterStartEpoch(a)||(b.updatedAt||0)-(a.updatedAt||0);return chronological(a,b)})}
...
function EventTimelinePrecipitationProbability({point}:{point:EventTimelinePoint}){const type=eventWeatherPart(point).type,title=type==='snow'||type==='snowGrains'||type==='snowStars'||type==='iceCrystals'||type==='icePellets'||type==='snowShowers'||type==='graupelShowers'||type==='hailShowers'?'Wahrscheinlichkeit festen Niederschlags':type==='sleet'||type==='sleetShowers'||type==='wintryAfterThunder'?'Wahrscheinlichkeit winterlichen/gemischten Niederschlags':type==='thunderstorm'||type==='thunderstormHail'?'Gewitter-/Niederschlagswahrscheinlichkeit':type==='freezingRain'||type==='freezingDrizzle'?'Wahrscheinlichkeit gefrierenden Niederschlags':'Niederschlagswahrscheinlichkeit',icon=type==='snow'||type==='snowGrains'||type==='snowStars'||type==='iceCrystals'||type==='icePellets'||type==='snowShowers'||type==='graupelShowers'||type==='hailShowers'||type==='sleet'||type==='sleetShowers'||type==='wintryAfterThunder'?<Snowflake size={12}/>:type==='thunderstorm'||type==='thunderstormHail'?<CloudLightning size={12}/>:<CloudRain size={12}/>;return <span className="event-timeline-pop" title={title} aria-label={`${title} ${formatNumber(point.precipitationProbability)} Prozent`}>{icon}<b>{formatNumber(point.precipitationProbability)} %</b></span>}
function eventPrecipProbability(summary:EventSummary){return eventPrecipitationProbability(summary)}
function eventPrecipLabel(summary:EventSummary){const base=summary.precipitationTypeLabel||'Niederschlag';return summary.precipitationProbabilitySource==='hourly-window-average-fallback'?`${base} · Stunden-PoP-Mittel`:base}
function EventSummaryPrecipitationIcon({summary,size=14}:{summary:EventSummary;size?:number}){const text=`${summary.precipitationTypeLabel||''} ${summary.weatherLabel||''}`.toLocaleLowerCase('de-DE'),code=Number(summary.weatherCode);if(/gewitter/.test(text)||[95,96,97,98,99].includes(code))return <CloudLightning size={size}/>;if(/schnee|graupel|hagel|eisnadeln|eiskörner/.test(text)||[71,73,75,76,77,78,79,85,86,87,88,89,90,93,94].includes(code))return <Snowflake size={size}/>;return <CloudRain size={size}/>}
...
function buildTimingHint(summary:EventSummary,activity:EventActivity,environment:EventEnvironment){
 ...
 if(summary.coverageComplete===false)return'Datengrundlage unvollständig; vor dem Termin erneut aktualisieren'
...
function aviationVisibility(value:number|null|undefined){if(value===null||value===undefined)return'–';const n=Number(value);if(!Number.isFinite(n))return'–';if(n<1000)return`${Math.max(50,Math.round(n/50)*50)} m`;if(n<10000)return`${new Intl.NumberFormat('de-DE',{minimumFractionDigits:n<3000?1:0,maximumFractionDigits:1}).format(n/1000)} km`;return'≥ 10 km'}
function aviationCeiling(value:number|null|undefined){if(value===null||value===undefined)return'–';const n=Number(value);if(!Number.isFinite(n))return'–';if(n<100)return'unter 100 ft AGL';return`${Math.round(n/100)*100} ft AGL`} 
function flightHazardDetail(item:{id:string;detail:string;source?:string;value?:number;unit?:'kt'|'m'|'ft'},summary:EventSummary,unit:WindUnit){const hazards=normalizeEventFlightHazardSummary(summary.flightHazards);if(item.id==='wind'){const gust=item.unit==='kt'&&Number.isFinite(item.value)?Number(item.value):hazards?.gustMaxKt??summary.gustMax;if(gust==null)return item.detail||'nicht verfügbar';const formatted=wind(gust,unit);return item.source?item.detail.replace(/Böen\s+\d+(?:[.,]\d+)?\s*kt/i,`Böen ${formatted}`):`bis ${formatted}`}if(item.id==='visibility'){const value=item.unit==='m'&&Number.isFinite(item.value)?Number(item.value):hazards?.visibilityMinM??summary.visibilityMin;return item.source&&item.value==null?item.detail:aviationVisibility(value)}if(item.id==='ceiling'){const value=item.unit==='ft'&&Number.isFinite(item.value)?Number(item.value):hazards?.ceilingMinFt;return item.source&&item.value==null?item.detail:value==null?'nicht belastbar':`min. ca. ${aviationCeiling(value)}`}return item.detail}
function eventMetricLine(plan:EventPlan|null,unit:WindUnit){if(!plan)return'Noch keine Analyse.';return`${formatNumber(plan.summary.temperatureAvg)} °C · ${eventPrecipLabel(plan.summary)} ${formatNumber(eventPrecipProbability(plan.summary))} % · ${formatNumber(plan.summary.precipitationTotal,1)} mm · Wind ${wind(plan.summary.windMax??Number.NaN,unit)} · G ${wind(plan.summary.gustMax??Number.NaN,unit)}`}
...
export default function EventPlannerPanel({initialLocation,advancedMode,unit,canonicalHours=[],canonicalFusion=null,canonicalWeatherTwinApplied=false,backgroundOnly=false}:Props){
 ...
 const displayedEvents=useMemo(()=>sortEvents(savedEvents,sortMode,eventClock),[savedEvents,sortMode,eventClock])
 const expiredEvents=useMemo(()=>savedEvents.filter(item=>isEventCenterRecordExpired(item,eventClock)),[savedEvents,eventClock])
 const activeEvents=useMemo(()=>savedEvents.filter(item=>!isEventCenterRecordExpired(item,eventClock)),[savedEvents,eventClock])
 ...
 useEffect(()=>{const current=plan?.summary.flightHazards,normalized=normalizeEventFlightHazardSummary(current);if(!current||!normalized||current.ceilingMinFt===normalized.ceilingMinFt)return;setPlan(value=>value?{...value,summary:{...value.summary,flightHazards:normalized}}:value)},[plan])
 ...
 function validateInputs(location:Location|null,eventDate:string,eventStartTime:string,eventEndTime:string){
  ...
  if(eventStartTime===eventEndTime){setError('Start und Ende müssen verschieden sein.');return false}if(eventDate>addDays(localToday(),13)){setError('Der Event-Planer wertet die nächsten 14 Tage aus. Für spätere Termine bitte näher am Termin erneut prüfen.');return false}
  return true
 }
 ...
 const detailExpired=Boolean(currentSavedRecord&&isEventCenterRecordExpired(currentSavedRecord,eventClock))
 const renderedFlightHazards=normalizeEventFlightHazardSummary(plan?.summary.flightHazards)
 ...
 return <section className="event-planner">
  ...
  {workspaceView==='overview'&&<section className="event-center-shelf">
   <header><div><span>EVENT-CENTER</span><h5>Kompakte Übersicht</h5><small>{activeEvents.length} aktiv{expiredEvents.length?` · ${expiredEvents.length} abgelaufen`:''} · {favoriteEvents.length} Favorit{favoriteEvents.length===1?'':'en'}</small></div><div className="event-center-actions"><button type="button" className="primary event-new-button" onClick={startNewEvent}><CalendarRange size={15}/> Neu</button><label className="event-center-sort"><ArrowUpDown size={14}/><span>Sortierung</span><select value={sortMode} onChange={(event:ValueEvent)=>setSortMode(event.target.value as EventSortMode)} aria-label="Events sortieren"><option value="chronological">Chronologisch</option><option value="favorites">Favoriten zuerst</option><option value="updated">Zuletzt geändert</option><option value="title">Titel A–Z</option></select></label><button type="button" className="secondary" onClick={()=>void refreshStoredEvents(true,false,'overview')} disabled={bulkRefreshing||!activeFavoriteEvents.length}>{bulkRefreshing?<RefreshCw className="spin" size={15}/>:<BellRing size={15}/>} Favoriten</button><button type="button" className="secondary" onClick={()=>void refreshStoredEvents(false,false,'overview')} disabled={bulkRefreshing||!activeEvents.length}>{bulkRefreshing?<RefreshCw className="spin" size={15}/>:<RefreshCw size={15}/>} Alle</button><AppInfoHint label="Informationen zum Event-Center"><strong>Event-Center</strong><p>Standardmäßig chronologisch. Die Karten zeigen zuerst nur Termin, Ort und meteorologische Eckdaten. Aufklappen liefert eine Kurzbewertung; „Details & Ratschläge“ öffnet die vollständige Auswertung.</p></AppInfoHint></div></header>
   {savedEvents.length?<div className="event-center-grid compact">{displayedEvents.map(record=>{const active=record.id===selectedRecordId||record.id===currentSavedRecord?.id,recordPlan=record.plan,refreshing=refreshingIds.includes(record.id),expired=isEventCenterRecordExpired(record,eventClock);return <article key={record.id} className={`event-center-card compact${active?' active':''}${expired?' expired':''}`}>
    ...
    {expired?<button type="button" className="event-center-expired-delete" onClick={()=>removeRecord(record)} aria-label="Abgelaufenes Event entfernen" title="Abgelaufenes Event entfernen"><Trash2 size={14}/></button>:null}
    <details className="event-center-card-disclosure">
     <summary>
      <span className="event-center-card-overview">
       <span className="event-center-card-overview-head"><strong>{record.title||record.location.name}</strong><span className="event-center-card-tags"><span className="event-center-card-pill fill">{environmentLabel(record.environment)}</span><span className="event-center-card-pill">{activityLabel(record.activity)}</span>{expired?<span className="event-center-card-pill expired">Abgelaufen</span>:null}{record.change&&record.change.level!=='none'&&<span className={`event-center-card-pill signal-${record.change.level}`}>{record.change.badge}</span>}</span></span>
       <span className="event-center-card-meta"><span>{eventCompactRange(record)}</span><span>{record.location.name}</span></span>
       <span className="event-center-card-quick-weather"><span className="event-center-card-weather-visual"><WeatherPictogram code={recordPlan?.summary.weatherCode??0} intensity={recordPlan?.summary.weatherIntensity} phenomenon={recordPlan?.summary.weatherPhenomenon} day={recordPlan?.summary.isDay!==false} title={recordPlan?.summary.weatherLabel||label(recordPlan?.summary.weatherCode??0)}/><EventFeasibilityDot plan={recordPlan}/></span><span>{eventMetricLine(recordPlan,unit)}</span></span>
      </span>
     ...
     <div className="event-center-card-details">
      <div className="event-center-card-weather"><WeatherPictogram code={recordPlan?.summary.weatherCode??0} intensity={recordPlan?.summary.weatherIntensity} phenomenon={recordPlan?.summary.weatherPhenomenon} day={recordPlan?.summary.isDay!==false} title={recordPlan?.summary.weatherLabel||label(recordPlan?.summary.weatherCode??0)}/><div><b>{recordPlan?.advice.headline||'Noch keine Analyse'}</b><span>{recordPlan?.advice.summary||destinationLabel(record.location)}</span></div></div>
  ...
  {workspaceView==='detail'&&plan&&<section className={`event-plan-result status-${plan.advice.status}`}>
   ...
   <div className="event-guide-hero">
    <div className="event-guide-topbar">
     ...
     <div className="event-guide-status-wrap"><div className={`event-status-badge ${plan.advice.status}`}><EventFeasibilityDot plan={plan}/>{plan.advice.status==='good'?<ShieldCheck size={16}/>:<ShieldAlert size={16}/>}<strong>{statusLabel(plan.advice.status)}</strong></div>{detailExpired?<small className="event-expired-detail-badge">Abgelaufen · historischer Wettercheck</small>:currentSavedRecord?.change&&currentSavedRecord.change.level!=='none'?<small className={`event-inline-update ${currentSavedRecord.change.level}`}>{currentSavedRecord.change.summary}</small>:null}</div>
    </div>
    <div className="event-guide-main">
     <div className="event-guide-copy"><span>EVENT-CHECK</span><h5>{currentTitle}</h5><p>{destinationLabel(plan.location)}</p><strong>{plan.advice.headline}</strong><p>{plan.advice.summary}</p><div className="event-guide-inline-notes"><article><MapPin size={15}/><span>{timingHint}</span></article><article><BellRing size={15}/><span>Stand {lastUpdateText}</span></article></div><div className="event-result-toolbar">{detailExpired&&currentSavedRecord?<button type="button" className="secondary danger-lite" onClick={()=>{removeRecord(currentSavedRecord);setWorkspaceView('overview')}}><Trash2 size={15}/>Event entfernen</button>:<button type="button" className="secondary" onClick={()=>currentSavedRecord?void analyseEvent(undefined,true,true):saveCurrentPlan(false)} disabled={loading}>{currentSavedRecord?<RefreshCw className={loading?'spin':undefined} size={15}/>:<Star size={15}/>} {currentSavedRecord?(loading?'Aktualisiere …':'Event aktualisieren'):'Event speichern'}</button>}{currentSavedRecord?<button type="button" className="secondary" onClick={()=>toggleFavorite(currentSavedRecord)}><Star size={15} fill={currentSavedRecord.isFavorite?'currentColor':'none'}/>{currentSavedRecord.isFavorite?'Event-Favorit entfernen':'Event-Favorit'}</button>:<button type="button" className="secondary" onClick={()=>saveCurrentPlan(true)}><Star size={15}/>Als Event-Favorit speichern</button>}</div></div>
     <aside className="event-guide-weatherpanel"><small>Leitwetter</small><div className="event-guide-weatherrow"><WeatherPictogram code={plan.summary.weatherCode??0} intensity={plan.summary.weatherIntensity} phenomenon={plan.summary.weatherPhenomenon} day={plan.summary.isDay!==false} title={plan.summary.weatherLabel||label(plan.summary.weatherCode??0)}/><div><strong>{formatNumber(plan.summary.temperatureAvg)}°</strong><span>{plan.summary.weatherLabel||label(plan.summary.weatherCode??0)}</span></div></div><p>{outfitHint}</p><div className="event-guide-quickstats"><span><EventSummaryPrecipitationIcon summary={plan.summary}/>{eventPrecipLabel(plan.summary)} · {formatNumber(eventPrecipProbability(plan.summary))} %</span><span><Wind size={14}/>{wind(plan.summary.windMax??Number.NaN,unit)} · G {wind(plan.summary.gustMax??Number.NaN,unit)}</span>{plan.activity==='flight'?<span><Plane size={14}/>Sicht {aviationVisibility(renderedFlightHazards?.visibilityMinM??plan.summary.visibilityMin)} · Wolkenuntergrenze {aviationCeiling(renderedFlightHazards?.ceilingMinFt)}</span>:<span title="UV-Index und Sonnenscheindauer"><Sun size={14}/>UVI {formatUvi(plan.summary.uvMax??Number.NaN)} · Sonnenschein {eventSunshineLabel(plan.summary.sunshineDurationTotal)}</span>}</div></aside>
    </div>
   </div>
   ...
   {plan.activity==='flight'&&plan.summary.flightHazards?<section className={`event-flight-hazards ${plan.summary.flightHazards.overall}`}><header><div><Plane size={18}/><span><small>FLUGMETEOROLOGIE</small><strong>Hazard-Screening</strong></span></div><AppInfoHint label="Informationen zum Flugwetter-Screening" width={430}><strong>Quellen & Priorität</strong><p>MID priorisiert standort- und zeitbezogene ICAO-SIGMET und TAF; bei Nahterminen kommen METAR/SPECI und PIREP/AIREP hinzu. Mit autorisiertem WIFS-Zugang werden die maschinenlesbaren WAFS-SIGWX-Daten von WAFC London und WAFC Washington ausgewertet – also die Datenbasis der Significant-Weather-Charts. Nationale Wetterdienste fließen über den ICAO-Austausch ein; freigegebene Open-Data-Produkte werden, wo verfügbar, zusätzlich direkt genutzt (derzeit u. a. KNMI AIRMET/SIGMET). Die MID-Druckniveaudiagnose ergänzt diese amtlichen Quellen.</p><div className="event-flight-source-list">{plan.summary.flightHazards.sources?.map(source=><span key={source.id} className={source.status}><b>{source.label}</b><small>{source.status==='used'?'verwendet':source.status==='available'?'geprüft':source.status==='not-configured'?'Zugang nicht eingerichtet':'derzeit nicht erreichbar'}{source.detail?` · ${source.detail}`:''}</small></span>)}</div><p>Direkt zugangsbeschränkte oder nicht zur Weiterverarbeitung freigegebene nationale SWC-/Spezialprodukte werden nicht automatisiert aus Webseiten extrahiert.</p><p>Kein Ersatz für ein vorgeschriebenes amtliches Flugwetterbriefing.</p></AppInfoHint></header>{plan.summary.flightHazards.available?<div className="event-flight-hazard-grid">{plan.summary.flightHazards.items.map(item=><article key={item.id} className={item.level}><small>{item.label}</small><strong>{item.level==='caution'?'kritisch':item.level==='watch'?'beachten':'unauffällig'}</strong><span>{flightHazardDetail(item,plan.summary,unit)}</span></article>)}</div>:<div className="event-flight-unavailable">{plan.summary.flightHazards.note||'Flugprofildaten derzeit nicht verfügbar.'}</div>}{plan.summary.flightHazards.freezingLevelMin!=null?<footer>Nullgradgrenze im Zeitfenster: ca. {Math.round(plan.summary.flightHazards.freezingLevelMin/50)*50} m</footer>:null}</section>:null}

   <MidDisclosure className="event-detail-disclosure" summary={<><span><small>MEHR DETAILS</small><strong>Stündlicher Verlauf & Datenbasis</strong></span><ChevronDown size={18}/></>}>
    <div className="event-detail-disclosure-body">
     <div className="event-timeline-card"><header><div><span>ZEITFENSTER</span><h6>Stündlicher Verlauf</h6></div><AppInfoHint label="Informationen zum Stundenverlauf" width={390}><strong>Stündlicher Verlauf</strong><p>{advancedMode?'Best Match, unabhängige Modellfamilien und MID-Plausibilisierung; bei Nahterminen zusätzlich Nowcast. Die große Niederschlagswahrscheinlichkeit wird über das gesamte Eventfenster ausgewertet: bevorzugt aus Ensemble-Membern über die im Zeitraum aufsummierte Niederschlagsmenge; falls diese Auswertung nicht verfügbar ist, als zeitgewichtetes Mittel aller überlappenden Stundenwahrscheinlichkeiten – niemals nur als höchste Einzelstunde. Die Prozentwerte in den Karten gelten für das jeweils angegebene Stundenintervall; Open-Meteo weist Niederschlag und Sonnenscheindauer als Werte der vorangehenden Stunde aus, der Wettercode gilt dagegen für den Zeitpunkt am Stundenende. MID schneidet die Intervalle exakt auf Start und Ende des Events zu und lässt einen Wettercode am Intervallrand nicht rückwirkend eine trockene Stunde als Regenstunde erscheinen.':'Dieselbe plausibilisierte Stundenprognose wie in der übrigen App. Die große Niederschlagswahrscheinlichkeit gilt für den gesamten Eventzeitraum; die Karten zeigen nur die tatsächlich überlappenden Stundenintervalle.'}</p></AppInfoHint></header><div className="event-timeline">{plan.timeline.map(point=><article key={`${point.periodLabel||point.time}:${point.time}`}><time>{point.periodLabel||point.time}</time><WeatherPictogram code={point.weatherCode??0} intensity={point.weatherIntensity} phenomenon={point.weatherPhenomenon} day={point.isDay!==false} title={point.weatherLabel||label(point.weatherCode??0)}/><strong>{formatNumber(point.temperature)}°</strong><small className="event-timeline-weather-meta"><span>{point.weatherLabel||label(point.weatherCode??0)}</span><span aria-hidden="true"> · </span><EventTimelinePrecipitationProbability point={point}/></small><small className="event-timeline-compact-metrics"><span title="Wind und Böen" aria-label={`Wind ${wind(point.wind??Number.NaN,unit)}, Böen ${wind(point.gust??Number.NaN,unit)}`}><Wind size={12} aria-hidden="true"/><b>{wind(point.wind??Number.NaN,unit)} · G {wind(point.gust??Number.NaN,unit)}</b></span><span title="Sonnenscheindauer" aria-label={`Sonnenscheindauer ${sunshineMinutesLabel(point.sunshineDuration,point.durationMinutes??60)}`}><Sun size={12} aria-hidden="true"/><b>{sunshineMinutesLabel(point.sunshineDuration,point.durationMinutes??60)}</b></span></small></article>)}</div></div>
     <MidDisclosure className="event-model-disclosure" summary={<><Info size={16}/><span>Daten & Modellstand</span><ChevronDown size={15}/></>}><div className="event-model-updates"><header><div><span>MODELLSTAND</span><h6>Aktuelle Laufstände</h6></div><small>{lastUpdateText}</small></header>{plan.modelInfo?.summary?<div className="event-model-update-lead"><BellRing size={16}/><span>{plan.modelInfo.summary}</span></div>:null}{latestRuns.length>0&&<div className="event-run-grid">{latestRuns.map(run=><article key={`${run.id}:${run.initialisationTime}`}><strong>{run.label}</strong><small>Init {modelStamp(run.initialisationTime)}</small><small>Verfügbar {modelStamp(run.availabilityTime||run.initialisationTime)}</small></article>)}</div>}<footer><span>Quelle: {plan.source}</span><small>{plan.summary.modelFamilyCount?`${plan.summary.modelFamilyCount} unabhängige Modellgruppen geprüft. `:''}{plan.summary.rapidCycleUsed?'Rapid-Cycle-Läufe einbezogen. ':''}{advancedMode?'Varianten derselben Modellfamilie werden nicht mehrfach gewichtet.':''}</small></footer></div></MidDisclosure>
    </div>
   </MidDisclosure>
  </section>}
 </section>
