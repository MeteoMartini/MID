 ...
 return <section className="event-planner">
  ...
  {workspaceView==='detail'&&plan&&<section className={`event-plan-result status-${plan.advice.status}`}>
   ...
   <div className="event-metrics-grid">
    ...
    {plan.activity==='flight'?<article><Plane size={17}/><small>Sicht / Wolkenuntergrenze</small><strong>Sicht {aviationVisibility(plan.summary.flightHazards?.visibilityMinM??plan.summary.visibilityMin)}</strong><span>Wolkenuntergrenze {aviationCeiling(plan.summary.flightHazards?.ceilingMinFt)}</span></article>:<article><Sun size={17}/><small>UVI / Sicht / Sonnenscheindauer</small><strong>UVI {formatUvi(plan.summary.uvMax??Number.NaN)} · ☀ {eventSunshineLabel(plan.summary.sunshineDurationTotal)}</strong><span>Sicht min. {plan.summary.visibilityMin!=null?`${formatNumber(plan.summary.visibilityMin/1000,1)} km`:'–'}</span></article>}
   </div>
