 ...
 return <section className="event-planner">
  ...
  {workspaceView==='detail'&&plan&&<section className={`event-plan-result status-${plan.advice.status}`}>
   ...
   <div className="event-metrics-grid">
    ...
    <article><CloudRain size={17}/><small>Niederschlag</small><strong>{eventPrecipLabel(plan.summary)} {formatNumber(eventPrecipProbability(plan.summary))} %</strong><span>{formatNumber(plan.summary.precipitationTotal,1)} mm{plan.summary.precipitationProbabilitySource==='ensemble-members-dwd-event'&&plan.summary.precipitationProbabilitySignificant!=null?` · >5 mm ${formatNumber(plan.summary.precipitationProbabilitySignificant)} %`:''}</span></article>
