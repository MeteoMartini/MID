...
function operaMotion(previous:OperaMotionField|undefined,current:OperaMotionField,elapsedMs:number){if(!previous||previous.mass<2||current.mass<2||!Number.isFinite(previous.centroidEastKm)||!Number.isFinite(previous.centroidNorthKm)||!Number.isFinite(current.centroidEastKm)||!Number.isFinite(current.centroidNorthKm)||elapsedMs<8*60000)return{};const east=Number(current.centroidEastKm)-Number(previous.centroidEastKm),north=Number(current.centroidNorthKm)-Number(previous.centroidNorthKm),distance=Math.hypot(east,north),hours=elapsedMs/3600000,speed=distance/hours;if(!Number.isFinite(speed)||speed<2||speed>160||distance<1)return{};const direction=(Math.atan2(east,north)*180/Math.PI+360)%360;return{motionDirectionDeg:Number(direction.toFixed(1)),motionSpeedKmh:Number(speed.toFixed(1)),motionConfidence:distance>=8?'high' as const:distance>=3?'medium' as const:'low' as const,motionSource:'opera-field-tracking'}}
...
export async function analyseOperaRasterNowcast(frames:OperaRasterFrame[],lat:number,lon:number,signal?:AbortSignal):Promise<RadarNowcast>{
 ...
 let previousSample:OperaRasterSample|undefined,previousField:OperaMotionField|undefined,previousTime:number|undefined,arrivalMinutes:number|undefined,arrivalKind:RadarNowcast['arrivalKind'],arrivalStartAt:string|undefined,arrivalEndAt:string|undefined;
 const previous=[...sorted].reverse().find(frame=>Date.parse(frame.time)<=latestTime-9*60000);if(previous){try{previousTime=Date.parse(previous.time);const previousRaster=await loadOperaRasterData(previous.fileUrl,signal);previousSample=sampleOperaRaster(previousRaster,lat,lon);previousField=sampleOperaMotionField(previousRaster,lat,lon)}catch(error){if(signal?.aborted)throw error}}
 ...
 const probability=probabilityFor(sample,ageMinutes),peakRate=Math.max(sample.localRate,sample.nearbyRate),rateUncertain=sample.currentRate<.05&&peakRate>=.05,motion=operaMotion(previousField,latestField,previousTime===undefined?0:latestTime-previousTime),nowcastSeries=[...(previousTime!==undefined&&previousSample?[{time:new Date(previousTime).toISOString(),rate:previousSample.currentRate,nearbyRate:previousSample.nearbyRate,future:false}]:[]),{time:new Date(latestTime).toISOString(),rate:sample.currentRate,nearbyRate:sample.nearbyRate,future:false}];let summary:string;
 ...
 return{source:'opera',provider:'EUMETNET OPERA CIRRUS DBZH · 1 km / 5 min',quality:qualityForAge(ageMinutes),radarProbability:probability,currentRate:sample.currentRate,rawCurrentRate:sample.currentRate,peakRate,rateApproximate:true,rateUncertain,arrivalMinutes,arrivalKind,arrivalStartAt,arrivalEndAt,observedAt:new Date(latestTime).toISOString(),summary,coverage:true,license:'EUMETNET OPERA composite products, CC BY 4.0',timeline:sorted.map(frame=>frame.time),nowcastSeries,motionAnchors:latestField.anchors,...motion,diagnostics:{product:'CIRRUS DBZH',nativeResolutionKm:1,temporalResolutionMinutes:5,method:'ODIM-HDF5-Rasterpixel und 30-km-Umfeld direkt im Browser ausgewertet',nearestWetKm:sample.nearestWetKm,localRate:sample.localRate,nearbyRate:sample.nearbyRate,validPixels:sample.validPixels,wetPixels:sample.wetPixels,previousNearestWetKm:previousSample?.nearestWetKm,previousTime:previousTime?new Date(previousTime).toISOString():undefined,motionResolved:motion.motionDirectionDeg!==undefined,motionAnchorCount:latestField.anchors.length}};
}
