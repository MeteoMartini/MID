...
function probabilityFor(sample:OperaRasterSample,ageMinutes:number,siteThreshold:number,nearbyThreshold:number){let probability=5;if(sample.currentRate>=siteThreshold)probability=98;else if(sample.localRate>=siteThreshold)probability=88;else if(sample.nearbyRate>=nearbyThreshold)probability=78;else if((sample.nearestWetKm??Infinity)<=5)probability=72;else if((sample.nearestWetKm??Infinity)<=10)probability=58;else if((sample.nearestWetKm??Infinity)<=20)probability=38;else if((sample.nearestWetKm??Infinity)<=30)probability=22;if(ageMinutes>15)probability=Math.max(5,Math.round(probability*Math.max(.55,1-(ageMinutes-15)/60)));return probability}
