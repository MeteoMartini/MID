...
export async function analyseOperaRasterNowcast(frames:OperaRasterFrame[],lat:number,lon:number,signal?:AbortSignal):Promise<RadarNowcast>{
 ...
 const latestRaster=await loadOperaRasterData(latest.fileUrl,signal),sample=sampleOperaRaster(latestRaster,lat,lon),latestField=sampleOperaMotionField(latestRaster,lat,lon);if(!sample.covered)throw new Error('Der Standort liegt in einer OPERA-NoData-Zone.');
 ...
 return{source:'opera',provider:'EUMETNET OPERA CIRRUS DBZH · 1 km / 5 min',quality:qualityForAge(ageMinutes),radarProbability:probability,currentRate:sample.currentRate,rawCurrentRate:sample.currentRate,peakRate,rateApproximate:true,rateUncertain,arrivalMinutes,arrivalKind,arrivalStartAt,arrivalEndAt,observedAt:new Date(latestTime).toISOString(),summary,coverage:true,license:'EUMETNET OPERA composite products, CC BY 4.0',timeline:sorted.map(frame=>frame.time),nowcastSeries,motionAnchors:latestField.anchors,...motion,diagnostics:{product:'CIRRUS DBZH',nativeResolutionKm:1,temporalResolutionMinutes:5,method:'ODIM-HDF5-Rasterpixel und 30-km-Umfeld direkt im Browser ausgewertet',nearestWetKm:sample.nearestWetKm,localRate:sample.localRate,nearbyRate:sample.nearbyRate,validPixels:sample.validPixels,wetPixels:sample.wetPixels,previousNearestWetKm:previousSample?.nearestWetKm,previousTime:previousTime?new Date(previousTime).toISOString():undefined,motionResolved:motion.motionDirectionDeg!==undefined,motionAnchorCount:latestField.anchors.length,motionFrameCount:motion.motionFrameCount??motionRows.length,motionFit:motion.motionFit}};
}
