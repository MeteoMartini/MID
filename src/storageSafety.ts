...
const FAVORITE_SNAPSHOT_KEYS=new Set([FAVORITES_STORAGE_KEY,FAVORITES_SHADOW_KEY]);
...
export async function initializeStorageSafety(){const store=ensureNativeStorage();if(!store)return false;trimReconstructibleStorage(false);dropReserve();const records=await readRecords();for(const[key,record]of records){if(isMainModuleViewStateKey(key)){queueDelete(key);continue}if(!isDurableStorageKey(key))continue;const local=store.get(key),localRevision=durableSemanticRevision(key,local,other=>store.get(other)),mirroredRevision=mirrorRevision(key,record,records),localWins=local!==null&&(localRevision>mirroredRevision||(localRevision===mirroredRevision&&record.nativeCommitted!==false));if(localWins){fallback.set(key,local!);queueRecord(key,{value:local!,updatedAt:localRevision||Date.now(),nativeCommitted:true});continue}fallback.set(key,record.value);if(local!==record.value||record.nativeCommitted===false){const committed=nativeSetWithRecovery(key,record.value);if(committed)queueRecord(key,{...record,nativeCommitted:true,updatedAt:Date.now()})}}for(let index=0;index<store.storage.length;index++){const key=store.key(index);if(!key||!isDurableStorageKey(key)||records.has(key))continue;const value=store.get(key);if(value===null)continue;fallback.set(key,value);queueRecord(key,{value,updatedAt:durableSemanticRevision(key,value,other=>store.get(other))||Date.now(),nativeCommitted:true})}installBridge();scheduleReserve();return true}
