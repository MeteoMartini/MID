...
export function isMainModuleViewStateKey(key:string){return /^mid:module:[^:]+:open$/.test(key)||key.startsWith('mid:module-open-contract:')}
export function isDurableStorageKey(key:string){return !isMainModuleViewStateKey(key)&&(ROOT_DURABLE_KEYS.has(key)||(key.startsWith('mid:')&&!isTransientStorageKey(key)))}
