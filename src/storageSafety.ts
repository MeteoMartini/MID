...
export function readDurableStorageValue(key:string){const store=ensureNativeStorage();if(isDurableStorageKey(key)&&fallback.has(key))return fallback.get(key)??null;try{return store?.get(key)??null}catch{return fallback.get(key)??null}}
