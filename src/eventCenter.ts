...
export function eventPlanFreshness(plan:EventPlan|null|undefined){return{sourceRevision:Math.max(Number(plan?.sourceRevisionAt)||0,eventModelRevision(plan?.modelInfo)),refreshedAt:Number(plan?.refreshedAt)||0,startedAt:Number(plan?.refreshStartedAt)||0}}
export function compareEventPlanFreshness(left:EventPlan|null|undefined,right:EventPlan|null|undefined){const a=eventPlanFreshness(left),b=eventPlanFreshness(right);if(a.sourceRevision>0&&b.sourceRevision>0&&a.sourceRevision!==b.sourceRevision)return a.sourceRevision-b.sourceRevision;if(a.startedAt&&b.startedAt&&a.startedAt!==b.startedAt)return a.startedAt-b.startedAt;return a.refreshedAt-b.refreshedAt}
