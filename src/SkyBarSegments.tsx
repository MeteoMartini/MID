...
export function SkyBarHourCellsSvg({cells,left,right,chartW,centerY,keyPrefix='sky-cell'}:HourCellsProps){
 ...
 return <g data-mid-skybar-cells="24h">{cells.map((cell,index)=>{
  ...
  return <g key={`${keyPrefix}-${cell.key}`} data-skybar-hour={index} data-skybar-state={cell.state}>
   ...
   <rect x={x} y={y} width={size} height={size} rx={Math.min(2.2,size*.22)} fill="var(--mid-skycell-empty,transparent)" stroke="var(--mid-skycell-border,currentColor)" strokeOpacity={cell.state==='unavailable'?.34:.18} strokeWidth={.8}/>
