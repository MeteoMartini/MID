...
import {loadForecastFusion} from './forecastFusion'
...
export async function loadEventFlightHazards(lat:number,lon:number,elevation:number,startEpoch:number,endEpoch:number,signal?:AbortSignal,country?:string):Promise<EventFlightHazardSummary>{
 try{
  const [response,official,fusion]=await Promise.all([load(lat,lon,elevation,signal),loadOfficial(lat,lon,startEpoch,endEpoch,signal),loadForecastFusion(lat,lon,country,elevation,signal).catch(()=>null)]),hourly=response.data?.hourly
  ...
  const rucCeilingsFt=(fusion?.weatherHours??[]).filter(hour=>Number.isFinite(Number(hour.epoch))&&Number(hour.epoch)>=startEpoch-30*60*1000&&Number(hour.epoch)<=endEpoch+30*60*1000&&Number.isFinite(Number(hour.rucCeilingM))&&Number(hour.rucCeilingM)>=0&&Number(hour.rucCeilingM)<=15240).map(hour=>Number(hour.rucCeilingM)*3.28084),ceilingBasis=rucCeilingsFt.length?rucCeilingsFt:ceilings,ceilingMinFt=minNumber(ceilingBasis),ceilingSource=rucCeilingsFt.length?'DWD ICON-D2-RUC · CEILING':'MID-Druckniveau-Diagnose',visibilityMin=minNumber(visibility),gustMax=maxNumber(gusts),capeMax=maxNumber(capes),liMin=minNumber(lifted),cinMax=maxNumber(cin),thunderStrong=codes.some(code=>[95,96,97,98,99].includes(Math.round(code)))||(capeMax??0)>=1400||((capeMax??0)>=800&&(liMin??0)<=-2&&(cinMax??0)<100),thunderWatch=!thunderStrong&&((capeMax??0)>=500||(liMin??0)<=-1)
  ...
  const diagnosed:EventFlightHazardItem[]=[
   ...
   {id:'ceiling',label:'Wolkenuntergrenze',level:ceilingLevel,detail:ceilingMinFt===null?'nicht belastbar':`${rucCeilingsFt.length?'Modell-Ceiling':'diagnostisch'} min. ca. ${Math.round(ceilingMinFt/100)*100} ft AGL`,source:ceilingMinFt===null?undefined:ceilingSource,value:ceilingMinFt??undefined,unit:'ft'},
  ...
  const officialSignals=official.signals??[],items=mergeOfficial(diagnosed,officialSignals),sources:EventFlightSourceStatus[]=[{id:'model-profile',label:`${response.modelLabel||'Best Match'} / MID-Druckniveau`,status:'used'},{id:'dwd-ruc-ceiling',label:'DWD ICON-D2-RUC · CEILING',status:rucCeilingsFt.length?'used':'unavailable',detail:rucCeilingsFt.length?'Operatives Modell-Ceiling für das Event-Zeitfenster':'für das Event-Zeitfenster nicht verfügbar'},...(official.sources??[])],used=sourceLabels(sources),source=[response.modelLabel||'Best Match',...used.filter(label=>!label.startsWith(response.modelLabel||'Best Match'))].filter(Boolean).slice(0,4).join(' · '),resolvedCeilingMinFt=ceilingMinFt,resolvedVisibilityMinM=visibilityMin,resolvedGustMaxKt=gustMax
