...
export type EventFlightHazardItem={id:'thunderstorm'|'icing'|'turbulence'|'cat'|'ceiling'|'visibility'|'wind';label:string;level:EventFlightHazardLevel;detail:string}
export type EventFlightHazardSummary={available:boolean;overall:EventFlightHazardLevel;items:EventFlightHazardItem[];freezingLevelMin:number|null;ceilingMinFt:number|null;source:string;note?:string}
...
function overall(items:EventFlightHazardItem[]){return items.reduce<EventFlightHazardLevel>((best,item)=>levelRank(item.level)>levelRank(best)?item.level:best,'none')}
...
export async function loadEventFlightHazards(lat:number,lon:number,elevation:number,startEpoch:number,endEpoch:number,signal?:AbortSignal):Promise<EventFlightHazardSummary>{
 try{
  const response=await load(lat,lon,elevation,signal),hourly=response.data?.hourly
  if(!hourly?.time?.length)return{available:false,overall:'none',items:[],freezingLevelMin:null,ceilingMinFt:null,source:'MID Flugprofil',note:'Keine Druckniveau-Daten verfügbar.'}
  ...
  if(!indices.length)return{available:false,overall:'none',items:[],freezingLevelMin:null,ceilingMinFt:null,source:response.modelLabel||'Best Match',note:'Für das Event-Zeitfenster liegen noch keine Flugprofildaten vor.'}
  ...
  for(const index of indices){const points=profile(hourly,index);for(let row=0;row<points.length;row++){icing=Math.max(icing,icingAt(points[row]));const risk=turbulenceAt(points,row);turbulence=Math.max(turbulence,risk.turbulence);cat=Math.max(cat,risk.cat)}const ceiling=ceilingFor(points,elevation);if(ceiling!==null)ceilings.push(ceiling);const fz=valueAt(hourly,'freezing_level_height',index);if(fz!==null)freezing.push(fz);const vis=valueAt(hourly,'visibility',index);if(vis!==null)visibility.push(vis);const gust=valueAt(hourly,'wind_gusts_10m',index);if(gust!==null)gusts.push(gust);const cape=valueAt(hourly,'cape',index);if(cape!==null)capes.push(cape);const code=valueAt(hourly,'weather_code',index);if(code!==null)codes.push(code);const li=valueAt(hourly,'lifted_index',index);if(li!==null)lifted.push(li);const inhibition=valueAt(hourly,'convective_inhibition',index);if(inhibition!==null)cin.push(inhibition)}
  ...
  const items:EventFlightHazardItem[]=[
  ...
  return{available:true,overall:overall(items),items,freezingLevelMin:minNumber(freezing),ceilingMinFt,source:`${response.modelLabel||'Best Match'} · MID-Druckniveaudiagnose`}
 }catch(error){if(signal?.aborted)throw error;return{available:false,overall:'none',items:[],freezingLevelMin:null,ceilingMinFt:null,source:'MID Flugprofil',note:error instanceof Error?error.message:'Flugprofildaten nicht verfügbar.'}}
}
