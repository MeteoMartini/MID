...
export function summarizeDwdWarnings(samples:DwdWarningSample[],elevation=0,startLimit=samples.length){const relevant=samples.slice(0,Math.min(samples.length,Math.max(0,startLimit))),byKind=new Map<DwdWarningKind,DwdWarningSignal>();for(let index=0;index<relevant.length;index++){for(const signal of dwdWarningSignalsAt(relevant,index,elevation)){const previous=byKind.get(signal.kind);if(!previous||signal.level>previous.level||signal.level===previous.level&&signal.value>previous.value)byKind.set(signal.kind,signal)}}const apparentValues=relevant.map(sample=>finite(sample.apparent,finite(sample.temperature,Number.NaN))).filter(Number.isFinite),temperatureValues=relevant.map(sample=>finite(sample.temperature,Number.NaN)).filter(Number.isFinite);if(apparentValues.length&&temperatureValues.length){const maximum=Math.max(...apparentValues),minimum=Math.min(...temperatureValues);if(maximum>38)byKind.set('heat',{kind:'heat',level:3,title:'Extreme Wärmebelastung',symbol:'☀',detail:`Maximale gefühlte Temperatur ${Math.round(maximum)} °C; Tiefstwert der Lufttemperatur ${Math.round(minimum)} °C.`,value:maximum,unit:'°C'});else if(maximum>32&&minimum>=20)byKind.set('heat',{kind:'heat',level:1,title:'Starke Wärmebelastung',symbol:'☀',detail:`Maximale gefühlte Temperatur ${Math.round(maximum)} °C bei geringer nächtlicher Abkühlung (Tiefstwert ${Math.round(minimum)} °C).`,value:maximum,unit:'°C'});else byKind.delete('heat')}return[...byKind.values()].sort((a,b)=>b.level-a.level||a.kind.localeCompare(b.kind))}
