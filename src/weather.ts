...
function aggregateMembers(results:ModelResult[]){
 ...
 for(let lead=0;lead<allDates.length;lead++){
  ...
  let maxVals:{value:number;weight:number}[]=[],minVals:{value:number;weight:number}[]=[],rainVals:{value:number;weight:number}[]=[],cumulativeRainVals:{value:number;weight:number}[]=[],sunVals:{value:number;weight:number}[]=[],windVals:{value:number;weight:number}[]=[],gustVals:{value:number;weight:number}[]=[],modelSummaries:EnsembleModelDay[]=[];
