...
type TerrainMorphology={elevation?:number;slopeDeg:number;aspectDeg?:number;reliefM:number;positionIndexM:number;exposure:number};
...
type MorphologyContext={target?:TerrainMorphology;stations:Map<Station,TerrainMorphology>;surface?:LocalSurfaceContext;targetWindSpeed?:number;isDay?:boolean};
...
function terrainMorphologyFromElevations(values:number[]){const [center,north,south,east,west]=values,span=700,dzDy=(north-south)/span,dzDx=(east-west)/span,grade=Math.sqrt(dzDx*dzDx+dzDy*dzDy),slopeDeg=Math.atan(grade)*180/Math.PI,aspectDeg=slopeDeg<.6?undefined:(Math.atan2(-dzDx,-dzDy)*180/Math.PI+360)%360,outer=[north,south,east,west].filter(Number.isFinite),reliefM=outer.length?Math.max(center,...outer)-Math.min(center,...outer):0,positionIndexM=outer.length?center-outer.reduce((sum,v)=>sum+v,0)/outer.length:0,exposure=clampNumber(.5+positionIndexM/90+slopeDeg/120,0,1);return{elevation:center,slopeDeg,aspectDeg,reliefM,positionIndexM,exposure}}
async function terrainMorphologyForPoints(points:{lat:number;lon:number}[],signal?:AbortSignal){const result=new Map<number,TerrainMorphology>(),missing:number[]=[];points.forEach((point,index)=>{const cached=terrainMorphologyCache.get(terrainMorphologyKey(point.lat,point.lon));if(cached&&Date.now()-cached.at<7*86400000)result.set(index,cached.value);else missing.push(index)});for(let offset=0;offset<missing.length;offset+=20){const ids=missing.slice(offset,offset+20),samplePoints=ids.flatMap(index=>{const p=points[index];return[{lat:p.lat,lon:p.lon},offsetPoint(p.lat,p.lon,350,0),offsetPoint(p.lat,p.lon,-350,0),offsetPoint(p.lat,p.lon,0,350),offsetPoint(p.lat,p.lon,0,-350)]}),params=new URLSearchParams({latitude:samplePoints.map(p=>p.lat.toFixed(6)).join(','),longitude:samplePoints.map(p=>p.lon.toFixed(6)).join(',')}),raw=await j<any>(`https://api.open-meteo.com/v1/elevation?${params}`,signal),elevations=(Array.isArray(raw?.elevation)?raw.elevation:[]).map(Number);if(elevations.length<samplePoints.length)throw new Error('Copernicus-DEM-Profil unvollständig');ids.forEach((id,local)=>{const values=elevations.slice(local*5,local*5+5);if(values.every(Number.isFinite)){const profile=terrainMorphologyFromElevations(values);result.set(id,profile);terrainMorphologyCache.set(terrainMorphologyKey(points[id].lat,points[id].lon),{at:Date.now(),value:profile})}})}return result}
