...
export type RadarNowcastFrame={time:string;rate:number;nearbyRate?:number;nearestWetKm?:number;siteSupport?:number;hitClass?:'site'|'nearby'|'dry';future:boolean};
...
export async function radarNowcast(lat:number,lon:number,country?:string,signal?:AbortSignal,fast=false):Promise<RadarNowcast|null>{
 ...
 const[dwdSettled,operaSettled,nativeSettled]=await Promise.allSettled([dwdExpected?requestRadarStage(params,'dwd',signal,false):Promise.resolve(null),operaExpected?operaNowcast(lat,lon,signal):Promise.resolve(null),dwdExpected?nativeRadolanCurrentPoint(lat,lon,signal):Promise.resolve(null)]);abortError(signal);
 let dwdResult=dwdSettled.status==='fulfilled'?normaliseRadarNowcast(dwdSettled.value):null;const operaResult=operaSettled.status==='fulfilled'?operaSettled.value:null,native=nativeSettled.status==='fulfilled'?nativeSettled.value:null;
 if(dwdResult?.source==='dwd'){dwdResult=applyNativeObservation(dwdResult,native);return operaResult?.source==='opera'?mergeDwdOperaNowcast(dwdResult,operaResult):dwdResult}
