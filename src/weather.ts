...
function mergeEquivalentStationCandidates(a:Station,b:Station){
 const quality=(station:Station)=>networkQuality(station)*1.7-stationAgeMinutes(station.timestamp)/240-(station.distance??100000)/250000,primary=quality(a)>=quality(b)?a:b,secondary=primary===a?b:a,fields:(keyof Station)[]=['latitude','longitude','distance','height','timestamp','temperature','humidity','dewPoint','pressure','pressureReference','windSpeed','windDirection','windGust','visibility','cloudCover','ceilingHft','cloudBaseHft','precipitation','cloudObservation','cloudAnalysisMethod','trustFactor','siteClass'];
