...
export type Konrad3dCell={id:string;latitude:number;longitude:number;currentDistanceKm:number;relevanceDistanceKm:number;forecastDistanceKm?:number;forecastEffectiveDistanceKm?:number;forecastTime?:string;forecastLatitude?:number;forecastLongitude?:number;motionDirectionDeg?:number;arrivalMinutes?:number;isApproaching?:boolean;severity:number;trend:number;hailFlag:number;heavyRainFlag:number;gustFlag?:number;lightningRate:number;areaHail:number;areaLargeHail:number;speedKmh:number};
