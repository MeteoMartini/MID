...
export type Weather={latitude:number;longitude:number;elevation:number;timezone:string;current:Record<string,number|string>;hourly:Record<string,(number|string|null)[]>;daily:Record<string,(number|string|null)[]>};
export type Hour={time:string;temperature:number;apparent:number;humidity:number;dewPoint:number;precipitation:number;rain:number;showers:number;snowfall:number;probability:number;code:number;wind:number;gust:number;direction:number;cloud:number;cloudLow:number;cloudMid:number;cloudHigh:number;visibility:number;uvIndex:number;uvClearSky:number;isDay:boolean};
...
export async function forecast(lat:number,lon:number,signal?:AbortSignal){const p=new URLSearchParams({latitude:String(lat),longitude:String(lon),timezone:'auto',forecast_days:'14',models:'best_match',wind_speed_unit:'kn',current:['temperature_2m','relative_humidity_2m','dew_point_2m','apparent_temperature','is_day','precipitation','weather_code','cloud_cover','pressure_msl','wind_speed_10m','wind_direction_10m','wind_gusts_10m'].join(','),hourly:['temperature_2m','relative_humidity_2m','dew_point_2m','apparent_temperature','precipitation_probability','precipitation','rain','showers','snowfall','weather_code','cloud_cover','cloud_cover_low','cloud_cover_mid','cloud_cover_high','visibility','wind_speed_10m','wind_direction_10m','wind_gusts_10m','uv_index','uv_index_clear_sky','is_day'].join(','),daily:['weather_code','temperature_2m_max','temperature_2m_min','sunrise','sunset','precipitation_sum','precipitation_probability_max','wind_speed_10m_max','wind_gusts_10m_max','wind_direction_10m_dominant','uv_index_max'].join(',')});return j<Weather>(`https://api.open-meteo.com/v1/forecast?${p}`,signal)}
export async function airQuality(lat:number,lon:number,signal?:AbortSignal){const p=new URLSearchParams({latitude:String(lat),longitude:String(lon),timezone:'auto',current:['european_aqi','pm10','pm2_5','nitrogen_dioxide','ozone','uv_index','uv_index_clear_sky'].join(',')});return j<{current?:Record<string,number|string>}>(`https://air-quality-api.open-meteo.com/v1/air-quality?${p}`,signal)}
...
export function effectiveUvIndex(actual:number,clearSky:number,cloud:number,cloudLow:number,cloudMid:number,cloudHigh:number,code:number,isDay:boolean,precipitation=0,visibility=Number.POSITIVE_INFINITY){
 if(!isDay)return 0;
 // Open-Meteo uv_index is already cloud-adjusted. Use it unchanged whenever available.
 if(Number.isFinite(actual)&&actual>=0)return Number(actual.toFixed(1));
 if(!Number.isFinite(clearSky)||clearSky<=0)return 0;
 // Fallback only: estimate from clear-sky UVI if the cloud-adjusted field is unavailable.
 const low=Math.min(1,Math.max(0,cloudLow/100)),mid=Math.min(1,Math.max(0,cloudMid/100)),high=Math.min(1,Math.max(0,cloudHigh/100)),total=Math.min(1,Math.max(0,cloud/100));
 let factor=1-(0.58*low+0.30*mid+0.10*high);
 // Broken/thin clouds can occasionally enhance surface UV through scattering.
 if(total>=0.2&&total<=0.7&&low<0.35&&mid<0.45&&[1,2].includes(code))factor=Math.max(factor,1.05);
 if(code===3)factor=Math.min(factor,0.62);
 if([45,48].includes(code))factor*=0.52;
 if([51,53,55].includes(code))factor*=0.90;
 if([61,63,65].includes(code))factor*=0.80;
 if([80,81,82].includes(code))factor*=0.74;
 if([71,73,75,85,86].includes(code))factor*=0.80;
 if([95,96,99].includes(code))factor*=0.66;
 if(Number.isFinite(precipitation)&&precipitation>0)factor*=precipitation>=5?0.72:precipitation>=1?0.84:0.94;
 if(Number.isFinite(visibility)){if(visibility<5000)factor*=0.72;else if(visibility<10000)factor*=0.84;else if(visibility<20000)factor*=0.94;}
 factor=Math.min(1.12,Math.max(0.12,factor));
 return Number((clearSky*factor).toFixed(1));
}
...
export function mapHours(w:Weather):Hour[]{return (w.hourly.time as string[]).map((time,i)=>{const code=n(w.hourly.weather_code[i],3),cloud=n(w.hourly.cloud_cover[i],0),cloudLow=n(w.hourly.cloud_cover_low?.[i],0),cloudMid=n(w.hourly.cloud_cover_mid?.[i],0),cloudHigh=n(w.hourly.cloud_cover_high?.[i],0),visibility=n(w.hourly.visibility?.[i],Number.POSITIVE_INFINITY),precipitation=n(w.hourly.precipitation[i],0),isDay=n(w.hourly.is_day[i],0)===1,actualUv=n(w.hourly.uv_index[i],NaN),clearUv=n(w.hourly.uv_index_clear_sky?.[i],NaN);return{time,temperature:n(w.hourly.temperature_2m[i]),apparent:n(w.hourly.apparent_temperature[i]),humidity:n(w.hourly.relative_humidity_2m[i]),dewPoint:n(w.hourly.dew_point_2m[i]),precipitation,rain:n(w.hourly.rain[i],0),showers:n(w.hourly.showers[i],0),snowfall:n(w.hourly.snowfall[i],0),probability:n(w.hourly.precipitation_probability[i],0),code,wind:n(w.hourly.wind_speed_10m[i],0),gust:n(w.hourly.wind_gusts_10m[i],0),direction:n(w.hourly.wind_direction_10m[i],0),cloud,cloudLow,cloudMid,cloudHigh,visibility,uvIndex:effectiveUvIndex(actualUv,clearUv,cloud,cloudLow,cloudMid,cloudHigh,code,isDay,precipitation,visibility),uvClearSky:Number.isFinite(clearUv)?clearUv:0,isDay}}).filter(x=>Number.isFinite(x.temperature))}
