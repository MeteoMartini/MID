...
import {formatDwdWarningDetailWithDirection,formatDwdWarningValue,summarizeDwdWarnings,type DwdWarningLevel} from './dwdWarnings';
...
export function hazards(h:Hour[],_currentUv?:number,elevation=0,unit:WindUnit='kn'){
 ...
 return summarizeDwdWarnings(horizon,elevation,24).map(signal=>({level:dwdHazardClass(signal.level),title:signal.title,metric:formatDwdWarningValue(signal,unit),text:`${formatDwdWarningDetailWithDirection(signal,unit)} Automatisch aus dem Open-Meteo-Best-Match abgeleitet; keine amtliche Warnung.`,validFrom:signal.validFrom,validTo:signal.validTo})).sort((a,b)=>levelOrder[b.level]-levelOrder[a.level]);
}
