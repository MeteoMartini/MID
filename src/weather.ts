...
import {guardedOpenMeteoJson,isOpenMeteoRateLimitError,registerOpenMeteoCooldown,type OpenMeteoPriority} from './openMeteoGuard';
...
import {detectStableNightThermalRegime,stableNightThermalWeightFactor,type StableNightThermalSample} from './hyperlocalThermal';
...
export type Day={date:string;code:number;max:number;min:number;sunrise?:string;sunset?:string;sunshineDuration:number;precipitation:number;rain?:number;showers?:number;snowfall?:number;precipitationHours?:number;probability:number;probabilitySignificant?:number;probabilityWindows?:PrecipitationProbabilityWindow[];probabilitySource?:'ensemble-members-dwd'|'hourly-max-fallback'|'provider-no-probability';probabilityAvailable?:boolean;probabilityMemberCount?:number;wind:number;gust:number;gustAdjusted?:boolean;direction:number;uvMax:number;weatherSourceId?:string;weatherSourceLabel?:string};
...
export async function bestMatchModelInfo(lat:number,lon:number,country?:string,signal?:AbortSignal):Promise<BestMatchModelInfo>{
 ...
 return{summary:`Best Match stammt aus der Open-Meteo Forecast API. MID prüft zusätzlich verfügbare Rapid-Update-, Regional- und unabhängige Globalmodelle${rapidAvailable.length?` (${rapidAvailable.join(', ')})`:''}. DWD ICON-D2-RUC wird direkt auf Laufverfügbarkeit geprüft. Numerische RUC-Punktwerte können zusätzlich über den Worker-Punktadapter in die Forecast-Fusion eingehen; ohne dekodierten Punktpfad bleibt der DWD-Rohdatensatz wegen seines nativen Dreiecksgitters eine Verfügbarkeitsquelle. Die konkrete Best-Match-Quelle kann je Variable und Zeitraum wechseln.`,likelyChain:'',candidateModels,runs};
}
...
const FORECAST_CORE_CACHE_PREFIX='mid:forecast-core:v2:';
const FORECAST_CORE_LEGACY_CACHE_PREFIX='mid:forecast-core:v1:';
...
function parseForecastCoreCache(raw:string|null){try{if(!raw)return null;const parsed=JSON.parse(raw) as ForecastCoreCache,age=Date.now()-Number(parsed?.time);if(!parsed?.value||!Number.isFinite(age)||age<0||age>FORECAST_CORE_STALE_MS)return null;return{...parsed,age}}catch{return null}}
function forecastCoreCacheCoordinates(key:string){for(const prefix of[FORECAST_CORE_CACHE_PREFIX,FORECAST_CORE_LEGACY_CACHE_PREFIX])if(key.startsWith(prefix)){const match=key.slice(prefix.length).match(/^(-?\d+(?:\.\d+)?):(-?\d+(?:\.\d+)?)$/);if(match)return{lat:Number(match[1]),lon:Number(match[2])}}return null}
function readForecastCoreCache(lat:number,lon:number){try{const exact=parseForecastCoreCache(localStorage.getItem(forecastCoreCacheKey(lat,lon)));if(exact)return exact;const legacyExact=parseForecastCoreCache(localStorage.getItem(`${FORECAST_CORE_LEGACY_CACHE_PREFIX}${Number(lat).toFixed(4)}:${Number(lon).toFixed(4)}`));if(legacyExact){writeForecastCoreCache(lat,lon,legacyExact.value,legacyExact.time);return legacyExact}let best:(ForecastCoreCache&{age:number;distance:number})|null=null;for(let index=0;index<localStorage.length;index++){const key=localStorage.key(index)||'',coordinates=forecastCoreCacheCoordinates(key);if(!coordinates)continue;const entry=parseForecastCoreCache(localStorage.getItem(key));if(!entry)continue;const distance=haversine(lat,lon,coordinates.lat,coordinates.lon);if(distance>FORECAST_CORE_NEARBY_FALLBACK_M)continue;if(!best||entry.age<best.age||entry.age===best.age&&distance<best.distance)best={...entry,distance}}if(best){writeForecastCoreCache(lat,lon,best.value,best.time);return{time:best.time,value:best.value,age:best.age}}return null}catch{return null}}
function writeForecastCoreCache(lat:number,lon:number,value:Weather,time=Date.now()){try{writeBoundedStorage(localStorage,forecastCoreCacheKey(lat,lon),{time,value},[FORECAST_CORE_CACHE_PREFIX,FORECAST_CORE_LEGACY_CACHE_PREFIX],10,FORECAST_CORE_STALE_MS)}catch{}}
function validForecastCore(value:unknown):value is Weather{const row=value as Partial<Weather>|null;return Boolean(row&&typeof row==='object'&&row.current&&row.hourly&&row.daily&&Array.isArray(row.hourly.time)&&Array.isArray(row.daily.time))}
async function workerForecastCore(lat:number,lon:number,signal:AbortSignal|undefined,forceFresh:boolean,timeZone?:string,elevation?:number){if(!workerBaseCandidates('general').length)return null;try{const value=await fetchWorkerJson<Weather&{error?:string}>('forecast-core',{lat:Number(lat).toFixed(5),lon:Number(lon).toFixed(5),refresh:forceFresh?1:0,timezone:String(timeZone||'').trim()||undefined,elevation:Number.isFinite(Number(elevation))?Math.round(Number(elevation)):undefined},{purpose:'general',signal,timeoutMs:20000,cache:forceFresh?'no-store':'default',maxAgeMs:forceFresh?0:2*60*1000,staleIfErrorMs:FORECAST_CORE_STALE_MS,cacheKey:`forecast-core:${Number(lat).toFixed(3)}:${Number(lon).toFixed(3)}`});if(Number(value?._mid_core_proxy?.upstreamStatus)===429)registerOpenMeteoCooldown(Date.now()+120_000);return validForecastCore(value)?value:null}catch(error){if(signal?.aborted)throw error;return null}}
export async function forecast(lat:number,lon:number,signal?:AbortSignal,options:ForecastRequestOptions={}){const cached=readForecastCoreCache(lat,lon),priority=options.priority??'normal';if(!options.forceFresh&&cached&&cached.age<=FORECAST_CORE_FRESH_MS)return cached.value;const p=new URLSearchParams({latitude:String(lat),longitude:String(lon),timezone:'auto',forecast_days:'14',forecast_minutely_15:'24',past_minutely_15:'4',models:'best_match',wind_speed_unit:'kn',current:['temperature_2m','relative_humidity_2m','dew_point_2m','apparent_temperature','is_day','precipitation','rain','showers','snowfall','weather_code','cloud_cover','cloud_cover_low','cloud_cover_mid','cloud_cover_high','pressure_msl','wind_speed_10m','wind_direction_10m','wind_gusts_10m','visibility','cape','sunshine_duration'].join(','),minutely_15:['precipitation_probability','precipitation','rain','showers','snowfall','weather_code','sunshine_duration'].join(','),hourly:['temperature_2m','relative_humidity_2m','dew_point_2m','apparent_temperature','precipitation_probability','precipitation','rain','showers','snowfall','weather_code','cloud_cover','cloud_cover_low','cloud_cover_mid','cloud_cover_high','pressure_msl','wind_speed_10m','wind_direction_10m','wind_gusts_10m','uv_index','visibility','cape','lifted_index','convective_inhibition','total_column_integrated_water_vapour','is_day','sunshine_duration'].join(','),daily:['weather_code','temperature_2m_max','temperature_2m_min','sunrise','sunset','precipitation_sum','rain_sum','showers_sum','snowfall_sum','precipitation_hours','precipitation_probability_max','wind_speed_10m_max','wind_gusts_10m_max','wind_direction_10m_dominant','uv_index_max','sunshine_duration'].join(',')});const workerConfigured=workerBaseCandidates('general').length>0;if(workerConfigured){const proxied=await workerForecastCore(lat,lon,signal,options.forceFresh===true,options.timeZone,options.elevation);if(proxied){writeForecastCoreCache(lat,lon,proxied);return proxied}}try{const value=await j<Weather>(`https://api.open-meteo.com/v1/forecast?${p}`,signal,priority);writeForecastCoreCache(lat,lon,value);return value}catch(error){if(signal?.aborted)throw error;if(cached&&(isOpenMeteoRateLimitError(error)||priority==='foreground'))return cached.value;throw error}}
...
async function localBackground(points:{lat:number;lon:number;elevation?:number}[],context?:Location,signal?:AbortSignal):Promise<LocalBackgroundResult>{const models=hyperlocalBackgroundModelCandidates(context),key=localBackgroundKey(points,models),cached=localBackgroundCache.get(key);if(cached&&Date.now()-cached.at<=5*60000)return cached.value;let lastError:unknown;for(const modelId of models){try{const rows=await fetchLocalBackground(points,modelId,signal),value={rows,modelId,modelLabel:hyperlocalBackgroundModelLabel(modelId)};localBackgroundCache.set(key,{at:Date.now(),value});if(localBackgroundCache.size>24){const oldest=[...localBackgroundCache.entries()].sort((a,b)=>a[1].at-b[1].at).slice(0,localBackgroundCache.size-24);for(const [cacheKey] of oldest)localBackgroundCache.delete(cacheKey)}return value}catch(error){lastError=error;if(signal?.aborted)throw error}}if(cached&&Date.now()-cached.at<=20*60000)return cached.value;throw lastError??new Error('Lokaler Modellhintergrund nicht verfügbar')}
...
export function mapDays(w:Weather):Day[]{const sourceLabel=coreForecastSourceLabel(w),fallbackSource=coreForecastUsesIndependentFallback(w);return (w.daily.time as string[]).map((date,i)=>{
 const windPair=validateWindPair(n(w.daily.wind_speed_10m_max[i],0),n(w.daily.wind_gusts_10m_max[i],0)),leadHours=(localIsoEpoch(`${date}T12:00`,w.timezone,Number(w.utc_offset_seconds)||0)-Date.now())/3600000,rain=n(w.daily.rain_sum?.[i],0),showers=n(w.daily.showers_sum?.[i],0),snowfall=n(w.daily.snowfall_sum?.[i],0),precipitationHours=n(w.daily.precipitation_hours?.[i],0),rawProbability=n(w.daily.precipitation_probability_max[i],NaN),probabilityAvailable=Number.isFinite(rawProbability)&&w._mid_core_proxy?.precipitationProbabilityAvailable!==false,precipitationSignal=reconcileForecastPrecipitation({precipitation:n(w.daily.precipitation_sum[i],0),rain,showers,snowfall,probability:probabilityAvailable?rawProbability:0,probabilityUnavailable:!probabilityAvailable,code:n(w.daily.weather_code[i],3),leadHours});
 return({date,code:precipitationSignal.code,max:n(w.daily.temperature_2m_max[i]),min:n(w.daily.temperature_2m_min[i]),sunrise:String(w.daily.sunrise?.[i]??'')||undefined,sunset:String(w.daily.sunset?.[i]??'')||undefined,sunshineDuration:n(w.daily.sunshine_duration?.[i],0),precipitation:precipitationSignal.precipitation,rain:precipitationSignal.rain,showers:precipitationSignal.showers,snowfall:precipitationSignal.snowfall,precipitationHours,probability:precipitationSignal.probability,probabilitySource:(probabilityAvailable?'hourly-max-fallback':'provider-no-probability') as Day['probabilitySource'],probabilityAvailable,wind:windPair.wind,gust:windPair.gust,gustAdjusted:windPair.adjusted,direction:n(w.daily.wind_direction_10m_dominant[i],0),uvMax:altitudeCorrectedUvi(n(w.daily.uv_index_max[i],0),w.elevation),weatherSourceId:fallbackSource?'met-norway-locationforecast':'best_match',weatherSourceLabel:sourceLabel});
...
export function dailyPrecipitationProbabilityTitle(day:Pick<Day,'probability'|'probabilitySignificant'|'probabilityWindows'|'probabilitySource'|'probabilityAvailable'|'probabilityMemberCount'>,hours:Hour[]=[]){
 if(day.probabilityAvailable===false||day.probabilitySource==='provider-no-probability')return'Niederschlagswahrscheinlichkeit im Ersatzmodell nicht verfügbar';
...
export function dailyPrecipitationProbabilityCompact(day:Pick<Day,'probability'|'probabilityWindows'|'probabilitySource'|'probabilityAvailable'>,hours:Hour[]=[]){
 if(day.probabilityAvailable===false||day.probabilitySource==='provider-no-probability')return'PoP –';
...
export async function eventEnsembleForecast(lat:number,lon:number,date:string,startTime:string,endTime:string,signal?:AbortSignal,forceRefresh=false):Promise<EventEnsembleForecast>{
 const fresh=forceRefresh?null:readEventEnsembleCache(lat,lon,date,startTime,endTime,ENSEMBLE_FRESH_CACHE_MS);if(fresh)return fresh;const selected=selectedEnsembleModelsForEvent(lat,lon,date),lead=eventLeadDayIndex(date);
