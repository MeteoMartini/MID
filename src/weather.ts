...
function buildEnsembleScenarios(results:ModelResult[],days:EnsembleDay[]):EnsembleScenarioCluster[]{
 ...
 const usedLabels=new Set<string>(),reference=clusters[0].points,labelled=clusters.map((cluster,index)=>{let label=index===0?scenarioLabel(cluster.points,days):scenarioRelativeLabel(cluster.points,reference,index);if(usedLabels.has(label))label=index===1?'abweichende zeitliche Verteilung':'weiterer alternativer Verlauf';usedLabels.add(label);return{...cluster,id:`scenario-${index+1}`,label,summary:scenarioSummary(label,cluster.points)}}),divergenceDate=scenarioDivergenceDate(labelled,dates);
