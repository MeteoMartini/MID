...
export type StationFieldSource={provider:string;stationName:string;stationId?:string;distanceKm?:number;observedAt?:string;networkClass?:'official'|'professional'|'pws'|'citizen'|'unknown';sourceType?:StationSourceType;weight?:number;qc?:string};
...
type EnsembleModel={id:string;label:string;metaId:string;family:string;independenceGroup:string;resolutionKm:number;updateHours:number;maxDays:number;bbox?:[number,number,number,number];distributionMode?:'members'|'mean-spread'};
...
const meanPriority=['dwd_icon_eps_ensemble_mean_seamless','ncep_hgefs025_ensemble_mean','ecmwf_ifs_europe_ensemble_mean','ecmwf_aifs_europe_ensemble_mean','ecmwf_ifs025_ensemble_mean','ecmwf_aifs025_ensemble_mean','ncep_gefs_ensemble_mean_seamless','cmc_gem_geps_ensemble_mean','google_weathernext2_ensemble_mean'];
...
const EVENT_ENSEMBLE_CACHE_PREFIX='mid:event-ensemble:v1:';
...
function selectedEnsembleModelsForEvent(lat:number,lon:number,date:string){const lead=eventLeadDayIndex(date),applicable=ensembleModels.filter(model=>modelApplies(model,lat,lon)&&model.maxDays>=lead+.5);return rankModels(withoutGlobalEcmwfDuplicates(applicable),ensemblePriority,8)}
...
function stationSourceReference(station:Station,weight?:number):StationFieldSource{return{provider:station.provider||'Messstation',stationName:station.name,stationId:station.stationId,distanceKm:Number.isFinite(station.distance)?Number((Number(station.distance)/1000).toFixed(1)):undefined,observedAt:station.timestamp,networkClass:station.networkClass,sourceType:station.sourceType,weight:Number.isFinite(weight)?Number(Number(weight).toFixed(3)):undefined,qc:station.qcLabel}}
function stationFieldSourcesForCandidates(candidates:{s:Station;w:number}[]):StationFieldSources{const fields:StationAnalysisField[]=['temperature','humidity','dewPoint','pressure','windSpeed','windDirection','windGust','visibility','cloudCover','ceilingHft','cloudBaseHft','precipitation'],out:StationFieldSources={};for(const field of fields){const rows=candidates.filter(({s,w})=>Number.isFinite(Number(s[field]))&&w>0).sort((a,b)=>b.w-a.w).slice(0,3);if(rows.length)out[field]=rows.map(({s,w})=>stationSourceReference(s,w))}return out}
function stationFieldSourcesFromAnalysis(analysis:ResidualResult):StationFieldSource[]{return[...analysis.accepted].map(station=>stationSourceReference(station,analysis.weights.get(station))).sort((a,b)=>(b.weight??0)-(a.weight??0)).slice(0,4)}
...
function analysisWeight(s:Station,targetElevation:number|undefined,targetUrban:UrbanClass,field:StationAnalysisField='temperature'){
 ...
 const trust=Number.isFinite(s.trustFactor)?Math.max(.25,Math.min(1.15,Number(s.trustFactor)/100)):1,site=fieldSiteCompatibility(field,targetUrban,candidateSiteClass(s)),localityScale=field==='pressure'?42:field==='windSpeed'||field==='windDirection'||field==='windGust'?32:field==='visibility'||field==='cloudCover'||field==='precipitation'?26:24,locality=.55+.65*Math.exp(-Math.pow(distanceKm/localityScale,1.22));return policy.quality*trust*site*locality*Math.exp(-Math.pow(distanceKm/Math.max(3,policy.distanceScaleKm),1.28))*Math.exp(-age/Math.max(20,policy.ageScaleMinutes))*Math.exp(-heightDiff/terrainScale)
}
 ...
 */
export async function eventEnsembleForecast(lat:number,lon:number,date:string,startTime:string,endTime:string,signal?:AbortSignal):Promise<EventEnsembleForecast>{
 const fresh=readEventEnsembleCache(lat,lon,date,startTime,endTime,ENSEMBLE_FRESH_CACHE_MS);if(fresh)return fresh;const selected=selectedEnsembleModelsForEvent(lat,lon,date),lead=eventLeadDayIndex(date);
 const settled=await settledMapLimited(selected,2,async model=>{const forecastDays=Math.max(1,Math.min(14,Math.ceil(Math.min(model.maxDays,lead+2)))),weather=await fetchEnsembleWeather(lat,lon,forecastDays,model.id,signal),modelResult=parseModelMembers(weather,model),totals=eventMemberPrecipitationTotals(weather,date,startTime,endTime);return{model,modelResult,totals}});if(signal?.aborted)throw new DOMException('Abgebrochen','AbortError');
 const fulfilled=settled.filter(row=>row.status==='fulfilled').map(row=>(row as PromiseFulfilledResult<{model:EnsembleModel;modelResult:ModelResult|null;totals:number[]}>).value),results=fulfilled.map(row=>row.modelResult).filter(Boolean) as ModelResult[],days=aggregateMembers(results),precipitationProbability=aggregateEventPrecipitationProbability(fulfilled,date,startTime,endTime),models=fulfilled.filter(row=>row.modelResult||row.totals.length>=2).map(row=>row.model.label),value={days,models,precipitationProbability} satisfies EventEnsembleForecast;
