...
export function recentSunshineDuration(w:Weather,options:{localCloudCover?:number;currentCloudCover?:number;isDay?:boolean}={}):RecentSunshineDuration{
 ...
 if(Number.isFinite(currentEpoch)&&times.length&&values.length){
  ...
  if(rows.length>=3){
   const gaps=rows.slice(1).map((row,index)=>(row.epoch-rows[index].epoch)/60000).filter(value=>Number.isFinite(value)&&value>0),intervalMinutes=gaps.length?Math.max(1,Math.round(gaps.reduce((sum,value)=>sum+value,0)/gaps.length)):15,latest=rows[rows.length-1];let seconds=rows.reduce((sum,row)=>sum+Math.max(0,row.value),0),adjusted=false;
   const localCloud=Number(options.localCloudCover),modelCloud=Number(options.currentCloudCover),isDay=options.isDay!==false;
   if(isDay&&Number.isFinite(localCloud)&&Number.isFinite(modelCloud)&&localCloud>=87.5&&modelCloud>=80){const cap=localCloud>=95?120:300,original=Math.max(0,latest.value),limited=Math.min(original,cap);if(limited<original){seconds-=original-limited;adjusted=true}}
   return{seconds:Math.max(0,seconds),coverageMinutes:Math.min(60,rows.length*intervalMinutes),intervalMinutes,source:'minutely_15',plausibilityAdjusted:adjusted};
  }
 }
 const currentValue=n(w.current?.sunshine_duration,NaN),currentInterval=Math.max(60,n(w.current?.interval,900));
 if(Number.isFinite(currentValue))return{seconds:Math.max(0,currentValue),coverageMinutes:Math.round(currentInterval/60),intervalMinutes:Math.round(currentInterval/60),source:'current',plausibilityAdjusted:false};
 const hourlyTimes=(w.hourly?.time??[]) as string[],hourlyValues=(w.hourly?.sunshine_duration??[]) as (number|string|null)[];
 if(Number.isFinite(currentEpoch)&&hourlyTimes.length&&hourlyValues.length){let best=-1,bestEpoch=-Infinity;for(let i=0;i<hourlyTimes.length;i++){const epoch=localIsoEpoch(String(hourlyTimes[i]),w.timezone,offset),value=n(hourlyValues[i],NaN);if(Number.isFinite(epoch)&&Number.isFinite(value)&&epoch<=currentEpoch&&epoch>bestEpoch){best=i;bestEpoch=epoch}}if(best>=0)return{seconds:Math.max(0,n(hourlyValues[best],0)),coverageMinutes:60,intervalMinutes:60,source:'hourly',plausibilityAdjusted:false}}
