...
export function mapDays(w:Weather):Day[]{return (w.daily.time as string[]).map((date,i)=>{
 const windPair=validateWindPair(n(w.daily.wind_speed_10m_max[i],0),n(w.daily.wind_gusts_10m_max[i],0)),leadHours=(localIsoEpoch(`${date}T12:00`,w.timezone,Number(w.utc_offset_seconds)||0)-Date.now())/3600000,rain=n(w.daily.rain_sum?.[i],0),showers=n(w.daily.showers_sum?.[i],0),snowfall=n(w.daily.snowfall_sum?.[i],0),precipitationHours=n(w.daily.precipitation_hours?.[i],0),precipitationSignal=reconcileForecastPrecipitation({precipitation:n(w.daily.precipitation_sum[i],0),rain,showers,snowfall,probability:n(w.daily.precipitation_probability_max[i],0),code:n(w.daily.weather_code[i],3),leadHours});
 return({date,code:precipitationSignal.code,max:n(w.daily.temperature_2m_max[i]),min:n(w.daily.temperature_2m_min[i]),sunrise:String(w.daily.sunrise?.[i]??'')||undefined,sunset:String(w.daily.sunset?.[i]??'')||undefined,sunshineDuration:n(w.daily.sunshine_duration?.[i],0),precipitation:precipitationSignal.precipitation,rain:precipitationSignal.rain,showers:precipitationSignal.showers,snowfall:precipitationSignal.snowfall,precipitationHours,probability:precipitationSignal.probability,wind:windPair.wind,gust:windPair.gust,gustAdjusted:windPair.adjusted,direction:n(w.daily.wind_direction_10m_dominant[i],0),uvMax:altitudeCorrectedUvi(n(w.daily.uv_index_max[i],0),w.elevation),weatherSourceId:'best_match',weatherSourceLabel:'Open-Meteo Best Match'});
