...
function metarCloudCover(row:any):number|undefined{
 ...
 const layers=Array.isArray(row.clouds)?row.clouds:Array.isArray(row.cloudLayers)?row.cloudLayers:typeof row.clouds==='string'?row.clouds.split(/[,; ]+/):[];
 const values=layers.map((layer:any)=>map[String(typeof layer==='string'?layer:layer?.cover??layer?.amount??layer?.code??'').toUpperCase()]).filter(Number.isFinite);
