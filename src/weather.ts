...
export type Hour={time:string;epoch:number;timezone:string;temperature:number;apparent:number;humidity:number;dewPoint:number;pressure:number;precipitation:number;rain:number;showers:number;snowfall:number;probability:number;code:number;wind:number;gust:number;gustAdjusted?:boolean;direction:number;cloud:number;lowCloud:number;uvIndex:number;visibility:number;cape:number;liftedIndex?:number;convectiveInhibition?:number;columnWaterVapour?:number;sunshineDuration?:number;weatherSourceId?:string;weatherSourceLabel?:string;weatherBundleKind?:'best-match'|'coherent-model'|'nowcast';isDay:boolean};
...
export type Day={date:string;code:number;max:number;min:number;sunrise?:string;sunset?:string;sunshineDuration:number;precipitation:number;probability:number;wind:number;gust:number;gustAdjusted?:boolean;direction:number;uvMax:number;weatherSourceId?:string;weatherSourceLabel?:string};
...
export type EnsembleDay={date:string;maxMean:number;maxLow:number;maxHigh:number;maxQ25:number;maxQ75:number;minMean:number;minLow:number;minHigh:number;minQ25:number;minQ75:number;precipitationMean:number;precipitationLow:number;precipitationHigh:number;precipitationProbability:number;sunshineDurationMean:number;sunshineDurationLow:number;sunshineDurationHigh:number;windMean:number;windLow:number;windHigh:number;windQ25:number;windQ75:number;gustMean:number;gustLow:number;gustHigh:number;gustQ25:number;gustQ75:number;modelCount:number;memberCount:number;modelSummaries?:EnsembleModelDay[]};
...
export type RadarNowcast={source:'dwd'|'opera'|'rainviewer'|'model';provider:string;quality:RadarNowcastQuality;radarProbability:number;currentRate?:number;rawCurrentRate?:number;peakRate?:number;rateApproximate?:boolean;rateUncertain?:boolean;seasonalEchoProfile?:'winter-sensitive'|'transition'|'summer-filter';seasonalEchoLabel?:string;siteEchoThreshold?:number;nearbyEchoThreshold?:number;arrivalMinutes?:number;endMinutes?:number;arrivalKind?:'site'|'nearby'|'approximate';arrivalStartAt?:string;arrivalEndAt?:string;endAt?:string;endOpenEnded?:boolean;endUncertain?:boolean;observedAt?:string;summary:string;coverage?:boolean;coverageExpected?:boolean;temporaryUnavailable?:boolean;expectedSource?:string;radarLayer?:string;timeline?:string[];nowcastSeries?:RadarNowcastFrame[];license?:string;motionDirectionDeg?:number;motionSpeedKmh?:number;motionConfidence?:'high'|'medium'|'low';motionSource?:string;motionFrameCount?:number;motionFit?:number;motionPairCount?:number;motionDirectionConvention?:'towards';steeringDirectionDeg?:number;steeringSpeedKmh?:number;motionAnchors?:RadarMotionAnchor[];diagnostics?:Record<string,unknown>};
export type Konrad3dCell={id:string;latitude:number;longitude:number;currentDistanceKm:number;siteBearingDeg?:number;relevanceDistanceKm:number;forecastDistanceKm?:number;forecastEffectiveDistanceKm?:number;forecastUncertaintyKm?:number;forecastTime?:string;forecastLatitude?:number;forecastLongitude?:number;trackForecastTime?:string;trackForecastLatitude?:number;trackForecastLongitude?:number;trackForecastUncertaintyKm?:number;motionDirectionDeg?:number;arrivalMinutes?:number;isApproaching?:boolean;severity:number;trend:number;hailFlag:number;heavyRainFlag:number;gustFlag?:number;lightningRate:number;areaHail:number;areaLargeHail:number;speedKmh:number};
...
const ensemblePriority=['icon_d2_eps','icon_eu_eps','icon_seamless_eps','ecmwf_ifs_europe_ensemble','ecmwf_aifs_europe_ensemble','ecmwf_ifs025_ensemble','ecmwf_aifs025_ensemble','ncep_gefs05','gem_global_ensemble','google_weathernext2_ensemble'];
...
export async function reverseLocation(lat:number,lon:number,elevation?:number,signal?:AbortSignal):Promise<Location>{
 ...
 try{
  const d=await j<any>(`https://api.bigdatacloud.net/data/reverse-geocode-client?${p}`,signal),city=String(d.city||'').trim(),locality=String(d.locality||'').trim(),country=String(d.countryName||'').trim(),admin1=String(d.principalSubdivision||'').trim(),name=locality||city||admin1||country||`${formatDecimal(lat,2,2)}°, ${formatDecimal(lon,2,2)}°`,admin=(d.localityInfo?.administrative??[]) as any[],excluded=new Set([name,city,locality,country,admin1].filter(Boolean).map(value=>String(value).toLocaleLowerCase('de-DE'))),admin2=admin.map((item:any)=>String(item?.name||'').trim()).find((value:string)=>value&&!excluded.has(value.toLocaleLowerCase('de-DE')));
...
function cachedEeaStation(lat:number,lon:number){try{const parsed=JSON.parse(localStorage.getItem(EEA_STATION_CACHE_KEY)||'null') as {lat?:number;lon?:number;station?:AirQualityStationMeta;at?:number}|null;if(!parsed?.station?.available||!Number.isFinite(parsed.lat)||!Number.isFinite(parsed.lon)||!Number.isFinite(parsed.at)||Date.now()-Number(parsed.at)>30*86400000||haversine(lat,lon,Number(parsed.lat),Number(parsed.lon))>100000)return null;return{...parsed.station,cached:true,cachedAt:new Date(Number(parsed.at)).toISOString()} satisfies AirQualityStationMeta}catch{return null}}
...
export async function airQualityStation(lat:number,lon:number,signal?:AbortSignal){
 ...
 try{const station=await fetchWorkerJson<AirQualityStationMeta>('air-quality-station',{lat,lon},{purpose:'general',signal,timeoutMs:14000,cache:'default'});if(station.available)storeEeaStation(lat,lon,station);return station}catch(error){failures.push(error instanceof Error?error.message:String(error))}
...
function analysisWeight(s:Station,targetElevation:number|undefined,targetUrban:UrbanClass,field:StationAnalysisField='temperature'){
 const policy=fieldWeightPolicy(s.provider,s.networkClass,field),distanceKm=Math.max(.1,(s.distance??100000)/1000),age=stationAgeMinutes(s.timestamp),heightDiff=Number.isFinite(s.height)&&Number.isFinite(targetElevation)?Math.abs(Number(s.height)-Number(targetElevation)):120,terrainScale=field==='pressure'?1250:field==='windSpeed'||field==='windDirection'||field==='windGust'?1150:field==='visibility'||field==='cloudCover'||field==='precipitation'?820:950;
 const trust=Number.isFinite(s.trustFactor)?Math.max(.25,Math.min(1.15,Number(s.trustFactor)/100)):1,site=fieldSiteCompatibility(field,targetUrban,candidateSiteClass(s));return policy.quality*trust*site*Math.exp(-Math.pow(distanceKm/Math.max(3,policy.distanceScaleKm),1.28))*Math.exp(-age/Math.max(20,policy.ageScaleMinutes))*Math.exp(-heightDiff/terrainScale)
}
...
const localBackgroundCache=new Map<string,{at:number;value:LocalBackground[]}>();
function localBackgroundKey(points:{lat:number;lon:number;elevation?:number}[]){return points.slice(0,18).map(point=>`${point.lat.toFixed(3)}:${point.lon.toFixed(3)}:${Number.isFinite(point.elevation)?Math.round(Number(point.elevation)/25)*25:'x'}`).join('|')}
async function localBackground(points:{lat:number;lon:number;elevation?:number}[],signal?:AbortSignal):Promise<LocalBackground[]>{const selected=points.slice(0,18),key=localBackgroundKey(selected),cached=localBackgroundCache.get(key);if(cached&&Date.now()-cached.at<=5*60000)return cached.value;const p=new URLSearchParams({latitude:selected.map(x=>x.lat.toFixed(5)).join(','),longitude:selected.map(x=>x.lon.toFixed(5)).join(','),elevation:selected.map(x=>Number.isFinite(x.elevation)?String(Math.round(Number(x.elevation))):'nan').join(','),timezone:'GMT',forecast_days:'1',models:'best_match',wind_speed_unit:'kn',current:['temperature_2m','relative_humidity_2m','dew_point_2m','pressure_msl','wind_speed_10m','wind_direction_10m','wind_gusts_10m','visibility','cloud_cover','precipitation'].join(',')});try{const raw=await j<any>(`https://api.open-meteo.com/v1/forecast?${p}`,signal),list=Array.isArray(raw)?raw:[raw],value=list.map((item:any)=>{const c=item?.current??{},n=(v:any)=>Number.isFinite(Number(v))?Number(v):undefined;return{temperature:n(c.temperature_2m),humidity:n(c.relative_humidity_2m),dewPoint:n(c.dew_point_2m),pressure:n(c.pressure_msl),windSpeed:n(c.wind_speed_10m),windDirection:n(c.wind_direction_10m),windGust:n(c.wind_gusts_10m),visibility:n(c.visibility),cloudCover:n(c.cloud_cover),precipitation:n(c.precipitation)}});localBackgroundCache.set(key,{at:Date.now(),value});if(localBackgroundCache.size>24){const oldest=[...localBackgroundCache.entries()].sort((a,b)=>a[1].at-b[1].at).slice(0,localBackgroundCache.size-24);for(const [cacheKey] of oldest)localBackgroundCache.delete(cacheKey)}return value}catch(error){if(cached&&Date.now()-cached.at<=20*60000)return cached.value;throw error}}
...
function directStationField(candidates:Station[],obsKey:keyof Station,targetElevation:number|undefined,targetUrban:UrbanClass,absoluteLimit:number,min:number,max:number):ResidualResult{const raw=candidates.map(station=>({station,value:stationFieldValue(station,obsKey),weight:analysisWeight(station,targetElevation,targetUrban,obsKey as StationAnalysisField)})).filter(row=>Number.isFinite(row.value)&&row.value>=min&&row.value<=max&&row.weight>.0001),accepted=new Set<Station>(),weights=new Map<Station,number>();if(!raw.length)return{accepted,weights};const med=median(raw.map(row=>row.value)),mad=median(raw.map(row=>Math.abs(row.value-med))),limit=Math.max(absoluteLimit,Number.isFinite(mad)?mad*3.7:0),filtered=raw.filter(row=>Math.abs(row.value-med)<=limit),use=filtered.length?filtered:raw.sort((a,b)=>b.weight-a.weight).slice(0,1),sum=use.reduce((total,row)=>total+row.weight,0);if(!sum)return{accepted,weights};for(const row of use){accepted.add(row.station);weights.set(row.station,row.weight)}const value=use.reduce((total,row)=>total+row.value*row.weight,0)/sum,variance=use.reduce((total,row)=>total+(row.value-value)**2*row.weight,0)/sum;return{value,uncertainty:Math.sqrt(Math.max(0,variance)),accepted,weights}}
type CloudReconciliation={value?:number;method?:string;clearReports:number;cloudyReports:number};
function explicitSkyWeight(station:Station){const distanceKm=Math.max(.1,Number(station.distance??100000)/1000),age=stationAgeMinutes(station.timestamp),kind=station.cloudObservation==='cavok'?1.35:station.cloudObservation==='clear'?1.2:1;return networkQuality(station)*kind*Math.exp(-Math.pow(distanceKm/46,1.2))*Math.exp(-age/105)}
function reconcileHyperlocalCloudCover(candidates:Station[],residualValue:number|undefined,directValue:number|undefined,targetValue:number|undefined):CloudReconciliation{
 const finite=[residualValue,directValue,targetValue].find(value=>Number.isFinite(value)),baseline=finite===undefined?undefined:Number(finite),authoritative=candidates.filter(station=>station.cloudObservation&&stationAgeMinutes(station.timestamp)<=100&&Number(station.distance??999999)<=65000&&Number.isFinite(station.cloudCover)&&(station.networkClass==='official'||station.networkClass==='professional'||/metar|aviationweather|dwd/i.test(String(station.provider||''))));
 const clear=authoritative.filter(station=>(station.cloudObservation==='cavok'||station.cloudObservation==='clear')&&(!Number.isFinite(station.visibility)||Number(station.visibility)>=8000)&&(!Number.isFinite(station.precipitation)||Number(station.precipitation)<=.2)),cloudy=authoritative.filter(station=>station.cloudObservation==='layers'&&Number(station.cloudCover)>=60),sum=(rows:Station[])=>rows.reduce((total,station)=>total+explicitSkyWeight(station),0),clearWeight=sum(clear),cloudyWeight=sum(cloudy),nearest=(rows:Station[])=>rows.length?Math.min(...rows.map(station=>Number(station.distance??999999))):Infinity,nearestClear=nearest(clear),nearestCloudy=nearest(cloudy);
...
async function hyperlocalAnalysis(input:Station[],lat:number,lon:number,elevation:number|undefined,context:Location|undefined,signal?:AbortSignal,fast=false):Promise<Station|null>{
 ...
 const target=backgrounds[0],urban=contextUrbanClass(context),authoritative=(station:Station)=>fieldWeightPolicy(station.provider,station.networkClass,'visibility').sensitiveAllowed,temp=residualField(ranked,backgrounds,target,'temperature','temperature',elevation,urban,2.6,-65,65),humidity=residualField(ranked,backgrounds,target,'humidity','humidity',elevation,urban,15,0,100),dew=residualField(ranked,backgrounds,target,'dewPoint','dewPoint',elevation,urban,3.8,-80,45),pressure=residualField(ranked,backgrounds,target,'pressure','pressure',elevation,urban,5.5,870,1085,s=>s.pressureReference==='QFF'||s.pressureReference==='MSL'),windSpeed=residualField(ranked,backgrounds,target,'windSpeed','windSpeed',elevation,urban,9,0,120),windDirection=residualCircularField(ranked,backgrounds,target,elevation,urban),gust=residualField(ranked,backgrounds,target,'windGust','windGust',elevation,urban,14,0,180),visibility=residualField(ranked,backgrounds,target,'visibility','visibility',elevation,urban,20000,50,100000,authoritative),cloudCover=residualField(ranked,backgrounds,target,'cloudCover','cloudCover',elevation,urban,38,0,100,authoritative),ceiling=directStationField(ranked.filter(authoritative),'ceilingHft',elevation,urban,60,0,500),cloudBase=directStationField(ranked.filter(authoritative),'cloudBaseHft',elevation,urban,60,0,500),precipitation=residualField(ranked,backgrounds,target,'precipitation','precipitation',elevation,urban,3.5,0,60,authoritative);
 if(temp.value===undefined)return fallback;
 const analyses=[temp,humidity,dew,pressure,windSpeed,windDirection,gust,visibility,cloudCover,ceiling,cloudBase,precipitation],acceptedSet=new Set<Station>(),combinedWeights=new Map<Station,number>();
...
async function metarStations(lat:number,lon:number,radiusKm=140,signal?:AbortSignal,fast=false):Promise<Station[]>{
 ...
 if(workerBaseCandidates('metar').length){try{const data=await fetchWorkerJson<any>('',{lat,lon,radius_km:radiusKm,fast:fast?1:0},{purpose:'metar',signal,timeoutMs:fast?6000:10000,maxAgeMs:fast?180000:300000,staleIfErrorMs:fast?600000:1200000,cacheKey:`stations:${lat.toFixed(3)}:${lon.toFixed(3)}:${radiusKm}:${fast?1:0}`}),stations=parseMetarStations(parseMetarRows(data),lat,lon);if(stations.length)return stations}catch{}}
...
function mergeDwdOperaNowcast(dwd:RadarNowcast,opera:RadarNowcast):RadarNowcast{
 ...
 return{...dwd,quality,radarProbability,peakRate:Math.max(Number(dwd.peakRate)||0,Number(opera.peakRate)||0),motionDirectionDeg:Number.isFinite(Number(dwd.motionDirectionDeg))?dwd.motionDirectionDeg:opera.motionDirectionDeg,motionSpeedKmh:Number.isFinite(Number(dwd.motionSpeedKmh))?dwd.motionSpeedKmh:opera.motionSpeedKmh,motionConfidence:dwd.motionConfidence??opera.motionConfidence,motionSource:dwd.motionSource??opera.motionSource,motionAnchors:dwd.motionAnchors?.length?dwd.motionAnchors:opera.motionAnchors,nowcastSeries:dwd.nowcastSeries?.length?dwd.nowcastSeries:opera.nowcastSeries,provider:`${dwd.provider} · OPERA CIRRUS-Abgleich`,license:[dwd.license,'EUMETNET OPERA composite products, CC BY 4.0'].filter(Boolean).join(' · '),diagnostics:{...(dwd.diagnostics||{}),operaCrossCheck:{available:true,provider:opera.provider,probability:operaProbability,currentRate:opera.currentRate,peakRate:opera.peakRate,observedAt:opera.observedAt,quality:opera.quality,summary:opera.summary,weight:operaWeight,discrepancy}}};
}
...
export function mapDays(w:Weather):Day[]{return (w.daily.time as string[]).map((date,i)=>{const windPair=validateWindPair(n(w.daily.wind_speed_10m_max[i],0),n(w.daily.wind_gusts_10m_max[i],0)),leadHours=(localIsoEpoch(`${date}T12:00`,w.timezone,Number(w.utc_offset_seconds)||0)-Date.now())/3600000,precipitationSignal=reconcileForecastPrecipitation({precipitation:n(w.daily.precipitation_sum[i],0),probability:n(w.daily.precipitation_probability_max[i],0),code:n(w.daily.weather_code[i],3),leadHours});return({date,code:precipitationSignal.code,max:n(w.daily.temperature_2m_max[i]),min:n(w.daily.temperature_2m_min[i]),sunrise:String(w.daily.sunrise?.[i]??'')||undefined,sunset:String(w.daily.sunset?.[i]??'')||undefined,sunshineDuration:n(w.daily.sunshine_duration?.[i],0),precipitation:precipitationSignal.precipitation,probability:precipitationSignal.probability,wind:windPair.wind,gust:windPair.gust,gustAdjusted:windPair.adjusted,direction:n(w.daily.wind_direction_10m_dominant[i],0),uvMax:altitudeCorrectedUvi(n(w.daily.uv_index_max[i],0),w.elevation),weatherSourceId:'best_match',weatherSourceLabel:'Open-Meteo Best Match'})}).filter(d=>Number.isFinite(d.max)&&Number.isFinite(d.min)&&d.max>=d.min)}
...
function pseudoModelFromMeanSpread(w:Weather,definition:EnsembleMeanModel):ModelResult|null{
 ...
 for(let member=0;member<z.length;member++){
  const daily=new Map<string,{t:number[];p:number[]}>();
...
async function meanFallback(lat:number,lon:number,signal?:AbortSignal){
 ...
 const days=aggregateMembers(results),scenarios=buildEnsembleScenarios(results,days),activeModels=results.map(result=>result.model),runs=await modelRunMetas(activeModels.map(model=>({id:model.metaId,label:model.label,kind:'ensemble' as const})),signal);
...
export async function ensembles(lat:number,lon:number,signal?:AbortSignal){
 ...
 const activeModels=results.map(x=>x.model),days=aggregateMembers(results),scenarios=buildEnsembleScenarios(results,days),runs=await modelRunMetas(activeModels.map(x=>({id:x.metaId,label:x.label,kind:'ensemble' as const})),signal);
 ...
 throw new Error(`Keine ausreichend vollständigen Ensemble-Daten: ${failedCount} von ${selected.length} priorisierten Mitgliedermodell-Abrufen fehlgeschlagen; die offizielle Ensemble-Mittel-/Spread-Reserve lieferte ebenfalls keine auswertbare Tagesreihe.`);
}
