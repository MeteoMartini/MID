...
import {formatDwdWarningDetailWithDirection,formatDwdWarningDirection,formatDwdWarningValue,summarizeDwdWarnings,warningCurrentStartIndex,type DwdWarningKind,type DwdWarningLevel,type DwdWarningSignal} from './dwdWarnings';
...
const ENSEMBLE_FRESH_CACHE_MS=60*60*1000;
...
const ensembleFullPriority=['ecmwf_ifs_europe_ensemble','ecmwf_ifs025_ensemble','ncep_gefs05','ncep_aigefs025','gem_global_ensemble','google_weathernext2_ensemble','bom_access_global_ensemble','icon_seamless_eps','knmi_harmonie_arome_cy43_eps','icon_eu_eps','icon_d2_eps'] as const;
function selectedFullEnsembleModels(lat:number,lon:number){return orderedModels(selectedEnsembleModels(lat,lon),[...ensembleFullPriority])}
...
export type HazardItem={level:HazardLevel;title:string;text:string;metric?:string;displayMetric?:string;displayText?:string;scopeLabel?:string;precisionLabel?:string;validFrom?:string;validTo?:string;kind?:DwdWarningKind;lowerIntensity?:boolean;conditional?:boolean};
...
function hazardWindBand(kmh:number,unit:WindUnit){const converted=unit==='kmh'?kmh:unit==='ms'?kmh/3.6:unit==='mph'?kmh/1.609344:kmh/1.852,step=unit==='kmh'?10:unit==='ms'?2:5,band=hazardBand(converted,step);if(!band)return'';const suffix=unit==='kmh'?'km/h':unit==='ms'?'m/s':unit==='mph'?'mph':'kt',format=(value:number)=>unit==='ms'?formatDecimal(value,0,0):String(Math.round(value));return`${format(band.lower)}–${format(band.upper)} ${suffix}`}
function hazardAmountBand(value:number,kind:DwdWarningKind,windowHours?:number){const number=Number(value);if(!Number.isFinite(number))return'';if(kind==='fog')return'Sicht unter 200 m';if(kind==='heat'){const band=hazardBand(number,2);return band?`ca. ${Math.round(band.lower)}–${Math.round(band.upper)} °C`:''}if(kind==='frost'){const band=hazardBand(number,2);return band?`${Math.round(band.lower)} bis ${Math.round(band.upper)} °C`:''}if(kind==='heavyRain'||kind==='continuousRain'){const step=number<20?2:number<60?5:10,band=hazardBand(number,step);return band?`ca. ${Math.max(0,Math.round(band.lower))}–${Math.max(0,Math.round(band.upper))} mm${windowHours?`/${windowHours} h`:''}`:''}if(kind==='snow'){const step=number<10?2:5,band=hazardBand(number,step);return band?`ca. ${Math.max(0,Math.round(band.lower))}–${Math.max(0,Math.round(band.upper))} cm${windowHours?`/${windowHours} h`:''}`:''}return''}
function hazardPresentation(signal:DwdWarningSignal,hours:Hour[],unit:WindUnit){const conditional=hazardConvectiveContext(signal,hours),direction=formatDwdWarningDirection(signal),directionInline=direction?`${direction.charAt(0).toLocaleLowerCase('de-DE')}${direction.slice(1)}`:'',windBand=signal.kind==='wind'||signal.kind==='snowdrift'?hazardWindBand(Number(signal.value),unit):'',amountBand=hazardAmountBand(Number(signal.value),signal.kind,signal.windowHours),displayMetric=signal.kind==='wind'?windBand:signal.kind==='snowdrift'?[hazardAmountBand(Number(signal.secondaryValue), 'snow',signal.windowHours),windBand&&`${windBand} Böen`].filter(Boolean).join(' · '):amountBand,scopeLabel=conditional?'örtlich':signal.kind==='fog'||signal.kind==='ice'?'kleinräumig':signal.kind==='heavyRain'?'örtlich bis gebietsweise':signal.kind==='continuousRain'||signal.kind==='snow'||signal.kind==='snowdrift'?'gebietsweise':signal.kind==='heat'||signal.kind==='frost'?'regional':'lokal',precisionLabel=conditional?'Ortstreffer unsicher':signal.kind==='fog'||signal.kind==='ice'?'kleinräumig variabel':'Prognosebereich';let displayText='';if(signal.kind==='wind')displayText=conditional?`In Schauernähe sind örtlich Böen im Bereich ${windBand||formatDwdWarningValue(signal,unit)} möglich${directionInline?` ${directionInline}`:''}. Ob die stärksten Böen den Standort treffen, bleibt räumlich unsicher.`:`Für den Standort wird ein Böenbereich von ${windBand||formatDwdWarningValue(signal,unit)} erwartet${directionInline?` ${directionInline}`:''}. Einzelspitzen können lokal darüber oder darunter liegen.`;else if(signal.kind==='thunderstorm')displayText='Gewitter sind kleinräumig und treffen nicht jeden Ort. Der Hinweis beschreibt das mögliche Auftreten im Prognosefenster, nicht einen sicheren Zelltreffer.';else if(signal.kind==='heavyRain')displayText=`${conditional?'Örtlich':'Gebietsweise'} ${displayMetric||formatDwdWarningValue(signal,unit)} möglich. Niederschlagskerne können kleinräumig deutlich abweichen; die Summe ist deshalb als Bereich dargestellt.`;else if(signal.kind==='continuousRain')displayText=`Gebietsweise ${displayMetric||formatDwdWarningValue(signal,unit)} möglich. Lokale Summen können wegen Zugbahn und Staueffekten abweichen.`;else if(signal.kind==='snow')displayText=`Gebietsweise ${displayMetric||formatDwdWarningValue(signal,unit)} Neuschnee möglich. Höhe, Untergrund und kleinräumige Temperaturunterschiede können die tatsächliche Schneemenge verändern.`;else if(signal.kind==='snowdrift')displayText=`Schneeverwehungen sind gebietsweise möglich. Neuschnee und Böen werden bewusst als Bereiche statt als punktgenaue Spitzenwerte dargestellt.`;else if(signal.kind==='fog')displayText='Örtlich kann die Sicht unter etwa 200 m sinken. Nebelgrenzen und Sichtweiten können sich auf kurzer Distanz stark unterscheiden.';else if(signal.kind==='ice')displayText='Glätte beziehungsweise Glatteis kann punktuell auftreten. Untergrund, Niederschlagsphase und Temperatur können sich auf kurzer Distanz unterscheiden.';else if(signal.kind==='heat')displayText=`Regional ist eine Wärmebelastung im Bereich ${displayMetric||formatDwdWarningValue(signal,unit)} möglich. Exposition, Bebauung und nächtliche Abkühlung beeinflussen die tatsächliche Belastung.`;else if(signal.kind==='frost')displayText=`Regional sind Temperaturen im Bereich ${displayMetric||formatDwdWarningValue(signal,unit)} möglich. Bodennahe Werte und geschützte Lagen können stärker abweichen.`;else displayText=signal.detail;return{displayMetric,displayText,scopeLabel,precisionLabel,conditional}}
...
export function hazards(h:Hour[],_currentUv?:number,elevation=0,unit:WindUnit='kn'){
 ...
 return summarizeDwdWarnings(horizon,elevation,24).map(signal=>{const presentation=hazardPresentation(signal,horizon,unit);return{level:dwdHazardClass(signal.level),title:signal.title,metric:formatDwdWarningValue(signal,unit),text:`${formatDwdWarningDetailWithDirection(signal,unit)} Automatisch aus dem Open-Meteo-Best-Match abgeleitet; keine amtliche Warnung.`,...presentation,validFrom:signal.validFrom,validTo:signal.validTo,kind:signal.kind,lowerIntensity:signal.lowerIntensity}}).sort((a,b)=>levelOrder[b.level]-levelOrder[a.level]);
}
