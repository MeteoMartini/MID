...
export async function forecast(lat:number,lon:number,signal?:AbortSignal,options:ForecastRequestOptions={}){const cached=readForecastCoreCache(lat,lon),priority=options.priority??'normal';if(!options.forceFresh&&cached&&cached.age<=FORECAST_CORE_FRESH_MS)return cached.value;const useDirectFirst=priority==='foreground';let directError:unknown=null;if(useDirectFirst){try{const value=await directOpenMeteoCoreForecast(lat,lon,signal,priority);writeForecastCoreCache(lat,lon,value);return value}catch(error){if(signal?.aborted)throw error;directError=error}}const proxied=await workerForecastCore(lat,lon,signal,options.forceFresh===true,options.timeZone,options.elevation);if(proxied){writeForecastCoreCache(lat,lon,proxied);return proxied}if(!useDirectFirst){try{const value=await directOpenMeteoCoreForecast(lat,lon,signal,priority);writeForecastCoreCache(lat,lon,value);return value}catch(error){if(signal?.aborted)throw error;directError=error}}if(cached)return cached.value;if(directError)throw directError;throw new Error('Open-Meteo Best Match ist vorübergehend nicht erreichbar.')}
