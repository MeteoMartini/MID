...
function render(map:L.Map,canvas:HTMLCanvasElement,raster:OperaRaster,frameKey:string){
 const size=map.getSize(),memory=Number((navigator as Navigator&{deviceMemory?:number}).deviceMemory)||4,pixelCount=size.x*size.y,coarse=typeof matchMedia==='function'&&matchMedia('(pointer:coarse)').matches,step=coarse||memory<=4||pixelCount>220000?2:1,width=Math.max(1,Math.ceil(size.x/step)),height=Math.max(1,Math.ceil(size.y/step)),ratio=Math.min(1.25,window.devicePixelRatio||1),renderWidth=Math.max(1,Math.round(width*ratio)),renderHeight=Math.max(1,Math.round(height*ratio));
 ...
 const context=canvas.getContext('2d',{alpha:true});if(!context)return;const zoom=map.getZoom(),mapBounds=map.getBounds(),cacheKey=projectedViewportCacheKey({frame:frameKey,zoom,bounds:{south:mapBounds.getSouth(),west:mapBounds.getWest(),north:mapBounds.getNorth(),east:mapBounds.getEast()},width:size.x,height:size.y,renderWidth,renderHeight}),cached=getProjectedCanvas(cacheKey);
 if(cached){context.clearRect(0,0,renderWidth,renderHeight);context.drawImage(cached,0,0);return}
 const image=context.createImageData(renderWidth,renderHeight),pixels=image.data,worldSize=256*2**zoom,pixelOrigin=map.getPixelOrigin(),worldLeft=pixelOrigin.x,worldTop=pixelOrigin.y,screenScale=step/ratio;
 ...
 context.putImageData(image,0,0);setProjectedCanvas(cacheKey,canvas);
}
...
export default function OperaRasterOverlay({frame,opacity,onStatus}:{frame:OperaRasterFrame;opacity:number;onStatus?:(status:Status,message?:string)=>void}){
 ...
 useEffect(()=>{let active=true,raf=0;const pane=map.getPane('overlayPane');if(!pane){onStatus?.('error','Leaflet-Overlayebene nicht verfügbar.');return}
  ...
  let raster:OperaRaster|undefined;const redraw=()=>{if(!active||!raster)return;cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>{try{render(map,canvas,raster!,frame.fileUrl);if(active)onStatus?.('ready')}catch(error){if(active)onStatus?.('error',error instanceof Error?error.message:String(error))}})};
