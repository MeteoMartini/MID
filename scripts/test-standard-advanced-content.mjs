...
for(const token of ["lazy(()=>import('./CrossSectionPanel'))",'title="Cross Section · Streckenbriefing"','<LazyCrossSection/>','title="Meteogramme"',"lazy(()=>import('./MeteogramPanel'))"])if(!flight.includes(token))failures.push(`Flugmeteorologie-Gruppierung fehlt: ${token}`);
if(flight.includes('To be continued')||flight.includes('CrossSectionFuture'))failures.push('Die reaktivierte Cross Section enthält noch den Pausenvertrag.');
...
console.log('Standard-/Erweitert-Inhalte geprüft: Erststartmodule sind eingeklappt, Flugmeteorologie mit aktivem Streckenbriefing und Meteogramm sowie Widget/PNG bleibt dem erweiterten Modus vorbehalten, Quellen öffnen als schließbares Fußzeilen-Popover.');
