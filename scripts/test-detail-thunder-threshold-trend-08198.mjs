...
for(const token of [
 ...
 'signalScore>=DETAIL_THUNDER_RISK_DISPLAY_THRESHOLD'
...
for(const token of [
 ...
 'thunderSignalScore=Math.max(0,...hourlyThunderRisks.map(risk=>risk.signalScore))',
 'stormy=thunderSignalScore>=DETAIL_THUNDER_RISK_DISPLAY_THRESHOLD||thunderDirect',
...
if(compiled.diagnostics?.length)failures.push('detailThunderRisk.ts konnte nicht transpiliert werden.');
 ...
 if(!threshold||threshold.signalScore<30)failures.push(`Ein plausibles Mehrindexsignal ab 30 % wird nicht ausgegeben: ${threshold?.signalScore??'null'}`);
 ...
 if(capeOnly)failures.push(`CAPE ohne Feuchte/Auslösung erzeugt weiterhin ein Signal: ${capeOnly.signalScore} %`);
}
