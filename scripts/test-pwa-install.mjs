...
const failures=[];
for(const token of ["import {PwaInstallButton} from './PwaInstallButton';",'<PwaInstallButton suppressTransientHint={suppressPwaInstallHint}/>'])if(!app.includes(token))failures.push(`App-Einbindung fehlt: ${token}`);
if(!app.includes('suppressPwaInstallHint={MODERN_MAP_MODULES.includes(activeNavSection as DashboardModuleId)}'))failures.push('Kartenmodule unterdrücken den transienten Installationshinweis nicht.');
...
for(const token of ['.header-install-button','.pwa-install-hint','.pwa-install-backdrop','.pwa-install-dialog','@media(max-width:620px)'])if(!styles.includes(token))failures.push(`Responsive Installationsoberfläche fehlt: ${token}`);
if(failures.length){console.error(`PWA-Installationsprüfung fehlgeschlagen:\n- ${failures.join('\n- ')}`);process.exit(1)}
console.log('PWA-Installationsprüfung bestanden: kompakter Kopfzeilenstatus, persistenter Hinweis, direkter Chromium-Dialog, iOS-Anleitung und Standalone-Erkennung sind vorhanden.');
