...
for(const token of ['trigger={chartTooltipTrigger()}','content={<TrendTooltip','content={rainCumulative?<RainTooltip','content={<WindTooltip','function EnsembleTemperatureWeatherOverlay','<EnsemblePrecipShape','<EnsembleHazardShape','precipVisualType','SunshineScaleLegend'])need('Vollständige Ensemble-Funktion',panel,token);
...
for(const token of ['rightAxisWidth:compact?34:44','leftAxisWidth:compact?42:54','professionalEnsembleLayout(compactTrendTooltip,exporting)','professionalEnsembleLayout(compact,exporting)','professionalEnsembleLayout(compactChart,exportingKind===\'precipitation\')','const ENSEMBLE_RAIN_EXPORT_CHART_WIDTH=992;','const ENSEMBLE_TEMP_EXPORT_CHART_WIDTH=992;','const ENSEMBLE_WIND_EXPORT_CHART_WIDTH=992;'])need('Vertikale Tagesausrichtung',panel,token);
