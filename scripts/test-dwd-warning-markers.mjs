...
for(const token of ['EnsembleHazardBadges','EnsembleWeatherStrip','ensemble-day-hazards','ensemble-weather-strip'])if(!ensemble.includes(token)&&!styles.includes(token))failures.push(`Kompakter Ensemble-Hazardmarker fehlt: ${token}`);
