...
for(const token of ["type LayoutMode='standard'|'advanced'","return stored==='advanced'||stored==='full'?'advanced':'standard'",'Standardmodus','Erweiterter Modus','mode-${layoutMode}',"import {AppInfoHint as InfoHint} from './AppInfoPopover';",'function ModeExplanation',"advancedMode={layoutMode==='advanced'}"]){
...
for(const token of ["document.addEventListener('pointerdown',dismiss,true)","document.addEventListener('keydown',escape)",'anchorRef.current?.contains(target)||layerRef.current?.contains(target)','createPortal(<div ref={layerRef}'])if(!portal.includes(token))failures.push(`Gemeinsame App-Portal-/Außenklick-Schließlogik fehlt: ${token}`);
for(const token of ["document.addEventListener('pointerdown',dismiss,true)","document.addEventListener('keydown',escape)",'refs.trigger.current?.contains(target)||refs.layer.current?.contains(target)','createPortal(<div ref={layer}'])if(!ensemble.includes(token))failures.push(`Ensemble-Portal-/Außenklick-Schließlogik fehlt: ${token}`);
