...
assert.ok(radar.includes('name="mid-synoptic-fronts"')&&radar.includes('<MemoCompositeFronts')&&radar.includes('<MemoRegionalFrontalZones')&&radar.includes('<SynopticFrontCanvasFallback'),'Lokale typisierte Fronten und großräumige θe-Frontalzonen müssen zusätzlich unabhängig vom MapLibre-GeoJSON-Layer sichtbar gerendert werden.');
...
assert.ok(radar.includes("isoheights'] as const")&&radar.includes('product.elevation'),'500-hPa-Isohypsen müssen den offiziellen DWD-WMS-Linienpfad analog zu Isobaren nutzen können.');
assert.ok(radar.includes('hasGridIsoheights')&&radar.includes('vectorIsoheightFrame.isoheights')&&radar.includes('IsoheightCanvasFallback'),'Synoptik muss bei WMS-Ausfall geglättete Grid-/Vektor-Isohypsen per Canvas-Fallback rendern.');
