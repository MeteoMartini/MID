...
for(const token of ["label:'Karten'","candidates:['composite']","!['current','warnings','short-term','forecast','ensemble','composite'].includes(id)","openSettings('notifications')","openSettings('favorites')","openSettings('system')"])assert.ok(app.includes(token),`Navigation fehlt: ${token}`);
...
assert.ok(contract.includes('Aktuell · Warnungen · Kurzfrist · 7 Tage · Karten · Mehr'),'Navigationsvertrag ist nicht aktualisiert');
