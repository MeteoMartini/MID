...
for(const token of ["label:'Komposit'","candidates:['composite']","!['short-term','forecast','ensemble','composite'].includes(id)","openSettings('notifications')","openSettings('favorites')","openSettings('system')"])assert.ok(app.includes(token),`Navigation fehlt: ${token}`);
assert.ok(!app.includes("{id:'current',label:'Aktuell'"),'Aktuell darf nicht als Bottom-Bar-Tab zurückkehren');
...
assert.ok(radar.includes('writeCompositeSettings({basemap,viewMode,showRadar'),'Kompositzustand muss gespeichert werden');
...
assert.ok(contract.includes('Kurzfrist · 7 Tage · 14 Tage · Komposit · Mehr'),'Navigationsvertrag ist nicht aktualisiert');
...
console.log('MID 0.9.84.91: Navigation, Kompositpersistenz, sichtbare Isohypsenpfade und native ICON-D2-RUC-Niederschlagsphase für den Radarlayer geschützt.');
