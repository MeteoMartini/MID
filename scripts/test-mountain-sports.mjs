...
for(const token of ["mode==='mountain-profile'","mode==='snow-observations'",'aerialway:station','SCHNEE','overpass-api.de','api.open-meteo.com/v1/elevation','cacheKey:url.toString()','requestedLatitude:lat','sameLift:best.sameLift'])if(!worker.includes(token))failures.push(`Worker fehlt: ${token}`);
const packageJson=JSON.parse(pkg);if(packageJson.version!=='0.7.94')failures.push('Paketversion ist nicht 0.7.94');
