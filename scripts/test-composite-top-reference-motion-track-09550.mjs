...
for(const token of ['referenceUrl','light_only_labels','dark_only_labels','mapOverlayOpacity','Karte oben','zIndex={790}','name="mid-motion-track"','zIndex:850','function PrecipitationMotionTrack','motionTrackPoints','(direction+180)%360','destinationPoint(site,upstreamBearing,distanceKm)','<Marker position={site} icon={motionTrackArrowheadIcon'])assert.ok(radar.includes(token),`Kompositvertrag fehlt: ${token}`);
