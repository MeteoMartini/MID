...
for(const token of ['referenceUrl','light_only_labels','dark_only_labels','mapOverlayOpacity','Karte oben','zIndex={790}','<Polyline pane="overlayPane" positions={[geometry.trackStart,site] as any} interactive={false}','function PrecipitationMotionTrack','viewportDiagonalKm=Math.max(12,segmentKm(southWest,northEast))','(resolved.direction+180)%360','trackStart=destinationPoint(site,upstreamBearing,resolved.speed*shaftLeadMinutes/60)','<Marker pane="mid-motion-labels" position={site} icon={motionTrackArrowheadIcon'])assert.ok(radar.includes(token),`Kompositvertrag fehlt: ${token}`);
