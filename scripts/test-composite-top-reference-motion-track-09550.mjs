...
for(const token of ['referenceUrl','light_only_labels','dark_only_labels','mapOverlayOpacity','Karte oben','zIndex={790}','name="mid-motion-track"','zIndex:850','function PrecipitationMotionTrack','motionTrackPoints','(direction+180)%360','destinationPoint(site,upstreamBearing,distanceKm)','<Marker position={arrowPoint} icon={motionTrackArrowheadIcon'])assert.ok(radar.includes(token),`Kompositvertrag fehlt: ${token}`);
...
console.log('Komposit geprüft: oberste Referenzkarte mit Deckkraft und eine Zugspur mit Pfeilspitze am Ort und upstream-Zeitlabels.');
