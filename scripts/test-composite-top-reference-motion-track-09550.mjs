...
for(const token of ['COMPOSITE_REFERENCE_TILEJSON','VectorReferenceLayer',"sourceLayer:'boundary'","sourceLayer:'place'","positron:{label:'Schlicht hell'","dark:{label:'Schlicht dunkel'",'mapOverlayOpacity','Orte / Grenzen','zIndex={790}','<Marker pane="mid-motion-labels" position={site} icon={icon}','function PrecipitationMotionTrack','trackKm=Math.max(.5,resolved.speed*leadMinutes/60)','(resolved.direction+180)%360','trackStart=destinationPoint(site,upstreamBearing,trackKm)'])assert.ok(contract.includes(token),`Kompositvertrag fehlt: ${token}`);
