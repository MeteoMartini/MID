...
for(const token of ['referenceUrl','light_only_labels','dark_only_labels','mapOverlayOpacity','Karte oben','zIndex={790}','<Marker pane="mid-motion-labels" position={geometry.trackMid} icon={motionTrackGraphicIcon','function PrecipitationMotionTrack','trackKm=Math.max(.5,resolved.speed*leadMinutes/60)','(resolved.direction+180)%360','trackStart=destinationPoint(site,upstreamBearing,trackKm)','<Marker pane="mid-motion-labels" position={geometry.trackMid} icon={motionTrackGraphicIcon'])assert.ok(radar.includes(token),`Kompositvertrag fehlt: ${token}`);
