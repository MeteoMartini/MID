...
for(const token of ['RELATIVE_SUN_RAYS','daylightSecondsForFourteenDay','relativeSunshineShare','RelativeSunshineIcon','intensity=.42+ratio*.58','sun-base sun-ray','sunshineWholeHoursLabel(item.bestSunshineDuration)'])assert.ok(cockpit.includes(token),`Relative 14d-Sonnendarstellung fehlt: ${token}`);
assert.ok(cockpit.includes('P10–P90 ${sunshineHoursLabel(item.sunshineDurationLow)}–${sunshineHoursLabel(item.sunshineDurationHigh)}'),'P10–P90-Text der 14d-Sonnenzeile muss im Inline-Detail erhalten bleiben.');
