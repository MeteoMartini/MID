...
for(const token of ['RELATIVE_SUN_RAYS','daylightSecondsForFourteenDay','relativeSunshineShare','RelativeSunshineIcon','Math.round(ratio*RELATIVE_SUN_RAYS.length)','sunshineShare*100'])assert.ok(cockpit.includes(token),`Relative 14d-Sonnendarstellung fehlt: ${token}`);
...
assert.ok(styles.includes('.cockpit-relative-sun')&&styles.includes('.long-range-subsection'),'Designvertrag für relative Sonne und Untersektionen fehlt.');
