...
const cacheIndex=installBody.indexOf('await cacheShell(CACHE)');
if(cacheIndex<0)failures.push('Ein Update wird nicht vorab vollständig in den Shell-Cache geschrieben.');
if(installBody.includes('prepareUpdate()')||installBody.includes('skipWaiting()'))failures.push('Ein bloß installiertes Update darf weder den aktiven Cache umschalten noch automatisch den laufenden Client übernehmen.');
const messageBody=serviceWorker.match(/self\.addEventListener\('message',[\s\S]*?self\.addEventListener\('push'/)?.[0]||'';
if(!messageBody.includes("case'SKIP_WAITING':case'MID_ACTIVATE_UPDATE':await prepareUpdate()")||!messageBody.includes('await self.skipWaiting()'))failures.push('Explizite Update-Aktivierung über MID_ACTIVATE_UPDATE fehlt.');
