...
const failures=[];
for(const token of ["MODEL_CHANGE_SETTINGS_KEY='mid:modelChangeSettings'",'Modelllauf-Änderungsradar','Änderungsradar für die nächsten drei Tage anzeigen','Bei materieller Änderung benachrichtigen','forecastMaterialChange:true','changeRadarEnabled={modelChangeSettings.enabled}'])if(!app.includes(token))failures.push(`App-Einbindung fehlt: ${token}`);
for(const token of ['function ModelRunChangeRadar','advancedMode&&changeRadarEnabled&&<ModelRunChangeRadar','Auswertung ausschließlich für die nächsten drei Tage','model-change-models','updateModelChangeRadar(locationKey,snapshot)'])if(!panel.includes(token))failures.push(`Ensemble-Änderungsradar fehlt: ${token}`);
