...
for(const token of ['#ffc229','#aeb3b9','const weatherStripVisual=','const precipitationKind=','const precipBandWidth=','const precipSunColor=','const sampleIntervalSeconds=','const sunVisualShare='])assert.ok(skybar.includes(token),`Gemeinsame Gesamt-Skybar fehlt: ${token}`);
for(const forbidden of ['Sonnenschein/Klarheit · Stufe','Gesamtbewölkung · Stufe',"layer:'sun'","layer:'cloud'","layer:'precip'",'underlayColor','underlayStrokeWidth'])assert.ok(!skybar.includes(forbidden),`Alter Single-/Drei-Layer-Skybarvertrag muss entfernt sein: ${forbidden}`);
