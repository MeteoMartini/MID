...
for(const token of [
 ...
 'nextValue=next?',
 'leftSource=previousValue===undefined?current:',
 'rightSource=nextValue===undefined?current:',
...
for(const token of ['#ffc229','#aeb3b9','const weatherStripVisuals=','const baseSkyVisual=','const precipitationOverlayVisual=','precipitationPhaseColor(parts.type)','precipitationPhaseColorLabel(parts.type)','precipitationIntensityDescriptor','const sampleIntervalSeconds=','const sunVisualShare=','return [...baseSegments,...precipSegments]'])assert.ok(skybar.includes(token),`Gemeinsame farbreine Gesamt-Skybar fehlt: ${token}`);
