...
for(const token of ['hazardY=Math.max(plotTop+8,cellY-16)','clipPathUnits="userSpaceOnUse"','clipPath={`url(#${clipId})`}','Math.min(.86','ensemble-chart-temperature','ensemble-chart-precipitation','ensemble-chart-wind'])need('Ensemble-Overlay',panel,token);
