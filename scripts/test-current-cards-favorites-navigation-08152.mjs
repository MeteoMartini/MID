...
for(const token of [
  ...
  'sourceChanged=Boolean(loc?.autolocated)!==Boolean(normalized.autolocated)',
  'const reconciled={...loc,autolocated:normalized.autolocated===true}',
  "activeFavoriteId=trackedSelectionActive?'':matchingFavorite(favorites,current)?.id??''",
...
if(app.includes("activeFavoriteId=current?.autolocated?'':"))failures.push('Die Schnellleiste leitet den Aktivstatus weiterhin direkt aus potenziell veralteten Ortsmetadaten statt aus der aktuellen Auswahlquelle ab.');
...
console.log('Aktuelle Daten und Favoriten geprüft: identische Info-Schaltflächen, direkter Menüsprung, wiederhergestellte Nahbereichszuordnung sowie eine explizit von der Ortsnähe getrennte Standort-Aktivmarkierung.');
