...
for(const token of ['function EnsembleTemperatureCanvas','function EnsembleTemperatureWeatherOverlay','tickStep=data.length>1?plotWidth/Math.max(1,data.length-1):plotWidth','layer="weather"','layer="hazards"'])need('Wetterband',panel,token);
...
forbid('Resize-Performance',frame,'entry.contentRect.height');
...
console.log('Achsengebundenes Ensemble-Wetterband und scrollschonender Recharts-3-Liveframe geprüft.');
