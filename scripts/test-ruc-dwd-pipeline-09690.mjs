...
must(fetcher.includes('LEAD_RE=')&&fetcher.includes('select_requested_files')&&fetcher.includes('lead%15==0')&&fetcher.includes('hourly_only')&&fetcher.includes('lead%60==0'),'RUC downloader must keep deterministic 15-minute leads for 0–6 h, then hourly to +14 h, while RUC-EPS remains hourly');
...
must(builder.includes('hybrid_targets')&&builder.includes('range(0,rapid_limit+1,15)')&&builder.includes("'deterministic':det_targets")&&builder.includes("'eps':eps_targets"),'RUC builder must use deterministic 15-minute targets for 0–6 h and hourly EPS targets through +14 h');
