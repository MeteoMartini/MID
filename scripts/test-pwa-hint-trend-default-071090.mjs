...
assert.match(app,/<div className="actions compact-actions">[\s\S]{0,320}<PwaInstallButton suppressTransientHint=\{suppressPwaInstallHint\}\/>[\s\S]{0,260}<button className="settings-button"/,'App-Status muss weiterhin links neben dem Einstellungsbutton stehen; der Sektionen-Button darf davor liegen.');
assert.doesNotMatch(app,/<footer>[\s\S]{0,350}<PwaInstallButton\/>/,'alter breiter Installationsbutton darf nicht im Footer bleiben');
assert.match(app,/DEFAULT_FORECAST_DISPLAY_SETTINGS:ForecastDisplaySettings=\{showSevenDaySummary:true,showDwdPrecipitationTypeRadar:true,confidenceDisplayMode:'signal',skybarDisplayMode:'band'\}/,'7-Tage-Trend, Standard-Konfidenzmodus oder Skybar-Default ist nicht korrekt aktiviert');
