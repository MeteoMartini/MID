...
for(const token of ['cellSlotWidth=plotWidth/dayCount','cellX=plotLeft+slotIndex*cellSlotWidth','cellWidth=Math.max(1,cellSlotWidth)','centerX=cellX+cellWidth/2','radius=slotIndex===0||slotIndex===dayCount-1?3:0','<ResponsiveEnsembleTooltip compact={compact}'])if(!panel.includes(token))fails.push(`Temperatur-Chart fehlt: ${token}`);
