...
assert.ok(app.includes('Sonnenschein · gelb')&&app.includes('Bewölkung · grau')&&app.includes('Niederschlag · nach Phase')&&app.includes('Regen/Sprühregen/Schauer blau, Schnee/Eis hellblau, Misch-/gefrierende Phase violett, Gewitter/Graupel/Hagel purpur')&&app.includes('Gesamtbewölkung und Sonnenscheindauer werden als getrennte meteorologische Größen ausgewertet')&&app.includes('Dieser Fallback ist keine Sonnenscheindauer')&&app.includes('Gelb und Grau werden nie gleichzeitig gezeichnet')&&app.includes('Niederschlag liegt als eigenständige farbreine Lage darüber'),'Hinweis muss den exklusiven Wolken/Sonnen-Grundband- und Niederschlags-Overlay-Vertrag erklären.');
...
assert.ok(detailSkyBar.includes('if(sunshineShare<=.5)return 0;')&&detailSkyBar.includes('if(daylight&&sunshineDirect)')&&detailSkyBar.includes('if(daylight&&!sunshineDirect)')&&detailSkyBar.includes('const openSkyShare=clamp01(1-boundedCloud/100)')&&detailSkyBar.includes("color:'#ffc229'"),'Gelb muss direkte Sonnenscheindauer verwenden und den komplementären Anteil nur als Wolkenlücken-Fallback zulassen.');
assert.ok(detailSkyBar.includes('if(sunshineShare!==null&&Number.isFinite(sunshineShare))return clamp01(sunshineShare);')&&detailSkyBar.includes('if(cloudKnown){')&&detailSkyBar.includes('Sonnenscheindauer nicht verfügbar'),'Skybar muss direkte Sonnenscheindauer physikalisch eigenständig halten und Wolkenlücken-Fallback ausdrücklich kennzeichnen.');
...
assert.ok(cockpit.includes('probabilityHours=displayHours.filter(hour=>hour.time.startsWith(day.date))')&&cockpit.includes('calendarDayHours=probabilityHours')&&cockpit.includes('daySkyBarSource=calendarDayHours.length?calendarDayHours:dayHours')&&cockpit.includes('daySkyBarSegments=cockpitDaySkyBarSegments(daySkyBarSource)')&&cockpit.includes('daySkyBarHourCells=detailSkyBarHourCells(daySkyBarSource.slice(0,24))'),'Die Tageskachel muss für Skybar und Stundenquadrate den vollständigen Kalendertag (24 h) statt nur des Tagesfensters verwenden, während der appweite PoP-Vertrag seinen probabilityHours-Alias behält.');
