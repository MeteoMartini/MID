...
assert.ok(cockpit.includes('probabilityHours=displayHours.filter(hour=>hour.time.startsWith(day.date))')&&cockpit.includes('forecastLocalDaySkyBar(displayHours,day.date,location')&&cockpit.includes('daySkyBarSegments=daySkyBarData?.segments??[]')&&cockpit.includes('daySkyBarHourCells=daySkyBarData?.cells??[]')&&!cockpit.includes('daySkyBarSource.slice(0,24)'),'Die Tageskachel muss für Skybar und Stundenquadrate den vollständigen lokalen Kalendertag einschließlich 23-/25-h-DST-Tagen verwenden, während der PoP-Vertrag unverändert bleibt.');
