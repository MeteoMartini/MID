...
assert.ok(cockpit.includes('probabilityHours=hours.filter(hour=>hour.time.startsWith(day.date))')&&cockpit.includes('calendarDayHours=probabilityHours')&&cockpit.includes('daySkyBarSegments=cockpitDaySkyBarSegments(calendarDayHours.length?calendarDayHours:dayHours)'),'Die Tageskachel-Skybar muss den vollständigen Kalendertag (24 h) statt nur des Tagesfensters verwenden, während der appweite PoP-Vertrag seinen probabilityHours-Alias behält.');
