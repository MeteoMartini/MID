...
assert.match(briefing.runWhere(data,icing),/Raum|Rhein|km|Korridor/);const verticalDetail=briefing.verticalHazardDetail(icing,40);assert.match(verticalDetail,/\d+ ft MSL–\d+ ft MSL.*40-km-Korridor/);assert.match(verticalDetail,/3500 ft MSL–10000 ft MSL/,'Geometrische Profilgrenzen müssen durchgehend als ft MSL beschriftet sein.');
