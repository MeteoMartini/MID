...
assert.match(briefing.runWhere(data,icing),/Raum|Rhein|km|Korridor/);const verticalDetail=briefing.verticalHazardDetail(icing,40);assert.match(verticalDetail,/(?:SFC|\d+ ft AGL|FL\d{3})–(?:\d+ ft AGL|FL\d{3}).*40-km-Korridor/);assert.match(verticalDetail,/3500 ft AGL–FL100/,'Unter FL050 muss das Vertikalbriefing in 100-ft-genauer AGL-Darstellung statt als gerundetes FL040 beschriften.');
