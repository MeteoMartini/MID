...
const failures=[];
for(const token of ['const secureThreshold=72','secureUntilIndex=data.findIndex(row=>row.confidence<secureThreshold)','secureCount=secureUntilIndex===-1?Math.min(data.length,14):secureUntilIndex','<small>Weitgehend gesichert</small>','secureSummary','secureDetail','weitgehend gesichert bis'])if(!panel.includes(token))failures.push(`Prognose-Kompass fehlt: ${token}`);
