...
for(const token of ['function useCompactEnsembleChart()','width={compactTrendTooltip?47:68}','width={compactTrendTooltip?40:58}','width={compactChart?47:68}','width={compactChart?40:58}','width={compact?40:58}'])if(!ensemble.includes(token))failures.push(`Responsive Ensemble-Logik fehlt: ${token}`);
