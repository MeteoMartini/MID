...
for(const token of ['iconSize:[19,24]','iconAnchor:[10,18]','popupAnchor:[0,-18]'])if(!radar.includes(token))failures.push(`Halbierter Standortmarker fehlt: ${token}`);
for(const token of ['function useCompactEnsembleChart()','width={compactTrendTooltip?47:68}','width={compactTrendTooltip?4:58}','width={compactChart?47:68}','width={compactChart?40:58}'])if(!ensemble.includes(token))failures.push(`Responsive Ensemble-Logik fehlt: ${token}`);
