...
const pkg=JSON.parse(pkgText),baseline=JSON.parse(baselineText);if(!/^0\.8\.(?:27\.\d+|28\.0)$/.test(pkg.version))failures.push(`Version: package.json ${pkg.version}`);if(baseline.releaseVersion!==pkg.version)failures.push(`Version: Baseline ${baseline.releaseVersion} statt ${pkg.version}`);
