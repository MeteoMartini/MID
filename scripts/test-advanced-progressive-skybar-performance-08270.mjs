...
for(const token of ['function EnsembleForecastCompass','MID Prognose-Kompass','Wie geht’s weiter?','const d=useMemo<TrendRow[]>','hoursByDate=new Map<string,Hour[]>','requestIdleCallback','setStyle(current=>current.left===next.left'])need('Prognose-Kompass/Performance',ensemble,token);
...
const pkg=JSON.parse(pkgText),baseline=JSON.parse(baselineText);if(!/^0\.8\.(?:27\.\d+|28\.\d+|29\.\d+|3[0-9]\.\d+)$/.test(pkg.version))failures.push(`Version: package.json ${pkg.version}`);if(baseline.releaseVersion!==pkg.version)failures.push(`Version: Baseline ${baseline.releaseVersion} statt ${pkg.version}`);
