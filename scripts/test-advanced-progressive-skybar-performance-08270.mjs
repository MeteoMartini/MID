...
for(const token of ['const weatherStripVisuals=','const baseSkyVisual=','const precipitationOverlayVisual=','precipitationPhaseColor(parts.type)','precipitationPhaseColorLabel(parts.type)','const precipBandWidth=','const sampleIntervalSeconds=','const sunVisualShare=','xPositions?:number[]','export function detailSkyBarSegments(','return [...baseSegments,...precipSegments]'])need('Gemeinsame Skybar-Logik',skybar,token);
...
for(const token of ['function EnsembleForecastCompass','MID Prognose-Kompass','Prognoseentwicklung','const d=useMemo<TrendRow[]>','hoursByDate=new Map<string,Hour[]>','requestIdleCallback','setStyle(current=>current.left===next.left'])need('Prognose-Kompass/Performance',ensemble+await readFile(new URL('../src/ForecastConfidence.tsx',import.meta.url),'utf8'),token);
