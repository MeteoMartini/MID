...
for(const token of ["import SubseasonalTrendPanel from './SubseasonalTrendPanel'","import LongRangeModelComparison from './LongRangeModelComparison'",'<span>TREND 14D+</span>','<SubseasonalTrendPanel location={location} advancedMode={advancedMode}/>','<LongRangeModelComparison models={models}/>'])need('LongRangePanel',panel,token);
...
if(packageVersion!=='0.9.77.0')failures.push(`Package: erwartet 0.9.77.0, gefunden ${packageVersion}`);
...
if(failures.length){console.error('MID v0.9.77.0 Trend-14d+-Vertrag fehlgeschlagen:\n- '+failures.join('\n- '));process.exit(1)}
console.log('MID v0.9.77.0: Trend 14d+ mit EC46/GEFS, Wochenblöcken, Equal-Family-Multi-Modell und saisonalem Direktvergleich geschützt.');
