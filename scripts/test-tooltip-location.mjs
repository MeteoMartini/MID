...
const failures=[];
for(const token of ['compact-trend-tooltip','trend-tooltip-temperature-table','trend-tooltip-portal','createPortal','Klima 1991–2020','Sonnenschein','Konsistenz'])if(!ensemble.includes(token)&&!styles.includes(token))failures.push(`Kompakter Ensemble-Tooltip fehlt: ${token}`);
for(const token of ['detail-chart-popover','detail-chart-popover-portal','detail-tooltip-grid','openDetailTooltip','createPortal','onPointerDown={event=>openDetailTooltip(event,i)}','Tooltip schließen'])if(!app.includes(token)&&!styles.includes(token))failures.push(`Kompakter Detail-Tooltip fehlt: ${token}`);
