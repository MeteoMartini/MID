...
const [app,css,extreme]=await Promise.all([
 ...
 readFile(new URL('../src/midC18WarningTimeline.css',import.meta.url),'utf8'),
 readFile(new URL('../src/ExtremeWeatherOutlookPanel.tsx',import.meta.url),'utf8')
]);

for(const token of ['function WarningEventTab(','officialAlertIsRelevant(','automaticHazardIsRelevant(','warning-event-tab','warning-event-track','official-warning-${a.id}','mid-warning-${id}','warmCompositePanel','warmExtremeWeatherOutlook'])assert.ok(app.includes(token),`Warnereignis-Vertrag fehlt: ${token}`);
