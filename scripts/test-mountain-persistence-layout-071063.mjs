...
assert.match(app,/localStorage\.setItem\(mountainConfigKey\(item\.location\),JSON\.stringify\(item\.mountain\)\)/,'Bergprofile werden nicht separat je Favorit gespeichert');
assert.match(app,/queueMicrotask\(\(\)=>void persistStateNow\(\)\)/,'dauerhafte IndexedDB-/Cache-Sicherung nach Bergprofiländerung fehlt');
