...
for(const token of ['const weatherStripVisual=','const precipitationKind=','const precipBandWidth=','const precipSunColor=','const sampleIntervalSeconds=','const sunVisualShare=','xPositions?:number[]','export function detailSkyBarSegments(','#ffc229','#aeb3b9','var(--param-precipitation)'])if(!skybar.includes(token))failures.push(`Gemeinsamer Sonne-/Bewölkungsbalken fehlt: ${token}`);
for(const token of ["layer:'sun'","layer:'cloud'","layer:'precip'",'function sunSample(','function cloudSample(','function precipitationSample(','DetailSkyBarPoint'])if(skybar.includes(token))failures.push(`Veraltete Drei-Layer-Skybar ist noch aktiv: ${token}`);
