...
for(const token of ["import {detailSkyBarHourCells,detailSkyBarSegments} from './detailSkyBar';",'detailSkyBarSegments(p,left,right,W,skyBarY)','data-mid-skybar="react"','data-mid-sky-legend="react"', "skybarDisplayMode==='squares'"])if(!app.includes(token))failures.push(`Deklarative stündliche Wetterdarstellung fehlt: ${token}`);
for(const token of ['const weatherStripVisuals=','const baseSkyVisual=','const precipitationOverlayVisual=','precipitationPhaseColor(parts.type)','precipitationIntensityDescriptor','const sampleIntervalSeconds=','const sunVisualShare=','xPositions?:number[]','export function detailSkyBarSegments(','#ffc229','#aeb3b9','return [...baseSegments,...precipSegments]'])if(!skybar.includes(token))failures.push(`Gemeinsamer farbreiner Wetterstreifen fehlt: ${token}`);
if(skybar.includes('const precipSunColor=')||skybar.includes('color-mix(in srgb'))failures.push('Skybar darf Sonne/Bewölkung und Niederschlag nicht mehr zu Mischfarben verrechnen.');
for(const token of ["layer:'sun'","layer:'cloud'",'function sunSample(','function cloudSample(','function precipitationSample(','DetailSkyBarPoint'])if(skybar.includes(token))failures.push(`Veraltete Drei-Layer-Skybar ist noch aktiv: ${token}`);
