...
for(const token of ['formatGmtOffset','clockTick','place-meta','Z)`','niceRange(','nicePositiveRange(','temperatureTicks.map','rainTicks.map','windTicks.map','directionIndices.map','data-skybar-y={skyBarY}','nowBadgeY=4','iconY=narrowChart?48:50','skyBarY=narrowChart?70:72','cloudTop=narrowChart?90:94','selectedMarkerTop=iconY-10','className="selected-hour-line"','y1={selectedMarkerTop}'])if(!app.includes(token))failures.push(`Detail-/Ortsdarstellung fehlt: ${token}`);

const laneMatch=app.match(/nowBadgeY=(\d+),iconY=narrowChart\?(\d+):(\d+),skyBarY=narrowChart\?(\d+):(\d+),cloudTop=narrowChart\?(\d+):(\d+)/);
if(!laneMatch)failures.push('Die vertikale Lane-Reihenfolge konnte nicht ausgewertet werden.');
else{
 const [,now,narrowIcon,wideIcon,narrowSky,wideSky,narrowPlot,widePlot]=laneMatch.map(Number);
 if(!(now<narrowIcon&&narrowIcon<narrowSky&&narrowSky<narrowPlot))failures.push('Schmale Ansicht folgt nicht JETZT → Piktogramme → Band → Diagramm.');
 if(!(now<wideIcon&&wideIcon<wideSky&&wideSky<widePlot))failures.push('Breite Ansicht folgt nicht JETZT → Piktogramme → Band → Diagramm.');
}
...
console.log('v0.7.79.2 geprüft: JETZT, Wetterpiktogramme, Sonnenschein-/Bewölkungsband und Diagramm folgen der gewünschten Reihenfolge; die ausgewählte Stundenlinie reicht bis zur Piktogramm-Lane.');
