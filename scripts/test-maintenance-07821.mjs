...
for(const token of ['formatDwdWarningCompactValue(signal,unit)','dailyHazards(d,hours,elevation,unit,1)'])if(!app.includes(token))failures.push(`Kompakte 7-Tage-Warnwerte fehlen: ${token}`);
for(const token of ['<b>MID-Hinweise</b>','ensemble-hazard-tooltip','row.hazards.map'])if(!ensemble.includes(token))failures.push(`Ensemble-Warnhinweise fehlen im Tooltip: ${token}`);
for(const token of ['function EnsembleHazardShape','function EnsembleTemperatureWeatherOverlay','layer="hazards"','DWD_WARNING_COLORS'])if(!ensemble.includes(token)&&!styles.includes(token))failures.push(`Kompakte Ensemble-Warnmarker fehlen: ${token}`);
