...
for(const token of ['Best-Match-Warnhinweise','ensemble-hazard-tooltip','row.hazards.map'])if(!ensemble.includes(token))failures.push(`Ensemble-Warnhinweise fehlen im Tooltip: ${token}`);
for(const token of ['EnsembleHazardShape','hazardPoints','ensemble-hazard-marker'])if(ensemble.includes(token)||styles.includes(token))failures.push(`Veraltete Ensemble-Warnmarker sind noch vorhanden: ${token}`);
