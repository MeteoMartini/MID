...
for(const token of ["if(steeringValid)return{direction:normalizeBearing(steeringDirection)",'CanvasOverlay','mid-motion-time-arrow-shaft','drawLine(start,target','pane="mid-motion-labels" position={site}','name="mid-motion-labels"']) assert.ok(radar.includes(token),`Zeitpfeil-/Pane-Vertrag fehlt: ${token}`);
