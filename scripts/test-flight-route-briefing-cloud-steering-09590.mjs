...
for(const token of ["if(steeringValid)return{direction:normalizeBearing(steeringDirection)",'function motionTrackGraphicIcon(','<Marker pane="mid-motion-labels" position={site} icon={motionTrackGraphicIcon','geometry.ticks.map(tick=><Marker','position={tick.position} icon={motionTimeIcon(','name="mid-motion-labels"']) assert.ok(radar.includes(token),`Zeitpfeil-/Pane-Vertrag fehlt: ${token}`);
