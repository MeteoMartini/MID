...
for(const token of ["if(steeringValid)return{direction:normalizeBearing(steeringDirection)",'<Polyline pane="overlayPane" positions={[geometry.trackStart,site] as any} interactive={false}',"className:'mid-motion-time-arrow-shaft-core'",'positions={tick.labelConnector as any}','pane="mid-motion-labels" position={site}','name="mid-motion-labels"']) assert.ok(radar.includes(token),`Zeitpfeil-/Pane-Vertrag fehlt: ${token}`);
