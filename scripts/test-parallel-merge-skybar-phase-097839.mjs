...
assert.ok(cockpit.includes('calendarDayHours=probabilityHours')&&cockpit.includes('daySkyBarSource=calendarDayHours.length?calendarDayHours:dayHours')&&cockpit.includes('daySkyBarSegments=cockpitDaySkyBarSegments(daySkyBarSource)')&&cockpit.includes('daySkyBarHourCells=detailSkyBarHourCells(daySkyBarSource)')&&!cockpit.includes('daySkyBarSource.slice(0,24)'),'Kalendertags-Skybar bzw. Stundenquadrate müssen alle realen Stunden einschließlich DST behalten.');
...
assert.ok(phase.includes("type==='snow'||type==='snowShowers'||type==='snowGrains'||type==='snowStars'||type==='iceCrystals'||type==='icePellets'||type==='graupelShowers'")&&phase.includes("type==='freezingRain'||type==='freezingDrizzle'||type==='sleet'||type==='sleetShowers'")&&phase.includes("type==='hailShowers'||type==='thunderstorm'||type==='thunderstormHail'"),'Niederschlagsphasen werden nicht vollständig klassifiziert.');
...
assert.ok(app.includes('Niederschlag · nach Phase')&&app.includes('Regen/Sprühregen/Schauer blau, Schnee/Eis hellblau, Misch-/gefrierende Phase violett, Gewitter/Graupel/Hagel purpur'),'Skybar-Legende beschreibt die Phasenfarben nicht.');
