...
const failures=[];
for(const token of ['Keine Warnung','no-hazard'])if(!(app+styles).includes(token))failures.push(`Warnfreier 7-Tage-Hinweis fehlt: ${token}`);
...
for(const token of ['<b>MID-Hinweise</b>','row.hazards.map'])if(!ensemble.includes(token))failures.push(`Ensemble-Warnungen fehlen im Tooltip: ${token}`);
