...
for(const token of ['ICloudBackupSettings','Daten &amp; Synchronisation','Ein Stand auf allen MID-Web-Apps','Synchronisation & Integrationen'])if(!app.includes(token))failures.push(`Einstellungsreferenz fehlt: ${token}`);
if(failures.length){console.error('Einstellungs-/Ensemble-Referenzprüfung fehlgeschlagen:\n- '+failures.join('\n- '));process.exit(1)}
console.log('I.1-I.5-Synchronisations- und Ensemble-Referenzfunktionen geprüft.');
