...
const [weather,app,contract]=await Promise.all([readFile(new URL('../src/weather.ts',import.meta.url),'utf8'),readFile(new URL('../src/App.tsx',import.meta.url),'utf8'),readFile(new URL('../src/sunshineDuration.ts',import.meta.url),'utf8')]);
const failures=[];
...
if(!weather.includes('reconcileSunshineDuration({valueSeconds:row.value,intervalSeconds')||!contract.includes('rawSeconds=boundedSunshineSeconds(input.valueSeconds,intervalSeconds)'))failures.push('Ein einzelnes Sonnenscheinintervall wird nicht über den zentralen Vertrag auf seine physikalisch mögliche Länge begrenzt.');
...
if(!weather.includes('reconcileSunshineDuration({valueSeconds:hourlyValues[best],intervalSeconds:3600')||!contract.includes('Math.min(cap,Math.max(0,number))'))failures.push('Der Stunden-Fallback ist nicht über den zentralen Vertrag auf 60 Minuten begrenzt.');
if(!app.includes('sunshineMinutesLabel')||!contract.includes("return value==='–'?value:`${value} min`"))failures.push('Stundenfenster verwenden nicht durchgehend den zentralen reinen Minutenvertrag.');
