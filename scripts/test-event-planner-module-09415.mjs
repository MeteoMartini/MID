...
for(const token of ["const LazyEventPlanner=lazy(()=>import('./EventPlannerPanel'))","case'event-planner':return <CollapsibleModule","id=\"event-planner\" title=\"Eventplaner\"","{id:'planning',label:'Planen & Profile',modules:['event-planner','travel-planner','mountain','water']}"])if(!app.includes(token))failures.push(`Navigation/Rendering fehlt: ${token}`);
...
console.log('Planer-Integration geprüft: Eventplaner und Reiseplaner bleiben separat schaltbar und sind mit Berg-/Wasserprofilen unter „Planen & Profile“ gebündelt.');
