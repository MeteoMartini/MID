...
requireTokens('Kompositbild',radar,['function PrecipitationMotionTrack({site','upstreamBearing=(resolved.direction+180)%360','viewportDiagonalKm=Math.max(12,segmentKm(southWest,northEast))','showMotionField=showMotion','<MemoPrecipitationMotionTrack site={[lat,lon]}','function motionTrackArrowheadIcon(','mid-motion-track-arrowhead','<Marker pane="mid-motion-labels" position={site} icon={motionTrackArrowheadIcon','Zeitpfeil {Math.round(motionDirection)}°','steeringCloudCenterHpa','mid-motion-labels']);
...
requireTokens('Darstellung',styles,['.settings-toggle-card{','.radar-nowcast-strip{','.radar-nowcast-track{','.radar-nowcast-now{','.radar-nowcast-wet{','.mid-motion-track-arrowhead{','.composite-map-interaction{']);
