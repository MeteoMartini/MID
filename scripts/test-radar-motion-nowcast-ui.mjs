...
requireTokens('Kompositbild',radar,['function resolveEchoApproachTrack(','analysis.motionAnchors??[]','item.cross<=item.width','showMotionField=showMotion','<MemoEchoApproachTrack track={approachTrack}','mid-echo-approach-track','Echo-Zugspur ${Math.round(approachTrack.direction)}°','mid-motion-labels']);
...
requireTokens('Darstellung',styles,['.settings-toggle-card{','.radar-nowcast-strip{','.radar-nowcast-track{','.radar-nowcast-now{','.radar-nowcast-wet{','.mid-echo-approach-track','.composite-map-interaction{']);
...
console.log('Radar-Bewegungsfeld und optionale Nowcast-Leiste geprüft: reale Echoanker, Standort-ETA, Einstellungspersistenz und verfügbare −1/+2-h-Zeitstände sind vorhanden.');
