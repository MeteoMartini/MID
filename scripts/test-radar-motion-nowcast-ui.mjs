...
requireTokens('Kompositbild',radar,['function PrecipitationMotionTrack({site','upstreamBearing=(resolved.direction+180)%360','trackKm=Math.max(.5,resolved.speed*leadMinutes/60)','showMotionField=showMotion','<MemoPrecipitationMotionTrack site={[lat,lon]}','function motionTrackGraphicIcon(','mid-motion-track-graphic','<Marker pane="mid-motion-labels" position={site} icon={motionTrackGraphicIcon','Zeitpfeil {Math.round(motionDirection)}°','steeringCloudCenterHpa','mid-motion-labels']);
