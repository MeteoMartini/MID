...
requireTokens('Kompositbild',radar,['function PrecipitationMotionTrack({site','upstreamBearing=(resolved.direction+180)%360','trackKm=Math.max(.5,resolved.speed*leadMinutes/60)','showMotionField=showMotion','<MemoPrecipitationMotionTrack site={[lat,lon]}','function motionTrackCompositeIcon(','mid-motion-track-composite-svg','<Marker pane="mid-motion-labels" position={site} icon={icon}','Zeitpfeil {Math.round(motionDirection)}°','steeringCloudCenterHpa','mid-motion-labels']);
...
requireTokens('Darstellung',styles,['.settings-toggle-card{','.radar-nowcast-strip{','.radar-nowcast-track{','.radar-nowcast-now{','.radar-nowcast-wet{','.mid-motion-track-composite-svg{','.composite-map-interaction{']);
