...
const failures=[];
for(const token of ["const BACKUP_SCHEMA='mid-icloud-drive-backup'","const BACKUP_VERSION=3",'exportForecastVerificationArchive','importForecastVerificationArchive','crypto.subtle.digest','shareNavigator.share','collectPortableUserData','replacePortableUserData'])if(!backup.includes(token))failures.push(`Backup-Engine fehlt: ${token}`);
