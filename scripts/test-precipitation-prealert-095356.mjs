...
need('Meldungstext',worker,'Dauer ${');
need('Meldungsmenge',worker,'· ca. ${');
if(worker.includes('Radar bestätigt die Annäherung'))fail.push('Worker: redundanter Radar-Hinweis ist noch im Pushtext enthalten.');
if(worker.includes('voraussichtlich etwa'))fail.push('Worker: unnötige Dopplung im Pushtext ist noch enthalten.');
