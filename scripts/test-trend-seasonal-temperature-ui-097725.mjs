...
assert.ok(panel.includes('Poor-Man’s-Ensemble')&&panel.includes('eine Stimme je numerisch verfügbarer Modellfamilie'),'Season muss alle tatsächlich geladenen Modellfamilien gleichgewichtet als Poor-Man’s-Ensemble zusammenführen.');
...
assert.ok(seasonal.includes('freshModels.push(...c3s.models)')&&seasonal.includes('const freshDeduped=[...new Map(freshModels.map(model=>[model.family,model])).values()]'),'Alle verfügbaren numerischen Quellen müssen in den unabhängigen Modellfamilien-Pool einfließen.');
...
assert.ok(cockpit.includes('Kästchenintensität = Abweichung vom jeweiligen Klimamittel.'),'7-Tage-Legende muss die neue Tmin/Tmax-Kästchenlogik erklären.');
