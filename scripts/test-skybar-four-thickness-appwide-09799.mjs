...
assert.ok(app.includes('function currentTemperatureTrendSummary(values:number[])')&&app.includes('const signed=(delta:number)=>{const rounded=Math.round(delta)'),'Temperaturtrend muss Richtungswechsel robust zusammenfassen und sichtbare Kelvinbeträge ganzzahlig runden.');
