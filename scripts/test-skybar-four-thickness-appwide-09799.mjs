...
assert.ok(app.includes('function currentTemperatureDeltaLabel(start:number,end:number)')&&app.includes("currentThreadTargetEpoch=currentThreadStartEpoch+12*3600000"),'Temperaturdifferenz muss ausschließlich volle Stunden bis exakt +12 h vergleichen und ganzzahlig runden.');
...
assert.ok(app.includes('id="current-weather-metrics" className="metrics"')&&app.includes('Weitere aktuelle Werte')&&app.includes('Atmosphäre, Umwelt & Astronomie'),'Aktuell-Mehrbereich braucht eine klare visuelle Informationshierarchie.');
assert.ok(currentMoreTrend.includes('#current-weather-metrics')&&currentMoreTrend.includes('.current-more-head')&&currentMoreTrend.includes('grid-template-columns:repeat(2,minmax(0,1fr))'),'Aktuell-Mehrbereich muss als kompakter responsiver Informationsbereich gestaltet sein.');
assert.ok(app.includes('quickTapStart')&&app.includes('quickTapEnd')&&app.includes('Reihenfolge unter Favoriten verwalten ändern'),'Favoritenleiste muss Touch-Taps zuverlässig behandeln und Sortierung ausschließlich in die Verwaltung verweisen.');
