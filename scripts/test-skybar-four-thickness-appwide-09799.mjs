...
const [skybar,renderer,app,cockpit,styles,contract,pkgRaw,baselineRaw]=await Promise.all([
 read('src/detailSkyBar.ts'),read('src/SkyBarSegments.tsx'),read('src/App.tsx'),read('src/ForecastCockpit.tsx'),read('src/styles-src/30-modern.css'),read('MID_24H_PROFILE_STORY_AXIS_CONTRACT.md'),read('package.json'),read('MID_BASELINE.json')
]);
...
assert.ok(skybar.includes('const precipBandLevel=(amount:number):SkyBarThicknessIndex|null=>{')&&skybar.includes('if(amount<=0.5)return 0;')&&skybar.includes('if(amount<=4)return 1;')&&skybar.includes('if(amount<15)return 2;')&&skybar.includes('return 3;'),'Niederschlag muss die DWD-Intensitätsgrenzen plus MID-Darstellungsstufe ab 15 mm/h besitzen.');
