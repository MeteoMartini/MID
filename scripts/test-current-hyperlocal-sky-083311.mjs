...
const failures=[];
for(const token of ["import {hyperlocalSkyCondition} from './currentConditions';","reconciledCurrentPrecip.type==='none'?hyperlocalSkyCondition({","cloudObserved:fieldFresh('cloudCover')&&Number.isFinite(st?.cloudCover)","if(localSky){currentWeatherCode=localSky.code;currentWeatherLabel=localSky.label}"]){if(!app.includes(token))failures.push(`App-Anbindung fehlt: ${token}`)}
