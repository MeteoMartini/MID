...
assert.ok(forecastRows.includes('<small>Min</small>')&&forecastRows.includes('<small>Max</small>'),'Klassische 7-Tage-Karten dürfen statt ±K nur Min/Max semantisch kennzeichnen.');
...
for(const token of ['.seven-day-curve-temperature-halo','.seven-day-curve-time-label','.seven-day-curve-day-label','min-height:194px']){assert.ok(styleSource.includes(token),`7-Tage-Stylequelle fehlt: ${token}`);assert.ok(styleAggregate.includes(token),`7-Tage-Styleaggregat fehlt: ${token}`)}
...
console.log(`MID v${pkg.version}: 7-Tage-Kurve mit gemeinsamer Stundenachse, stündlichem Niederschlag, ECMWF-Farben und ohne Klimadelta geschützt.`);
