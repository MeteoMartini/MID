...
requireTokens('Stufe 3 · aktiver Wetterzwilling',engine,[
 'applyLocalTwinForecast(','applyLocalTwinHours(','return locallyAdjusted;','settings.nowcastAssimilation&&radar','privateSensorUrl','PRIVATE_SENSOR_INTEGRATION_ENABLED=true'
]);
...
// Eigene Sensoren sind seit der reaktivierten Stations-/Lüftungsintegration wieder zulässig.
// Dieser Stufentest darf deshalb weder eine frühere UI-Deaktivierung noch einen bestimmten
// Ort der Sensoroberfläche erzwingen; die Funktionsverträge werden in den dedizierten
// Stations- und Lüftungsregressionen geprüft.
...
requireTokens('App-Integration',app,['WeatherTwinSettingsPanel','finalizedHours=useMemo(()=>{const core=finalizeForecastHours(','displayHours=finalizedHours.hours','displayDays=useMemo','recordForecastCapture(','recordLiveTwinObservation(','twinActive={twinForecastActive}']);
