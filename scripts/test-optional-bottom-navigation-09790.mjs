...
for(const token of [
 ...
 "label:'Kurzfrist'","label:'7 Tage'","label:'14 Tage'","label:'Komposit'",'<span>Mehr</span>','bottomBarHidden','downDistance>=104','upDistance>=10','is-scroll-hidden','data-scroll-hidden',
 "dashboard-section-nav-list ${variant}${modernDrawer?' progressive':''}",'<details key={group.id}',"!['short-term','forecast','ensemble','composite'].includes(id)"
])assert.ok(app.includes(token),`Kanonischer Bottom-Bar-Vertrag fehlt: ${token}`);
assert.ok(!app.includes("{id:'current',label:'Aktuell'"),'Aktuell darf nicht als Primärtab zurückkehren');
...
for(const token of [
 '/* MID v0.9.84.89 · kanonische iOS-27-inspirierte Floating Bottom Bar.','.navigation-bottom-tabs .dashboard-section-quick.dashboard-bottom-tabs','position:fixed!important','bottom:max(9px,var(--mid-safe-bottom))!important','max-width:620px!important','backdrop-filter:blur(30px) saturate(1.45)','min-height:54px!important','white-space:nowrap!important','word-break:keep-all!important','.dashboard-section-quick.dashboard-bottom-tabs.is-scroll-hidden','transform:translate3d(0,calc(100% - 13px),0)!important','@media(max-width:850px) and (orientation:landscape)','@media(prefers-reduced-motion:reduce)'
...
console.log('Kanonische Bottom-Bar: Kurzfrist/7T/14T/Komposit/Mehr, Legacy-Aufräumung, sanftes Autohide und Radarfarb-Isolation geprüft.');
