...
const testPath='scripts/test-mid-18-2-4-d2-trend-night-098580.mjs';

assert.equal(pkg.version,'0.9.85.80','D.2 bleibt Bestandteil von MID v0.9.85.80.');
...
assert.ok(!app.includes('index===0&&Number.isFinite(temp)?temp:Number(hour.temperature)'),'12-h-Trend darf keinen lokal korrigierten Einzel-Startwert in eine andere Reihe mischen.');
for(const token of [
...
]) assert.ok(app.includes(token),`Robuste Verlaufssummary fehlt: ${token}`);

assert.ok(astronomy.includes('export function solarTimelineWindow'),'Zentrale minutengenaue Solar-Geometrie fehlt.');
assert.ok(app.includes('solarTimelineWindow(currentThreadStartEpoch,currentThreadEndEpoch'),'Aktuelles Wetter muss dieselbe Solar-Geometrie verwenden.');
...
for(const token of [
  ...
  'var(--mg-night,#5b667c)'
]) assert.ok(design.includes(token),`Nachtband-Vertrag der Temperaturkurve fehlt: ${token}`);
...
console.log('MID v0.9.85.80 D.2: konsistente hyperlokale Trendreihe, mehrphasige Verlaufssummary und gemeinsame dezente Nachtgeometrie geschützt.');
