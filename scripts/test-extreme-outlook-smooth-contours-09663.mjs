...
const [canvasSource,overlay,modelledAreas,geojsonSource,panel,pkgRaw,baselineRaw,changelog,implementation,workerSource]=await Promise.all([
 read('src/extremeOutlookAreaCanvas.ts'),read('src/ExtremeOutlookAreaOverlay.tsx'),read('src/extremeOutlookModelledAreas.ts'),read('src/extremeOutlookAreaGeoJson.ts'),read('src/ExtremeWeatherOutlookPanel.tsx'),read('package.json'),read('MID_BASELINE.json'),read('CHANGELOG.md'),read('MID_IMPLEMENTATION_0.9.66.3.md'),read('worker-src/00-core-observations.js')
]);
