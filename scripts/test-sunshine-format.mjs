...
const contract=fs.readFileSync(new URL('../src/sunshineDuration.ts',import.meta.url),'utf8');
const failures=[];
for(const [name,source] of [['Sunshine-Contract',contract],['Ensemble-Tooltip',ensemble]]){
...
for(const token of ['sunshineHoursFormatter.format','sunshineHoursLabel(d.sunshineDuration)'])if(!(contract+app).includes(token))failures.push(`7-Tage-Sonnenscheindauer fehlt: ${token}`);
...
if(!ensemble.includes('P10–P90 ${formatSunshineHours(row.sunshineLowHours)}–${formatSunshineHours(row.sunshineHighHours)} h'))failures.push('Ensemble-Tooltip: P10–P90-Bezeichnung der Sonnenscheindauer fehlt');
