...
assert.ok(deviceSync.includes("pagehide=()=>{const current=readDeviceSyncConfig();if(current.pendingChangedAt)void pushDeviceSync(current,false).catch(()=>undefined)}"),'Unveränderter Geräte-Sync würde beim Verlassen weiterhin unnötige KV-Writes erzeugen.');

// Bestehende Scheduler-Einsparungen bleiben und die neuen Write-Sparmaßnahmen kommen hinzu.
for(const token of ['mid-push-schedule-v1','mid-push-schedule-index-v2','pushEntryCadenceMinutes','pushScheduleItemDue','metadataMissing||changed&&after!==before','pushRegistrationSignature','PUSH_HEARTBEAT_WRITE_MS=30*60*1000',"schema:'mid.kv-operations-audit.v3'",'heartbeatWritesPerDayCeiling:48',"deviceSyncPagehideWrites:'nur bei ausstehender lokaler Änderung (pendingChangedAt) und nur bei geändertem Inhalts-Hash'"])assert.ok(push.includes(token),`KV-Sparvertrag fehlt: ${token}`);
...
assert.ok(push.includes('if(!unchanged){entry.updatedAt=now;await putPushEntry(env,key,entry)}await pushScheduleIndexUpsert(env,key,entry)'),'Unveränderte Push-Registrierungen werden nicht write-frei behandelt oder der Scheduler-Index wird nicht repariert.');
...
const store=new Map(),metadataStore=new Map(),puts=[];
const kv={
 ...
 async put(key,value,options){puts.push({key,value,options});store.set(key,value);if(options?.metadata)metadataStore.set(key,options.metadata)},
 async delete(key){store.delete(key);metadataStore.delete(key)},
 async list(options={}){const prefix=String(options.prefix||''),keys=[...store.keys()].filter(key=>key.startsWith(prefix)).map(name=>({name,metadata:metadataStore.get(name)}));return{keys,list_complete:true}}
};
...
const first=await worker.default.fetch(request(),env),firstJson=await first.json();assert.equal(first.status,200,JSON.stringify(firstJson));assert.equal(puts.length,2,'Erste Registrierung erzeugt genau den Subscription-Write plus einmalig den Scheduler-Index.');
const second=await worker.default.fetch(request(),env),secondJson=await second.json();assert.equal(second.status,200,JSON.stringify(secondJson));assert.equal(secondJson.unchanged,true,'Zweite identische Registrierung wird nicht als unverändert erkannt.');assert.equal(puts.length,2,'Identische Wiederregistrierung erzeugt weiterhin einen unnötigen KV-Write.');
...
assert.equal(auditResponse.status,200,JSON.stringify(audit));assert.equal(audit.schema,'mid.kv-operations-audit.v3');assert.equal(audit.after?.heartbeatWritesPerDayCeiling,48);assert.ok(audit.after?.listOperationsPerDayCeiling<=4);assert.match(audit.after?.subscriptionRegistrationWrites||'',/0 Writes/);
