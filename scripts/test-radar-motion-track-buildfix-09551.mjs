...
for(const active of ['function motionTrackPoints','function PrecipitationMotionTrack','const upstreamBearing=(direction+180)%360','<MemoPrecipitationMotionTrack','<Marker position={site} icon={motionTrackArrowheadIcon']) assert.ok(radar.includes(active),`Aktiver Einzelzugspur-Vertrag fehlt: ${active}`);
