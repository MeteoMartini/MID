...
for(const active of ['function motionTrackPoints','function PrecipitationMotionTrack','const upstreamBearing=(direction+180)%360','<MemoPrecipitationMotionTrack','<Marker position={site} icon={motionTrackArrowheadIcon','const trackCandidates=visible.length>=2?visible:points','trackStart=[trackCandidates.at(-1)!.lat,trackCandidates.at(-1)!.lon] as [number,number]']) assert.ok(radar.includes(active),`Aktiver Einzelzugspur-Vertrag fehlt: ${active}`);
