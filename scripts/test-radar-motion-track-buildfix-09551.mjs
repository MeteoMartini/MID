...
for(const active of ['function PrecipitationMotionTrack','upstreamBearing=(resolved.direction+180)%360','<MemoPrecipitationMotionTrack','<Marker pane="mid-motion-labels" position={geometry.trackMid} icon={motionTrackGraphicIcon','const trackKm=Math.max(.5,resolved.speed*MOTION_AXIS_LEAD_MINUTES/60)','trackStart=destinationPoint(site,upstreamBearing,trackKm)','geometry.ticks.map(tick=>']) assert.ok(radar.includes(active),`Aktiver Einzelzugspur-Vertrag fehlt: ${active}`);
