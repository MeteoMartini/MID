...
for(const active of ['function PrecipitationMotionTrack','upstreamBearing=(resolved.direction+180)%360','<MemoPrecipitationMotionTrack','<Marker pane="mid-motion-labels" position={site} icon={motionTrackGraphicIcon','trackKm=Math.max(.5,resolved.speed*leadMinutes/60)','trackStart=destinationPoint(site,upstreamBearing,trackKm)','geometry.ticks.map(tick=>']) assert.ok(radar.includes(active),`Aktiver Einzelzugspur-Vertrag fehlt: ${active}`);
