...
for(const token of ['drySignal','dryAdjustedHour','reconcileForecastDaysWithHours','dailyDry','mosmixDry'])assert.ok(fusionSource.includes(token),`fehlender Nowcast-Konsistenzvertrag: ${token}`);
...
try{
 ...
 const fusion={active:true,hours:[{time:'2026-08-02T08:00:00.000Z',epoch:baseHour.epoch,temperature:18,dewPoint:12,humidity:70,pressure:1018,precipitation:0,probability:1,wind:4,gust:8}],mosmix:{available:true,applied:true,quality:.95},days:[],sources:[],schema:'mid.forecast-fusion.v1',version:3,generatedAt:new Date(now).toISOString(),summary:'',strategy:''};
 const fusedHours=mod.applyForecastFusionHours([baseHour],[baseDay],[fusedDay],fusion);
 assert.equal(fusedHours[0].precipitation,0,'trockener Tages- und MOSMIX-Konsens muss kleine Best-Match-Menge entfernen');
 assert.ok(fusedHours[0].probability<=4,`trockener Konsens muss kurzfristige Wahrscheinlichkeit deutlich senken, erhalten ${fusedHours[0].probability}`);
 assert.equal(fusedHours[0].code,3,'trockener Konsens muss Regen-Wettercode in passenden Bewölkungscode umwandeln');
