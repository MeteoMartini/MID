...
for(const token of ['seven-day-temperature-gradient','seven-day-curve-rainbar','dailyTemperatureTone(day.min','dailyTemperatureTone(day.max','WeatherPictogram'])assert.ok(cockpit.includes(token),`Kurvenübersicht unvollständig: ${token}`);
