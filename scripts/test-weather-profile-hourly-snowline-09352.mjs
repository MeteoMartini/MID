...
for(const token of ['function shortTermProfileHourlyPoints(hours:Hour[],adjusted:ShortTermForecastPoint[],timezone:string,now=Date.now())','const windowEnd=now+PROFILE_WINDOW_MS','precipitationPresentationHours(hours).filter(hour=>hour.epoch<windowEnd&&hour.epoch+HOUR_MS>now)',"durationMinutes>=55?'1 h':`${durationMinutes} min`",'const chartSourcePoints=profileDisplayPoints.length?profileDisplayPoints:hourlyPoints.slice(0,25)'])need('24-h-Profil',cockpit,token);
