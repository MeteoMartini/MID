...
for(const token of ['function motionTrackGraphicIcon(','<Marker pane="mid-motion-labels" position={site} icon={motionTrackGraphicIcon','tickMinutes=Array.from({length:tickCount},(_,index)=>stepMinutes*(index+1))','geometry.ticks.map(tick=><Marker','position={tick.position} icon={motionTimeIcon(']) assert.ok(radar.includes(token),`Sichtbarer Zeitpfeil-Schaft fehlt: ${token}`);
