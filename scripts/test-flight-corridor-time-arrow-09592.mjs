...
for(const token of ['<Polyline pane="overlayPane" positions={[geometry.trackStart,site] as any} interactive={false}',"className:'mid-motion-time-arrow-shaft-halo'","className:'mid-motion-time-arrow-shaft-core'",'positions={tick.labelConnector as any}','position={site} icon={motionTrackArrowheadIcon']) assert.ok(radar.includes(token),`Sichtbarer Zeitpfeil-Schaft fehlt: ${token}`);
