...
for(const token of ['function motionTrackGraphicIcon(','<Marker pane="mid-motion-labels" position={geometry.trackMid} icon={motionTrackGraphicIcon','const MOTION_AXIS_TICK_MINUTES=[15,30,45,60] as const','geometry.ticks.map(tick=><Marker','position={tick.position} icon={motionTimeIcon(']) assert.ok(radar.includes(token),`Sichtbarer Zeitpfeil-Schaft fehlt: ${token}`);
