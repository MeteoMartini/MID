...
for(const token of ['function motionTrackCompositeIcon(','return <Marker pane="mid-motion-labels" position={site} icon={icon}','<Marker pane="mid-motion-labels" position={site} icon={icon}','tickMinutes=Array.from({length:tickCount},(_,index)=>stepMinutes*(index+1))','geometry.ticks.map((tick,index)=>','label=motionTimeLabel(']) assert.ok(radar.includes(token),`Sichtbarer Zeitpfeil-Schaft fehlt: ${token}`);
