...
try{
 ...
 globalThis.fetch=async(input,init={})=>{const url=new URL(input instanceof Request?input.url:String(input));if(url.origin==='https://api.netatmo.com'&&url.pathname==='/oauth2/token')return new Response(JSON.stringify({access_token:'secret-access-token',refresh_token:'secret-refresh-token',expires_in:10800}),{status:200,headers:{'content-type':'application/json'}});if(url.origin==='https://api.netatmo.com'&&url.pathname==='/api/getstationsdata')return new Response(JSON.stringify({body:{devices:[{_id:'device-1',station_name:'Gartenstation',place:{location:[7.04,50.81],altitude:58},dashboard_data:{Pressure:1017.2,time_utc:Math.floor(Date.now()/1000)},modules:[{_id:'outdoor-1',type:'NAModule1',module_name:'Außen',dashboard_data:{Temperature:23.4,Humidity:61,time_utc:Math.floor(Date.now()/1000)}},{_id:'wind-1',type:'NAModule2',module_name:'Wind',dashboard_data:{WindStrength:18,GustStrength:32,WindAngle:245,time_utc:Math.floor(Date.now()/1000)}},{_id:'rain-1',type:'NAModule3',module_name:'Regen',dashboard_data:{Rain:.6,time_utc:Math.floor(Date.now()/1000)}}]}]}}),{status:200,headers:{'content-type':'application/json'}});throw new Error('Unerwarteter Abruf '+url.origin+url.pathname)};
 response=await worker.fetch(new Request(`https://worker.test/?mode=netatmo-callback&state=${encodeURIComponent(state)}&code=code-1`),env);if(response.status!==200)throw new Error('OAuth-Callback zeigt keine Rücksprungseite.');const callbackHtml=await response.text();if(!callbackHtml.includes('Netatmo wurde mit MID verbunden'))throw new Error('OAuth-Erfolg wird auf der Rücksprungseite nicht bestätigt.');
