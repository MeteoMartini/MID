...
for(const token of ['regularShortTermPoints','cockpit-now90','cockpit-meteogram-pro','WeatherPictogram code={point.code}','cockpit-day-regime','regimeText=forecastRegimeLabel(regime,precipitationForm?.label)','precipitationAmountLabel(day)','cockpit-fourteen-row'])assert.ok(cockpit.includes(token),`Klarheitsbaustein fehlt: ${token}`);
