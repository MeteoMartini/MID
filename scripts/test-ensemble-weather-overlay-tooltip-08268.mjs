...
for(const token of ['function EnsembleTemperatureWeatherOverlay','const centerX=Math.round((plotLeft+(row.x+.5)*dayWidth)*2)/2','cellWidth=Math.max(10,Math.min(dayWidth*.88,dayWidth-2))','cellY=plotBottom-cellHeight','hazardY=Math.max(plotTop+8,cellY-18)','<EnsemblePrecipShape','<EnsembleHazardShape','row.precipVisualLabel','% Best Match'])need('Wetterband/Tooltip',panel,token);
