...
for(const token of ['function EnsembleTemperatureWeatherOverlay','cellX=centerX-dayWidth*.46','cellWidth=dayWidth*.92','cellY=plotBottom-cellHeight','hazardY=Math.max(plotTop+8,cellY-10)','<EnsemblePrecipShape','<EnsembleHazardShape','row.precipVisualLabel','% Best Match'])need('Wetterband/Tooltip',panel,token);
