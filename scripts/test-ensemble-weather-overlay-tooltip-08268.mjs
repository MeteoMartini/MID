...
const failures=[];
for(const token of ['function EnsembleTemperatureWeatherOverlay','cellSlotWidth=plotWidth/dayCount','cellY=Math.max(plotTop+8,plotBottom-cellHeight-7)','hazardY=Math.max(plotTop+8,cellY-16)','function EnsemblePrecipShape','function EnsembleHazardShape'])if(!panel.includes(token))failures.push(`Overlay fehlt: ${token}`);
