...
for(const token of ['trend-tooltip-portal','createPortal','visualViewport','position:fixed!important','max-height:calc(100dvh - 16px)'])if(!ensemble.includes(token)&&!styles.includes(token))failures.push(`Viewportgebundener Trend-Tooltip fehlt: ${token}`);
if(!ensemble.includes('allowEscapeViewBox={{x:true,y:true}}'))failures.push('Portal-Tooltip wird von Recharts weiterhin intern geclippt.');
