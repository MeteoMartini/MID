...
const required=[
 ...
 [worker,"WORKER_VERSION='0.7.94'"],[worker,"mode==='push-config'"],[worker,'async scheduled('],[docs,'https://mid-data-proxy.martinmolkentin.workers.dev'],[docs,'MID_PUSH_SUBSCRIPTIONS'],[docs,'VAPID_PRIVATE_KEY'],[docs,'*/5 * * * *']
];
for(const[source,needle]of required)if(!source.includes(needle))throw new Error(`Fehlender v0.7.94-Baustein: ${needle}`);
