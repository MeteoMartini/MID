...
for(const token of ['className="search-input-shell"','const focusSearchInput=','onPointerDown={()=>setOpen(true)}','lastTrackedPointerSelection=useRef(0)','quickTapStart','quickTapEnd','Reihenfolge unter Favoriten verwalten ändern','className={`favorite-toggle${locationIsFavorite?\' active\':\'\'}`} onClick={()=>toggleFavorite(loc)}'])if(!app.includes(token))failures.push(`Interaktion fehlt: ${token}`);
