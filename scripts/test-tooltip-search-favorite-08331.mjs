...
for(const token of ['className="search-input-shell"','const focusSearchInput=','onPointerDown={()=>setOpen(true)}','lastTrackedPointerSelection=useRef(0)','Math.hypot(event.clientX-tap.x,event.clientY-tap.y)>18','className={`favorite-toggle${locationIsFavorite?\' active\':\'\'}`} onClick={()=>toggleFavorite(loc)}'])if(!app.includes(token))failures.push(`Interaktion fehlt: ${token}`);
