...
for(const token of ['sunshine-tooltip-line single-value','precipitation-tooltip-line single-value','className="tooltip-meta-value"','P10–P90 ${formatSunshineHours(row.sunshineLowHours)}–${formatSunshineHours(row.sunshineHighHours)} h','` · ${row.bestPrecipitationProbabilityCompact}`'])if(!panel.includes(token))failures.push(`Tooltip fehlt: ${token}`);
for(const token of ['className="search-input-shell"','const focusSearchInput=','onPointerDown={()=>setOpen(true)}','lastTrackedPointerSelection=useRef(0)','const distance=Math.hypot(event.clientX-active.startX,event.clientY-active.startY);if(!active.dragging){if(distance<8)return','className={`favorite-toggle${locationIsFavorite?\' active\':\'\'}`} onClick={()=>toggleFavorite(loc)}'])if(!app.includes(token))failures.push(`Interaktion fehlt: ${token}`);
