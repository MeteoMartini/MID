...
for(const token of ['suppressedRef=useRef(false)','tooltip.prepareOpen();desktop.onPointerDown(event)','rainTooltip.prepareOpen();rainDesktop.onPointerDown(event)'])need('Ensemble-Ersttipper',ensemble,token);
if((ensemble.match(/tooltip\.prepareOpen\(\);desktop\.onPointerDown\(event\)/g)||[]).length<2)failures.push('Temperatur- und Winddiagramm besitzen nicht beide den Pointer-down-Vorlauf.');
