...
try{
 ...
 const server=http.createServer((req,res)=>{res.setHeader('content-type','application/json');const url=new URL(req.url,'http://localhost');if(url.searchParams.get('mode')==='ruc-health')res.end(JSON.stringify({configured:true,ready:true,fresh:true,version:'0.9.69.5'}));else res.end(JSON.stringify({ok:true,version:'0.9.69.5'}));});await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));
