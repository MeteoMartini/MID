...
try{
 ...
 const port=server.address().port;result=await run(process.execPath,['tools/cloudflare/check_worker_health.mjs','--url',`http://127.0.0.1:${port}/`,'--expected-version','0.9.69.5','--worker','mid-worker','--version-id',vid,'--require-ruc-ready','--retries','1']);server.close();assert.equal(result.code,0,result.stderr);
 const staleServer=http.createServer((req,res)=>{res.setHeader('content-type','application/json');const url=new URL(req.url,'http://localhost');if(url.searchParams.get('mode')==='ruc-health')res.end(JSON.stringify({configured:true,ready:false,fresh:false,schemaValid:true,backend:'pages',run:'2026-08-29T12:00',generatedAt:'2026-08-29T12:10:00Z',ageHours:6.5,pointCount:542040,timeCount:15,epsMemberCount:20,reason:'RUC-Lauf nicht frisch',version:'0.9.69.5'}));else res.end(JSON.stringify({ok:true,version:'0.9.69.5'}));});await new Promise(resolve=>staleServer.listen(0,'127.0.0.1',resolve));
 const stalePort=staleServer.address().port;result=await run(process.execPath,['tools/cloudflare/check_worker_health.mjs','--url',`http://127.0.0.1:${stalePort}/`,'--expected-version','0.9.69.5','--worker','mid-worker','--version-id',vid,'--require-ruc-ready','--retries','1']);staleServer.close();assert.equal(result.code,0,result.stderr);assert.match(result.stderr,/bootstrap-sicher/);
 const brokenServer=http.createServer((req,res)=>{res.setHeader('content-type','application/json');const url=new URL(req.url,'http://localhost');if(url.searchParams.get('mode')==='ruc-health')res.end(JSON.stringify({configured:true,ready:false,fresh:false,schemaValid:false,backend:'pages',run:'2026-08-29T12:00',pointCount:542040,timeCount:15,epsMemberCount:20,reason:'Schema\/Pages-Profil ungültig',version:'0.9.69.5'}));else res.end(JSON.stringify({ok:true,version:'0.9.69.5'}));});await new Promise(resolve=>brokenServer.listen(0,'127.0.0.1',resolve));
 const brokenPort=brokenServer.address().port;result=await run(process.execPath,['tools/cloudflare/check_worker_health.mjs','--url',`http://127.0.0.1:${brokenPort}/`,'--expected-version','0.9.69.5','--worker','mid-worker','--version-id',vid,'--require-ruc-ready','--retries','1']);brokenServer.close();assert.notEqual(result.code,0,'Ungültiges RUC-Schema darf trotz Stale-Bootstrap nicht akzeptiert werden');
