...
for(const token of ['keep_vars:true','no_bundle:true','compatibility_date','kv_namespaces','r2_buckets','MID_RUC_BINDING_AUTO_APPROVED','Unbekannte/noch nicht sicher abgebildete Worker-Bindings','versions'])assert.ok(prepare.includes(token),`Dynamische Wrangler-Konfiguration fehlt: ${token}`);
...
try{
 ...
 assert.equal(config.keep_vars,true);assert.equal(config.no_bundle,true);assert.equal(config.kv_namespaces[0].binding,'MID_PUSH_SUBSCRIPTIONS');assert.equal(config.r2_buckets[0].binding,'MID_DWD_RUC_DATA');assert.ok(!serialized.includes('must-not-leak'),'Plaintext-/Secretwerte dürfen nie in generierte Dateien gelangen.');
 ...
 const server=http.createServer((req,res)=>{res.setHeader('content-type','application/json');const url=new URL(req.url,'http://localhost');if(url.searchParams.get('mode')==='ruc-health')res.end(JSON.stringify({configured:true,ready:true,fresh:true,version:'0.9.69.3'}));else res.end(JSON.stringify({ok:true,version:'0.9.69.3'}));});await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));
 const port=server.address().port;result=await run(process.execPath,['tools/cloudflare/check_worker_health.mjs','--url',`http://127.0.0.1:${port}/`,'--expected-version','0.9.69.3','--worker','mid-worker','--version-id',vid,'--require-ruc-ready','--retries','1']);server.close();assert.equal(result.code,0,result.stderr);
