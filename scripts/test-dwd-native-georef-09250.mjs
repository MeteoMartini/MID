...
for(const token of ['latitude=Number(location.latitude)','longitude=Number(location.longitude)','coordinateLabel(latitude,longitude)','Standort wird bewusst nicht in das amtliche Originalbild eingezeichnet.','dwd-precip-type-radar__source-image'])need('Location-/Originalprodukt-Durchleitung',radar,token);
...
for(const token of ['DWD_DIRECT_IMAGE','dwdPrecipitationTypeImagePosition','radarCropWindow','RasterPolynomial','DWD_SOURCE_RASTER_GRID','LazyDwdPrecipitationMap'])reject('Keine aktive PNG-/Kartenverortung',radar,token);
...
if(failures.length){console.error('DWD Originalprodukt-/WGS84-Sicherheitsprüfung fehlgeschlagen:\n- '+failures.join('\n- '));process.exit(1)}
console.log('DWD-Originalprodukt geschützt: kein irreführender Marker im amtlichen Bild; Standortkoordinaten bleiben unverändert verfügbar und Legacy-WGS84-Hilfen dormant.');
