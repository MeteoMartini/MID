...
assert.ok(preload.includes('STARTUP_PRELOAD_STATION_TIMEOUT_MS=7500')&&preload.includes('await delay(35);if(signal.aborted)throw signal.reason'),'Stations-Schnellstart muss priorisiert, nur minimal versetzt und hart begrenzt erfolgen.');
assert.ok(preload.includes('STARTUP_PRELOAD_ENSEMBLE_TIMEOUT_MS=12000')&&preload.includes("await delay(constrained?650:440);if(signal.aborted)throw signal.reason;return ensembles(location.latitude,location.longitude,signal,'foreground')"),'Ensemble-Splashstart muss nach den kritischen Istwetterdaten folgen und den begrenzten, abbrechbaren Foreground-/Bootstrap-Pfad verwenden.');
...
assert.ok(preload.includes('Promise.race([forecastRequest.promise,stationRequest.promise,radarRequest.promise,delay(700)]).then(()=>preloadInterfaceChunks(ensemble))'),'Nichtkritische UI-Chunks dürfen den kritischen Forecast-/Stationsstart nicht mehr konkurrierend ausbremsen.');
...
for(const token of ['startupForecastForLocation','startupStationForLocation','startupStationEnrichmentForLocation','startupRadarForLocation','startupEnsembleForLocation'])assert.ok(app.includes(token),`App muss den Splash-Promise teilen statt den Request zu duplizieren: ${token}`);
