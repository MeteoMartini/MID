...
assert.ok(preload.includes('STARTUP_PRELOAD_STATION_TIMEOUT_MS=7500')&&preload.includes('await delay(120);if(signal.aborted)throw signal.reason'),'Stations-Schnellstart muss leicht versetzt und hart begrenzt erfolgen.');
assert.ok(preload.includes('STARTUP_PRELOAD_ENSEMBLE_TIMEOUT_MS=12000')&&preload.includes("await delay(260);if(signal.aborted)throw signal.reason;return ensembles(location.latitude,location.longitude,signal,'foreground')"),'Ensemble-Splashstart darf nur den begrenzten, abbrechbaren Foreground-/Bootstrap-Pfad verwenden.');
