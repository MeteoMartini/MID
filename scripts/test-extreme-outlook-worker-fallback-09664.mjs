...
try{
 ...
 assert.equal(result.scope,'DACH');assert.equal(result.delivery,'browser-direct');assert.equal(result.grid.pointCount,117);assert.equal(result.cells.length,117);assert.equal(result.periods.length,4);assert.equal(result.thresholds.probability.overviewMin,40);assert.equal(result.thresholds.probability.hazardMin,10);assert.equal(result.quality.ensembleMembers,20);assert.ok(result.cells.every(cell=>Object.keys(cell.periods).length===4));assert.ok(result.cells.every(cell=>Object.values(cell.periods).every(period=>Object.keys(period.probabilityFields||{}).length===5&&Object.values(period.probabilityFields||{}).every(levels=>levels.length===4))),'Direktberechnung muss vollständige I1–I4-Felder für alle Gefahren liefern.');
