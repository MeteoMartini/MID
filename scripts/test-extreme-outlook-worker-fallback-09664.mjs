...
assert.equal(pkg.version,'0.9.66.4');
...
for(const token of ["loadDirectDachExtremeOutlook","OUTLOOK_FRESH_MS=20*60*1000","OUTLOOK_STALE_MS=12*60*60*1000","dailyWorkerLimit","rememberWorkerLimit","delivery:'browser-direct'","delivery:'local-cache'","readOutlookCache(OUTLOOK_STALE_MS)","cacheKey:'dach-extreme-outlook:v2'",'kostenfreie Direktabruf von ICON-D2-EPS'])assert.ok(client.includes(token),`Client-Fallbackvertrag fehlt: ${token}`);
...
try{
 ...
 assert.equal(result.scope,'DACH');assert.equal(result.delivery,'browser-direct');assert.equal(result.grid.pointCount,117);assert.equal(result.cells.length,117);assert.equal(result.periods.length,4);assert.equal(result.thresholds.probability.overviewMin,40);assert.equal(result.thresholds.probability.hazardMin,10);assert.equal(result.quality.ensembleMembers,20);assert.ok(result.cells.every(cell=>Object.keys(cell.periods).length===4));
 assert.equal(calls.filter(call=>call.url.hostname==='ensemble-api.open-meteo.com').length,2);assert.equal(calls.filter(call=>call.url.pathname==='/v1/dwd-icon').length,2);assert.equal(calls.filter(call=>call.url.pathname.endsWith('/meta.json')).length,1);assert.equal(calls.filter(call=>call.url.hostname==='ensemble-api.open-meteo.com').reduce((sum,call)=>sum+call.url.searchParams.get('latitude').split(',').length,0),117);
}finally{globalThis.fetch=originalFetch}

assert.ok(changelog.startsWith('## 0.9.66.4'));
...
console.log('MID 0.9.66.4: Worker-Tageslimit, kanonische Browser-Direktberechnung, persistenter Stale-Cache, Abrufbudget und verständlicher Datenweg geprüft.');
