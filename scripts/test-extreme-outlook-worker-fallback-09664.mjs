...
try{
 ...
 assert.equal(result.scope,'Mitteleuropa');assert.equal(result.delivery,'browser-direct');assert.ok(result.grid.pointCount>=140&&result.grid.pointCount<=170);assert.equal(result.cells.length,result.grid.pointCount);assert.equal(result.quality.dataCoveragePct,100);assert.equal(result.periods.length,4);assert.equal(result.thresholds.probability.overviewMin,40);assert.equal(result.thresholds.probability.hazardMin,10);assert.equal(result.quality.ensembleMembers,20);assert.ok(result.cells.every(cell=>Object.keys(cell.periods).length===4));assert.ok(result.cells.every(cell=>Object.values(cell.periods).every(period=>Object.keys(period.probabilityFields||{}).length===5&&Object.values(period.probabilityFields||{}).every(levels=>levels.length===4))),'Direktberechnung muss vollständige I1–I4-Felder für alle Gefahren liefern.');
 const ensembleCalls=calls.filter(call=>call.url.hostname==='ensemble-api.open-meteo.com'),diagnosticCalls=calls.filter(call=>call.url.pathname==='/v1/dwd-icon');assert.equal(ensembleCalls.length,Math.ceil(result.grid.pointCount/32));assert.equal(diagnosticCalls.length,ensembleCalls.length);assert.equal(calls.filter(call=>call.url.pathname.endsWith('/meta.json')).length,1);assert.equal(ensembleCalls.reduce((sum,call)=>sum+call.url.searchParams.get('latitude').split(',').length,0),result.grid.pointCount);
