...
if(!/cellWidth=Math\.max\(10,Math\.min\(cellSlotWidth\*\.54,cellSlotWidth-6\)\)/.test(panel))fail.push('Kompakte Tageskästchen-Geometrie fehlt.');
if(!panel.includes('const centerX=Math.round(((data.length>1?plotLeft+row.x*tickStep:plotLeft+plotWidth/2))*2)/2'))fail.push('Tageskästchen-Zentrierung fehlt.');
