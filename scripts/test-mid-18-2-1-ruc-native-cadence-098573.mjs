...
assert.ok(worker.includes("state=rapid.state15||{}")&&worker.includes("nativeVisibilitySeconds:Number.isFinite(visibility)?900:undefined")&&worker.includes("nativeCeilingSeconds:Number.isFinite(ceiling)?900:undefined"),'Worker must expose only field-specific native 15-minute state provenance.');
...
assert.ok(fusion.includes('nativeVisibilitySeconds?:number')&&fusion.includes('nativeCeilingSeconds?:number')&&fusion.includes('visibilityWeight=')&&fusion.includes('ceilingWeight=')&&fusion.includes('stateVisibility=visibilityWeight?rapidStateBlend')&&fusion.includes('stateCeiling=ceilingWeight?rapidStateBlend')&&fusion.includes('nativeVisibilitySeconds:visibilityWeight?900:row.nativeVisibilitySeconds')&&fusion.includes('nativeCeilingSeconds:ceilingWeight?900:row.nativeCeilingSeconds'),'Forecast finalization must consume only field-specific native 15-minute visibility/ceiling without pretending absent fields exist.');
