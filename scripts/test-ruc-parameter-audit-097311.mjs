...
const [audit,fetcher,builder,pack,worker,fusion,extreme,weather]=await Promise.all([
  read('MID_RUC_PARAMETER_AUDIT_0.9.73.11.md'),read('tools/ruc/fetch_and_build_ruc.py'),read('tools/ruc/build_ruc_bundle.py'),read('tools/ruc/ruc_pack.py'),read('worker-src/00-core-observations.js'),read('src/forecastFusion.ts'),read('worker-src/25-dach-extreme-outlook.js'),read('src/weather-src/00-types-models-search.tsfrag')
]);
...
for(const token of ['rapid-severe-15m.bin','rapid-solar-15m.bin','specialist-hourly.bin','rapid-phase-15m.bin','rapid-reflectivity-15m.bin','mid.dwd.ruc.rapid-extreme.v2'])assert.ok(builder.includes(token),`Builder audit product missing: ${token}`);
...
assert.ok(weather.includes('Zustandskern 1 h bis +14 h')&&weather.includes('Niederschlag 5 min bis +6 h'),'Visible model text must report the parameter-native RUC cadence.');
