...
for(const token of ["RAPID_REQUIRED=('TOT_PREC','CAPE_ML','CIN_ML')","'DBZ_CMAX'","'CAPE_MU'","'UH_MAX'","'ECHOTOPinM'","'HAIL_GSP'","'RAIN_GSP'","'SNOW_GSP'","RAPID_STATE_OPTIONAL_15=('VIS','CEILING')","SPECIALIST_HOURLY_OPTIONAL=('CAPE_MU','CIN_MU','VIS','CEILING','HZEROCL','SNOWLMT','CLCM','CLCH','T_G','H_SNOW')"])assert.ok(fetcher.includes(token),`Fetcher audit integration missing: ${token}`);
assert.ok(!fetcher.match(/RAPID_OPTIONAL_15=.*ASWDIR_S/),'Historical solar diagnostics must not force unused full-grid solar data into the free operational fetch path.');
assert.ok(!fetcher.includes("'LAPSE_RATE','SRH','WSHEAR_U','WSHEAR_V'"),'Ambiguous lvt1 SRH/shear fields must remain deferred.');
for(const token of ['rapid-severe-15m.bin','rapid-solar-15m.bin','specialist-hourly.bin','rapid-phase-15m.bin','rapid-reflectivity-15m.bin','mid.dwd.ruc.rapid-extreme.v4'])assert.ok(builder.includes(token),`Builder audit product missing: ${token}`);
assert.ok(pack.includes("FieldSpec('visibility', 'm', 10.0)"),'Visibility must use a non-clipping 10 m int16 scale.');
for(const token of ['capeMu','uhMax','lpiMax','echoTopM','hailGsp','rucCeilingM','rucFreezingLevelM','rucSnowlineM'])assert.ok(worker.includes(token)||fusion.includes(token),`Worker/client diagnostic missing: ${token}`);
assert.ok(pack.includes('SOLAR_15M_FIELDS')&&builder.includes('rapid-solar-15m.bin'),'Solar wire support may remain available for a future non-Pages backend without forcing current free-profile ingestion.');
