...
const [overlay,canvasSource,panel,styles,pkgRaw,baselineRaw,changelog,implementation,workerSource]=await Promise.all([
 read('src/ExtremeOutlookAreaOverlay.tsx'),read('src/extremeOutlookAreaCanvas.ts'),read('src/ExtremeWeatherOutlookPanel.tsx'),read('src/styles.css'),read('package.json'),read('MID_BASELINE.json'),read('CHANGELOG.md'),read('MID_IMPLEMENTATION_0.9.66.2.md'),read('worker-src/00-core-observations.js')
]);
const pkg=JSON.parse(pkgRaw),baseline=JSON.parse(baselineRaw),test='scripts/test-extreme-outlook-area-rendering-09662.mjs';

assert.equal(pkg.version,'0.9.66.2');
...
for(const token of ['CanvasOverlay','drawExtremeOutlookAreas','extremeProbabilityOpacity(signal.probability)','EXTREME_INTENSITY_COLORS[signal.intensity]','moveend','zoomend','resize','render'])assert.ok(overlay.includes(token),`Canvas-Overlayvertrag fehlt: ${token}`);
for(const token of ['map.project(coordinate)','grid.latStep*.515','grid.lonStep*.515','context.fill()','context.stroke()','devicePixelRatio','Math.min(2'])assert.ok(canvasSource.includes(token),`Canvas-Flächenvertrag fehlt: ${token}`);
...
assert.ok(panel.includes('Flächenfarbe = Intensität · Deckkraft und Prozentzahl = Wahrscheinlichkeit'));
...
const calls={move:0,line:0,fill:0,stroke:0,transform:[]},context={beginPath(){},moveTo(){calls.move++},lineTo(){calls.line++},closePath(){},fill(){calls.fill++},stroke(){calls.stroke++},setTransform(...values){calls.transform=values},clearRect(){},lineJoin:'',lineCap:'',fillStyle:'',strokeStyle:'',lineWidth:0},canvas={width:0,height:0,style:{},getContext:()=>context},map={getCanvas:()=>({clientWidth:320,clientHeight:240}),project:([lon,lat])=>({x:lon*10,y:lat*10})};
Object.defineProperty(globalThis,'devicePixelRatio',{value:3,configurable:true});
canvasModule.drawExtremeOutlookAreas(map,canvas,[{lat:50,lon:8,color:'#269b83',opacity:.62},{lat:48,lon:12,color:'#e7b92f',opacity:.7}],{latStep:.8,lonStep:.75});
assert.equal(canvas.width,640,'Canvas-Pixeldichte muss auf 2× begrenzt sein.');assert.equal(canvas.height,480);assert.deepEqual(calls.transform,[2,0,0,2,0,0]);assert.equal(calls.fill,2,'Jede Gefahrenzelle benötigt eine sichtbare Füllung.');assert.equal(calls.stroke,2,'Jede Gefahrenzelle benötigt eine Kontur.');assert.equal(calls.move,4);assert.equal(calls.line,12);
assert.ok(changelog.startsWith('## 0.9.66.2'));
...
console.log('MID 0.9.66.2: sichtbare, georeferenzierte DACH-Gefahrenflächen und reduzierte regionale Marker geprüft.');
