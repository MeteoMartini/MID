...
for(const token of [
 ...
 'combinedPrecipitationPreview','temperatureAnomaly','confidenceGradientStops','stopColor={color}','TemperatureValueDot',"presentation!=='full'||temperatureOpen","presentation!=='full'||rainOpen","presentation!=='full'||windOpen",'ensemble-temperature-value','<Cell key={`rain-cell-${row.date}`','uncertaintyOpacity(row'
...
assert.ok(cockpit.includes('cockpit-fourteen-compact-meta')&&cockpit.includes('{compactGustLabel(item.bestGust,unit)}'), 'Cockpit-14-Tage-Übersicht nennt Wind und Böen nicht.');
