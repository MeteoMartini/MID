...
assert.ok(fetcher.includes("SPECIALIST_15M_OPTIONAL=('VIS','CEILING','HZEROCL','SNOWLMT')"),'Native specialist 15-minute RUC products must be staged without becoming hard requirements.');
assert.ok(fetcher.includes("SPECIALIST_HOURLY_OPTIONAL=('VIS','CEILING','HZEROCL','SNOWLMT','CLCM','CLCH','T_G','H_SNOW')"),'Specialist hourly fallback products must remain available.');
...
for(const token of ["rapid5_times=rapid_targets(a.run,5,6)","rapid15_times=rapid_targets(a.run,15,6)","rapid-5m.bin","rapid-15m.bin","rapid-extreme.json","rapid-reflectivity-15m.bin","rapid-severe-15m.bin","rapid-solar-15m.bin","rapid-specialist-15m.bin","specialist-hourly.bin","rapid-phase-15m.bin"])assert.ok(builder.includes(token),`Builder rapid contract missing: ${token}`);
for(const token of ['RAPID_5M_FIELDS','RAPID_15M_FIELDS','REFLECTIVITY_15M_FIELDS','SEVERE_15M_FIELDS','SOLAR_15M_FIELDS','SPECIALIST_15M_FIELDS','SPECIALIST_HOURLY_FIELDS','PHASE_15M_FIELDS'])assert.ok(pack.includes(token),`Wire spec missing: ${token}`);
...
assert.ok(cockpit.includes('Niederschlag 5 min bis +6 h')&&cockpit.includes('Konvektion/Reflektivität')&&cockpit.includes('Sicht/Ceiling/Nullgrad-/Schneefallgrenze 15 min bis +6 h'),'Model text must expose parameter-native cadence.');
