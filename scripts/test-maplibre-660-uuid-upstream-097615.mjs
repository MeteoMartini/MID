...
const [pkgRaw,lockRaw,baselineRaw,policy,rules,implementation]=await Promise.all([
 read('package.json'),read('package-lock.json'),read('MID_BASELINE.json'),read('MID_DEPENDENCY_UPGRADE_POLICY.md'),read('MID_BRANCH_RULESET.json'),read('MID_IMPLEMENTATION_0.9.78.28.md')
]);
...
assert.ok(implementation.includes('MapLibre GL JS 6.7.0')&&implementation.includes('uuid@7.0.3')&&implementation.includes('Branch-Schutz'));
console.log(`MID v${pkg.version}: MapLibre 6.7.0, transitive UUID-Upstreamgrenze und Stable-Branch-Schutzvorlage geprüft.`);
