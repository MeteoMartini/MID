...
for(const token of['sunshine_duration','sunshineDurationLow','sunshineDurationHigh','bestSunshineHours','Sonnenscheindauer:</b>','Bandbreite von','sunshine-scale-legend'])if(!ensemble.includes(token)&&!styles.includes(token)&&!read('../src/weather.ts').includes(token))failures.push(`Sonnenscheindauer-Auswertung fehlt: ${token}`);
...
console.log('Interaktionen geprüft: Suche schließt, Tooltips bleiben getrennt von der Legende, beide Tagesachsen sind symmetrisch eingerückt, Best-Match-Sonnenscheindauer und ENS-P10–P90 sind eingebunden, Diagrammnavigation funktioniert und Stationswind wird in kt normalisiert.');
