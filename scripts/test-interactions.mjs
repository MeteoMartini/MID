...
const failures=[];
for(const token of ['function RainTooltip(','content={<RainTooltip advancedMode={advancedMode} compact={compactChart} onActive={rainTooltip.markActive} onDismiss={rainTooltip.dismiss}/>','bestPrecipitation','precipitationProbability'])if(!ensemble.includes(token))failures.push(`Niederschlags-Tooltip fehlt: ${token}`);
