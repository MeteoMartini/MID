...
for(const token of ['trend-tooltip-portal','createPortal','visualViewport','viewportWidth<=560','max-height:calc(100dvh - 16px)'])if(!ensemble.includes(token)&&!styles.includes(token))failures.push(`Viewportgebundener Trend-Tooltip fehlt: ${token}`);
if(!ensemble.includes('allowEscapeViewBox={{x:true,y:true}}'))failures.push('Recharts muss den Portal-Tooltip ohne interne Clip-Grenze auslösen können');
...
for(const token of['sunshine_duration','sunshineDurationLow','sunshineDurationHigh','bestSunshineHours','Sonnenscheindauer</b>','<em>P10–P90</em>','sunshine-scale-legend'])if(!ensemble.includes(token)&&!styles.includes(token)&&!read('../src/weather.ts').includes(token))failures.push(`Sonnenscheindauer-Auswertung fehlt: ${token}`);
