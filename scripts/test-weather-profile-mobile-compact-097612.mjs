...
const implementation=read('MID_IMPLEMENTATION_0.9.76.12.md');

assert.equal(pkg.version,baseline.releaseVersion,'Paket- und Baseline-Version müssen synchron sein.');
assert.ok(changelog.includes('## v0.9.76.12'),'Changelog-Eintrag v0.9.76.12 fehlt.');
...
assert.ok(cockpit.includes('cockpit-weather-profile__window-note'),'Die frühere Zeitpille wurde nicht in eine zurückhaltende Kopfzeile überführt.');
...
assert.ok(cockpit.includes('chartPaddingLeft=compactProfile?68:chartViewportWidth<=860?76:88'),'Linker Diagrammrand wurde nicht kompakter ausgelegt.');
assert.ok(cockpit.includes('chartPaddingRight=compactProfile?34:chartViewportWidth<=860?38:44'),'Rechter Diagrammrand wurde nicht kompakter ausgelegt.');
...
assert.ok(css.includes('.cockpit-weather-profile .cockpit-meteogram-pro__svg .night-band{opacity:.34;pointer-events:none}'),'Nachtstunden sind noch zu dominant markiert.');
assert.ok(css.includes('.cockpit-weather-profile .profile-bottom-date{'),'CSS für die kompaktere untere Tagesmarke fehlt.');
assert.ok(css.includes('.cockpit-weather-profile .cockpit-weather-profile__window-note{'),'CSS für die zurückhaltende Kopfzeilen-Notiz fehlt.');
