...
assert.ok(pictogram.includes("if([96,99].includes(c))return'thunder-solid'")&&pictogram.includes("kind==='thunder-solid'?<><Lightning")&&!pictogram.includes("kind==='thunder-solid'?<><Rain"),'WMO 96/99 müssen neutral Graupel/Hagel zeigen und dürfen keinen Regen erfinden.');
...
assert.ok(radarPhase.includes('explicitGraupel=[87,88]')&&radarPhase.includes('explicitHail=[89,90]')&&radarPhase.includes('explicitGraupelHail=[96,99]')&&radarPhase.includes('explicitRain=[51,53,55,61,63,65,80,81,82,91,92]')&&!radarPhase.includes('explicitThunder=[95,97,98]'),'Radar-/Modell-Phasenoverlay muss Graupel, Hagel und die neutrale WMO-96/99-Familie unterscheiden und 98 nicht als Niederschlagsbeleg behandeln.');
assert.ok(ensemble.includes("if([87,88].includes(code))return'graupel'")&&ensemble.includes("if([89,90].includes(code))return'hail'")&&ensemble.includes("if([96,99].includes(code))return'solid'")&&ensemble.includes("if([93,94].includes(code))return'wintry'"),'Ensemble-Wetterband muss Graupel, Hagel, neutral Graupel/Hagel und phasenoffenen winterlichen Niederschlag unterscheiden.');
assert.ok(ensemble.includes("row.precipVisualType==='none'&&!row.precipVisualThunder")&&ensemble.includes("type:'none'|'rain'|'snow'|'mixed'|'graupel'|'hail'|'solid'|'wintry'"),'WMO 98 muss im Ensemble als Gewitter ohne erfundene Niederschlagsphase sichtbar bleiben.');
...
assert.ok(pictogram.includes("afterThunderShower=[91,92].includes(wmoCode)")&&pictogram.includes(".includes(kind)&&!afterThunderShower"),'WMO 91/92 müssen den Tages-/Nachthimmelskörper unterdrücken.');
...
assert.ok(precipitation.includes("warmPhaseProtected=[76,77,78,79,87,88,89,90,93,94,96,99].includes(code)"),'Feste Sonderphasen sowie Hagel/Graupel und phasenoffene WMO-Codes dürfen nicht durch eine Warmboden-Schneekorrektur in Regen umgedeutet werden.');
assert.ok(phaseColor.includes("type==='snow'||type==='snowShowers'||type==='snowGrains'||type==='snowStars'||type==='iceCrystals'||type==='icePellets'||type==='graupelShowers'")&&phaseColor.includes("type==='hailShowers'||type==='thunderstorm'||type==='thunderstormHail'")&&phaseColor.includes("type==='freezingRain'||type==='freezingDrizzle'||type==='sleet'||type==='sleetShowers'||type==='wintryAfterThunder'"),'Neue festen/gemischten Phasen müssen mit getrennten Schnee/Graupel-, Misch-/Gefrier- und Gewitter/Hagel-Farbfamilien in der appweiten Niederschlagsfarbe enthalten sein.');
...
assert.match(app,/function mountainNewSnowLabel\(value:number\)\{return Number\.isFinite\(value\)\?`\$\{formatDecimalFixed\(Math\.max\(0,value\),1\)\} cm`:'–'\}/,'Neuschneeakkumulation muss Dezimalpräzision behalten und fehlende Werte als Strich darstellen.');
assert.ok(app.includes('<MidMetric label="Neuschnee letzte 24 h" value={mountainNewSnowLabel(level.pastSnow24Cm)}'),'Der rückblickende Neuschnee muss in der aktuellen Höhenleiste über den präzisen Label-Helfer angezeigt werden.');
