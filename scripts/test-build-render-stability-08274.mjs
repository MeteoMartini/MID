...
for(const token of ["minify:'oxc'","cssMinify:'lightningcss'","reportCompressedSize:false",'rolldownOptions','codeSplitting',"name:'ReactVendor'","name:'ChartsVendor'"])if(!vite.includes(token))fail.push(`Vite-Konfiguration fehlt: ${token}`);
for(const token of ["minify:'esbuild'","cssMinify:'esbuild'",'manualChunks','rollupOptions'])if(vite.includes(token))fail.push(`Deprecated Vite-Konfiguration ist aktiv: ${token}`);
if(/name:['"]MapLibre/i.test(vite))fail.push('MapLibre darf nicht in einen erzwungenen Vendor-Chunk verschoben werden.');
