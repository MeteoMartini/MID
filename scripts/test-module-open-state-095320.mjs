...
assert.match(app,/const MODULES_DEFAULT_CLOSED=\['composite','ensemble','long-range','forecast-verification','travel-planner','event-planner','flight-meteorology','weather-maps','widget'\]/,'Große Standardmodule müssen bei der Vertragsmigration konsistent geschlossen initialisiert werden');
...
assert.match(app,/history\.replaceState\(null,'',`\$\{location\.pathname\}\$\{location\.search\}`\)/,'Stale Dashboard-Hash muss bei Vertragsmigration entfernt werden');
for(const id of ['composite','ensemble','long-range','forecast-verification','travel-planner','event-planner','flight-meteorology','weather-maps','widget']) assert.match(app,new RegExp(`id=\\"${id}\\"[\\s\\S]{0,180}defaultOpen=\\{false\\}`),`${id} muss denselben standardmäßig geschlossenen Hauptmodulvertrag verwenden`);
