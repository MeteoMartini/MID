...
assert(/explicitHail=\[89,90,96,99\]\.includes\(code\)/.test(overlay)&&/explicitSnowGrains=\[77\]\.includes\(code\)/.test(overlay)&&/phase:'graupel'/.test(overlay),'Hagel, Schneekörner und konservativ abgeleitete Graupel/Eiskörner müssen aus beobachtungsnahen Signalen ableitbar sein.');
