...
assert(/explicitHail=\[89,90\]\.includes\(code\)/.test(overlay)&&/explicitGraupelHail=\[96,99\]\.includes\(code\)/.test(overlay)&&/explicitIceCrystals=code===76/.test(overlay)&&/explicitSnowGrains=code===77/.test(overlay)&&/explicitSnowStars=code===78/.test(overlay)&&/explicitIcePellets=code===79/.test(overlay)&&/phase:'graupel-hail'/.test(overlay)&&/phase:'graupel'/.test(overlay),'Hagel, neutral Graupel/Hagel, Schneegriesel, Schneesterne, Eisnadeln, Eiskörner und konservativ abgeleiteter Graupel müssen aus beobachtungsnahen Signalen ableitbar sein.');
