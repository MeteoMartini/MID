...
const required={
 ...
 weather:['mapHours(w:Weather)','mapMinutely15(w:Weather)','mapDays(w:Weather)','leadHours:(epoch-Date.now())/3600000',"sunshineDuration,weatherSourceId:'best_match',weatherSourceLabel:'Open-Meteo Best Match'"],
 ...
 app:['finalizeForecastHours(twinHours,baseDisplayDays','observedTemperature:finalizationObservedTemperature','hour-tooltip-source'],
...
try{
 ...
 const shower=reconcileForecastPrecipitation({precipitation:.4,rain:.05,showers:.35,probability:35,code:61,cloud:55,lowCloud:10,cape:300,sunshineDuration:1800,isDay:true,leadHours:96});assert.ok([80,81,82].includes(shower.code));assert.equal(shower.rain,0);assert.ok(shower.showers>0);
