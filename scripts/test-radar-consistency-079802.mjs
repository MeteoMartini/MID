...
for(const token of ["showMotion=showRadar&&Number.isFinite(motionDirection)&&Number.isFinite(motionSpeed)&&motionSpeed>=2",'Zug nach {Math.round(motionDirection)}°','Zugrichtung nach {Math.round(motionDirection)}°'])if(!radar.includes(token))failures.push(`Zugrichtungsbeschriftung: ${token}`);
