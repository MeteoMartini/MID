...
for(const token of ["motionSource:'multi-frame-grid-correlation'","motionDirectionConvention:'towards'",'observedMotionTimes=',"method:'WMS GetMap + GetFeatureInfo je Analyseframe; Zugrichtung aus mehreren großräumigen Radarframes'"])if(!worker.includes(token))failures.push(`Mehrframe-Zugrichtung: ${token}`);
for(const token of ["motionReliable=analysis?.motionConfidence==='high'||analysis?.motionConfidence==='medium'",'Zug nach {Math.round(motionDirection)}°','Zugrichtung nach {Math.round(motionDirection)}°'])if(!radar.includes(token))failures.push(`Zugrichtungsbeschriftung: ${token}`);
