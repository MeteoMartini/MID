...
for(const token of ["motionAvailable=showRadar&&Number.isFinite(motionDirection)&&Number.isFinite(motionSpeed)&&motionSpeed>=2","showMotion=showMotionOverlay&&motionAvailable",'motionLabel=motionAvailable?`Zugbahn ${Math.round(motionDirection)}°','Zugbahn {Math.round(motionDirection)}°'])if(!radar.includes(token))failures.push(`Zugrichtungsbeschriftung: ${token}`);
