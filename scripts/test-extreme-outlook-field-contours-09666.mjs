...
assert.equal(pkg.version,'0.9.66.6');
...
assert.ok(changelog.startsWith('## 0.9.66.6'));
...
for(const token of ["mid:travel-water-climate:1991-2020:v4:",'COASTAL_WATER_MAX_DISTANCE_KM=80','Promise.allSettled(WATER_REFERENCE_YEARS.map(loadYear))'])assert.ok(travel.includes(token),`Angehängter SST-0.9.66.5-Vertrag ging verloren: ${token}`);
...
console.log('MID 0.9.66.6: vollständige I1–I4-Wahrscheinlichkeitsfelder, gradientengeformte DACH-Gefahrengebiete, Entfernung identischer Sechseck-Stützkerne und Erhalt des SST-0.9.66.5-Ausgangsstands geprüft.');
