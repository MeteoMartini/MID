...
for(const token of ["(row.precipVisualType==='none'&&!row.precipVisualThunder))return null","<PrecipitationGlyph type={row.precipVisualType}",'size={row.precipVisualSize}','thunder={row.precipVisualThunder}'])need(token);
