...
assert.ok(radar.includes("hasRenderableIsoheights=Boolean(vectorIsoheightFrame&&hasGridIsoheights)")&&radar.includes("synopticReady=visibleModelLines==='isobars'?(hasRenderableSynopticFrame&&hasRenderableIsobars):visibleModelLines==='isoheights'?Boolean(hasRenderableSynopticFrame&&hasRenderableIsoheights):Boolean((hasRenderableSynopticFrame&&hasRenderableIsobars)&&hasRenderableIsoheights)"), 'Die Bereitschaftslogik für Synoptik/Isohypsen muss den kontrollierten geglätteten Vektor-/Gridpfad getrennt vom Isobaren-WMS bewerten.');
assert.ok(radar.includes("{visibleModelLines!=='off'&&(dominantModelFrame||vectorIsoheightFrame||hasGridCenters)&&<Pane name=\"mid-model-lines\""), 'Das Modelllinien-Pane muss auch bei reinen Grid-Isohypsen gerendert werden.');
