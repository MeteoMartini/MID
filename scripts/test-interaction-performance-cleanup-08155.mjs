...
const expectedDormant=new Set(['RouteWeatherPanel.tsx','CrossSectionPanel.tsx','connectedStation.ts','routeWeather.ts','SynopticPanel.tsx','synoptic.ts']),expectedRadarRuntime=new Set(['DwdPrecipitationMap.tsx','HymecNgOverlay.tsx','HymecNgSource.ts']),unexpectedDormant=sourceNames.filter(name=>!reachable.has(join(srcDir,name))&&!expectedDormant.has(name)),unexpectedActive=[...expectedDormant].filter(name=>reachable.has(join(srcDir,name))),missingRadarRuntime=[...expectedRadarRuntime].filter(name=>!reachable.has(join(srcDir,name)));
...
if(missingRadarRuntime.length)failures.push(`Aktive georeferenzierte DWD-Niederschlagskarte verliert benötigte Laufzeitmodule: ${missingRadarRuntime.join(', ')}`);

if(failures.length){console.error('Interaktions-/Performancebereinigung fehlgeschlagen:\n- '+failures.join('\n- '));process.exit(1)}
console.log('Interaktionsperformance geprüft: Randwischen blockiert normales Scrollen nicht mehr, Favoritenpositionierung und Drag-Reorder sind rAF-gedrosselt, Ansichtswechsel vermeiden Voll-Layoutmessungen; deaktivierte Altmodule bleiben dormant und die neue DWD-Niederschlagskarten-Pipeline ist bewusst im aktiven Bundlepfad.');
