...
const expectedDormant=new Set(['RouteWeatherPanel.tsx','CrossSectionPanel.tsx','connectedStation.ts','routeWeather.ts','SynopticPanel.tsx','synoptic.ts']),unexpectedDormant=sourceNames.filter(name=>!reachable.has(join(srcDir,name))&&!expectedDormant.has(name)),unexpectedActive=[...expectedDormant].filter(name=>reachable.has(join(srcDir,name)));
