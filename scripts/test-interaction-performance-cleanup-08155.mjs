...
const expectedDormant=new Set(['RouteWeatherPanel.tsx','routeWeather.ts','SynopticPanel.tsx','synoptic.ts','DwdPrecipitationMap.tsx','HymecNgOverlay.tsx','HymecNgSource.ts','extremeOutlookAreaGeoJson.ts']),unexpectedDormant=sourceNames.filter(name=>!reachable.has(join(srcDir,name))&&!expectedDormant.has(name)),unexpectedActive=[...expectedDormant].filter(name=>reachable.has(join(srcDir,name)));
