...
const expectedDormant=new Set(['RouteWeatherPanel.tsx','CrossSectionPanel.tsx','connectedStation.ts','routeWeather.ts']),unexpectedDormant=sourceNames.filter(name=>!reachable.has(join(srcDir,name))&&!expectedDormant.has(name)),unexpectedActive=[...expectedDormant].filter(name=>reachable.has(join(srcDir,name)));
...
console.log('Interaktionsperformance geprüft: Randwischen blockiert normales Scrollen nicht mehr, Favoritenpositionierung und Drag-Reorder sind rAF-gedrosselt, Ansichtswechsel vermeiden Voll-Layoutmessungen und deaktivierte Stations- und Cross-Section-Logik bleibt aus dem aktiven Bundlepfad.');
