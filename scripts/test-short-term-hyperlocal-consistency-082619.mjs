...
const [shortTerm,anchorCore,presentation,app,pkg,baseline]=await Promise.all([
...
const bridge=presentation.slice(0,presentation.indexOf('\n\nfunction dateKey'));
const failures=[];
...
try{
 ...
 let core=anchorCore.replace(/^import .*$/gm,'').replace('export function forecastLocalAnchorFromCurrent','function forecastLocalAnchorFromCurrent').replace(/\bfinite\b/g,'anchorFinite').replace(/\bobservedSkyCode\b/g,'anchorObservedSkyCode');
 source=`const React={createElement:(...args)=>({args})};\nconst useMemo=(factory)=>factory();\nconst useState=(value)=>[value,()=>{}];\nconst CloudFog=()=>null; const CloudLightning=()=>null; const Droplets=()=>null; const Gauge=()=>null; const Navigation=()=>null; const Thermometer=()=>null; const WindIcon=()=>null;\nconst significantHourlyThunderRisk=()=>null;\nconst precipitationParts=(input)=>{const wet=Number(input.precipitation)>=.01;return{displayCode:wet?61:Number(input.code)||0,weatherLabel:wet?'leichter Regen':'kein Niederschlag',type:wet?'rain':'none'};};\nconst reconcileForecastPrecipitation=input=>({precipitation:Math.max(0,Number(input.precipitation)||0),rain:Math.max(0,Number(input.rain)||0),showers:Math.max(0,Number(input.showers)||0),snowfall:Math.max(0,Number(input.snowfall)||0),probability:Math.max(0,Math.min(100,Number(input.probability)||0)),code:Math.round(Number(input.code)||0),traceSuppressed:false});\nconst coherentSunshineDurationSeconds=input=>input.valueSeconds===null||input.valueSeconds===undefined?null:Math.min(Math.max(0,Number(input.intervalSeconds)||3600),Math.max(0,Number(input.valueSeconds)||0));\nconst label=(code)=>Number(code)===10?'Feuchter Dunst':Number(code)===5?'Trockener Dunst':Number(code)===45?'Nebel':Number(code)===3?'Bedeckt':Number(code)===2?'Teilweise bewölkt':'Klar'; const wind=(value,unit)=>String(Math.round(Number(value)||0))+' '+String(unit); const formatDecimal=(value,digits=1)=>Number(value).toFixed(digits);\nconst DWD_WIND_THRESHOLDS_KMH=[];\nconst parseReportedVisibilityPhenomenon=()=>undefined;\nconst classifyVisibilityPhenomenon=(input)=>{const visibility=Number(input?.visibility),humidity=Number(input?.humidity),code=Math.round(Number(input?.weatherCode)||0);if(Number.isFinite(visibility)){if(visibility<1000&&humidity>=80)return{kind:'fog',label:'Nebel',displayCode:code===48||code===49?code:45};if(visibility<=8000&&humidity>=80)return{kind:'mist',label:'Feuchter Dunst',displayCode:10};if(visibility<=5000&&humidity<80)return{kind:'haze',label:'Trockener Dunst',displayCode:5}}return{kind:'none',label:'Keine meteorologische Sichttrübung abgeleitet'};};\nconst stationFieldObservationUsable=(station,field,now)=>{\n const limits={temperature:75,humidity:85,dewPoint:85,pressure:210,windSpeed:70,windDirection:70,windGust:55,visibility:55,cloudCover:75,ceilingHft:95,cloudBaseHft:95,precipitation:40};\n if(!station||!Number.isFinite(Number(station[field])))return false;\n const source=station.fieldSources?.[field]?.[0],stamp=Date.parse(source?.observedAt??station.fieldObservedAt?.[field]??station.timestamp??'');\n const age=Number.isFinite(stamp)?Math.max(0,(now-stamp)/60000):9999;\n return age<(limits[field]??75);\n};\nconst blendRadarAtTarget=()=>null;\n${core}\nconst buildForecastLocalAnchor=forecastLocalAnchorFromCurrent;\n${bridge}\n${source}`;
 ...
 if(anchor.code!==10)failures.push(`Wetterzustand: 6 km Sicht bei 90 % rF muss feuchten Dunst (10) liefern, erhalten ${anchor.code}.`);
 ...
 if(!first)failures.push('Erster Kurzfristpunkt fehlt.');
  ...
  if(first.code!==10)failures.push(`Sichtkonsistenz: erster Punkt hat Code ${first.code} statt feuchtem Dunst (10).`);
 ...
 const mixed=mod.shortTermAnchorFromCurrent({...station,timestamp:new Date(now-2*60000).toISOString(),fieldObservedAt:{temperature:new Date(now-90*60000).toISOString(),windSpeed:new Date(now-2*60000).toISOString(),windGust:new Date(now-2*60000).toISOString()}},current,now);
 if(Math.abs(mixed.temperature-20)>.01)failures.push(`Feldalter: 90 Minuten alte Temperatur darf nicht durch einen frischen Gesamt-/Windzeitstempel künstlich aktuell werden (${mixed.temperature} °C).`);
 if(Math.abs(mixed.wind-6)>.05)failures.push(`Feldalter: frischer Wind muss trotz alter Temperatur weiter als Beobachtungsanker nutzbar bleiben (${mixed.wind} kt).`);
 const stale=mod.shortTermAnchorFromCurrent({...station,timestamp:new Date(now-220*60000).toISOString()},current,now);
 if(stale.active)failures.push('Vollständig veraltete Stationsdaten aktivieren weiterhin die lokale Kurzfristangleichung.');
