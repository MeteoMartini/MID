...
assert.ok(app.includes('Die WMO-Sonnenscheindauer bleibt als eigenständiger Strahlungsparameter erhalten')&&app.includes('nie mit 100 % minus Bewölkung gleichgesetzt')&&app.includes('Wolkenlücken-Darstellung'),'Skybar-Hinweis muss direkte WMO-Sonnenscheindauer und Wolkenlücken-Fallback strikt trennen.');
assert.ok(skybar.includes('if(cloudKnown){')&&skybar.includes('if(daylight&&sunshineDirect){')&&skybar.includes('if(daylight&&!sunshineDirect){')&&skybar.includes('if(sunshineShare!==null&&Number.isFinite(sunshineShare))return clamp01(sunshineShare);'),'Skybar muss zeitlich ausgerichtete direkte Sonnenscheindauer als eigenen Parameter verwenden und den Wolkenkomplement nur bei fehlender Sonnenscheindauer als Wolkenlücken-Fallback zulassen.');
...
console.log(`MID v${pkg.version}: Sonnenscheindauer bleibt zeitlich am Vorwärtsslot ausgerichtet und wird in der Skybar strikt vom Wolkenlücken-Fallback getrennt.`);
