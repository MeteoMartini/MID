...
for(const token of ['function useTemperatureAxisCenters','cellX=index===0?bandLeft:(centers[index-1]+centerX)/2','cellRight=index===dayCount-1?bandRight:(centerX+centers[index+1])/2','centerX=centers[index]','stroke="none"','ensemble-sky-strip-separator','ensemble-sky-strip-outline'])if(!panel.includes(token))failures.push(`Achsgebundene Wetterleiste fehlt: ${token}`);
