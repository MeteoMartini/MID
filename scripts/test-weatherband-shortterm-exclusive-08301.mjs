...
for(const token of ['cellX=plotLeft+(slotIndex/dayCount)*plotWidth','cellRight=plotLeft+((slotIndex+1)/dayCount)*plotWidth','centerX=plotLeft+((slotIndex+.5)/dayCount)*plotWidth','stroke="none"','ensemble-sky-strip-separator','ensemble-sky-strip-outline'])if(!panel.includes(token))failures.push(`Achsgebundene Wetterleiste fehlt: ${token}`);
