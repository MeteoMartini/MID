...
for(const token of ["onPointerUp={event=>{if(event.pointerType!=='mouse'){event.preventDefault();setSelectedId",'onClick={()=>setSelectedId(current=>current===point.id?\'\':point.id)}'])if(!shortTerm.includes(token))fails.push(`Kurzfrist fehlt: ${token}`);
