...
const fails=[];
for(const token of ['type="search"','inputMode="search"','enterKeyHint="search"','autoCapitalize="none"','autoCorrect="off"','spellCheck={false}','debounceMs=/^\\d{2,8}$/.test(term)?35:60'])if(!app.includes(token))fails.push(`App fehlt: ${token}`);
