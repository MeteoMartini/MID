...
need('Radar-Modell-Phase',radar,'OPERA-CIRRUS-Echomaske + ICON-D2 als radar-/modellgestützte Phasenklassifikation');
need('Radar-Modell-Phase',phase,'operaRasterPoint');
need('Radar-Modell-Phase',phase,'wetBulbStull');
need('Radar-Modell-Phase',phase,"minimumDbz=phase.phase==='snow'||phase.phase==='freezing'?5:7");
need('Radar-Modell-Phase',phase,'Math.abs(targetMs-operaMs)>12*60000');
need('Radar-Modell-Phase',phase,'Math.abs(modelMs-targetMs)>20*60000');
need('Radar-Modell-Phase',phase,'modelMs-referenceMs>5*60*60000');
need('Radar-Modell-Phase',phase,"phase.phase==='uncertain'||phase.confidence==='eingeschränkt'");
need('Radar-Modell-Phase',phase,'subdivisions=1');
forbid('Radar-Modell-Phase',phase,"phase:'hail'");
...
for(const token of ['precipitation-phase-grid','wet_bulb_temperature_2m','snowfall_height','freezing_level_height','const rows=35,cols=49','valid<coordinates.length*.9','modelTimeMs-referenceMs>5*60*60000'])need('Worker-Radarphasenraster',worker,token);
