...
if(pkg.dependencies?.['maplibre-gl']!=='6.7.0')failures.push('Package: maplibre-gl 6.7.0 ist nicht fest gepinnt.');
...
if(lock.packages?.['node_modules/maplibre-gl']?.version!=='6.7.0')failures.push('Lockfile enthält nicht MapLibre GL JS 6.7.0.');
...
need('Tages-PoP',weather,"probabilitySource?:'ensemble-members-dwd'|'daily-wet-derived'|'hourly-max-fallback'");
need('Tages-PoP',weather,'precipitationProbability:weightedProbability(probabilityValues,DWD_PRECIPITATION_PROBABILITY_THRESHOLD_MM,true)');
need('Tages-PoP',weather,'precipitationProbabilitySignificant:weightedProbability(probabilityValues,DWD_SIGNIFICANT_PRECIPITATION_PROBABILITY_THRESHOLD_MM,true)');
...
need('Tages-PoP-Fallback',weather,":'hourly-max-fallback' as const");
...
need('Tages-PoP-Cockpit',cockpit,'dailyPrecipitationProbabilityTitle(day,probabilityHours)');
...
need('Radar-Modell-Phase',phase,"explicitHail=[89,90].includes(code)");
need('Radar-Modell-Phase',phase,"explicitGraupelHail=[96,99].includes(code)");
for(const token of ['explicitIceCrystals=code===76','explicitSnowGrains=code===77','explicitSnowStars=code===78','explicitIcePellets=code===79'])need('Radar-Modell-Phase',phase,token);
