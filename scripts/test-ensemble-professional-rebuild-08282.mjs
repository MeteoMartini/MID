...
const failures=[];
for(const token of ['type ProfessionalEnsembleLayout','function professionalEnsembleLayout','function ResponsiveEnsembleTooltip','function EnsembleDateAxisTick','cellSlotWidth=plotWidth/dayCount','cellX=plotLeft+slotIndex*cellSlotWidth','cellWidth=Math.max(1,cellSlotWidth)','professionalEnsembleLayout(compactTrendTooltip,exporting)','professionalEnsembleLayout(compact,exporting)','professionalEnsembleLayout(compactChart,exportingKind===\'precipitation\')'])if(!panel.includes(token))failures.push(`Professionelle Chart-Engine fehlt: ${token}`);
