...
const failures=[];
for(const token of ['type ProfessionalEnsembleLayout','function professionalEnsembleLayout','function ResponsiveEnsembleTooltip','function EnsembleDateAxisTick','function useTemperatureAxisCenters','function resolvedEnsembleDayCenters','cellX=index===0?bandLeft:(centers[index-1]+centerX)/2','cellRight=index===dayCount-1?bandRight:(centerX+centers[index+1])/2','cellWidth=Math.max(1,cellRight-cellX)','professionalEnsembleLayout(compactTrendTooltip,exporting)','professionalEnsembleLayout(compact,exporting)','professionalEnsembleLayout(compactChart,exportingKind===\'precipitation\')'])if(!panel.includes(token))failures.push(`Professionelle Chart-Engine fehlt: ${token}`);
