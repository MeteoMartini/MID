...
for(const token of ["nowcastButtonDetail=(visibleNowcastMix.length||thunder?.nearbyCells?.length)?`K3D ${thunder?.nearbyCells?.length||0} · MIX ${visibleNowcastMix.length}`:'keine Zellen'",'KONRAD3D liefert derzeit keine aktuellen Zellobjekte','DWD NowCastMIX ist erreichbar'])need(radar,token,'Livequellenstatus fehlt');
