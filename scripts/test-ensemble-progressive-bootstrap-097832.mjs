...
for(const token of ["requestPriority=ens.length?'normal':'foreground'",'ensembles(loc.latitude,loc.longitude,ensembleController.signal,requestPriority)','if(value.bootstrap)scheduleRetry(12_000)','setEnsembleRuns(current=>value.runs?.length?value.runs:current)'])assert.ok(app.includes(token),`Progressiver Ensemble-Apppfad fehlt: ${token}`);
for(const token of ['fastEnsembleBootstrap(','memberEnsembleBootstrap(','meanEnsembleBootstrap(','ENSEMBLE_BOOTSTRAP_MEMBER_IDS','ENSEMBLE_BOOTSTRAP_MEAN_IDS','Promise.any([','bootstrap:true'])assert.ok(weather.includes(token),`Schneller Ensemble-Bootstrap fehlt: ${token}`);
