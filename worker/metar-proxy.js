...
const DWD_KONRAD3D_ROOTS=['https://opendata.dwd.de/weather/radar/konrad3d/','https://opendatao.dwd.de/weather/radar/konrad3d/'];
const EEA_AIR_QUALITY_STATION_ENDPOINTS=['https://air.discomap.eea.europa.eu/arcgis/rest/services/AirQuality/AirQualityDownloadServiceEUMonitoringStations/MapServer/0/query','https://eeha.discomap.eea.europa.eu/arcgis/rest/services/AirQuality/AirQualityDownloadServiceEUMonitoringStations/MapServer/0/query'];
const DWD_RADOLAN_YW_ROOT='https://opendata.dwd.de/weather/radar/radolan/yw/';
const DWD_RADOLAN_HISTORY_ROOTS={rw:'https://opendata.dwd.de/weather/radar/radolan/rw/',ry:'https://opendata.dwd.de/weather/radar/radolan/ry/',sf:'https://opendata.dwd.de/weather/radar/radolan/sf/'};
...
const WORKER_VERSION='0.7.110.0';
...
async function openMeteoModelMeta(url){
 const model=String(url.searchParams.get('model')||'').trim(),kind=url.searchParams.get('kind')==='forecast'?'forecast':'ensemble';if(!/^[a-z0-9_]{3,80}$/i.test(model))return json({error:'ungültiges Modell'},400,{'cache-control':'no-store'});
 const host=kind==='forecast'?'https://api.open-meteo.com':'https://ensemble-api.open-meteo.com',response=await fetchWithDeadline(`${host}/data/${model}/static/meta.json?cache_buster=${Date.now()}`,{headers:{Accept:'application/json'}},9000),body=await response.text();
 if(!response.ok)return json({error:`Open-Meteo Metadaten HTTP ${response.status}`},response.status>=500?502:response.status,{'cache-control':'no-store'});
 try{return json(JSON.parse(body),200,{'cache-control':'public, max-age=300'})}catch{return json({error:'Ungültige Modellmetadaten'},502,{'cache-control':'no-store'})}
}
...
function parseKonradCells(xml,lat,lon,observedAt){
 const referenceMs=Date.parse(observedAt),cells=[];for(const match of konradFeatureMatches(xml)){
  ...
  const intensity=blocks(feature,'intensity')[0]||feature,hymec=blocks(feature,'hymec')[0]||'',lightning=blocks(feature,'lightning')[0]||'',tracking=blocks(feature,'tracking')[0]||'',currentDistanceKm=distance(lat,lon,clat,clon)/1000,forecasts=konradForecasts(feature,lat,lon,referenceMs),bestForecast=forecasts.reduce((best,row)=>!best||row.distanceKm<best.distanceKm?row:best,undefined),bestEffectiveForecast=forecasts.reduce((best,row)=>!best||row.effectiveDistanceKm<best.effectiveDistanceKm?row:best,undefined),centerApproaching=Boolean(bestForecast&&bestForecast.distanceKm+2<currentDistanceKm),relevanceDistanceKm=Math.min(currentDistanceKm,bestEffectiveForecast?.effectiveDistanceKm??Infinity),arrivalMinutes=currentDistanceKm<=20?0:centerApproaching&&bestForecast&&bestForecast.distanceKm<=35?bestForecast.minutes:undefined;
  ...
  cells.push({id:konradAttribute(attrs,'identifier')||tagValue(feature,'identifier')||`cell-${cells.length+1}`,latitude:clat,longitude:clon,currentDistanceKm,siteBearingDeg:Number(bearingTowards(lat,lon,clat,clon).toFixed(1)),relevanceDistanceKm,forecastDistanceKm:bestForecast?.distanceKm,forecastEffectiveDistanceKm:bestEffectiveForecast?.effectiveDistanceKm,forecastUncertaintyKm:bestForecast?.uncertaintyKm,forecastTime:bestForecast?.time,forecastLatitude:bestForecast?.latitude,forecastLongitude:bestForecast?.longitude,motionDirectionDeg:bestForecast?Number(bearingTowards(clat,clon,bestForecast.latitude,bestForecast.longitude).toFixed(1)):undefined,arrivalMinutes,isApproaching:centerApproaching,severity,trend,hailFlag,heavyRainFlag,gustFlag,lightningRate,areaHail,areaLargeHail,speedKmh});
 }
 return cells.sort((a,b)=>a.relevanceDistanceKm-b.relevanceDistanceKm);
}
function konradSummary(cell){const level=cell.severity>=3?'stark':cell.severity>=2?'moderat':'schwach',trend=cell.trend>0?'verstärkend':cell.trend<0?'abschwächend':'stabil',currentDistance=Number(cell.currentDistanceKm),forecastDistance=Number(cell.forecastDistanceKm),near=Number.isFinite(currentDistance)&&currentDistance<=20,atSite=Number.isFinite(currentDistance)&&currentDistance<1,arrival=!near&&cell.isApproaching&&Number.isFinite(cell.arrivalMinutes)&&Number(cell.arrivalMinutes)>0?` · mögliche Annäherung in etwa ${Math.round(cell.arrivalMinutes)} min`:Number.isFinite(forecastDistance)&&forecastDistance>currentDistance+2?' · Zellzentrum zieht voraussichtlich nicht näher heran':'';return`${atSite?'KONRAD3D-Zelle unmittelbar am Standort':`KONRAD3D-Zelle ${Math.max(1,Math.round(currentDistance))} km entfernt`}${arrival} · ${level} · ${trend}.`}
async function dwdKonrad3dNowcast(lat,lon,country=''){
 if(!inGermanyBounds(lat,lon)&&normalizeCountryCode(country)!=='DE')return{available:false,coverage:false,provider:'DWD KONRAD3D',product:'KONRAD3D',cellsFound:0,nearbyCells:[],summary:'KONRAD3D ist auf Deutschland und das unmittelbare Umfeld begrenzt.',temporalResolutionMinutes:5,license:'Daten: Deutscher Wetterdienst'};
 const errors=[];let staleResult=null;
 for(const root of DWD_KONRAD3D_ROOTS){try{
  ...
  const xml=await fetchText(new URL(latest.name,root).toString(),{'Accept':'application/xml,text/xml,*/*'}),cells=parseKonradCells(xml,lat,lon,observedAt),nearbyCells=cells.filter(cell=>cell.relevanceDistanceKm<=80).slice(0,8),nearest=nearbyCells[0];
  return{available:true,coverage:true,provider:'DWD KONRAD3D',product:'KONRAD3D',observedAt,ageMinutes,cellsFound:cells.length,nearbyCells,nearest,summary:nearest?konradSummary(nearest):'Keine relevante KONRAD3D-Zelle im Umkreis von 80 km.',temporalResolutionMinutes:5,license:'Daten: Deutscher Wetterdienst',diagnostics:{sourceRoot:new URL(root).hostname}};
...
function radolanProductTimestamp(filename,product){const match=String(filename).match(new RegExp(`^raa01-${product}_10000-(\\d{10})-dwd---bin\\.hdf5$`,'i'));if(!match)return undefined;const value=match[1],year=2000+Number(value.slice(0,2)),month=Number(value.slice(2,4))-1,day=Number(value.slice(4,6)),hour=Number(value.slice(6,8)),minute=Number(value.slice(8,10)),time=Date.UTC(year,month,day,hour,minute);return Number.isFinite(time)?time:undefined}
function radolanHistoryProxyUrl(request,product,filename){const url=new URL(request.url);url.search='';url.searchParams.set('mode','radolan-history-file');url.searchParams.set('product',product);url.searchParams.set('file',filename);return url.toString()}
async function radolanDirectoryFiles(product,lookbackHours=30){const root=DWD_RADOLAN_HISTORY_ROOTS[product];if(!root)throw new Error(`Unbekanntes RADOLAN-Produkt ${product}`);const html=await fetchText(root,{Accept:'text/html,application/xhtml+xml'}),now=Date.now(),pattern=new RegExp(`href=["'](raa01-${product}_10000-\\d{10}-dwd---bin\\.hdf5)["']`,'gi');return[...html.matchAll(pattern)].map(match=>match[1]).map(name=>({name,time:radolanProductTimestamp(name,product)})).filter(item=>Number.isFinite(item.time)&&item.time<=now+15*60000&&item.time>=now-lookbackHours*3600000).sort((a,b)=>a.time-b.time)}
async function radolanHistoryMetadata(request,lat,lon){if(!inGermanyBounds(lat,lon))return{coverage:false,provider:'DWD RADOLAN',hour:null,day:null,summary:'Außerhalb des deutschen RADOLAN-Rasters.'};const now=Date.now(),[rwResult,ryResult,sfResult]=await Promise.allSettled([radolanDirectoryFiles('rw',4),radolanDirectoryFiles('ry',4),radolanDirectoryFiles('sf',30)]),rw=rwResult.status==='fulfilled'?rwResult.value:[],ry=ryResult.status==='fulfilled'?ryResult.value:[],sf=sfResult.status==='fulfilled'?sfResult.value:[],latestRw=rw.at(-1),latestSf=sf.at(-1),rwAge=latestRw?Math.round((now-latestRw.time)/60000):Infinity,sfAge=latestSf?Math.round((now-latestSf.time)/60000):Infinity;let hour=null;if(latestRw&&rwAge>=-10&&rwAge<=55)hour={mode:'single',product:'RW',adjusted:true,label:'RADOLAN RW · angeeichte 1-h-Summe',observedAt:new Date(latestRw.time).toISOString(),frames:[{time:new Date(latestRw.time).toISOString(),fileUrl:radolanHistoryProxyUrl(request,'rw',latestRw.name)}]};else{const latestRy=ry.at(-1),latestRyTime=latestRy?.time??NaN,frames=ry.filter(item=>Number.isFinite(latestRyTime)&&item.time>latestRyTime-65*60000&&item.time<=latestRyTime+60000).slice(-13).map(item=>({time:new Date(item.time).toISOString(),fileUrl:radolanHistoryProxyUrl(request,'ry',item.name)}));if(frames.length)hour={mode:'sum',product:'RY',adjusted:false,label:'RADOLAN RY · nicht angeeichte 5-min-Summen',observedAt:frames.at(-1).time,frames}}const day=latestSf&&sfAge>=-10&&sfAge<=150?{mode:'single',product:'SF',adjusted:true,label:'RADOLAN SF · angeeichte gleitende 24-h-Summe',observedAt:new Date(latestSf.time).toISOString(),frames:[{time:new Date(latestSf.time).toISOString(),fileUrl:radolanHistoryProxyUrl(request,'sf',latestSf.name)}]}:null;return{coverage:Boolean(hour||day),provider:'DWD RADOLAN',hour,day,license:'DWD Open Data',diagnostics:{rwAgeMinutes:Number.isFinite(rwAge)?rwAge:undefined,sfAgeMinutes:Number.isFinite(sfAge)?sfAge:undefined,rwFiles:rw.length,ryFiles:ry.length,sfFiles:sf.length,errors:[rwResult,ryResult,sfResult].map((result,index)=>result.status==='rejected'?`${['RW','RY','SF'][index]}: ${result.reason instanceof Error?result.reason.message:String(result.reason)}`:'').filter(Boolean)}}}
async function radolanHistoryFileResponse(request){const url=new URL(request.url),product=String(url.searchParams.get('product')||'').toLowerCase(),filename=String(url.searchParams.get('file')||''),root=DWD_RADOLAN_HISTORY_ROOTS[product],time=radolanProductTimestamp(filename,product),maxAge=product==='sf'?31*3600000:product==='rw'?6*3600000:4*3600000;if(!root||!time||Math.abs(Date.now()-time)>maxAge)return json({error:'Ungültige oder veraltete RADOLAN-Rückschau-Datei.',version:WORKER_VERSION},400,{'cache-control':'no-store'});const upstream=new URL(filename,root).toString(),response=await fetch(upstream,{headers:{Accept:'application/x-hdf5,application/octet-stream,*/*','User-Agent':`MID-weather-dashboard/${WORKER_VERSION}`},cf:{cacheTtl:product==='sf'?3600:900,cacheEverything:true}});if(!response.ok)return json({error:`DWD RADOLAN ${product.toUpperCase()} HTTP ${response.status}`,version:WORKER_VERSION},response.status,{'cache-control':'no-store'});return new Response(response.body,{status:200,headers:{'content-type':'application/x-hdf5','content-disposition':`inline; filename="${filename}"`,'access-control-allow-origin':'*','cache-control':product==='sf'?'public, max-age=3600':'public, max-age=900','x-mid-worker-version':WORKER_VERSION,'x-mid-source':`DWD RADOLAN ${product.toUpperCase()}`}})}
...
function geoSphereSnowTimestamp(raw,feature){return safeDate(feature?.properties?.timestamp??feature?.properties?.reference_time??feature?.timestamp??(Array.isArray(raw?.timestamps)?raw.timestamps.at(-1):raw?.timestamp))}
async function geoSphereSnowMeasurement(lat,lon,elevation){
 if(!geoSphereApplies(lat,lon))return{available:false,provider:'GeoSphere Austria',reason:'außerhalb Österreichs'};
 const targetElevation=number(elevation),meta=await geoSphereMetadata(),nearby=meta.map(st=>({...st,distanceM:distance(lat,lon,st.lat,st.lon),heightDifferenceM:targetElevation!==undefined&&st.elevation!==undefined?st.elevation-targetElevation:undefined})).filter(st=>st.distanceM<=25000&&(targetElevation===undefined||st.elevation===undefined||Math.abs(st.elevation-targetElevation)<=350)).sort((a,b)=>(a.distanceM+Math.abs(a.heightDifferenceM||0)*45)-(b.distanceM+Math.abs(b.heightDifferenceM||0)*45)).slice(0,18);
 if(!nearby.length)return{available:false,provider:'GeoSphere Austria',reason:'keine Station innerhalb 25 km und ±350 m Höhenunterschied'};
 const parameters=['HS','SH','SNOW_DEPTH'];let lastError='';
 for(const parameter of parameters){
  const url=new URL(GEOSPHERE_CURRENT);url.searchParams.set('parameters',parameter);url.searchParams.set('station_ids',nearby.map(st=>st.id).join(','));url.searchParams.set('output_format','geojson');
  try{
   const raw=await fetchJson(url.toString()),features=Array.isArray(raw?.features)?raw.features:Array.isArray(raw?.data)?raw.data:Array.isArray(raw)?raw:[],byId=new Map(nearby.map(st=>[st.id,st])),candidates=[];
   for(const feature of features){const id=geoSphereFeatureId(feature),station=byId.get(id);if(!station)continue;const value=geoSphereParam(feature,parameter),observedAt=geoSphereSnowTimestamp(raw,feature),ageMs=observedAt?Date.now()-Date.parse(observedAt):Infinity;if(value===undefined||value<0||value>1000||!Number.isFinite(ageMs)||ageMs< -15*60000||ageMs>3*3600000)continue;candidates.push({available:true,valueCm:value,stationName:station.name,stationId:station.id,stationElevation:station.elevation,distanceKm:station.distanceM/1000,heightDifferenceM:station.heightDifferenceM??0,observedAt,provider:'GeoSphere Austria / TAWES',parameter,rawUnit:'cm'})}
   candidates.sort((a,b)=>(a.distanceKm*1000+Math.abs(a.heightDifferenceM)*45)-(b.distanceKm*1000+Math.abs(b.heightDifferenceM)*45));if(candidates.length)return candidates[0];
  }catch(error){lastError=error instanceof Error?error.message:String(error)}
 }
 return{available:false,provider:'GeoSphere Austria / TAWES',reason:'keine aktuelle, höhennahe Schneehöhenmessung',error:lastError||undefined};
}
...
function precipitationAnchors(image,rateAt,lat,lon,latDelta,lonDelta,maxAnchors=18,minimumRate=.12){const columns=7,rows=7,anchors=[];for(let row=0;row<rows;row++)for(let column=0;column<columns;column++){const x0=Math.floor(column*image.width/columns),x1=Math.max(x0+1,Math.floor((column+1)*image.width/columns)),y0=Math.floor(row*image.height/rows),y1=Math.max(y0+1,Math.floor((row+1)*image.height/rows)),step=Math.max(1,Math.floor(Math.min(x1-x0,y1-y0)/8));let total=0,weightedX=0,weightedY=0,peak=0,samples=0;for(let y=y0;y<y1;y+=step)for(let x=x0;x<x1;x+=step){const rate=rateAt(x,y);if(!Number.isFinite(rate)||rate<minimumRate)continue;const weight=Math.sqrt(Math.min(80,rate));total+=weight;weightedX+=x*weight;weightedY+=y*weight;peak=Math.max(peak,rate);samples++}if(total<=0||samples<2)continue;const px=weightedX/total,py=weightedY/total,anchorLat=lat+((image.height/2-py)/(image.height/2))*latDelta,anchorLon=lon+((px-image.width/2)/(image.width/2))*lonDelta;anchors.push({lat:Number(anchorLat.toFixed(5)),lon:Number(anchorLon.toFixed(5)),rate:Number(peak.toFixed(2))})}return anchors.sort((a,b)=>b.rate-a.rate).slice(0,maxAnchors)}
...
function dwdMapSample(image,lat,lon,latDelta,lonDelta,echoProfile=seasonalEchoProfile(lat)){
 ...
 return{center,nearby:Math.max(...near,0),visible:center>=echoProfile.siteThreshold||near.some(v=>v>=echoProfile.nearbyThreshold),motionAnchors:precipitationAnchors(image,sample,lat,lon,latDelta,lonDelta,20,echoProfile.anchorThreshold),...precipitationCentroid(image,sample,kmPerPixelX,kmPerPixelY)};
}
...
async function dwdMotionField(base,layer,lat,lon,time,echoProfile=seasonalEchoProfile(lat,time)){const latDelta=2.35,lonDelta=latDelta/Math.max(.35,Math.cos(lat*Math.PI/180)),u=new URL(base);u.searchParams.set('service','WMS');u.searchParams.set('version','1.1.1');u.searchParams.set('request','GetMap');u.searchParams.set('layers',layer);u.searchParams.set('styles','');u.searchParams.set('srs','EPSG:4326');u.searchParams.set('bbox',`${lon-lonDelta},${lat-latDelta},${lon+lonDelta},${lat+latDelta}`);u.searchParams.set('width','257');u.searchParams.set('height','257');u.searchParams.set('format','image/png');u.searchParams.set('transparent','true');u.searchParams.set('exceptions','application/vnd.ogc.se_xml');if(Number.isFinite(time))u.searchParams.set('time',new Date(time).toISOString());const response=await fetch(u.toString(),{headers:{Accept:'image/png,*/*'},cf:{cacheTtl:120,cacheEverything:true}});if(!response.ok)throw new Error(`DWD Bewegungsfeld HTTP ${response.status}`);const image=await decodePng(await response.arrayBuffer()),sample=(x,y)=>{x=clamp(Math.round(x),0,image.width-1);y=clamp(Math.round(y),0,image.height-1);const index=(y*image.width+x)*4;return dwdPixelRate(image.rgba[index],image.rgba[index+1],image.rgba[index+2],image.rgba[index+3])};return{time:Number.isFinite(time)?time:Date.now(),lat,lon,latDelta,lonDelta,anchors:precipitationAnchors(image,sample,lat,lon,latDelta,lonDelta,20,echoProfile.anchorThreshold),grid:motionGrid(image,sample,81,echoProfile.motionThreshold)}}
...
async function radarMeteorologicalContext(lat,lon){
 const url=new URL(OPEN_METEO_FORECAST);url.searchParams.set('latitude',String(lat));url.searchParams.set('longitude',String(lon));url.searchParams.set('hourly','temperature_2m,wet_bulb_temperature_2m,snowfall,weather_code,cape,wind_speed_925hPa,wind_direction_925hPa,wind_speed_850hPa,wind_direction_850hPa,wind_speed_700hPa,wind_direction_700hPa,wind_speed_500hPa,wind_direction_500hPa');url.searchParams.set('past_hours','2');url.searchParams.set('forecast_hours','4');url.searchParams.set('timezone','GMT');url.searchParams.set('wind_speed_unit','kmh');url.searchParams.set('cell_selection','nearest');
 const data=await fetchJson(url.toString()),hourly=data?.hourly??{},times=Array.isArray(hourly.time)?hourly.time:[];if(!times.length)return{};const now=Date.now(),index=times.reduce((best,value,i)=>{const epoch=Date.parse(`${String(value).replace(/Z$/,'')}Z`),delta=Math.abs(epoch-now);return delta<best.delta?{index:i,delta}:best},{index:0,delta:Infinity}).index,read=name=>number(hourly?.[name]?.[index]);return{observedAt:safeDate(`${String(times[index]).replace(/Z$/,'')}Z`),temperature:read('temperature_2m'),wetBulb:read('wet_bulb_temperature_2m'),snowfall:read('snowfall'),weatherCode:read('weather_code'),cape:read('cape'),levels:[925,850,700,500].map(level=>({level,speed:read(`wind_speed_${level}hPa`),directionFrom:read(`wind_direction_${level}hPa`)})).filter(row=>Number.isFinite(row.speed)&&Number.isFinite(row.directionFrom))};
}
...
function steeringMotionFromContext(context,peakRate=0){
 const levels=Array.isArray(context?.levels)?context.levels:[];if(levels.length<2)return{};const convective=Number(context?.cape)>=300||Number(peakRate)>=8||[95,96,97,99].includes(Number(context?.weatherCode)),weights=convective?{925:.08,850:.27,700:.40,500:.25}:{925:.18,850:.45,700:.30,500:.07};let east=0,north=0,total=0;const directions=[];for(const row of levels){const weight=weights[row.level]??0;if(weight<=0)continue;const towards=(Number(row.directionFrom)+180)%360,vector=vectorFromTowards(towards,Number(row.speed));east+=vector.east*weight;north+=vector.north*weight;total+=weight;directions.push(towards)}if(total<=0)return{};east/=total;north/=total;const speed=Math.hypot(east,north),direction=directionFromVector(east,north),spread=directions.length?directions.reduce((sum,value)=>sum+angularDifference(value,direction),0)/directions.length:180;if(!Number.isFinite(speed)||speed<2)return{};return{motionDirectionDeg:Number(direction.toFixed(1)),motionSpeedKmh:Number(speed.toFixed(1)),motionConfidence:levels.length>=3&&spread<=45?'medium':'low',motionSource:convective?'steering-flow-850-700-500hPa':'steering-flow-925-850-700hPa',motionFit:Number(Math.max(0,1-spread/180).toFixed(2)),motionDirectionConvention:'towards',steeringLevels:levels.map(row=>row.level),steeringSpreadDeg:Number(spread.toFixed(1))};
}
...
function hybridRadarMotion(radar,steering,konrad){
 const valid=value=>Number.isFinite(Number(value?.motionDirectionDeg))&&Number.isFinite(Number(value?.motionSpeedKmh))&&Number(value.motionSpeedKmh)>=2,radarValid=valid(radar),steeringValid=valid(steering),konradValid=valid(konrad);if(radarValid){let result={...radar};if(konradValid&&angularDifference(radar.motionDirectionDeg,konrad.motionDirectionDeg)<=55){result={...result,...blendMotionVectors(radar,konrad,.72),motionConfidence:radar.motionConfidence==='high'?'high':'medium',motionSource:`${radar.motionSource}+KONRAD3D`}}if(steeringValid){const difference=angularDifference(result.motionDirectionDeg,steering.motionDirectionDeg);if(difference<=65){result={...result,...blendMotionVectors(result,steering,.78),motionConfidence:result.motionConfidence==='high'?'high':'medium',motionSource:`${result.motionSource}+Schwerpunktströmung`,steeringDirectionDeg:steering.motionDirectionDeg,steeringSpeedKmh:steering.motionSpeedKmh}}else if(difference>=125&&result.motionConfidence==='low'&&konradValid){result={...konrad,motionConfidence:'medium',motionSource:'KONRAD3D statt widersprüchlicher Radarverschiebung'}}}return result}
 if(konradValid){if(steeringValid&&angularDifference(konrad.motionDirectionDeg,steering.motionDirectionDeg)<=70)return{...konrad,...blendMotionVectors(konrad,steering,.72),motionConfidence:'medium',motionSource:'KONRAD3D+Schwerpunktströmung',steeringDirectionDeg:steering.motionDirectionDeg,steeringSpeedKmh:steering.motionSpeedKmh};return{...konrad,motionConfidence:'medium',motionSource:'KONRAD3D-Zellverlagerung'}}
 if(steeringValid)return{...steering,motionConfidence:steering.motionConfidence||'low'};return{};
}
...
function radarResultFromFrames(source,provider,quality,frames,license,options={}){
 ...
 if(current.rate>=siteThreshold){radarProbability=98;arrivalMinutes=0;arrivalKind='site';peakRate=current.rate;rateUncertain=current.uncertain;const ending=ongoingWetEnd(future,siteWet,latestObserved.time);endAt=ending.endAt?new Date(ending.endAt).toISOString():undefined;endMinutes=ending.endMinutes;endOpenEnded=Boolean(ending.endOpenEnded);endUncertain=Boolean(ending.endUncertain);summary=`Niederschlag am Standort erkannt: ${radarRateLabel(current.rate)}.`}
...
async function px250Metadata(request,lat,lon){
 // Für die Kartenüberlagerung wird das räumlich passende Standortradar
 // bevorzugt. Das nationale HX-Komposit liegt in einer projizierten
 // Deutschlandmatrix und darf nur als Fallback dienen, weil ein einfaches
 // Leaflet-ImageOverlay die Projektion sonst sichtbar verschieben kann.
 const rangeKm=150,candidates=nearestPxSites(lat,lon,rangeKm).slice(0,5),diagnostics=[];
 for(const site of candidates){try{const item=await pxSiteLatest(site);diagnostics.push(`${item.name}: ${Number.isFinite(item.ageMinutes)?item.ageMinutes:'?'} min`);if(!Number.isFinite(item.observedMs)||item.ageMinutes>30||item.ageMinutes<-10)continue;const fileUrl=new URL(request.url);fileUrl.search='';fileUrl.searchParams.set('mode','px250-file');fileUrl.searchParams.set('product','px250');fileUrl.searchParams.set('site',item.code);fileUrl.searchParams.set('file',item.latest.file);return{available:true,product:'px250',productName:'DWD PX250 Standortradar',site:item.code,siteName:item.name,stationId:item.wmo,radarLat:item.lat,radarLon:item.lon,distanceKm:Math.round(item.distanceKm*10)/10,observedAt:item.observedAt,ageMinutes:item.ageMinutes,fileUrl:fileUrl.toString(),nativeResolutionM:250,rangeKm,georeferencing:'local-site'}}catch(error){diagnostics.push(error instanceof Error?error.message:String(error))}}
 if(inGermanyBounds(lat,lon)){try{const hx=await hxLatest();if(Number.isFinite(hx.observedMs)&&hx.ageMinutes<=30&&hx.ageMinutes>=-10){const fileUrl=new URL(request.url);fileUrl.search='';fileUrl.searchParams.set('mode','px250-file');fileUrl.searchParams.set('product','hx');fileUrl.searchParams.set('file',hx.latest.file);return{available:true,product:'hx',productName:'DWD HX Deutschlandkomposit',site:'hx',siteName:'Deutschlandkomposit HX · Fallback',coverage:'Deutschland',distanceKm:0,observedAt:hx.observedAt,ageMinutes:hx.ageMinutes,fileUrl:fileUrl.toString(),nativeResolutionM:250,rangeKm:650,georeferencing:'projected-national-fallback',warning:'Nationales HX-Komposit wird nur verwendet, wenn kein frisches lokales PX250-Standortradar verfügbar ist.'}}}catch(error){diagnostics.push(error instanceof Error?error.message:String(error))}}
 ...
 return{available:false,stale:Boolean(candidates.length),nativeResolutionM:250,rangeKm,site:nearest.code,siteName:nearest.name,distanceKm:Math.round(distanceKm*10)/10,reason:`Kein höchstens 30 Minuten alter lokaler PX250-Stand und kein aktuelles HX-Fallback verfügbar. Nächster Standort ${nearest.name} in ${Math.round(distanceKm)} km. ${diagnostics.slice(0,5).join(' · ')}`};
}
...
function pushFavoriteUrl(appUrl,favorite){try{const url=new URL(String(appUrl||'https://meteomartini.github.io/MID/'));url.searchParams.set('mid-favorite',String(favorite?.id||''));url.searchParams.set('mid-lat',String(favorite?.latitude??''));url.searchParams.set('mid-lon',String(favorite?.longitude??''));url.searchParams.set('mid-name',String(favorite?.name||'Ort').replace(/^Modelllaufänderung\s*·\s*/i,'').replace(/^Aktueller Standort\s*·\s*/i,''));if(favorite?.country)url.searchParams.set('mid-country',String(favorite.country));return url.toString()}catch{return String(appUrl||'https://meteomartini.github.io/MID/')}}
function pushLocationPayload(favorite){return{favoriteId:String(favorite?.id||''),latitude:number(favorite?.latitude),longitude:number(favorite?.longitude),locationName:String(favorite?.name||'Ort').replace(/^Modelllaufänderung\s*·\s*/i,'').replace(/^Aktueller Standort\s*·\s*/i,''),country:String(favorite?.country||'')}}
...
function validPushNotificationInterval(value){const minutes=Math.round(Number(value));return PUSH_NOTIFICATION_INTERVALS.includes(minutes)?minutes:60}
function pushNotificationReady(lastAt,intervalMinutes,now=Date.now()){const previous=Date.parse(String(lastAt||''));return!Number.isFinite(previous)||now-previous>=validPushNotificationInterval(intervalMinutes)*60000}
...
async function pushSubscribe(request,env){
 if(!pushConfigured(env))return json({error:'Push-Dienst ist im Worker noch nicht konfiguriert.',version:WORKER_VERSION},503,{'cache-control':'no-store'});
 if(!pushOriginAllowed(request,env))return json({error:'Origin für Push-Verwaltung nicht zugelassen.',version:WORKER_VERSION},403,{'cache-control':'no-store'});
 let body;try{body=await request.json()}catch{return json({error:'Ungültiger JSON-Inhalt.',version:WORKER_VERSION},400,{'cache-control':'no-store'})}
 const subscription=validPushSubscription(body?.subscription);if(!subscription)return json({error:'Ungültiges Push-Abonnement.',version:WORKER_VERSION},400,{'cache-control':'no-store'});
 const key=`sub:${await pushHash(subscription.endpoint)}`,existing=await env.MID_PUSH_SUBSCRIPTIONS.get(key,{type:'json'}).catch(()=>null),notificationIntervalMinutes=validPushNotificationInterval(body?.notificationIntervalMinutes),entry={subscription,favorites:validPushFavorites(body?.favorites),notificationIntervalMinutes,lastNotificationAt:existing?.lastNotificationAt||'',appUrl:String(body?.appUrl||env.MID_APP_URL||'https://meteomartini.github.io/MID/').slice(0,500),userAgent:String(body?.userAgent||'').slice(0,300),state:existing?.state&&typeof existing.state==='object'?existing.state:{},createdAt:existing?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
 await env.MID_PUSH_SUBSCRIPTIONS.put(key,JSON.stringify(entry));return json({ok:true,favorites:entry.favorites.length,notificationIntervalMinutes,version:WORKER_VERSION},200,{'cache-control':'no-store'})
}
...
function pushThunderState(result){const candidates=Array.isArray(result?.nearbyCells)?result.nearbyCells:[],eligible=candidates.map(cell=>{const currentDistanceKm=number(cell?.currentDistanceKm),forecastDistanceKm=number(cell?.forecastDistanceKm),effectiveDistanceKm=number(cell?.forecastEffectiveDistanceKm??cell?.relevanceDistanceKm),arrivalMinutes=number(cell?.arrivalMinutes),near=Number.isFinite(currentDistanceKm)&&currentDistanceKm<=20,approaching=Boolean(!near&&cell?.isApproaching&&Number.isFinite(effectiveDistanceKm)&&effectiveDistanceKm<=35&&(!Number.isFinite(arrivalMinutes)||arrivalMinutes<=60));return{cell,currentDistanceKm,forecastDistanceKm,effectiveDistanceKm,arrivalMinutes,near,approaching,rank:near?currentDistanceKm:(effectiveDistanceKm??999)}}).filter(row=>row.near||row.approaching).sort((a,b)=>Number(a.rank)-Number(b.rank)),selected=eligible[0];if(!selected)return{active:false};const{cell,currentDistanceKm,forecastDistanceKm,effectiveDistanceKm,arrivalMinutes,near,approaching}=selected,status=near?(currentDistanceKm<1?'at-site':'near'):'approaching';return{active:true,key:String(cell.id||`${Math.round(Number(cell.latitude)||0)}:${Math.round(Number(cell.longitude)||0)}`),status,currentDistanceKm,forecastDistanceKm,effectiveDistanceKm,arrivalMinutes:approaching&&Number.isFinite(arrivalMinutes)&&arrivalMinutes>0?arrivalMinutes:undefined}}
function pushVisibleLocationName(favorite){return String(favorite?.name||'Ort').replace(/^Modelllaufänderung\s*·\s*/i,'').replace(/^Aktueller Standort\s*·\s*/i,'').trim()||'Ort'}
function pushLocationPhrases(favorite){const tracked=String(favorite?.id||'')==='tracked-location'||/^Aktueller Standort\s*·/i.test(String(favorite?.name||'')),name=pushVisibleLocationName(favorite);return tracked?{at:'am Standort',toward:'dem Standort',from:'vom Standort',name:'Standort'}:{at:`bei ${name}`,toward:name,from:`von ${name}`,name}}
function thunderPushBody(result,favorite){const place=pushLocationPhrases(favorite),current=Number(result?.currentDistanceKm),forecast=Number(result?.forecastDistanceKm),arrival=Number(result?.arrivalMinutes);if(result?.status==='at-site'||Number.isFinite(current)&&current<1)return`Eine Gewitterzelle befindet sich unmittelbar ${place.at}. Keine amtliche Warnung.`;if(result?.status==='near'||Number.isFinite(current)&&current<=20)return`Eine Gewitterzelle befindet sich aktuell etwa ${Math.max(1,Math.round(current))} km ${place.from} entfernt. Keine amtliche Warnung.`;const currentText=Number.isFinite(current)?` · aktuell etwa ${Math.max(1,Math.round(current))} km entfernt`:'',approachText=Number.isFinite(arrival)&&arrival>0?` · größte berechnete Annäherung in ${Math.round(arrival)} min${Number.isFinite(forecast)?` auf etwa ${Math.max(0,Math.round(forecast))} km`:''}`:'';return`Eine Gewitterzelle nähert sich ${place.toward}${currentText}${approachText}. Keine amtliche Warnung.`}
...
async function evaluatePushEntry(key,entry,env,weatherCache,thunderCache,forecastCache){
 const subscription=validPushSubscription(entry?.subscription);if(!subscription){await env.MID_PUSH_SUBSCRIPTIONS.delete(key);return}
 const favorites=validPushFavorites(entry?.favorites),state=entry?.state&&typeof entry.state==='object'?entry.state:{},appUrl=String(entry?.appUrl||env.MID_APP_URL||'https://meteomartini.github.io/MID/'),notificationIntervalMinutes=validPushNotificationInterval(entry?.notificationIntervalMinutes);let expired=false,changed=false,lastNotificationAt=String(entry?.lastNotificationAt||'');
 const canNotify=()=>pushNotificationReady(lastNotificationAt,notificationIntervalMinutes),markNotificationSent=()=>{lastNotificationAt=new Date().toISOString();changed=true;return lastNotificationAt};
 for(const favorite of favorites){
  const old=state[favorite.id]??{},next={...old};
  if(favorite.rules.precipitationStart){
   const cacheKey=`${favorite.latitude.toFixed(4)}:${favorite.longitude.toFixed(4)}`;let weather=weatherCache.get(cacheKey);if(!weather){weather=pushWeatherState(favorite);weatherCache.set(cacheKey,weather)}
   try{
    const result=await weather,eventKey=result.upcomingKey||'',triggerActive=result.active&&!old.precipitationActive,triggerUpcoming=!result.active&&result.upcomingMinutes!==undefined&&eventKey!==old.rainEventKey,trigger=triggerActive||triggerUpcoming;let advanceState=!trigger;
    if(trigger&&canNotify()){
     const place=pushLocationPhrases(favorite),body=triggerActive?`${result.kind} hat ${place.at} begonnen.`:`${result.kind} beginnt ${place.at} voraussichtlich in ${result.upcomingMinutes} Minuten.`,sent=await sendWebPush(subscription,{title:`MID · Niederschlag ${place.at}`,body,tag:`mid-rain-${favorite.id}`,url:pushFavoriteUrl(appUrl,favorite),...pushLocationPayload(favorite),timestamp:Date.now(),renotify:true},env);
    ...
    if(advanceState){next.precipitationActive=result.active;if(!result.active&&result.upcomingMinutes===undefined)next.rainEventKey=''}
    changed=true
   }catch(error){next.precipitationError=error instanceof Error?error.message:String(error);changed=true}
  }
  if(favorite.rules.thunderstormApproach&&!expired){
   const cacheKey=`${favorite.latitude.toFixed(4)}:${favorite.longitude.toFixed(4)}:${favorite.country}`;let thunder=thunderCache.get(cacheKey);if(!thunder){thunder=dwdKonrad3dNowcast(favorite.latitude,favorite.longitude,favorite.country);thunderCache.set(cacheKey,thunder)}
   try{
    const result=pushThunderState(await thunder),trigger=result.active&&(!old.thunderActive||result.key!==old.thunderCellKey);let advanceState=!trigger;
    if(trigger&&canNotify()){
     const place=pushLocationPhrases(favorite),sent=await sendWebPush(subscription,{title:`MID · Gewitter ${place.at}`,body:thunderPushBody(result,favorite),tag:`mid-thunder-${favorite.id}`,url:pushFavoriteUrl(appUrl,favorite),...pushLocationPayload(favorite),timestamp:Date.now(),renotify:true},env);
     if(sent.expired){expired=true;break}
     if(sent.ok){next.lastThunderNotificationAt=markNotificationSent();advanceState=true}
    }
    if(advanceState){next.thunderActive=result.active;next.thunderCellKey=result.active?result.key:''}
    changed=true
   }catch(error){next.thunderError=error instanceof Error?error.message:String(error);changed=true}
  }
  if(favorite.rules.forecastMaterialChange&&!expired){
   const cacheKey=`${favorite.latitude.toFixed(4)}:${favorite.longitude.toFixed(4)}`;let forecast=forecastCache.get(cacheKey);if(!forecast){forecast=pushForecastState(favorite);forecastCache.set(cacheKey,forecast)}
   try{
    const result=await forecast,reference=old.forecastReferenceSnapshot||old.forecastSnapshot,events=pushForecastChangeEvents(reference,result),newSignature=result.signature!==old.forecastSnapshot?.signature,trigger=newSignature&&Boolean(reference)&&events.length>0&&old.forecastNotifiedSignature!==result.signature;let advanceState=!trigger;
    if(trigger&&canNotify()){
     const visibleName=favorite.name.replace(/^Modelllaufänderung\s*·\s*/i,''),extra=events.length>2?` · plus ${events.length-2} weitere`:'',sent=await sendWebPush(subscription,{title:`MID · Prognoseänderung bei ${visibleName}`,body:`${events.slice(0,2).map(pushForecastEventText).join(' · ')}${extra}`,tag:`mid-model-change-${favorite.id}`,url:pushFavoriteUrl(appUrl,favorite),...pushLocationPayload(favorite),timestamp:Date.now(),renotify:true},env);
     if(sent.expired){expired=true;break}
     if(sent.ok){next.forecastNotifiedSignature=result.signature;next.forecastReferenceSnapshot=result;next.lastForecastChangeNotificationAt=markNotificationSent();advanceState=true}
    }
    if(newSignature&&advanceState){if(!reference)next.forecastReferenceSnapshot=result;next.forecastSnapshot=result;next.forecastMaterialChanges=events}
    if(newSignature)changed=true
   }catch(error){next.forecastChangeError=error instanceof Error?error.message:String(error);changed=true}
  }
  state[favorite.id]=next
 }
 if(expired){await env.MID_PUSH_SUBSCRIPTIONS.delete(key);return}
 if(changed)await env.MID_PUSH_SUBSCRIPTIONS.put(key,JSON.stringify({...entry,subscription,favorites,notificationIntervalMinutes,lastNotificationAt,state,checkedAt:new Date().toISOString()}))
}

function deviceSyncConfigured(env){return Boolean(env?.MID_PUSH_SUBSCRIPTIONS)}
function validDeviceSyncKey(value){const key=String(value||'').replace(/[^A-Za-z0-9_-]/g,'');return key.length>=32&&key.length<=160?key:''}
async function deviceSyncStorageKey(syncKey){return`sync:${await pushHash(`mid-device-sync:${syncKey}`)}`}
async function deviceSyncPush(request,env){
 if(!deviceSyncConfigured(env))return json({error:'Gerätesynchronisation ist im Worker nicht konfiguriert.',version:WORKER_VERSION},503,{'cache-control':'no-store'});
 if(!pushOriginAllowed(request,env))return json({error:'Nicht freigegebener Ursprung.',version:WORKER_VERSION},403,{'cache-control':'no-store'});
 const body=await request.json().catch(()=>null),syncKey=validDeviceSyncKey(body?.syncKey),blob=body?.blob,updatedAt=String(body?.updatedAt||new Date().toISOString());
 if(!syncKey||!blob||typeof blob.iv!=='string'||typeof blob.data!=='string')return json({error:'Ungültiger Synchronisationsstand.',version:WORKER_VERSION},400,{'cache-control':'no-store'});
 if(blob.iv.length>256||blob.data.length>1800000)return json({error:'Synchronisationsstand ist zu groß.',version:WORKER_VERSION},413,{'cache-control':'no-store'});
 const key=await deviceSyncStorageKey(syncKey),entry={schema:'mid-device-sync',version:1,updatedAt,deviceId:String(body?.deviceId||'').slice(0,160),blob:{iv:blob.iv,data:blob.data}};
 await env.MID_PUSH_SUBSCRIPTIONS.put(key,JSON.stringify(entry),{expirationTtl:180*24*3600});
 return json({ok:true,updatedAt,version:WORKER_VERSION},200,{'cache-control':'no-store'});
}
async function deviceSyncPull(request,env){
 if(!deviceSyncConfigured(env))return json({error:'Gerätesynchronisation ist im Worker nicht konfiguriert.',version:WORKER_VERSION},503,{'cache-control':'no-store'});
 if(!pushOriginAllowed(request,env))return json({error:'Nicht freigegebener Ursprung.',version:WORKER_VERSION},403,{'cache-control':'no-store'});
 const body=await request.json().catch(()=>null),syncKey=validDeviceSyncKey(body?.syncKey);if(!syncKey)return json({error:'Ungültiger Synchronisationscode.',version:WORKER_VERSION},400,{'cache-control':'no-store'});
 const entry=await env.MID_PUSH_SUBSCRIPTIONS.get(await deviceSyncStorageKey(syncKey),{type:'json'}).catch(()=>null);if(!entry?.blob)return json({error:'Synchronisationsstand nicht gefunden.',version:WORKER_VERSION},404,{'cache-control':'no-store'});
 return json({ok:true,updatedAt:entry.updatedAt,blob:entry.blob,version:WORKER_VERSION},200,{'cache-control':'no-store'});
}
...
export default{async fetch(request,env){
 ...
 const radiusKm=Math.min(250,Math.max(25,Number(u.searchParams.get('radius_km'))||140)),fastStations=u.searchParams.get('fast')==='1',openSenseEnabled=!fastStations&&env?.ENABLE_OPENSENSEMAP!=='false',sources=[
 ...
 return json({data:rows,providers,diagnostics:{radiusKm,fast:fastStations,rows:rows.length,errors,sourceRows:Object.fromEntries(sources.map((x,i)=>[x.name,settled[i].status==='fulfilled'?settled[i].value.length:0]))},version:WORKER_VERSION});
 },
