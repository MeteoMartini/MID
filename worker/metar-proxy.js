...
async function operaListedFrames(reference=Date.now()){const days=[operaNominalTime(reference),operaNominalTime(reference-24*60*60000)],results=await Promise.allSettled(days.map(operaListDay)),errors=results.filter(result=>result.status==='rejected').map(result=>result.reason instanceof Error?result.reason.message:String(result.reason)),keys=[...new Set(results.flatMap(result=>result.status==='fulfilled'?result.value:[]))],frames=keys.map(key=>({key,time:operaTimeFromKey(key)})).filter(frame=>Number.isFinite(frame.time)).sort((a,b)=>a.time-b.time);if(!frames.length)throw new Error(errors.join(' | ')||'OPERA-S3-Index lieferte keine verwertbaren Frames.');return frames}
