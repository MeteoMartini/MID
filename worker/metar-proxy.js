...
const MOUNTAIN_LIFT_TYPES=new Set(['cable_car','gondola','chair_lift','mixed_lift','drag_lift','t-bar','j-bar','platter','magic_carpet','funicular']);
function mountainRole(value=''){const role=String(value||'').toLowerCase();if(/bottom|lower|valley|tal/.test(role))return'valley';if(/mid|middle|mittel/.test(role))return'middle';if(/top|upper|summit|berg/.test(role))return'summit';return''}
function mountainEle(value){const match=String(value??'').replace(',','.').match(/-?\d+(?:\.\d+)?/),n=match?Number(match[0]):NaN;return Number.isFinite(n)&&n>=-100&&n<=9000?n:undefined}
async function overpassMountainElements(lat,lon){
 const query=`[out:json][timeout:24];(node(around:25000,${lat},${lon})["aerialway"="station"];way(around:25000,${lat},${lon})["aerialway"~"^(cable_car|gondola|chair_lift|mixed_lift|drag_lift|t-bar|j-bar|platter|magic_carpet|funicular)$"];);out tags center geom;`,errors=[];
 for(const endpoint of OVERPASS_ENDPOINTS)try{const url=new URL(endpoint);url.searchParams.set('data',query);const response=await fetch(url.toString(),{method:'GET',headers:{Accept:'application/json','User-Agent':`MID-weather-dashboard/${WORKER_VERSION}`},cf:{cacheTtl:21600,cacheEverything:true,cacheKey:url.toString()}});if(!response.ok)throw new Error(`${url.hostname} HTTP ${response.status}`);const data=await response.json();if(Array.isArray(data?.elements))return data.elements}catch(error){errors.push(error instanceof Error?error.message:String(error))}
 throw new Error(errors.join(' | ')||'OpenStreetMap Overpass nicht erreichbar');
}
async function fillMountainElevations(candidates){
 const missing=candidates.filter(item=>!Number.isFinite(item.elevation));if(!missing.length)return candidates;
 const url=new URL(OPEN_METEO_ELEVATION);url.searchParams.set('latitude',missing.map(item=>item.latitude).join(','));url.searchParams.set('longitude',missing.map(item=>item.longitude).join(','));
 try{const data=await fetchJson(url.toString()),elevations=Array.isArray(data?.elevation)?data.elevation:[];missing.forEach((item,index)=>{const value=number(elevations[index]);if(value!==undefined)item.elevation=Math.round(value)})}catch{}
 return candidates;
}
function dedupeMountainCandidates(candidates){const rows=[];for(const candidate of candidates){if(!Number.isFinite(candidate.latitude)||!Number.isFinite(candidate.longitude))continue;const existing=rows.find(item=>distance(item.latitude,item.longitude,candidate.latitude,candidate.longitude)<90);if(!existing){rows.push(candidate);continue}if(!existing.role&&candidate.role)existing.role=candidate.role;if(!existing.name&&candidate.name)existing.name=candidate.name;if(!Number.isFinite(existing.elevation)&&Number.isFinite(candidate.elevation))existing.elevation=candidate.elevation;if(existing.kind!=='station'&&candidate.kind==='station')existing.kind='station';if(!existing.liftId&&candidate.liftId)existing.liftId=candidate.liftId}return rows}
function derivedMountainProfile(lat,lon,elevation,name=''){const valley=Math.max(0,Math.round(Number(elevation)||0)),summit=Math.max(valley+600,Math.max(1200,valley+1200));return{levels:[{role:'valley',name:name?`${name} · Talniveau`:'Talniveau',latitude:lat,longitude:lon,elevation:valley,source:'abgeleitet'},{role:'summit',name:name?`${name} · Bergniveau`:'Bergniveau',latitude:lat,longitude:lon,elevation:summit,source:'abgeleitet'}],source:'Abgeleitete Ausgangswerte',confidence:'low',checkedAt:new Date().toISOString(),diagnostics:{fallback:true}}}
async function mountainProfileData(lat,lon,name=''){
 let elements;try{elements=await overpassMountainElements(lat,lon)}catch(error){const elevation=await fillMountainElevations([{latitude:lat,longitude:lon}]);return{...derivedMountainProfile(lat,lon,elevation[0]?.elevation,name),diagnostics:{fallback:true,requestedLatitude:lat,requestedLongitude:lon,requestedName:name,error:error instanceof Error?error.message:String(error)}}}
 const candidates=[];
 for(const element of elements){const tags=element?.tags??{},type=String(tags.aerialway||''),label=String(tags.name||tags.ref||'').trim(),ele=mountainEle(tags.ele);
  if(element.type==='node'&&type==='station'){const latitude=number(element.lat),longitude=number(element.lon);if(latitude===undefined||longitude===undefined)continue;candidates.push({latitude,longitude,elevation:ele,name:label||'Seilbahnstation',role:mountainRole(tags['aerialway:station']),source:'OpenStreetMap-Station',distance:distance(lat,lon,latitude,longitude),kind:'station'})}
  if(element.type==='way'&&MOUNTAIN_LIFT_TYPES.has(type)&&Array.isArray(element.geometry)&&element.geometry.length>=2){const first=element.geometry[0],last=element.geometry.at(-1),base=label||'Lift',liftId=String(element.id??`${base}:${candidates.length}`);for(const[item,suffix,role]of[[first,'Tal','valley'],[last,'Berg','summit']]){const latitude=number(item?.lat),longitude=number(item?.lon);if(latitude===undefined||longitude===undefined)continue;candidates.push({latitude,longitude,name:`${base} · ${suffix}`,role,source:'OpenStreetMap-Liftende',distance:distance(lat,lon,latitude,longitude),kind:'lift-end',liftId})}}
 }
 await fillMountainElevations(candidates);
 const byLift=new Map();for(const candidate of candidates)if(candidate.liftId){const rows=byLift.get(candidate.liftId)||[];rows.push(candidate);byLift.set(candidate.liftId,rows)}for(const rows of byLift.values())if(rows.length===2&&Number(rows[0].elevation)>Number(rows[1].elevation)){rows[0].role='summit';rows[1].role='valley';rows[0].name=rows[0].name.replace(/ · Tal$/,' · Berg');rows[1].name=rows[1].name.replace(/ · Berg$/,' · Tal')}
 const valid=dedupeMountainCandidates(candidates).filter(item=>Number.isFinite(item.elevation)&&item.distance<=30000);if(valid.length<2){const elevation=await fillMountainElevations([{latitude:lat,longitude:lon}]);return{...derivedMountainProfile(lat,lon,elevation[0]?.elevation,name),diagnostics:{fallback:true,requestedLatitude:lat,requestedLongitude:lon,requestedName:name,osmCandidates:valid.length}}}
 const valleyPool=[...valid.filter(item=>item.role==='valley'),...valid],summitPool=[...valid.filter(item=>item.role==='summit'),...valid];let best;
 for(const valley of valleyPool)for(const summit of summitPool){if(valley===summit)continue;const gain=Number(summit.elevation)-Number(valley.elevation),span=distance(valley.latitude,valley.longitude,summit.latitude,summit.longitude),nearest=Math.min(valley.distance,summit.distance);if(gain<250||span>30000||nearest>22000)continue;const sameLift=Boolean(valley.liftId&&valley.liftId===summit.liftId),score=gain/16-valley.distance/420-summit.distance/900+(valley.role==='valley'?24:0)+(summit.role==='summit'?24:0)+(sameLift?58:0)+(valley.kind==='station'?6:0)+(summit.kind==='station'?6:0);if(!best||score>best.score)best={valley,summit,score,sameLift}}
 if(!best){const elevation=await fillMountainElevations([{latitude:lat,longitude:lon}]);return{...derivedMountainProfile(lat,lon,elevation[0]?.elevation,name),diagnostics:{fallback:true,requestedLatitude:lat,requestedLongitude:lon,requestedName:name,osmCandidates:valid.length,reason:'Kein ortsnahes zusammenhängendes Höhenpaar'}}}
 const{valley,summit}=best,target=(Number(valley.elevation)+Number(summit.elevation))/2,mids=valid.filter(item=>item.role==='middle'),middlePool=[...mids,...valid.filter(item=>!item.role&&item!==valley&&item!==summit)].filter(item=>Number(item.elevation)>Number(valley.elevation)+100&&Number(item.elevation)<Number(summit.elevation)-100&&distance(valley.latitude,valley.longitude,item.latitude,item.longitude)<22000&&distance(summit.latitude,summit.longitude,item.latitude,item.longitude)<22000),middle=middlePool.sort((a,b)=>Math.abs(Number(a.elevation)-target)-Math.abs(Number(b.elevation)-target)||a.distance-b.distance)[0],level=(item,role)=>({role,name:item.name,latitude:item.latitude,longitude:item.longitude,elevation:Math.round(item.elevation),source:item.source}),confidence=(valley.role==='valley'&&summit.role==='summit')||best.sameLift?'high':'medium';
 return{levels:[level(valley,'valley'),...(middle?[level(middle,'middle')]:[]),level(summit,'summit')],source:'OpenStreetMap + Copernicus GLO-90 via Open-Meteo',confidence,checkedAt:new Date().toISOString(),diagnostics:{requestedLatitude:lat,requestedLongitude:lon,requestedName:name,osmElements:elements.length,candidates:valid.length,sameLift:best.sameLift,explicitMiddle:Boolean(middle)}};
}
function parseMountainLevels(value){try{const rows=JSON.parse(String(value||'[]'));return(Array.isArray(rows)?rows:[]).slice(0,3).map(item=>({role:['valley','middle','summit'].includes(item?.role)?item.role:'',latitude:number(item?.latitude),longitude:number(item?.longitude),elevation:number(item?.elevation)})).filter(item=>item.role&&item.latitude!==undefined&&item.longitude!==undefined&&item.elevation!==undefined)}catch{return[]}}
async function geoSphereSnowObservations(levels){
 const meta=await geoSphereMetadata(),nearby=meta.map(st=>({...st,minDistance:Math.min(...levels.map(level=>distance(level.latitude,level.longitude,st.lat,st.lon))),minElevationDiff:Math.min(...levels.map(level=>Math.abs(Number(st.elevation??0)-level.elevation)))})).filter(st=>st.minDistance<=40000&&st.minElevationDiff<=500).sort((a,b)=>a.minDistance-b.minDistance||a.minElevationDiff-b.minElevationDiff).slice(0,18);if(!nearby.length)return[];
 let raw,param='SCHNEE';for(const candidate of['SCHNEE','HS'])try{const url=new URL(GEOSPHERE_CURRENT);url.searchParams.set('parameters',candidate);url.searchParams.set('station_ids',nearby.map(st=>st.id).join(','));url.searchParams.set('output_format','geojson');raw=await fetchJson(url.toString());param=candidate;break}catch{}if(!raw)return[];
 const features=Array.isArray(raw?.features)?raw.features:Array.isArray(raw?.data)?raw.data:Array.isArray(raw)?raw:[],byId=new Map(nearby.map(st=>[st.id,st])),timestamp=safeDate(Array.isArray(raw?.timestamps)?raw.timestamps.at(-1):raw?.timestamp),available=[];
 for(const feature of features){const id=geoSphereFeatureId(feature),station=byId.get(id),depth=geoSphereParam(feature,param);if(!station||depth===undefined||depth<0||depth>1500)continue;available.push({...station,depth,observedAt:safeDate(feature?.properties?.timestamp)||timestamp})}
 const used=new Set(),observations=[];for(const level of levels){const maxDistance=level.role==='valley'?25000:35000,maxHeightDiff=level.role==='middle'?300:400,candidate=available.filter(st=>!used.has(st.id)&&distance(level.latitude,level.longitude,st.lat,st.lon)<=maxDistance&&Math.abs(Number(st.elevation??0)-level.elevation)<=maxHeightDiff).sort((a,b)=>{const ad=distance(level.latitude,level.longitude,a.lat,a.lon)/1000+Math.abs(Number(a.elevation??0)-level.elevation)/80,bd=distance(level.latitude,level.longitude,b.lat,b.lon)/1000+Math.abs(Number(b.elevation??0)-level.elevation)/80;return ad-bd})[0];if(!candidate)continue;used.add(candidate.id);observations.push({role:level.role,depthCm:candidate.depth,stationName:candidate.name,stationId:candidate.id,elevation:candidate.elevation,distanceKm:Number((distance(level.latitude,level.longitude,candidate.lat,candidate.lon)/1000).toFixed(1)),observedAt:candidate.observedAt,provider:'GeoSphere Austria / TAWES Schneepegel',license:'CC BY 4.0'})}
 return observations;
}
async function snowObservationsData(lat,lon,country,levels){const code=directCountryCode(country),austria=code==='AT'||(!code&&geoSphereApplies(lat,lon));if(!austria)return{observations:[],provider:'Open-Meteo Modellschneehöhe',coverage:'Für diesen Ort ist derzeit keine passende frei zugängliche Live-Schneepegelquelle eingebunden.'};try{const observations=await geoSphereSnowObservations(levels);return{observations,provider:'GeoSphere Austria / TAWES',coverage:observations.length?'Passende aktuelle Schneepegel nach Entfernung und Höhendifferenz':'Keine ausreichend passende aktuelle TAWES-Schneehöhenmessung; Modellwerte bleiben verfügbar.'}}catch(error){return{observations:[],provider:'GeoSphere Austria / TAWES',coverage:'Schneepegel vorübergehend nicht verfügbar',sourceError:error instanceof Error?error.message:String(error)}}}
...
function validPushFavorites(value){return(Array.isArray(value)?value:[]).slice(0,21).map(item=>({id:String(item?.id||'').slice(0,100),name:String(item?.name||'Favorit').slice(0,100),latitude:number(item?.latitude),longitude:number(item?.longitude),country:String(item?.country||'').slice(0,40),targetType:item?.targetType==='tracked-location'?'tracked-location':'favorite',rules:{precipitationStart:Boolean(item?.rules?.precipitationStart),thunderstormApproach:Boolean(item?.rules?.thunderstormApproach)}})).filter(item=>item.id&&item.latitude!==undefined&&item.longitude!==undefined&&(item.rules.precipitationStart||item.rules.thunderstormApproach))}
...
async function evaluatePushEntry(key,entry,env,weatherCache,thunderCache){const subscription=validPushSubscription(entry?.subscription);if(!subscription){await env.MID_PUSH_SUBSCRIPTIONS.delete(key);return}const favorites=validPushFavorites(entry?.favorites),state=entry?.state&&typeof entry.state==='object'?entry.state:{},appUrl=String(entry?.appUrl||env.MID_APP_URL||'https://meteomartini.github.io/MID/');let expired=false,changed=false;for(const favorite of favorites){const targetKey=`${favorite.targetType}:${favorite.latitude.toFixed(4)}:${favorite.longitude.toFixed(4)}`,stored=state[favorite.id]??{},old=stored.targetKey===targetKey?stored:{},next={...old,targetKey};if(favorite.rules.precipitationStart){const cacheKey=`${favorite.latitude.toFixed(4)}:${favorite.longitude.toFixed(4)}`;let weather=weatherCache.get(cacheKey);if(!weather){weather=pushWeatherState(favorite);weatherCache.set(cacheKey,weather)}try{const result=await weather,eventKey=result.upcomingKey||'',triggerActive=result.active&&!old.precipitationActive,triggerUpcoming=!result.active&&result.upcomingMinutes!==undefined&&eventKey!==old.rainEventKey;if(triggerActive||triggerUpcoming){const tracked=favorite.targetType==='tracked-location',place=tracked?'am aktuellen Standort':`bei ${favorite.name}`,body=triggerActive?`${result.kind} hat ${place} begonnen.`:`${result.kind} beginnt ${place} voraussichtlich in ${result.upcomingMinutes} Minuten.`,sent=await sendWebPush(subscription,{title:tracked?'MID · Aktueller Standort':`MID · ${favorite.name}`,body,tag:`mid-rain-${favorite.id}`,url:appUrl,favoriteId:favorite.id,timestamp:Date.now(),renotify:true},env);if(sent.expired){expired=true;break}if(sent.ok){next.rainEventKey=eventKey||`active-${new Date().toISOString().slice(0,13)}`;next.lastRainNotificationAt=new Date().toISOString()}}next.precipitationActive=result.active;if(!result.active&&result.upcomingMinutes===undefined)next.rainEventKey='';changed=true}catch(error){next.precipitationError=error instanceof Error?error.message:String(error)}}if(favorite.rules.thunderstormApproach&&!expired){const cacheKey=`${favorite.latitude.toFixed(4)}:${favorite.longitude.toFixed(4)}:${favorite.country}`;let thunder=thunderCache.get(cacheKey);if(!thunder){thunder=dwdKonrad3dNowcast(favorite.latitude,favorite.longitude,favorite.country);thunderCache.set(cacheKey,thunder)}try{const result=pushThunderState(await thunder),trigger=result.active&&(!old.thunderActive||result.key!==old.thunderCellKey);if(trigger){const tracked=favorite.targetType==='tracked-location',distanceText=Number.isFinite(result.distanceKm)?` · Abstand etwa ${Math.round(result.distanceKm)} km`:'',arrivalText=Number.isFinite(result.arrivalMinutes)?` · mögliche Annäherung in ${Math.round(result.arrivalMinutes)} min`:'',target=tracked?'dem aktuellen Standort':`dem Favoritenort ${favorite.name}`,sent=await sendWebPush(subscription,{title:tracked?'MID · Gewitter · Aktueller Standort':`MID · Gewitter bei ${favorite.name}`,body:`Eine Gewitterzelle nähert sich ${target}${distanceText}${arrivalText}. Keine amtliche Warnung.`,tag:`mid-thunder-${favorite.id}`,url:appUrl,favoriteId:favorite.id,timestamp:Date.now(),renotify:true},env);if(sent.expired){expired=true;break}if(sent.ok)next.lastThunderNotificationAt=new Date().toISOString()}next.thunderActive=result.active;next.thunderCellKey=result.active?result.key:'';changed=true}catch(error){next.thunderError=error instanceof Error?error.message:String(error)}}state[favorite.id]=next}if(expired){await env.MID_PUSH_SUBSCRIPTIONS.delete(key);return}if(changed)await env.MID_PUSH_SUBSCRIPTIONS.put(key,JSON.stringify({...entry,subscription,favorites,state,checkedAt:new Date().toISOString()}))}
async function runPushSchedule(env){if(!pushConfigured(env))return{ok:false,reason:'not-configured'};const listed=await env.MID_PUSH_SUBSCRIPTIONS.list({prefix:'sub:',limit:250}),weatherCache=new Map(),thunderCache=new Map();await pushMapLimited(listed.keys??[],4,async item=>{const entry=await env.MID_PUSH_SUBSCRIPTIONS.get(item.name,{type:'json'}).catch(()=>null);if(entry)await evaluatePushEntry(item.name,entry,env,weatherCache,thunderCache)});return{ok:true,subscriptions:listed.keys?.length??0,complete:listed.list_complete!==false}}
...
export default{async fetch(request,env){
 ...
 const lat=Number(u.searchParams.get('lat')),lon=Number(u.searchParams.get('lon'));if(!Number.isFinite(lat)||!Number.isFinite(lon))return json({error:'lat/lon required',version:WORKER_VERSION},400);
  if(mode==='mountain-profile'){try{return json({...await mountainProfileData(lat,lon,u.searchParams.get('name')||''),version:WORKER_VERSION},200,{'cache-control':'private, max-age=0, no-store'})}catch(error){return json({...derivedMountainProfile(lat,lon,0,u.searchParams.get('name')||''),error:error instanceof Error?error.message:String(error),version:WORKER_VERSION},200,{'cache-control':'public, max-age=1800'})}}
  if(mode==='snow-observations'){const levels=parseMountainLevels(u.searchParams.get('levels'));if(levels.length<2)return json({error:'Mindestens Tal- und Bergniveau erforderlich',version:WORKER_VERSION},400);try{return json({...await snowObservationsData(lat,lon,u.searchParams.get('country')||'',levels),version:WORKER_VERSION,checkedAt:new Date().toISOString()},200,{'cache-control':'public, max-age=300'})}catch(error){return json({observations:[],provider:'Schneehöhen',coverage:'Messdaten nicht verfügbar',error:error instanceof Error?error.message:String(error),version:WORKER_VERSION},502,{'cache-control':'no-store'})}}
