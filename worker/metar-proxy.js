...
const WORKER_VERSION='0.7.101.0';
 ...
 for(const favorite of favorites){
  ...
  if(favorite.rules.thunderstormApproach&&!expired){
   ...
   try{
    ...
    if(trigger&&canNotify()){
     const distanceText=Number.isFinite(result.distanceKm)?` · Abstand etwa ${Math.round(result.distanceKm)} km`:'',arrivalText=Number.isFinite(result.arrivalMinutes)?` · mögliche Annäherung in ${Math.round(result.arrivalMinutes)} min`:'',sent=await sendWebPush(subscription,{title:`MID · Gewitter bei ${favorite.name}`,body:`Eine Gewitterzelle nähert sich dem Favoriten${distanceText}${arrivalText}. Keine amtliche Warnung.`,tag:`mid-thunder-${favorite.id}`,url:pushFavoriteUrl(appUrl,favorite),...pushLocationPayload(favorite),timestamp:Date.now(),renotify:true},env);
     if(sent.expired){expired=true;break}
