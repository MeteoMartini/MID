...
const WORKER_VERSION='0.7.95.26';
...
async function fetchWithDeadline(input,init={},timeoutMs=8000){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),Math.max(1000,timeoutMs));try{return await fetch(input,{...init,signal:controller.signal})}catch(error){if(controller.signal.aborted)throw new Error(`${typeof input==='string'?new URL(input).hostname:'Upstream'} Zeitüberschreitung nach ${timeoutMs} ms`);throw error}finally{clearTimeout(timer)}}
...
// Der offizielle generische GeoServer-Endpunkt steht zuerst. Workspace-Endpunkte
// bleiben als Fallback erhalten, verwenden dort aber Layernamen ohne "dwd:".
...
function wmsLayerLocalName(value){return String(value||'').trim().toLowerCase().split(':').at(-1)||''}
function sameWmsLayer(left,right){const a=String(left||'').trim().toLowerCase(),b=String(right||'').trim().toLowerCase();return Boolean(a&&b&&(a===b||wmsLayerLocalName(a)===wmsLayerLocalName(b)))}
...
async function dwdLightningPoints(lat,lon){
 const latSpan=2.1,lonSpan=2.1/Math.max(.28,Math.cos(lat*Math.PI/180)),bbox=[lon-lonSpan,lat-latSpan,lon+lonSpan,lat+latSpan].map(value=>value.toFixed(5)).join(','),requests=[];
 for(const base of[DWD_WFS_PRIMARY,DWD_WFS_BACKUP])for(const version of['2.0.0','1.0.0'])requests.push((async()=>{const url=new URL(base);url.searchParams.set('service','WFS');url.searchParams.set('version',version);url.searchParams.set('request','GetFeature');url.searchParams.set(version==='2.0.0'?'typeNames':'typeName','dwd:Accumulated_Flash_Geometry');url.searchParams.set('outputFormat','application/json');url.searchParams.set('srsName','EPSG:4326');url.searchParams.set(version==='2.0.0'?'count':'maxFeatures','750');url.searchParams.set('bbox',`${bbox},EPSG:4326`);const response=await fetchWithDeadline(url.toString(),{headers:{Accept:'application/geo+json,application/json','User-Agent':`MID-weather-dashboard/${WORKER_VERSION}`},cache:'no-store'},4500);if(!response.ok)throw new Error(`${new URL(base).hostname}/WFS ${version} HTTP ${response.status}`);const data=await response.json(),features=Array.isArray(data?.features)?data.features:[],points=[];
  ...
  if(!points.length)throw new Error(`${new URL(base).hostname}/WFS ${version}: keine Features`);const dated=points.filter(point=>Number.isFinite(Date.parse(point.observedAt||''))),newest=dated.map(point=>Date.parse(point.observedAt)).sort((a,b)=>a-b).at(-1),fresh=dated.length?points.filter(point=>{const stamp=Date.parse(point.observedAt||'');return Number.isFinite(stamp)&&stamp>=Date.now()-125*60000&&stamp<=Date.now()+10*60000}):points;if(dated.length&&!fresh.length)throw new Error(`${new URL(base).hostname}/WFS ${version}: Blitzstand veraltet (${newest?new Date(newest).toISOString():'Zeit unbekannt'})`);return{points:fresh.slice(0,750),provider:'DWD NowCastMIX Accumulated Flash Geometry',observedAt:fresh.map(point=>point.observedAt).filter(Boolean).sort().at(-1),coverage:'Deutschland · DWD-WFS-Blitzgeometrien'} })());
 try{return await Promise.any(requests)}catch(error){const messages=error instanceof AggregateError?error.errors.map(item=>item instanceof Error?item.message:String(item)):[error instanceof Error?error.message:String(error)];throw new Error(`DWD-Blitzpunkte nicht verfügbar: ${messages.slice(-4).join(' | ')}`)}
}
...
function satelliteProduct(capabilities,candidates,now=Date.now()){
 ...
 for(const candidate of candidates){const xml=capabilities[candidate.provider];if(!xml||!hasWmsLayer(xml,candidate.layer))continue;const all=dwdTimesFromCapabilities(xml,candidate.layer),latest=all.at(-1);
  ...
  if(Number.isFinite(latest)){
   const maxAgeMinutes=Number(candidate.maxAgeMinutes)||190,freshAgeMinutes=Number(candidate.freshAgeMinutes)||80;if(latest<now-maxAgeMinutes*60000||latest>now+15*60000)continue;
   const times=recentObservedTimes(all,now,maxAgeMinutes,30,15);if(times.length)products.push({...candidate,times:times.map(time=>new Date(time).toISOString()),latest,fresh:latest>=now-freshAgeMinutes*60000});continue;
  }
...
async function compositeTimes(lat,lon){const serverTime=new Date().toISOString(),result={satelliteDay:[],satelliteIr:[],satellitePrecip:[],mtgLightning:[],dwdLightning:[],dwdRadar:[],dwdRadarLayer:'',dwdRadarLatestOnly:false,mtgLightningLatestOnly:false,checkedAt:serverTime,serverTime,errors:[]};
 // Die Live-Grundebenen hängen nicht von einer langsamen GetCapabilities-
 // Auswertung ab. Die offiziellen WMS-Dienste liefern ohne TIME den jeweils
 // neuesten verfügbaren Dienststand. Präzise Quellenprüfungen laufen separat.
