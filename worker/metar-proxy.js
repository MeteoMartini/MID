...
const WORKER_VERSION='0.7.103.6';
...
function pushLocationPhrases(favorite){const tracked=String(favorite?.id||'')==='tracked-location'||/^Aktueller Standort\s*·/i.test(String(favorite?.name||'')),name=pushVisibleLocationName(favorite);return tracked?{at:'am Standort',toward:'dem Standort',from:'vom Standort',name:'Standort'}:{at:`in ${name}`,toward:name,from:`von ${name}`,name}}
 ...
 for(const favorite of favorites){
  ...
  if(favorite.rules.forecastMaterialChange&&!expired){
   ...
   try{
    ...
    if(trigger&&canNotify()){
     const visibleName=favorite.name.replace(/^Modelllaufänderung\s*·\s*/i,''),extra=events.length>2?` · plus ${events.length-2} weitere`:'',sent=await sendWebPush(subscription,{title:`MID · Prognoseänderung für ${visibleName}`,body:`${events.slice(0,2).map(pushForecastEventText).join(' · ')}${extra}`,tag:`mid-model-change-${favorite.id}`,url:pushFavoriteUrl(appUrl,favorite),...pushLocationPayload(favorite),timestamp:Date.now(),renotify:true},env);
     if(sent.expired){expired=true;break}
