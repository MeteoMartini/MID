...
function termsFor(...values){return values.flatMap(v=>normalize(v).split(/\s+/)).filter(x=>x.length>=3&&!['stadt','gemeinde','bezirk','landkreis','kreis','region','bundesland'].includes(x))}
...
function capLink(block){const links=linkObjects(block);return links.find(x=>String(x.type||'').toLowerCase().includes('cap+xml'))?.href||links.find(x=>/\/api\/v\d+\/warnings\//i.test(x.href||''))?.href||links.find(x=>/cap|\.xml(?:$|\?)/i.test(x.href||''))?.href||links.find(x=>/^https?:/i.test(x.href||''))?.href||tagValues(block,'link').find(x=>/^https?:/i.test(x))}
...
async function capFeedAlerts(feedUrl,source,lat,lon,name,region,district,language='de',maxDocuments=24){
 const xml=await fetchText(feedUrl),items=(blocks(xml,'entry').length?blocks(xml,'entry'):blocks(xml,'item')).map((item,index)=>({item,index,score:entryScore(item,name,region,district)})).sort((a,b)=>b.score-a.score||a.index-b.index),alerts=[];
 for(const x of items){const direct=parseCap(x.item,source,capLink(x.item),lat,lon,name,region,district,language);if(direct)alerts.push(direct)}
 const candidates=items.filter(x=>!parseCap(x.item,source,capLink(x.item),lat,lon,name,region,district,language)&&capLink(x.item)).slice(0,maxDocuments);
 for(let i=0;i<candidates.length;i+=6){const batch=candidates.slice(i,i+6);const results=await Promise.allSettled(batch.map(async x=>{const url=capLink(x.item),capXml=await fetchText(url);return parseCap(capXml,source,url,lat,lon,name,region,district,language)}));for(const r of results)if(r.status==='fulfilled'&&r.value)alerts.push(r.value)}
...
async function officialAlerts(lat,lon,country,name,region,district,language){
 const c=String(country||'').toUpperCase();if(c==='DE')return dwdAlerts(lat,lon,name,region,district,language);if(c==='US'){const alerts=await nwsAlerts(lat,lon);return{alerts:dedupeAlerts(alerts),provider:'NOAA / National Weather Service',coverage:'USA · amtliche CAP-Warnungen'}}
 const slug=FEED_SLUGS[c];if(slug){const url=`${METEOALARM_FEEDS}meteoalarm-legacy-atom-${slug}`,alerts=await capFeedAlerts(url,'MeteoAlarm / nationale Wetterbehörde',lat,lon,name,region,district,language,18);return{alerts,provider:'MeteoAlarm / nationale Wetterbehörde',coverage:'Europa · CAP-Warnungen nationaler Wetterdienste',sourceStatus:{primary:'MeteoAlarm Atom/CAP',endpoint:url}}}
...
async function metarRows(lat,lon,radiusKm){
 ...
 const response=await fetch(api,{headers:{'User-Agent':`MID-weather-dashboard/${WORKER_VERSION}`}});
...
export default{async fetch(request,env){
 ...
 return json({data:rows,providers:{metar:true,weatherUnderground:Boolean(env?.WEATHER_COM_API_KEY||env?.WU_API_KEY)},version:WORKER_VERSION});
}};
