...
function dwdMapSample(image,lat,latDelta,lonDelta){
 ...
 return{center,nearby:Math.max(...near,0),visible:center>0||near.some(v=>v>0),...precipitationCentroid(image,sample,kmPerPixelX,kmPerPixelY)};
}
...
function sampleRadarTile(image,lat){const cx=Math.floor(image.width/2),cy=Math.floor(image.height/2),kmPerPixel=156.54303392*Math.cos(lat*Math.PI/180)/(2**7),sample=(x,y)=>{x=clamp(Math.round(x),0,image.width-1);y=clamp(Math.round(y),0,image.height-1);const i=(y*image.width+x)*4;return rainViewerPixelRate(image.rgba[i],image.rgba[i+1],image.rgba[i+2],image.rgba[i+3])},center=sample(cx,cy),offsets=[3,5,8].map(k=>Math.max(1,k/kmPerPixel)),near=[];for(const o of offsets)for(const[a,b]of[[o,0],[-o,0],[0,o],[0,-o],[o*.7,o*.7],[-o*.7,o*.7]])near.push(sample(cx+a,cy+b));return{center,nearby:Math.max(...near,0),...precipitationCentroid(image,sample,kmPerPixel,kmPerPixel)}}
async function rainViewerRadarNowcast(lat,lon){
 ...
 const frames=[];for(const frame of past){const url=`${host}${frame.path}/256/7/${lat.toFixed(5)}/${lon.toFixed(5)}/2/0_0.png`,response=await fetch(url,{cf:{cacheTtl:600,cacheEverything:true}});if(!response.ok)continue;const sampled=sampleRadarTile(await decodePng(await response.arrayBuffer()),lat);frames.push({time:Number(frame.time)*1000,center:sampled.center,nearby:sampled.nearby,centroidEastKm:sampled.centroidEastKm,centroidNorthKm:sampled.centroidNorthKm,precipMass:sampled.precipMass,future:false})}
...
function radarMotionFromFrames(frames){const usable=frames.filter(frame=>!frame.future&&Number.isFinite(frame.time)&&Number.isFinite(frame.centroidEastKm)&&Number.isFinite(frame.centroidNorthKm)&&Number(frame.precipMass)>=3).sort((a,b)=>a.time-b.time);if(usable.length<2)return{};let first=usable[0],last=usable.at(-1);if(last.time-first.time<10*60000)return{};const east=last.centroidEastKm-first.centroidEastKm,north=last.centroidNorthKm-first.centroidNorthKm,distance=Math.hypot(east,north),hours=(last.time-first.time)/3600000,speed=distance/hours;if(!Number.isFinite(speed)||speed<2||speed>160||distance<1)return{};const direction=(Math.atan2(east,north)*180/Math.PI+360)%360,confidence=usable.length>=5&&distance>=5?'high':usable.length>=3&&distance>=2.5?'medium':'low';return{motionDirectionDeg:Number(direction.toFixed(1)),motionSpeedKmh:Number(speed.toFixed(1)),motionConfidence:confidence,motionSource:'radar-centroid-tracking',motionFrameCount:usable.length}}
