...
const EEA_AIR_QUALITY_STATIONS='https://discomap.eea.europa.eu/arcgis/rest/services/AirQuality/AirQualityDownloadServiceEUMonitoringStations/MapServer/0/query';
...
const WORKER_VERSION='0.7.103.0';
...
async function nearestEeaAirQualityStation(lat,lon){
 const url=new URL(EEA_AIR_QUALITY_STATIONS);url.searchParams.set('f','json');url.searchParams.set('where','1=1');url.searchParams.set('geometry',`${lon},${lat}`);url.searchParams.set('geometryType','esriGeometryPoint');url.searchParams.set('inSR','4326');url.searchParams.set('outSR','4326');url.searchParams.set('spatialRel','esriSpatialRelIntersects');url.searchParams.set('distance','75000');url.searchParams.set('units','esriSRUnit_Meter');url.searchParams.set('outFields','AirQualityStation,AQStationName,Country,CountryCode,AirQualityStationEoICode,stationClass');url.searchParams.set('returnGeometry','true');url.searchParams.set('resultRecordCount','100');
 const response=await fetch(url.toString(),{headers:{Accept:'application/json'},cf:{cacheTtl:86400,cacheEverything:true}});if(!response.ok)throw new Error(`EEA-Messstationsdienst HTTP ${response.status}`);const data=await response.json();if(data?.error)throw new Error(data.error?.message||'EEA-Messstationsdienst meldet einen Fehler.');
 const features=Array.isArray(data?.features)?data.features:[],stations=features.map(feature=>{const attributes=feature?.attributes||{},x=number(feature?.geometry?.x),y=number(feature?.geometry?.y);if(x===undefined||y===undefined)return null;const distanceKm=distance(lat,lon,y,x)/1000;return{name:String(attributes.AQStationName||attributes.AirQualityStation||attributes.AirQualityStationEoICode||'EEA-Messstation'),stationCode:String(attributes.AirQualityStation||''),eoiCode:String(attributes.AirQualityStationEoICode||''),country:String(attributes.Country||''),countryCode:String(attributes.CountryCode||''),stationClass:String(attributes.stationClass||''),latitude:y,longitude:x,distanceKm}}).filter(Boolean).filter(row=>Number.isFinite(row.distanceKm)).sort((a,b)=>a.distanceKm-b.distanceKm);
 const station=stations[0];if(!station)return{available:false,provider:'European Environment Agency (EEA)',reason:'Im Umkreis von 75 km wurde keine EEA-Messstation gefunden.'};return{available:true,...station,distanceKm:Math.round(station.distanceKm*10)/10,provider:'European Environment Agency (EEA)'};
}
...
function thunderPushBody(result){const current=Number(result?.currentDistanceKm),forecast=Number(result?.forecastDistanceKm),arrival=Number(result?.arrivalMinutes);if(result?.status==='at-site'||Number.isFinite(current)&&current<1)return'Eine Gewitterzelle befindet sich unmittelbar am Favoriten. Keine amtliche Warnung.';if(result?.status==='near'||Number.isFinite(current)&&current<=20)return`Eine Gewitterzelle befindet sich aktuell etwa ${Math.max(1,Math.round(current))} km vom Favoriten entfernt. Keine amtliche Warnung.`;const currentText=Number.isFinite(current)?` · aktuell etwa ${Math.max(1,Math.round(current))} km entfernt`:'',approachText=Number.isFinite(arrival)&&arrival>0?` · größte berechnete Annäherung in ${Math.round(arrival)} min${Number.isFinite(forecast)?` auf etwa ${Math.max(0,Math.round(forecast))} km`:''}`:'';return`Eine Gewitterzelle nähert sich dem Favoriten${currentText}${approachText}. Keine amtliche Warnung.`}
 ...
 for(const favorite of favorites){
  ...
  if(favorite.rules.precipitationStart){
   ...
   try{
    ...
    if(trigger&&canNotify()){
     const body=triggerActive?`${result.kind} hat am Favoriten begonnen.`:`${result.kind} beginnt voraussichtlich in ${result.upcomingMinutes} Minuten.`,sent=await sendWebPush(subscription,{title:`MID · ${favorite.name}`,body,tag:`mid-rain-${favorite.id}`,url:pushFavoriteUrl(appUrl,favorite),...pushLocationPayload(favorite),timestamp:Date.now(),renotify:true},env);
  ...
  if(favorite.rules.thunderstormApproach&&!expired){
   ...
   try{
    ...
    if(trigger&&canNotify()){
     const sent=await sendWebPush(subscription,{title:`MID · Gewitter bei ${favorite.name}`,body:thunderPushBody(result),tag:`mid-thunder-${favorite.id}`,url:pushFavoriteUrl(appUrl,favorite),...pushLocationPayload(favorite),timestamp:Date.now(),renotify:true},env);
     if(sent.expired){expired=true;break}
