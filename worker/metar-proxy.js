...
async function overpassMountainElements(lat,lon){
 ...
 for(const endpoint of OVERPASS_ENDPOINTS)try{const response=await fetch(endpoint,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded;charset=UTF-8','User-Agent':`MID-weather-dashboard/${WORKER_VERSION}`},body:`data=${encodeURIComponent(query)}`,cf:{cacheTtl:21600,cacheEverything:true}});if(!response.ok)throw new Error(`${new URL(endpoint).hostname} HTTP ${response.status}`);const data=await response.json();if(Array.isArray(data?.elements))return data.elements}catch(error){errors.push(error instanceof Error?error.message:String(error))}
...
async function mountainProfileData(lat,lon,name=''){
 let elements;try{elements=await overpassMountainElements(lat,lon)}catch(error){const elevation=await fillMountainElevations([{latitude:lat,longitude:lon}]);return{...derivedMountainProfile(lat,lon,elevation[0]?.elevation,name),diagnostics:{fallback:true,error:error instanceof Error?error.message:String(error)}}}
 const candidates=[];
 for(const element of elements){const tags=element?.tags??{},type=String(tags.aerialway||''),label=String(tags.name||tags.ref||'').trim(),ele=mountainEle(tags.ele);
  ...
  if(element.type==='way'&&MOUNTAIN_LIFT_TYPES.has(type)&&Array.isArray(element.geometry)&&element.geometry.length>=2){const first=element.geometry[0],last=element.geometry.at(-1),base=label||'Lift';for(const[item,suffix]of[[first,'Tal'],[last,'Berg']]){const latitude=number(item?.lat),longitude=number(item?.lon);if(latitude===undefined||longitude===undefined)continue;candidates.push({latitude,longitude,name:`${base} ${suffix}`,role:'',source:'OpenStreetMap-Liftende',distance:distance(lat,lon,latitude,longitude)})}}
 }
 await fillMountainElevations(candidates);const valid=dedupeMountainCandidates(candidates).filter(item=>Number.isFinite(item.elevation)&&item.distance<=30000);if(valid.length<2){const elevation=await fillMountainElevations([{latitude:lat,longitude:lon}]);return{...derivedMountainProfile(lat,lon,elevation[0]?.elevation,name),diagnostics:{fallback:true,osmCandidates:valid.length}}}
 const bottoms=valid.filter(item=>item.role==='valley'),tops=valid.filter(item=>item.role==='summit'),mids=valid.filter(item=>item.role==='middle');
 const valley=(bottoms.length?bottoms:valid).sort((a,b)=>(a.elevation+b.distance/1500)-(b.elevation+a.distance/1500))[0];
 const summit=(tops.length?tops:valid).filter(item=>item!==valley&&distance(valley.latitude,valley.longitude,item.latitude,item.longitude)<=35000).sort((a,b)=>(b.elevation-b.distance/4000)-(a.elevation-a.distance/4000))[0];
 if(!summit||summit.elevation<=valley.elevation+150)return derivedMountainProfile(lat,lon,valley.elevation,name);
 const target=(valley.elevation+summit.elevation)/2,middlePool=[...mids,...valid.filter(item=>!item.role&&item!==valley&&item!==summit)].filter(item=>item.elevation>valley.elevation+120&&item.elevation<summit.elevation-120),middle=middlePool.sort((a,b)=>Math.abs(a.elevation-target)-Math.abs(b.elevation-target)||a.distance-b.distance)[0];
 const level=item=>({role:item.role,name:item.name,latitude:item.latitude,longitude:item.longitude,elevation:Math.round(item.elevation),source:item.source});
 valley.role='valley';summit.role='summit';if(middle)middle.role='middle';const levels=[level(valley),...(middle?[level(middle)]:[]),level(summit)],confidence=bottoms.length&&tops.length?'high':'medium';
 return{levels,source:'OpenStreetMap + Copernicus GLO-90 via Open-Meteo',confidence,checkedAt:new Date().toISOString(),diagnostics:{osmElements:elements.length,candidates:valid.length,explicitBottom:bottoms.length,explicitMiddle:mids.length,explicitTop:tops.length}};
}
