...
async function modelContours(lat,lon){
 ...
 const fetchRow=async rowIndex=>{const url=new URL('https://api.open-meteo.com/v1/forecast'),latitudes=Array.from({length:cols},()=>lats[rowIndex].toFixed(4)),longitudes=lons.map(value=>value.toFixed(4));url.searchParams.set('latitude',latitudes.join(','));url.searchParams.set('longitude',longitudes.join(','));url.searchParams.set('hourly','pressure_msl,geopotential_height_500hPa');url.searchParams.set('past_hours','2');url.searchParams.set('forecast_hours','5');url.searchParams.set('models',domain.model);url.searchParams.set('timezone','GMT');url.searchParams.set('cell_selection','nearest');url.searchParams.set('elevation','nan');const response=await fetch(url.toString(),{headers:{Accept:'application/json','User-Agent':`MID-weather-dashboard/${WORKER_VERSION}`},cf:{cacheTtl:900,cacheEverything:true}});if(!response.ok)throw new Error(`Open-Meteo Modelllinien HTTP ${response.status}`);const points=openMeteoRows(await response.json());if(points.length!==cols)throw new Error(`Open-Meteo lieferte ${points.length} statt ${cols} Rasterpunkte in Zeile ${rowIndex+1}.`);return points};
 const rowSets=await Promise.all(Array.from({length:rows},(_,rowIndex)=>fetchRow(rowIndex))),points=rowSets.flat(),times=points[0]?.hourly?.time||[],frames=[];
