...
3. Temperaturidentität bleibt an die Parameterrolle gebunden: Tmin verwendet ausschließlich Blautöne aus `--param-temperature-min`, Tmax ausschließlich Rottöne aus `--param-temperature-max`, eine allgemeine Temperaturkurve `--param-temperature`. Ausschließlich die Zahlfarbe einzelner Tmin-/Tmax-Werte darf anhand der Abweichung vom zugehörigen Klimamittel variieren; Hintergrund und Rahmen bleiben neutral und die Farbfamilie darf niemals wechseln. Kurzfristige und aktuelle Einzeltemperaturen (einschließlich „Nächste 90 Minuten“ und stündlicher Tagesdetails) werden neutral in `var(--text)` dargestellt, also im Hellmodus dunkel/schwarz und im Dunkelmodus hell/weiß. Blau/Rot bleibt ausschließlich den Tagesextrema Tmin/Tmax vorbehalten.
...
- Tmin bleibt ausschließlich in der blauen Farbfamilie. Die **Zahlfarbe** folgt der signierten Abweichung vom klimatologischen Tmin: deutlich kälter = sehr kräftig/dunkel, Klimamittel = Referenzblau, deutlich milder = sichtbar hellere/entsättigte Blautönung. Hintergrund und Rahmen des Wertes werden dabei nicht eingefärbt.
- Tmax bleibt ausschließlich in der roten Farbfamilie. Die **Zahlfarbe** folgt der signierten Abweichung vom klimatologischen Tmax: deutlich kühler = sichtbar hellere/entsättigte Rottönung, Klimamittel = Basisrot, deutlich wärmer = sehr kräftig/dunkel. Hintergrund und Rahmen des Wertes werden dabei nicht eingefärbt.
