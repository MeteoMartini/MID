const CACHE='mid-shell-v0.7.95.13';
...
async function navigateClientsForUpdate(){const windows=await self.clients.matchAll({type:'window',includeUncontrolled:true});await Promise.all(windows.map(async client=>{try{const url=new URL(client.url);url.searchParams.set('mid-retry',VERSION);url.searchParams.set('_mid_reload',String(Date.now()));await client.navigate(url.toString())}catch{}}))}
...
async function rollbackDocument(response,active){try{const html=await response.text(),version=versionFromCache(active),banner=`<aside id="mid-rollback-banner" style="position:fixed;z-index:99999;left:8px;right:8px;bottom:8px;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;border-radius:14px;background:#102338;color:#eef6ff;box-shadow:0 12px 36px rgba(0,0,0,.4);font:12px system-ui,sans-serif"><span><b>MID-Rückfallversion v${version}</b><br><small style="opacity:.78">Die vorherige geprüfte Version ist aktiv.</small></span><button style="min-height:36px;padding:7px 11px;border:0;border-radius:9px;background:#248dff;color:white;font-weight:700" onclick="(async(button)=>{button.disabled=true;button.textContent='Aktuelle Version wird getestet …';const r=await navigator.serviceWorker.ready,w=navigator.serviceWorker.controller||r.active,c=new MessageChannel();c.port1.onmessage=e=>{if(e.data&&e.data.ok){document.getElementById('mid-rollback-banner')?.remove();const u=new URL(location.href);u.searchParams.set('mid-retry',String(Date.now()));location.replace(u.toString())}else{button.disabled=false;button.textContent='Aktuelle Version erneut testen'}};w.postMessage({type:'MID_USE_CURRENT'},[c.port2])})(this)">Aktuelle Version erneut testen</button></aside>`;return new Response(html.replace('</body>',banner+'</body>'),{status:response.status,statusText:response.statusText,headers:{'content-type':'text/html; charset=utf-8','cache-control':'no-store'}})}catch{return response}}

self.addEventListener('install',event=>{event.waitUntil(cacheShell(CACHE))});
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const meta=await readMeta(),names=await shellCaches();let next,retryNewerRollback=false;if(meta.mode==='rollback'&&names.includes(String(meta.activeCache||''))&&String(meta.rolledBackFrom||'')===VERSION){next={...meta,workerVersion:VERSION}}else if(meta.mode==='rollback'&&names.includes(String(meta.activeCache||''))){const previous=String(meta.activeCache||'');retryNewerRollback=true;next={...meta,workerVersion:VERSION,activeCache:CACHE,previousCache:previous,mode:'updating',pending:{targetVersion:VERSION,previousCache:previous,startedAt:Date.now()}}}else if(meta.pending?.targetVersion===VERSION){next={...meta,workerVersion:VERSION,activeCache:CACHE,mode:'updating'}}else{const prior=names.find(name=>name!==CACHE)||String(meta.activeCache||'');next={...meta,workerVersion:VERSION,activeCache:CACHE,previousCache:prior&&prior!==CACHE?prior:meta.previousCache||'',mode:'normal'}}await writeMeta(next);await cleanupShellCaches(next);await self.clients.claim();if(retryNewerRollback)await navigateClientsForUpdate()})())});
