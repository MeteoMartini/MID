...
**v0.9.76.0 TypeScript-7-Kompatibilität:** Der gemeinsame Browser-/PWA-/iOS-Quellkern ist isoliert von TypeScript 5.9.3 auf exakt 7.0.2 migriert. App- und Node-Typecheck benötigen keine fachliche Quellcode- oder Konfigurationsaufweichung; React 18.3.1, Vite 6.4.3 und plugin-react 4.7.0 bleiben unverändert. Nur die von TypeScript 7 nicht mehr exportierte Strada-JavaScript-API für API-basierte Regressionen kommt über den gepinnten Side-by-side-Alias `typescript-strada` 6.0.3; Produkt-Typecheck und `tsc` bleiben 7.0.2. Der neue Required Vertrag schützt Lockfile, No-Emit-Typecheck und Plattformgleichlauf. Das nächste native Gate bleibt die macOS-/Xcode-Simulator-Qualitätssicherung.

**v0.9.76.11 gemeinsamer DWD-Ortsausschnitt:** „Wolken + Niederschlagsart“ verwendet in Browser/PWA und Capacitor-iOS denselben standortzentrierten Ausschnitt des unveränderten amtlichen DWD-Original-PNG. Eine an 17 sichtbaren DWD-Stadtankern sowie fünf deutschen Randorten geprüfte bilineare Ansichtsabbildung positioniert den Marker, ohne das Bild als georeferenzierten Kartenlayer auszugeben. Die eingebettete DWD-Originallegende bleibt sichtbar und Klicks werden unverändert auf die Originalpixel des vollständigen PNG zurückgerechnet. Die Reisewetteränderungen aus v0.9.76.9 und das modernisierte 24-h-Profil aus v0.9.76.10 bleiben erhalten; das nächste native Gate bleibt die macOS-/Xcode-Simulator-Qualitätssicherung.
...
**v0.9.76.12 Parallelstand-Konsolidierung:** Die nahezu zeitgleich auf derselben v0.9.76.10-Basis entstandenen v0.9.76.11-Linien sind ohne Rollback vereinigt. Der standortzentrierte DWD-Ortsausschnitt mit Originallegende und Originalpixel-Auswertung bleibt vollständig erhalten; zusätzlich nutzt das gemeinsame 24-h-Wetterprofil kompaktere mobile Ränder, ruhigere und besser geschützte Achsenbeschriftungen, eine deutlich dezentere Nachtmarkierung sowie eine vereinfachte gemeinsame obere/untere Zeitachse. Browser/PWA und iOS teilen weiterhin denselben React/Vite-Kern. Keine Worker-Fachänderung; das nächste native Gate bleibt macOS-/Xcode-Simulator-QA.
